...
for (const [index, filename] of chapterFiles.entries()) {
  ...
  let tableNumber = 0;
  content = content.replace(/<table([^>]*)>/g, (m, attrs, offset) => {
    const before = content.slice(0, offset);
    const h = [...before.matchAll(/<h3[^>]*>([\s\S]*?)<\/h3>/g)].pop();
    const topic = h ? h[1].replace(/<[^>]+>/g, '') : title;
    return `<div class="table-scroll" tabindex="0" role="region" aria-label="Table: ${topic}, ${++tableNumber}"><table${attrs}>`;
...
<div class="rail-legend" aria-label="Level colors">
