...
CH=len([f for f in os.listdir(Path(__file__).parent) if re.match(r'c\d\d\.md$',f)])
...
if a.h2!=[f's{i}' for i in range(1,CH+1)]:a.errors.append('Chapter headings '+str(a.h2))
if re.search(r'<!--\s*(FIGURE|MOCKUP):',s):a.errors.append('Unrendered figure')
text=' '.join(a.text)+' '+' '.join(re.findall(r'aria-label="([^"]*)"',s))
if '\u2014' in text:a.errors.append('Em dash remains')
for w in ['labelled','honours','greyed','behavioural','behaviour','summarised','colour','organisation','authorise','prioritise','defence','analyse','centre']:
    if re.search(r'\b'+w+r'\b',text):a.errors.append('British spelling: '+w)
