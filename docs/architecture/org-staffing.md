...
plus every plan of a scope project, and every task filed under either, read by
index so a workspace past the health task cap still counts every row. A whole
workspace role holds the remainder no narrower role covers, read over the
capped workspace fetch. `counted` says which of the two a row used and how many
projects, plans and tasks it read, so health and the task list cite the same
rows and a reader can tell when a count is a floor.
