...
`withdraw`. Accepting applies at once through the one apply core in
orgInit.ts (applyRole, applyProjects, applyMove, applyRetire, plus new
applyScope, applyBudget, applyTrust, applyRoutine, applyProjectMeta,
applyAdopt) with `human_decision` from the signed in user (web has no token,
so `refuseUnlessHuman` passes; a CLI accept is the person at a plain shell
with no session). A proposal resolves when every change is applied or skipped.
...
`cast org apply op-N` walks the changes interactively at a shell. The stack
