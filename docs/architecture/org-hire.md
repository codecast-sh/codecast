...
(W1), W2 D4 (decision grants), org-staffing.md (S4 to S7, S12, S16, S19) and
org-roles-run-work.md (W7). The growth pack in `~/src/platform/packs/growth`
...
the escalation. A hire with open human items is a role working within what
...
with the instance, release, evidence links and the proposed change, labelled
`template-lesson`, and links it from the instance row. For `learn.review:
codecast` that project is **Codecast: Templates** in the Codecast workspace,
created with this slice so the first lesson has somewhere to land on day one;
for `publisher` it is the project the publishing workspace names on the
template row (`review_project_id`), defaulting to the project the template
was first hired on. The role never edits its release (the snapshot is read
only) and never edits the publisher's checkout from an instance; the pilot's
write-back into the canonical checkout was the founder's own machine and
stays a publisher privilege.
...
**The role page** already has eleven tabs and W7 R3 makes Scope the first.
A template role adds no tab wall: setup, what it may do, routines with
readiness, the scoreboard and the update line are sections inside
`RoleScopeView` (ct-52468 owns it; the slot is agreed there), shown only when
the role has an instance, with at most one extra tab, Template, for the
release, changelog, host and bindings. The header's waiting-on-you line and
the R1 escalation (H5) are how the person meets the setup list without
opening the page.

- **W8.1 Contract and backend.** Tables with access keys, manifest v2 (done
  ...
  `createTask` paused and `activateRoutine`, the Codecast: Templates project.
