...
The DM and thread composers check the sender's own permission, carried by `chat.listChannels` as `viewer_user_id` and `viewer_slack_auth` on the existing Slack link feed. A person without a token sees “Reply in Slack, too”; a token missing `chat:write` asks them to allow replies. Both open personal Slack authorization from the composer and return to the same conversation, preserving its draft. Until permission arrives, “Also send to Slack” is unchecked and disabled, and sends are explicitly `sync_local_only`. The live feed enables future sends after authorization; earlier local messages stay local. A cached permission belonging to another viewer is treated as unknown.
