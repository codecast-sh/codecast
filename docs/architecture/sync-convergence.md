...
- **Truncation is explicit.** Every cap that fires names itself in
  ...
  one so overflow costs one row, and the cap rises to 100. The pin writers refuse past
  the cap (`pinCapExceeded`): the explicit `patchConversation` mutation throws a clear
  error, and the generic dispatch patch rail drops the pin field and logs (a throw there
  would fail the whole outbox entry, not one gesture), so the server echo reverts the
  optimistic pin. The limit is enforced where the user can see it.
