...
`cast workflow run-daemon` resolves a run without a stored graph through
`resolveWorkflowSource(run.workflow_name)`, the same lookup `cast workflow
run <name>` uses. The run's primary session is a hand of the role
