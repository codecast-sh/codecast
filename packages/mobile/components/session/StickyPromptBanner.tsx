...
import { Animated, Easing, PanResponder, Pressable, StyleSheet, View as RNView, type LayoutChangeEvent } from 'react-native';
import Feather from '@expo/vector-icons/Feather';
import * as Haptics from 'expo-haptics';
import { Text } from '@/components/Themed';
import { Theme, Spacing } from '@/constants/Theme';

export type StickyPrompt = { id: string; ordinal: number; text: string };
...
export function StickyPromptBanner({ prompt, top, translateY, onJump, onDismiss, onHeight }: Props) {
  ...
  const responder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_e, gs) => gs.dy < -6 && Math.abs(gs.dy) > Math.abs(gs.dx) * 1.5,
  ...
  const label = `Prompt ${shown.ordinal}: ${shown.text}`;
    ...
    >
      <RNView style={styles.pillShadow} onLayout={handleLayout} {...responder.panHandlers}>
        <Pressable
          ...
          accessibilityHint="Jumps to this prompt"
          ...
          onAccessibilityAction={(e) => {
            if (e.nativeEvent.actionName === 'expand') setExpanded((v) => !v);
