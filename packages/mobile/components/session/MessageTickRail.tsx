...
// prompt (user rows only, already sampled by `sampleTicks`; the machine rows
// the sheet hides never get a tick), the active prompt wider. Tap opens
...
// The wrap is much wider than the tick strip so the absolutely positioned
// scrub label has real width to measure against. Yoga resolves an absolute
// child's width from its parent's content box: inside the 12pt strip a label
// anchored at `right: 20` gets a negative available width and its text
// collapses to zero columns, painting an empty bubble.
const RAIL_WRAP_WIDTH = 240;
const TICK_PITCH = 7;
const TICK_HEIGHT = 2;
const TICK_WIDTH = 6;
...
export function MessageTickRail({ ticks, promptCount, activeMessageId, visible, onOpen, onScrub, onScrubEnd }: Props) {
  ...
  return (
    // Transparent wide wrap: gives the scrub label width to lay out in and
    // passes every touch through (box-none). The narrow tick strip inside is
    // the only touch target — the panHandlers and the locationY math must
    // stay on it, because locationY is relative to the touch TARGET.
    <Animated.View
      pointerEvents={visible ? 'box-none' : 'none'}
      style={[styles.railWrap, { opacity, transform: [{ translateY: -(ticks.length * TICK_PITCH) / 2 }] }]}
    >
      <View
        style={styles.rail}
...
// position feedback the finger gets. Anchored left of the tick strip at the
// active tick's y, inside the wide wrap so it both inherits the fade and has
