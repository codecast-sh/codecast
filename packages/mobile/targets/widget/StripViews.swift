    ...
    var body: some View {
        Group {
            if state.sessions.count > 1 {
                ...
                StripHeader(state: state, headlineLines: 2, detailLines: 2)
                    .padding(.horizontal, 16)
                    .padding(.vertical, 14)
            }
        }
...
// A running clock since the session started. `timerInterval` is the form
// Live Activities keep ticking without a push; the plain date styles fall
// back to words ("7 minutes") inside a widget.
struct Clock: View {
    let since: Date
    var size: CGFloat = 13
    ...
    var body: some View {
        Text(timerInterval: since...Date.distantFuture, countsDown: false, showsHours: false)
            .font(Mono.medium(size))
            .monospacedDigit()
    ...
    var body: some View {
        ...
        case .working:
            if let started = session.started {
                Clock(since: started, size: 11, tint: Palette.dim)
            }
...
struct IslandTrailing: View {
    ...
    var body: some View {
        if state.waiting > 0 {
            ...
            Clock(since: started, size: 12, tint: Palette.text)
        } else {
