...
export default function TasksScreen() {
  ...
  const viewNesting = useMemo(() => {
    const byId = new Map<string, { indent: number; visibleDescendants: number }>();
    for (const rows of Object.values(groupedTasks)) {
      for (const row of buildTaskTree(rows)) {
        byId.set(row.task._id, { indent: row.indent, visibleDescendants: row.descendantCount });
      }
    }
    return byId;
  }, [groupedTasks]);
  ...
  const renderTaskGroup = useCallback(
    ...
    [groupedTasks, groupBy, router, updateTask, viewNesting, familyIndex],
  );
