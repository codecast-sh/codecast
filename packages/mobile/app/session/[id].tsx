...
import { parseInboundSessionMessage, isAgentMessage, parseAgentAuthoredMessage, isSessionUpdateBatch, parseSessionUpdateBatch, parseUserMessage, isScheduledTaskMessage, parseChatWakePrompt, parseHuddleSummaryTag, isToolResultCarrier, type ChatWakePrompt } from '@codecast/web/components/sessionMessage';
...
function SessionMessageBlock({ from, name, body, timestamp, variant }: { from: string; name?: string; body: string; timestamp?: number; variant?: 'update' | 'agent' }) {
  ...
  return (
    <RNView style={[styles.sessionMessageBlock, isAgentReport && { borderLeftColor: accent + '99', backgroundColor: accent + '0d' }]}>
      <RNView style={styles.sessionMessageHeader}>
        ...
        <RNText style={[styles.sessionMessageLabel, isAgentReport && { color: accent + 'b3' }]}>{isAgentReport ? 'Report from' : variant === 'update' ? 'Update from' : 'Message from'}</RNText>
        {hasRealSender ? (
