...
export type OrgTaskStatusChange = { kind: "task_status"; task: string; status: "done" | "dropped"; reason: string };
...
export const ORG_CHANGE_APPLY_RANK: Record<OrgChangeKind, number> = {
  plan_status: 0, task_status: 1, project_status: 2,
...
export const TASK_STATUS_CHANGES: readonly OrgTaskStatusChange["status"][] = ["done", "dropped"];
...
export function describeTenure(t: OrgTenureSpec | { kind: "standing" } | { kind: "program"; ends: { plan?: unknown; project?: unknown; date?: number }; then: "retire" | "review" } | null | undefined, names?: { plan?: string; project?: string }): string {
  ...
  const e: any = t.ends;
  const ends = e.plan !== undefined ? `with ${names?.plan ?? e.plan}` : e.project !== undefined ? `with ${names?.project ?? e.project}` : `on ${new Date(e.date).toISOString().slice(0, 10)}`;
