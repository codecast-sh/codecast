...
describe("routine readiness", () => {
  ...
  test("missing authority and evidence block, in order", () => {
    expect(routineReadiness(manifest, manifest.routines[1]!, state(), now).missing).toEqual(["authority ads-spend not granted", "evidence ads_read has no pass", "runs as propose: trust is understand"]);
  });
