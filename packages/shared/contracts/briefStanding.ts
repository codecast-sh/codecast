...
// its title or its short id. The date in parentheses at the end is the day
// the role wrote the sentence; it is the only clock the line has, so the
// frame and the panel both read a line's age from it. A line with no date
// has no age.
//
...
export function parseStandingLine(line: string): StandingLine | null {
  ...
  const project = strip(raw.slice(0, colon));
  let rest = raw.slice(colon + 1).trim();
  const date = rest.match(TRAILING_DATE);
  let written_on: string | null = null;
  let written_at: number | null = null;
  if (date) {
    const at = Date.parse(`${date[1]}T00:00:00Z`);
    if (!Number.isNaN(at)) { written_on = date[1]; written_at = at; rest = rest.slice(0, date.index).trim(); }
  }
