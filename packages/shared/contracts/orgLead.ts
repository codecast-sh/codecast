...
const wholeWorkspace = (r: LeadRole) => (r.scope?.project_ids ?? []).length === 0 && (r.scope?.plan_ids ?? []).length === 0;

export function leadScopeChange<R extends LeadRole>(projectId: unknown, role: R, roles: readonly R[]): LeadScopeChange<R> {
  if (wholeWorkspace(role)) return { kind: "whole_workspace" };
  ...
  if (parent && !wholeWorkspace(parent) && !scopeListsProject(parent, projectId)) return { kind: "outside_parent", parent };
  return { kind: "add" };
}
