...
describe("parseStandingSection", () => {
  test("reads the lines under the heading, the project before the colon, the date at the end", () => {
    const brief = [
      ...
      "- Billing (2026-09-20): invoices ship again since Tuesday",
      "- Empty:",
      "",
      "## Goals: Ashot",
      "- Not standing: a goal",
    ...
    expect(lines.map((l) => l.project)).toEqual(["Union goals", "pj-abc Growth", "Billing"]);
    expect(lines[2]).toMatchObject({ text: "invoices ship again since Tuesday", written_on: "2026-09-20" });
