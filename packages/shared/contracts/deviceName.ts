 ...
 *
 * Show a machine's chosen name without the OS prefix and .local suffix.
 * Provisioned hosts with generated IP or UUID names read by kind instead.
 */
...
/** Scaleway Mac hosts default to a UUID instead of a name a person chose. */
...
/** A clean display name: chosen names when present, generated hostnames by kind. */
export function deviceDisplayName(d: DeviceNameSource | undefined | null): string {
  ...
  if (d.is_remote && (!host.trim() || isCloudAssignedHostname(d.label) || MAC_UUID_HOSTNAME.test(host.replace(/\.local$/i, "")))) return "Remote Mac";
