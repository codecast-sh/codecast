...
export function routineReadiness(manifest: ReadinessManifest, routine: ReadinessRoutine, state: ReadinessState, now = Date.now()): RoutineReadiness {
  ...
  for (const id of routine.requires?.authority ?? []) {
    ...
    else if (grant.expires_at !== undefined && grant.expires_at <= now) { missing.push(`authority ${id} expired`); lapsed = true; }
  }
  for (const id of routine.requires?.evidence ?? []) {
    ...
    else if (maxAge !== undefined && now - record.observed_at > maxAge) { missing.push(`evidence ${id} is ${describeAge(now - record.observed_at)} old`); lapsed = true; }
  }
  const wantsApply = routine.mode === "apply";
  const acts = (routine.requires?.authority ?? []).some((id) => ACTING_KINDS.has(authorityKinds.get(id) ?? ""));
  let mode: "propose" | "apply" = wantsApply ? "apply" : "propose";
  if (wantsApply && acts && state.trust === "understand") { mode = "propose"; missing.push("runs as propose: trust is understand"); }
  if (wantsApply && lapsed) mode = "propose";
  // Ready means activation may be offered: nothing missing except the trust
  // note, which lowers the mode rather than blocking the routine.
  const blocking = missing.filter((m) => !m.startsWith("runs as propose"));
  return { ready: blocking.length === 0, mode, missing };
}
