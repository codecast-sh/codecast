...
function roleFlags(r: RoleSignals): HealthFlag[] {
  ...
  if (breaches.length) {
    ...
    const history = earlier ? `; flagged at ${plural(earlier, "earlier review")} in a row, so this is breach ${streak}` : "; first breach on record";
    flags.push({ code: "overloaded", severity: breaches.length >= 2 || streak >= STABILITY.split_after_breaches.value ? "blocker" : "warn", detail: `${who} holds ${breaches.join(", ")}${history}` });
  }
