...
export const FLAG_CODES = ["overloaded", "wide_span", "idle", "slow_to_recommend", "review_stall", "cap_hit", "unowned", "no_charter", "chatter"] as const;
...
function roleFlags(r: RoleSignals): HealthFlag[] {
  ...
  const over = (key: RoleCapacityKey, n: number, word: string, suffix = "") => { if (n > capacity(key)) breaches.push(`${plural(n, word)}${suffix} (model: ${capacity(key)})`); };
  over("open_tasks", r.load.open_tasks, "open task");
  over("in_flight", r.load.in_flight, "task", " in flight");
  over("active_plans", r.load.active_plans, "active plan");
  over("live_hands", r.load.live_hands, "live hand");
  if (breaches.length) flags.push({ code: "overloaded", severity: breaches.length >= 2 ? "blocker" : "warn", detail: `${who} holds ${breaches.join(", ")}` });
