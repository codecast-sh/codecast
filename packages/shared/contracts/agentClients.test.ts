...
describe("fork and switch capability helpers", () => {
  ...
  it("a native-fork client always declares fork", () => {
    for (const d of Object.values(AGENT_CLIENTS)) {
      if (d.forkCmd) expect(d.capabilities.fork).toBe(true);
    }
  });

  it("reconstitution is claude, codex, gemini, grok and pi — the daemon's transcript writers", () => {
    ...
    expect(agentReconstitutes("grok")).toBe(true);
    expect(agentReconstitutes("pi")).toBe(true);
    // SQLite-owned session stores: nothing codecast can write.
    for (const t of ["cursor", "opencode"]) expect(agentReconstitutes(t)).toBe(false);
  });

  it("a blank session can become any agent; one with history only a rebuildable one", () => {
    expect(canSessionBecomeAgent("cursor", 0)).toBe(true);
    expect(canSessionBecomeAgent("cursor", undefined)).toBe(true);
    expect(canSessionBecomeAgent("cursor", 12)).toBe(false);
    expect(canSessionBecomeAgent("opencode", 1)).toBe(false);
    expect(canSessionBecomeAgent("pi", 12)).toBe(true);
