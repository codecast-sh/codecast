...
export const AGENT_CLIENTS: Record<AgentClientId, AgentClientDescriptor> = {
  ...
  muse: {
    ...
    agentFileTargets: {
      // Project AGENTS.md verified via `muse init --dry-run` ("Muse Code reads
      // this file as project rules"). No user-level instruction file verified
      // (no inspect oracle exposes personal-rules loading), so `user` stays
      // honestly absent.
      instructionFile: { project: "AGENTS.md", format: "markdown" },
      // All three dirs verified live with a scratch HOME: `muse skills
      // install --scope user` lands at $CONFIG_DIR/skills/<name> (i.e.
      // ~/.config/muse/skills), and `muse skills list --source` shows
      // $HOME/.agents/skills under user and .agents/skills under project
      // (with --trust-workspace). No muse-native project dir: a planted
      // .muse/skills is ignored.
      skillsDir: {
        user: "~/.config/muse/skills",
        shared: "~/.agents/skills",
        sharedProject: ".agents/skills",
      },
      // `mcpServers` in ~/.config/muse/settings.json ({"schema_version":1,...}):
      // the path confirmed by the loader erroring without schema_version, the
      // key confirmed by the startup unknown-member oracle naming a bogus
      // sibling but not mcpServers, and entry resolution confirmed by
      // `muse mcp login <name>` starting OAuth for a planted entry. No
      // project-level settings file: planted .agents/settings.json and
      // .muse/settings.json are both ignored. Same surgical-edit rule as
      // codex's config.toml: a shared user-owned file, never rewrite.
      mcpConfig: { user: "~/.config/muse/settings.json", shape: "json_mcpservers" },
      // `muse mcp login|logout <server>` (OAuth for streamable-HTTP entries).
      // The native manager outranks the file write in capabilitySupport.
      nativeManager: { mcp: "muse mcp" },
      // OMITTED, honestly: hooksConfig, agentsDir, commands dir, pluginSettings
