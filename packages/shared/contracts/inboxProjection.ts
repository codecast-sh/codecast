...
// Deterministic 12h gap cut by updated_at over members NOT in a deliberate
// window: pinned, dismissed, stashed and owned rows are someone's explicit act
// and never fold (nor bridge a gap that should hide the caller's cruft). Rows
// outside the selection (CLI label extras, foreign channels) are fold exempt on
// every channel — they are simply not in `members`. Queued work
// (has_pending_messages) is about to move, so it never folds either. Fold never
// changes membership; it splits the tally and the default rendering.
export function computeFold(
  ...
  for (const [id, m] of members) {
    if (m.windows.some((w) => w !== "recent")) continue; // deliberate window: exempt
...
export function projectInbox<Row extends ProjectableInboxRow>(
  ...
  return { entries, placements, tally, set_digest: digestProjection(entries), truncated };
}
