...
- **One Chrome, many agents.** Each session owns one tab and sees only its own: \`tabs\` lists yours, \`tabs --all\` every agent's. Act only on yours, and \`--new-tab\` only for a genuine second page. To work in another agent's tab on purpose, name it: \`tab switch <id>\` shares it with your session (the human's own tabs can never be named). Nothing in a session's ordinary flow reaches another session's tab or the browser itself; \`stop --all\` and \`start --fresh\` are the deliberate exceptions and close everyone's tabs. State persists until \`cast browser stop\` (\`--wipe\` also removes the profile copy; \`start --fresh\` starts signed out). The clone holds the human's logins — treat that access as theirs. Modal dialogs are dismissed automatically and never block.
- **The human's Chrome first.** Sessions land in the human's Chrome whenever the extension is paired and connected: your first verb settles the session there, and \`cast browser target\` says which browser you are on. When the extension was paired but is not connected, a verb starts the bridge host and waits for the extension to find it, so a fresh boot or a dead host costs seconds. If a verb still reports the extension is not connected, Chrome is closed or the extension is disabled: tell the human and wait, rather than moving work that needs their logins to the clone. Only a machine that was never paired needs \`cast browser extension setup\`, run once by the human, and \`cast browser extension status\` tells the two cases apart. Your tabs there sit in the one \`Cast\` tab group among the human's own tabs: act only on tabs you opened, never on theirs. \`cast browser target real\` moves a session that settled on the clone; \`--real\` and \`--clone\` on a verb choose for one verb. \`grant\` and \`login\` run only on the clone; \`eval\` runs in both. Never move a session to the clone to stay out of the human's way, for a quick check, or to verify a page: their Chrome is the place for all of it, and a session that leaves it does so only because the human asked or because the extension is unavailable, said out loud either way.
