...
- **One Chrome, many agents.** Each session owns one background tab in the \`Cast\` tab group. \`tabs\` lists yours, \`tabs --all\` every agent's. Act only on yours, never the human's tabs. Use \`--new-tab\` only for a second page. \`tab switch <id>\` deliberately shares another agent's tab with your session. \`cast browser stop\` closes your session's tab. Modal dialogs are dismissed automatically.
- **Connection recovery.** \`cast browser target\` reports the browser; \`cast browser target real\` restores the human's Chrome if an earlier command chose something else. \`cast browser extension status\` checks the bridge. Commands start the bridge host if needed and wait for reconnection. If still disconnected, the human checks that Chrome is running and the extension is enabled. Pairing is the human's one-time \`cast browser extension setup\` step. A missing pairing, failed command, or unavailable verb is not permission to launch another browser.
...
The separate browser remains available for cases the human explicitly approves after the normal Chrome route cannot do the work, or when they specifically request it. Only then consult \`cast browser help start\` for the last-resort controls. Keep ordinary browser work on their Chrome.
${BROWSER_SNIPPET_END}
`;
...
export const SNIPPET_CATALOG: SnippetDescriptor[] = [
  ...
  },
  {
    ...
    detail:
      ...
      "own background tabs. All ordinary commands, including `start`, use your Chrome; " +
