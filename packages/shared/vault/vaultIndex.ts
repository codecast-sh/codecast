...
import { scanEntityRefs, type VaultEntityOccurrence } from "./entityRef";
...
export interface IndexedNote {
  ...
  /** Codecast objects this note links to. Kept beside the parse rather than
   *  inside it because the reference grammar is markdown links to app URLs,
   *  which the indexing parser strips to their display text. */
  entityRefs: VaultEntityOccurrence[];
}
...
/** The distinct objects a note references — one key per object, however many
 *  times the note links it. */
function entityKeysOf(note: IndexedNote): string[] {
  const keys = new Set<string>();
  for (const occurrence of note.entityRefs ?? []) keys.add(occurrence.ref.key);
  return [...keys];
}

/** Always sorted: candidate order decides which file an ambiguous link lands
 *  on, so it must not depend on the order files happened to be indexed in. */
const sortedPaths = (set: Set<string> | undefined): string[] =>
  set ? [...set].sort((a, b) => (a < b ? -1 : a > b ? 1 : 0)) : [];

/** One name, several files: prefer a candidate in the linking file's own
 *  folder (Obsidian's rule), else the alphabetically first — and say it was
 *  ambiguous either way so the UI can offer to disambiguate. */
function pickCandidate(
  candidates: string[],
  fromPath: string,
): { path: string; isAmbiguous: boolean; candidates: string[] } {
...
export class VaultIndex {
  ...
  /** entity key (`task:ct-40561`) → files referencing that object. Local, so
   *  "which notes mention this task" costs nothing at the server. */
  ...
  upsert(path: string, content: string): void {
    ...
    const note: IndexedNote = {
      ...
      parsed,
      entityRefs: parsed ? scanEntityRefs(content) : [],
    };
  ...
  rename(oldPath: string, newPath: string): void {
    ...
    // Renaming ONTO an existing path replaces it — that is what rename(2) does
    // on disk and what the UI's overwrite prompt means. Retire the clobbered
    // file completely; leaving its aliases, tags and backlinks registered would
    // strand them under a path whose content is now somebody else's.
    const clobbered = this.notes.get(newPath);
    if (clobbered) {
      for (const key of this.detach(clobbered)) touched.add(key);
      this.notes.delete(newPath);
      this.dropOutgoing(newPath);
    }
  ...
  /** Every indexed path, sorted — attachments included, since embeds resolve
   *  to them. Filter with `isVaultMarkdownPath` for note-only views. */
  paths(): string[] {
  ...
  resolveLinkInfo(
    ...
    const named = byKey.length ? byKey : sortedPaths(this.nameIndex.get(lc(stripKnownExtension(norm))));
    if (named.length) return pickCandidate(named, fromPath);

    // 3. Aliases — a real filename always wins over someone's alias.
    const aliased = sortedPaths(this.aliasIndex.get(key));
    if (aliased.length) return pickCandidate(aliased, fromPath);
  ...
  /** The codecast objects this note links, in document order. */
  entityRefs(path: string): VaultEntityOccurrence[] {
    return this.notes.get(path)?.entityRefs ?? [];
  }

  /** Notes that reference one object, by its `entityRefKey`. This is the honest
   *  local half of "what connects to this task": the vault knows which of its
   *  own notes point at the object. Whether the object points BACK — a task
   *  description linking a note — lives on the server and is not answerable
   *  from here. */
  notesReferencingEntity(key: string): string[] {
    return sortedPaths(this.entityIndex.get(key));
  }

  /** Every object the vault references at least once, sorted by key. */
  referencedEntities(): { key: string; notes: string[] }[] {
    return [...this.entityIndex.keys()]
      .sort()
      .map((key) => ({ key, notes: sortedPaths(this.entityIndex.get(key)) }));
  }
  ...
  private attach(note: IndexedNote): Set<string> {
    ...
    // Entity keys are NOT added to `touched`: they name codecast objects, not
    // vault files, so no wiki link's resolution can depend on them.
    for (const key of entityKeysOf(note)) addTo(this.entityIndex, key, note.path);
    return touched;
  }
  ...
  private detach(note: IndexedNote): Set<string> {
    ...
    for (const key of entityKeysOf(note)) removeFrom(this.entityIndex, key, note.path);
    return touched;
  }
