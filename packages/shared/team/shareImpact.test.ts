...
describe("labels", () => {
  ...
  test("the menu actions say what one click exposes", () => {
    const m = shareMenuActions(impact, NOW);
    expect(m.fromToday).toEqual({ label: "Share from today", detail: "New sessions only. The 100 sessions you already have stay private." });
    expect(m.everything).toEqual({ label: "Share all 112 sessions", detail: "Everything, Jun 3 to today." });
    const empty = shareMenuActions({ ...impact, sessions: 0, older: 0, olderThanDay: 0, first: null, last: null }, NOW);
    expect(empty.fromToday.detail).toBe("New sessions only. Nothing older there.");
    expect(empty.everything.label).toBe("Share everything");
    const hidden = shareMenuActions({ ...impact, hidden: 3 }, NOW);
    expect(hidden.everything.detail).toBe("Everything, Jun 3 to today. 3 sessions you hid by hand stay hidden.");
  });
