...
test("sync status acknowledgment requires an applied mutation and credentials", async () => {
  const sync = new SyncService({ convexUrl: "http://localhost:0", userId: "u", authToken: "test" });
  let fail = true;
  (sync as any).client = { mutation: async () => { if (fail) throw new Error("offline"); return { applied: true }; } };
  ...
  for (const result of [undefined, null, {}, { applied: false, reason: "missing_session" }, { applied: false, reason: "stale_status" }]) {
    (sync as any).client = { mutation: async () => result };
    expect(await sync.updateSessionAgentStatus("conv", "connected", 3, undefined, undefined, undefined, null)).toBe(false);
  }
