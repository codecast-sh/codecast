...
export async function runAgentPrompt(args: string[]): Promise<void> {
  ...
  if (!explicit && process.env.TMUX) {
    const pane = await tmuxRunAsync(["display-message", "-p", "-t", process.env.TMUX_PANE || "", "#{@codecast_session_id}\t#{session_name}\t#{session_created}"]);
    if (pane.status === 0) {
      const [session, name, created] = pane.stdout.trimEnd().split("\t");
      source = resolveAgentPromptSource(session, cache);
      if (!source) {
        const launch = read("tmux-spawns.json")[name];
        const elapsed = Number(created) * 1000 - launch?.timestamp;
