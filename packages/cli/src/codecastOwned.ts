...
export function isCodecastOwnedHomePath(rel: string): boolean {
  ...
  // `.codecast-owned.<file>` beside a settings file is the ownership ledger applyOwnedJson keeps (capabilities/ownedJson.ts).
  if (path.posix.basename(clean).startsWith(".codecast-owned.")) return true;
