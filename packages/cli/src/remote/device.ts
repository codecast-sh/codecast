...
const DEVICE_BINDING_FILE = path.join(defaultConfigDir(), ".device_binding.json");
const CONFIG_FILE = path.join(defaultConfigDir(), "config.json");
...
/** Touch this file in the config dir to make a machine a remote device. */
export const REMOTE_DEVICE_MARKER = "remote-device";
let remoteMarkerPresent: boolean | null = null;

/**
 * True when this process runs on a remote box (a cloud Mac or Linux host). A
 * remote device only serves conversations explicitly moved to it (owner == this
 * device) — it must never adopt, resume, or spawn sessions on its own, and it
 * never rotates the pushed copy of the primary's Claude login it runs on.
 *
 * Two ways to declare it. The Linux provisioner sets CODECAST_REMOTE_DEVICE=1
 * in the daemon's systemd unit. A Mac's LaunchAgent is generated by codecast
 * with no environment of its own, so a Mac remote is marked by the file
 * ~/.codecast/remote-device instead (`touch` it, restart the daemon). Without
 * either, a cloud Mac registered with `cast hosts add` runs a full primary
 * daemon: it refreshed the pushed login on its own schedule and, the refresh
 * token being single use, logged the laptop out about once a day (InterGalactic,
 * 2026-09-13 through 09-16).
 */
export function isRemoteDevice(): boolean {
  if (process.env.CODECAST_REMOTE_DEVICE === "1") return true;
  if (remoteMarkerPresent === null) {
    remoteMarkerPresent = fs.existsSync(path.join(defaultConfigDir(), REMOTE_DEVICE_MARKER));
  }
  return remoteMarkerPresent;
}

/** Forget the cached marker lookup. Tests only. */
export function resetRemoteDeviceForTests(): void {
  remoteMarkerPresent = null;
}
