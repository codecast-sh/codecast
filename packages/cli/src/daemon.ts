...
import { TranscriptDirWatcher, transcriptDirWatcherConfig, decodePiCwdSlug, type TranscriptDirEvent } from "./transcriptDirWatcher.js";
import {
...
async function executeRemoteCommand(
  ...
  try {
    switch (command) {
      ...
      case "start_session": {
        const parsed = commandArgs ? JSON.parse(commandArgs) : {};
        // parsed.agent_type comes off JSON.parse (untyped), and the queue may carry
        // either the daemon spelling (AgentClientId) or the convex spelling
        // ("claude_code"). fromConvexAgentType is the ONE normalization for both —
        // it maps every known client (incl. pi) to its AgentClientId and everything
        // else to "claude", reproducing the old codex/cursor/gemini-else-claude
        // ternary without another hand-rolled branch that silently drops pi.
        const agentType: AgentClientId = fromConvexAgentType(parsed.agent_type);
      ...
      case "resume_session": {
        ...
        // undefined here means "claude/unknown — use the default claude resume path";
        // a defined AgentClientId routes to that client's own resume. Normalize via the
        // single fromConvexAgentType (handles pi + the convex spelling), then collapse
        // claude back to undefined to preserve the original semantics.
        ...
        if (resumed) {
          ...
          // cursor-agent owns its own session store, and pi owns its own JSONL tree
          // (which lives under ~/.pi and is never a Claude transcript) — for neither
          // may we fabricate a Claude JSONL. Skip DB reconstitution for both and fall
          // through to the fresh cursor/pi spawn below.
          if (config?.convex_url && config?.auth_token && resumeAgentType !== "cursor" && resumeAgentType !== "pi") {
            try {
              const siteUrl = config.convex_url.replace(".cloud", ".site");
...
async function autoResumeSessionInner(sessionId: string, content: string, titleCache: TitleCache, cwdOverride?: string, conversationId?: string, agentTypeHint?: AgentClientId): Promise<boolean> {
  ...
  // pi owns its JSONL tree under ~/.pi; a missing file must NOT be reconstituted as a
  // fake Claude transcript (mirrors cursor). With the file absent, pi falls through to
  // the refuse branch below — an honest failure, not a fabricated claude session.
  const isPiResume = agentTypeHint === "pi";
  if (!isCursorResume && !isPiResume && !sessionFile && conversationId && config?.auth_token && config?.convex_url) {
  ...
  try {
    ...
    // The four glyph-prompt clients settle on ❯/›; pi has no prompt glyph (its
    // composer is a box + status bar), so a pi resume watches for its registry
    // readiness pattern (the status-bar context-budget segment) instead.
    const promptPattern = agentType === "pi" ? AGENT_CLIENTS.pi.promptReadyPattern : /[❯›]/;
