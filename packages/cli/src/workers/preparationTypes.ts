...
export type PreparationPage = Identity & { tokens: IngestToken[]; done: boolean; rss: number; peakRss: number; computeRemainingMs: number };
...
export function validPreparationPage(v: unknown): v is PreparationPage {
  return object(v) && keys(v,['cursor','generation','sequence','tokens','done','rss','peakRss','computeRemainingMs']) && id(v.cursor) && id(v.generation) && count(v.sequence) && tokens(v.tokens) && typeof v.done === 'boolean' && count(v.rss) && count(v.peakRss) && Number.isFinite(v.computeRemainingMs) && v.computeRemainingMs >= 0 && v.computeRemainingMs <= PREPARATION_COMPUTE_MS && Buffer.byteLength(JSON.stringify(v)) <= INGEST_PAGE_BYTES;
}
