...
for(const runtime of ['source','node','bun','compiled']) test.skipIf(runtime!=='source'&&!built)(`${runtime} actual CLI ingest entry reads file and SQLite without daemon boot`,async()=>{
  ...
  try {
    ...
    const parsed=await read('claude',file);expect(parsed.messages[0].content).toBe(text);expect(parsed.bytesConsumed).toBe(fs.statSync(file).size);expect(parsed.signatures).toHaveLength(1);expect(host.state.pid).toBeGreaterThan(1);
