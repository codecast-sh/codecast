...
test('normal-return retained work arms a due wake from cold and survives pause and stop',async()=>{
  const root=fs.mkdtempSync(path.join(os.tmpdir(),'f3-retry-retained-')),file=path.join(root,'source');fs.writeFileSync(file,'unchanged');
  let now=0,allowed=true,retain=true,runs=0;
  const owner=new TranscriptRetryOwner({allowed:()=>allowed,retryable:()=>false,now:()=>now,wakeDelayMs:1}),map=new Map();
  try{
    const sync=owner.create(map,file,{file,sessionId:'full',client:'claude'},async()=>{runs++;if(retain)expect(owner.defer(file,'full')).toBe(true);},{maxRetries:1});
    expect(owner.defer(file,'other')).toBe(false);
    await sync.invalidateAndAwait();expect(runs).toBe(1);now++;
    allowed=false;expect(owner.drain()).toBe(0);allowed=true;retain=false;
    expect(owner.drain()).toBe(1);await sync.awaitQueue();expect(runs).toBe(2);expect(owner.drain()).toBe(0);
    retain=true;await sync.invalidateAndAwait();sync.stop();now++;expect(owner.defer(file,'full')).toBe(false);expect(owner.drain()).toBe(0);
    expect(runs).toBe(3);
