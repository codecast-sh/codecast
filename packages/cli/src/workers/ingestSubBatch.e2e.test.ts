...
for(const scenario of [{mutant:false,kind:undefined},{mutant:true,kind:'direct'},{mutant:true,kind:'pending'}])for(const enabled of [false,true])test(`real service sub-batch custody worker=${enabled} mutant=${scenario.mutant} path=${scenario.kind??'all'}`,async()=>{
  ...
  try{
    if(mutant){
      const original=fs.readFileSync(path.resolve(import.meta.dir,'../syncService.ts'),'utf8'),needle='if (options?.onBatchAccepted) options.onBatchAccepted(batch.map(message => preparedIndexes!.get(message)!));';
    ...
    if(mutant){
      ...
      expect(row.review).toEqual({realService:true,directPending:true,acceptedSubset:25,lateFences:3,accounting:true,reconcileIndexes:true,unknownNoReceipt:true});
