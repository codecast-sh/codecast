...
export function configureDaemonWorkers(enabled: boolean, options: WorkerHostOptions = {}, scanOptions: WorkerHostOptions = {}, ingestOptions: WorkerHostOptions = {}): WorkerHost | null {
  closeDaemonWorkers();
  if (enabled && process.env.CODECAST_WORKER !== "1" && process.platform !== "win32") {
    ...
    ingest = new WorkerHost("ingest", ingestOptions);
  }
  return probe;
}
