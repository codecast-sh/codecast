...
test('an old HOME scan cannot install over a newer root or clear its inflight build',async()=>{
 ...
 await start(true);process.env.HOME=a;resetSessionFileIndexForTests();const old=refreshSessionFileIndex();
...
test('unknown rollout ownership fails closed while worker scan is pending despite a Claude mirror',async()=>{
 const home=temp();process.env.HOME=home;await start(true);resetSessionFileIndexForTests();const id='88888888-8888-4888-8888-888888888888';
...
test('worker roots and skills retain honest HOME ownership and refresh without synchronous walks',async()=>{
 ...
 process.env.HOME=a;await start(true);invalidateLocalProjectRoots();resetSkillsScanMemoForTests();expect(computeLocalProjectRoots()).toEqual([]);await refreshLocalProjectRoots();const first=computeLocalProjectRoots();expect(first).toContain(path.join(a,'src','one'));expect(computeLocalProjectRoots()).toBe(first);expect((await readAvailableSkills()).map(s=>s.name)).toContain('one');
...
test('unreadable fresh rollout evidence cannot become owned and root outage retains last good',async()=>{
 const home=temp();process.env.HOME=home;await start(true);resetSessionFileIndexForTests();const root=path.join(home,'.codex/sessions');fs.mkdirSync(root,{recursive:true});fs.mkdirSync(path.join(home,'src','good'),{recursive:true});
