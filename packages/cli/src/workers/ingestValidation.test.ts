...
const result = () => ({messages:[{role:'assistant' as const,content:'hello',timestamp:1}],bytesConsumed:100,fileSize:100,totalCount:1,maxRowId:0,signatures:['a'.repeat(64)],messageTitles:[null],handoffParents:[null],metadata:{warnings:[]}});
...
test('shared schema enforces client counts, signatures, metadata and nested record types',async()=>{
  ...
  for (const mutate of [
    ...
    (r:any)=>r.maxRowId=1,
    (r:any)=>r.signatures=[],
    (r:any)=>r.receiptSignatures=['a'.repeat(64)],
  ...
  const cursor={...result(),bytesConsumed:0,totalCount:3,maxRowId:9};
