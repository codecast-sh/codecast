...
test('shared schema enforces client counts, signatures, metadata and nested record types',async()=>{
  ...
  const cursor={...result(),receiptOccurrences:[5],rawBubbleCount:6,bytesConsumed:0,totalCount:3,maxRowId:9};
...
test('Cursor raw ordinals use snapshot bubble extent across filtering and count windows',async()=>{
  const cursorJob={...job,client:'cursorDb' as const,offset:2};
  const cursor={...result(),bytesConsumed:0,totalCount:4,maxRowId:9,rawBubbleCount:10_002,
  ...
  for(const receiptOccurrences of [[10_000,10_000],[10_001,10_000],[-1,10_000],[10_000,10_002],[10_000,1.5]]){
    await expect(validateIngestResult({...cursor,receiptOccurrences},cursorJob)).rejects.toThrow('schema');
  }
  await expect(validateIngestResult({...result(),rawBubbleCount:1},job)).rejects.toThrow('schema');
});
