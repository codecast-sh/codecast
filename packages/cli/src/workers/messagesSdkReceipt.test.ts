...
test('actual SDK receipt precedes log transfer and survives only later local failures', async () => {
  ...
  try {
    ...
    for (const scenario of scenarios) {
      ...
      const naturalCleanup = { sendSettled: true, closeObserved: false, cursors: cursors.size, hostPid: state.pid, generation: state.generation };
      emit({ event: 'receipt-natural-cleanup', scenario, ...naturalCleanup });
      ...
      const report = { scenario, result, offered, custody: batch.custody, receipt: batch.acceptedReceipt, valuePages, logs: logs.length, logParity: JSON.stringify(logs) === JSON.stringify(oldLogs), request: requests.at(-1), events, naturalCleanup, fixtureCleanup: { used: naturalCleanup.cursors > 0, reason: 'original forwarding host has no live-child close route', cursorsAfter: cursors.size } };
    ...
    for (const report of reports) {
      ...
      expect(report.naturalCleanup.sendSettled).toBe(true); expect(report.naturalCleanup.cursors).toBe(1);
      expect(report.fixtureCleanup.used).toBe(true); expect(report.fixtureCleanup.cursorsAfter).toBe(0); expect(report.request).toBe(requests[0]);
