...
test('actual SDK receipt precedes log transfer and survives only later local failures', async () => {
  ...
  try {
    ...
    emit({ event: 'receipt-baseline', baseline, logs: oldLogs.length, request: requests[0], responseBytes: Buffer.byteLength(body), responseSha256: createHash('sha256').update(body).digest('hex'), runtime: process.versions });
    for (const scenario of scenarios) {
      ...
      const host = { state, request: async (_operation: string, payload: unknown) => {
        ...
        if (payload.action === 'waitOutcome') {
          ...
          if (scenario === 'malformed-receipt') page = { ...page, receipt: { ...actualReceipt, extra: true } } as MessagesSdkPage;
        }
        return page;
    ...
    emit({ event: 'receipt-cleanup', listening: server.listening, requests: requests.length, cases: reports.length });
  }
});
