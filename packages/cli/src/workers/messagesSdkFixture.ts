...
if (process.argv.includes('_worker')) {
  ...
  try {
    ...
    for (const fixture of [
      ...
      emit({ event: 'case-outcome', case: fixture.name, before: describe(old), after: describe(newer), oldLogs, newLogs, custody: batch.custody, acceptedOutcome: batch.acceptedOutcome ? convexToJson(batch.acceptedOutcome as any) : null, requests: requests.slice(-2), child: host.state });
    ...
    const throwingResult = await observe(() => throwing.send({ getApiToken: () => 'synthetic-token', dispatch: init => fetch(`${url}/api/mutation`, init), onAccepted: value => { receipt.push(value); }, logger: () => { throw new Error('owned logger failure'); } }));
    emit({ event: 'case-outcome', case: 'throwing-logger', result: describe(throwingResult), receipt, custody: throwing.custody, acceptedOutcome: throwing.acceptedOutcome, child: host.state });
