...
export async function coreReview(f:any) {
  ...
  service.client={mutation:async(name:string,args:any,options:any)=>{
    ...
    const ids=args.messages.map((message:any)=>{
      const existing=message.message_uuid&&stored.find(row=>row.conversationId===args.conversation_id&&row.message_uuid===message.message_uuid);
      if(existing)return existing.id;
      const row={conversationId:args.conversation_id,...message,id:'row-'+stored.length};stored.push(row);return row.id;
    });
    await afterWrite?.(number);return {inserted:args.messages.length,ids};
  },query:async()=>[]};
  ...
  if(!selected||selected==='c1')for(const change of ['delete','replace','pending-append-replace']){
    ...
    const partialPending=pending[id].length;assert.equal(receipts(id).length,25);
  ...
  return {partialCleanup:3,fenceCases:16,expiredWaits:3,networkQueue:true,walOrdinal:true,globalAdmission:true,scheduledRetained:true};
}
