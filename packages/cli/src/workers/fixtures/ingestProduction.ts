...
try {
  ...
  await claude(exhaustion,'empty-exhaustion');assert.equal(getPosition(exhaustion),25*1024*1024);assert.ok(!rows.has('after-empty-exhaustion'));
  ...
  await d.processCursorSession(dbFile,'sqlite',home,sync,'user',undefined,cache,queue,pending,()=>{});assert.equal(getPosition(dbFile),3);
  db.close();
  ...
  const retry=file('retry',claudeLine('retry-data','durably queued'));sendFailure=true;await claude(retry,'retry');sendFailure=false;assert.equal(getPosition(retry),0);assert.ok(queued.some(q=>q.type==='addMessages' && q.params.messages.some((m:any)=>m.messageUuid==='retry-data')));
  ...
  let custodyReceipt;
  if (process.argv[3] === 'custody') { const {custody}=await import('./ingestCustody.js');custodyReceipt=await custody({d,home,getPosition,claudeLine}); }
  ...
  console.log(JSON.stringify({enabled,production:true,formats:8,parentParses,workerPid:pid,serialized:true,positions:true,queued:true,fallbackParses,adversarial:adversarialReceipt,custody:custodyReceipt,registration:registrationReceipt}));
