...
import * as localCursorParser from '../../cursorChatParser.js';
...
const parseCursorChatData = localCursorParser.parseCursorChatData;
...
mock.module('../../cursorChatParser.js',()=>({...localCursorParser,parseCursorChatData:(...args:Parameters<typeof parseCursorChatData>)=>{
  if(fallbackAllowed) fallbackParses++; else parentParses++;
  if(enabled&&!fallbackAllowed)throw new Error('supervisor Cursor database parse');
  return parseCursorChatData(...args);
}}));
...
if (['mutant','custody-mutant','custody','retry-mutant','ack','ack-mutant','ack-restart','registration','task-review','scheduled-review','custody-review','disappearance-review','emission-review','sub-batch-review','core-review'].includes(process.argv[3])) {
  ...
  if (['custody-review','disappearance-review','emission-review','sub-batch-review','core-review'].includes(process.argv[3])) { source += '\nexport const fixtureCustody={acceptedPending,acceptedPendingOwners,transcriptAckObserved,injectedMessageTs,geminiSyncedCounts,opencodeSyncedCounts,transcriptRetryOwners,reserveTranscriptReceipts,ingestStat,observeTranscriptAckWindow,receiptAccounting:()=>({count:acceptedPendingCount,bytes:acceptedPendingBytes,reserved:acceptedPendingReserved,reservedBytes:acceptedPendingReservedBytes})};\n';
  if (process.argv[3] === 'core-review') source += '\nexport const fixtureCore={publishPendingTranscript,retainedPendingWeights,pruneAcceptedTranscript,saveDaemonState,handleAuthFailure,resetAuthFailureCount,piSyncedSigs};\n';
  }
  ...
  if (process.argv[3] === 'core-review' && process.env.F3_FIXTURE_CORE_MUTANT) {
    const name=process.env.F3_FIXTURE_CORE_MUTANT;
    const mutations:Record<string,[string,string]>={
      c1:['    retirePendingTranscript(pending,sessionId,accepted);',''],
      'c2-queue':['    if (dispatchRefused) throw err;',''],
      c3:['transcriptRetryOwners.defer(','(() => false)('],
      c5:['  if(pendingTranscriptVersions.get(pending)!==version){transcriptCapacityRefused(sessionId,"pending publication changed");return;}',''],
    };
    const mutation=mutations[name];
    if(mutation){assert.ok(source.includes(mutation[0]));if(name!=='c3')assert.equal(source.split(mutation[0]).length,2);source=source.replaceAll(mutation[0],mutation[1]);}
  }
...
if (process.env.F3_FIXTURE_CORE_VALIDATOR) await import('./ingestCoreValidationControl.js');
...
try {
  ...
  if (process.argv[3] === 'core-review') {
    const {coreReview}=await import('./ingestCoreReview.js');
    const review=await coreReview({d,home,getPosition,setPosition});
    console.log(JSON.stringify({review,enabled,parentParses,workerPid:ingestWorkerHost()?.state.pid}));
    await closeFixtureWorkers();process.exit(0);
  }
  if (process.argv[3] === 'sub-batch-review') {
    const {subBatchReview}=await import('./ingestSubBatch.js');
    const review=await subBatchReview({d,home,getPosition});
    console.log(JSON.stringify({review,enabled,parentParses,workerPid:ingestWorkerHost()?.state.pid}));
    await closeFixtureWorkers();process.exit(0);
  }
