...
export async function custodyReview(f:any) {
  ...
  for(const kind of ['ack-throw','mapping-same','mapping-new','deadline','cancel','source-change']) {
    ...
    during=async()=>{
      ...
      if(kind==='deadline')await new Promise(resolve=>setTimeout(resolve,25));
      if(kind==='cancel')controller.abort();
      if(kind==='source-change'){fs.writeFileSync(p+'.next','assistant:\nreplacement occurrence\n');fs.renameSync(p+'.next',p);expected=1;}
    };
    try {
      const run=()=>cursor(p,id);
      if(kind==='deadline')await assert.rejects(withTranscriptDeadline(async()=>{await readTranscriptIngest({client:'cursor',file:p,sessionId:id,offset:baseline});(currentTranscriptDeadline() as any).end=performance.now()+10;await run();}));
  ...
  try{
    ...
    await until(()=>internal.geminiSyncedCounts.get(many)===2500);await tick();
  ...
  return {lateAcceptance:6,countMapping:true,legacyFreshAppend:true,mismatchCold:true,evictedReplacement:true,countReplacement:true,receiptCapacity:2048,healthyProgress:true,oversizedPendingBypass:true,scheduledOccurrences:2500};
}
