...
export async function custodyReview(f:any) {
  ...
  for(const client of ['claude','cursor','codex']) for(const concurrent of [false,true]) {
    ...
    await run();const position=getPosition(p),before=sent.length,stat=fs.promises.stat.bind(fs.promises);let observations=0;
    fs.promises.stat=(async(...args:any[])=>{const result=await (stat as any)(...args);if(String(args[0])===p&&++observations===2){fs.writeFileSync(p+'.next',replacement);fs.renameSync(p+'.next',p);if(concurrent)setPosition(p,position+1);}return result;}) as typeof fs.promises.stat;
    try{await assert.rejects(run(),/generation changed after offset lookup/);}finally{fs.promises.stat=stat;}
