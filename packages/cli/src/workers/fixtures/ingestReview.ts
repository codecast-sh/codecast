...
export async function taskReplay(f:any) {
  ...
  let duringPrimary:(()=>void)|undefined;
  ...
  const sync:any=new Proxy({
    ...
    offloadImages:async()=>{},
    addMessages:async(p:any)=>{sent.push(p);const action=duringPrimary;duringPrimary=undefined;action?.();if(primaryFailure)throw new Error('primary outage');return {ids:p.messages.map((_:any,i:number)=>String(i))};},
  ...
  const lostCalls=calls.filter(c=>c.conversation_id==='tasks-lost');
  ...
  d.fixtureReview.planModeTaskMap.set(legacyId,new Map([['1','ct-existing-one'],['2','ct-existing-two']]));
  ...
  const alternate=b+'.alternate.jsonl',alternateCreates=calls.length;fs.writeFileSync(alternate,fs.readFileSync(b));await run(alternate,'tasks-lost');assert.equal(calls.length,alternateCreates);
  const replacementCreates=calls.length;
  const newNative=fs.readFileSync(b,'utf8').replaceAll('tasks-lost-assistant','tasks-lost-new-assistant').replaceAll('tasks-lost-one','tasks-lost-new-one').replaceAll('tasks-lost-two','tasks-lost-new-two').replace('"taskId":"2"','"taskId":"4"');
  fs.writeFileSync(b+'.new-native',newNative);fs.renameSync(b+'.new-native',b);await run(b,'tasks-lost');
  assert.equal(calls.length,replacementCreates+2);assert.equal(tables.tasks.length,8);assert.equal(updates.at(-1)[0],tables.tasks[7].short_id);
  ...
  const observations=d.fixtureReview.permissionJustResolved;
  for(const change of ['add','delete-add','clear-add','clear']){
    fs.appendFileSync(flags,claudeLine('permission-'+change,'y')+claudeLine('primary-'+change,'new primary '+change));
    observations.add(id);const token=observations.capture(id);
    duringPrimary=()=>{if(change.startsWith('delete'))observations.delete(id);if(change.startsWith('clear'))observations.clear();if(change!=='clear')observations.add(id);};
    await run(flags,id);assert.equal(getPosition(flags),fs.statSync(flags).size);
    assert.ok(sent.at(-1).messages.every((m:any)=>m.content!=='y'));
    assert.equal(observations.has(id),change!=='clear');assert.notEqual(observations.capture(id),token);
  }
  fs.appendFileSync(flags,claudeLine('permission-only','y'));observations.add(id);const beforeOnly=sent.length;await run(flags,id);
  assert.equal(observations.has(id),false);assert.equal(sent.length,beforeOnly);assert.equal(getPosition(flags),fs.statSync(flags).size);
  return {backend:true,unknownBefore:true,lostResponse:true,nativeReplacement:true,legacyUpdate:true,numericUpdate:true,mappingFence:true,oneShotReplay:true,permissionEpoch:true};
}
