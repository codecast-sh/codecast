...
export async function scheduledReview(f:any){
  ...
  try{
    ...
    return {formats:8,scheduled:true,zeroManaged:true,unchanged:true,burstExhausted:true,stopped:true};
  }finally{if(timer)clearInterval(timer);owner.stop();db.close();op.close();}
}
