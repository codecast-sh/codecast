...
export async function subBatchReview(f:any) {
  ...
  const timer=globalThis.setTimeout,timedOut:number[][]=[];let intercepted=0,finish:undefined|((value:any)=>void);
  service.client.mutation=()=>new Promise(resolve=>{finish=resolve;});
  ...
  try{
    ...
    assert.equal(intercepted,1);assert.ok(finish);finish({inserted:1,ids:['late-unknown']});await new Promise(resolve=>setImmediate(resolve));assert.deepEqual(timedOut,[]);
  }finally{globalThis.setTimeout=timer;finish?.({inserted:1,ids:['late-unknown']});}
  return {realService:true,directPending:true,acceptedSubset:25,lateFences:3,accounting:true,reconcileIndexes:true,unknownNoReceipt:true,callbackCaptured:true,timeoutNoReceipt:true};
}
