...
export async function ingestRetainedWeight(value: unknown, checkpoint: () => void = () => {}): Promise<number> {
  let weight = 0;
  for await (const tokens of ingestTokenPages(value)) {
    for (const token of tokens) weight += 2 * (Buffer.byteLength(JSON.stringify(token)) + 1) + 64;
    checkpoint();
    await new Promise<void>(resolve => setImmediate(resolve));
  }
  checkpoint();
