...
import type { MessagesSdkPayload } from "./messagesSdkTypes.js";
import { PayloadCodecCursors } from "./payloadCodecCursors.js";
import type { PayloadCodecPayload } from "./payloadCodecTypes.js";
...
import type { IngestPayload } from "./ingestTypes.js";
import { PreparationCursors } from "./preparationCursors.js";
import type { PreparationPayload } from "./preparationTypes.js";
export function runWorker(kind: string): void {
  ...
  const ingests = new IngestCursors();
  const preparations = new PreparationCursors();
  const messagesSdk = new MessagesSdkCursors();
  const payloadCodec = new PayloadCodecCursors();
  ...
  process.stdin.on("data", (chunk: Buffer) => {
    try {
      for (const frame of decoder.push(chunk)) {
        ...
        if (frame.operation === "messagesSdk" || frame.operation === "payloadCodec") {
          inflight.add(frame.id!);
          const deadline = setTimeout(finish, remaining);
          const operation = frame.operation;
          const work = operation === "messagesSdk"
            ? messagesSdk.request(frame.payload as MessagesSdkPayload, frame.id!)
            : payloadCodec.request(frame.payload as PayloadCodecPayload, frame.id!);
          void work.then(result => {
            if (Date.now() >= frame.deadline!) return finish();
            send({ v: 1, kind: frame.kind, type: "result", id: frame.id, operation, result });
          }, () => {
            send({ v: 1, kind: frame.kind, type: "error", id: frame.id, error: "operation_failed" });
          }).finally(() => { clearTimeout(deadline); inflight.delete(frame.id!); });
          continue;
        }
        if (frame.operation === "prepare") {
          inflight.add(frame.id!);
          const deadline = setTimeout(finish, remaining);
          void preparations.request(frame.payload as PreparationPayload, frame.id!).then(result => {
            if (Date.now() >= frame.deadline!) return finish();
            send({ v: 1, kind: frame.kind, type: "result", id: frame.id, operation: "prepare", result });
          }, () => {
            send({ v: 1, kind: frame.kind, type: "error", id: frame.id, error: "operation_failed" });
          }).finally(() => { clearTimeout(deadline); inflight.delete(frame.id!); });
          continue;
        }
        if (frame.operation === "ingest") {
          inflight.add(frame.id!);
          const deadline = setTimeout(finish, remaining);
          void ingests.request(frame.payload as IngestPayload, frame.id!).then(result => {
            if (Date.now() >= frame.deadline!) return finish();
            send({ v: 1, kind: frame.kind, type: "result", id: frame.id, operation: "ingest", result });
          }, () => {
            send({ v: 1, kind: frame.kind, type: "error", id: frame.id, error: "operation_failed" });
          }).finally(() => { clearTimeout(deadline); inflight.delete(frame.id!); });
          continue;
