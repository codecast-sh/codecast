...
import { ByteReceiver, bytePage, describeBytes, type BytePage } from './payloadBytes.js';
...
type Attempt = {
  ...
  credit?: ReturnType<typeof deferred<void>>;
...
export class MessagesSdkCursors {
  ...
  private stop(cursor: Cursor) {
    ...
    attempt.fetch.reject(error); attempt.captured.reject(error); attempt.credit?.reject(error);
    if (!attempt.responseEnded) attempt.response?.error(error);
    ...
    try {
      ...
      if (payload.action === 'push') {
        ...
        if (payload.action === 'responseStart') {
          if (attempt.receiver) throw new Error('messages SDK response duplicate');
          attempt.receiver = new ByteReceiver();
          const body = payload.body ? new ReadableStream<Uint8Array>({
            start: controller => { attempt.response = controller; },
            pull: () => { attempt.credit?.resolve(); attempt.credit = undefined; },
            cancel: () => { attempt.responseEnded = true; attempt.credit?.reject(new Error('SDK response cancelled')); },
          }, { highWaterMark: 1 }) : null;
          attempt.fetch.resolve(new Response(body, { status: payload.status }));
        } else if (payload.action === 'responsePush') {
          if (!attempt.receiver || !attempt.response || attempt.responseEnded) throw new Error('messages SDK response not open');
          if ((attempt.response.desiredSize ?? 0) <= 0) { attempt.credit = deferred<void>(); await attempt.credit.promise; }
          this.current(key, cursor);
          attempt.response.enqueue(attempt.receiver.push(payload.page));
        } else if (payload.action === 'responseEnd') {
          if (!attempt.receiver || attempt.responseEnded) throw new Error('messages SDK response not open');
          attempt.receiver.finish(payload.descriptor); attempt.responseEnded = true; attempt.response?.close();
          ...
          if (!attempt.responseEnded) { attempt.response?.error(attempt.errorReference); attempt.responseEnded = true; }
          attempt.credit?.reject(attempt.errorReference);
