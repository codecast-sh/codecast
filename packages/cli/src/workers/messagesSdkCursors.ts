...
import type { MessagesSdkPage, MessagesSdkPayload, SdkCapture, SdkReceipt } from './messagesSdkTypes.js';
...
type Cursor = { generation: string; sequence: number; conversationId: string; messageCount: number; plan?: string; input?: IngestAssembler; messages?: PreparedWireMessage[]; attempt?: Attempt; budget: PayloadBudget; timer: ReturnType<typeof setTimeout>; busy: boolean };
...
export class MessagesSdkCursors {
  ...
  private start(cursor: Cursor, apiToken: string, receipt: SdkReceipt): Attempt {
    if (!cursor.messages || cursor.attempt) throw new Error('messages SDK not ready');
    if (receipt.messageCount !== cursor.messageCount || cursor.plan && cursor.plan !== receipt.plan) throw new Error('messages SDK plan mismatch');
    cursor.plan = receipt.plan;
    const attempt: Attempt = { receipt: Object.freeze({ ...receipt }), accepted: false, offset: 0, captured: deferred<SdkCapture>(), fetch: deferred<Response>(), outcome: Promise.resolve(), logs: [], responseEnded: false, waiting: false };
    ...
    attempt.outcome = sdk.mutation('messages:addMessages' as any, { conversation_id: cursor.conversationId, messages: cursor.messages, api_token: apiToken }, { skipQueue: true }).then(
      value => { attempt.accepted = true; return { kind: 'success', value, logs: attempt.logs }; },
  ...
  async request(payload: MessagesSdkPayload, id: string): Promise<MessagesSdkPage> {
    ...
    try {
      ...
      if (payload.action === 'push') {
        ...
        const attempt = this.start(cursor, payload.apiToken, payload.receipt);
        ...
        return { ...address, stage: 'outcome', ...(attempt.accepted ? { receipt: attempt.receipt } : {}) };
