...
for(const mode of ['ack','ack-mutant']) test(`actual production queued reread ACK ${mode}`,async()=>{
  ...
  for(const name of ['tmux','ps','lsof','claude','codex','gemini','opencode','pi','grok'])fs.writeFileSync(path.join(bin,name),'#!/bin/sh\nexit 1\n',{mode:0o755});
  try {
    const fixture=run(process.execPath,[path.join(import.meta.dir,'fixtures/ingestProduction.ts'),'true',mode],{env:{...process.env,HOME:root,TMUX_TMPDIR:tmp,TMUX:'',PATH:bin+path.delimiter+process.env.PATH,NODE_ENV:'test'},timeout:60_000,maxBuffer:3*1024*1024});
