...
export type SdkReceipt = Readonly<{ plan: string; attempt: string; workerGeneration: number; messageCount: number }>;
export function validSdkReceipt(v: unknown): v is SdkReceipt {
  return record(v) && Object.keys(v).length === 4 && ownFields(v, ['plan', 'attempt', 'workerGeneration', 'messageCount']) && identity(v.plan) && identity(v.attempt) && count(v.workerGeneration) && v.workerGeneration > 0 && count(v.messageCount) && v.messageCount <= 1_000_000;
}
...
export type MessagesSdkPayload = { action: 'open'; generation: string; conversationId: string; messageCount: number; budgetMs: number } | Address & (
  ...
  { action: 'begin'; apiToken: string; receipt: SdkReceipt } |
...
export type MessagesSdkPage = Address & (
  { stage: 'ready' } |
  { stage: 'outcome'; receipt?: SdkReceipt } |
...
export function validMessagesSdkPayload(v: unknown): v is MessagesSdkPayload {
  ...
  if (v.action === 'begin') return ownFields(v, [...base, 'apiToken', 'receipt']) && boundedText(v.apiToken, 16384) && validSdkReceipt(v.receipt);
  if (v.action === 'responseStart') return ownFields(v, [...base, 'status', 'body']) && Number.isInteger(v.status) && v.status >= 200 && v.status <= 599 && typeof v.body === 'boolean';
...
export function validMessagesSdkPage(v: unknown): v is MessagesSdkPage {
  ...
  if (v.stage === 'outcome') return ownFields(v, [...base, 'receipt']) && (!Object.hasOwn(v, 'receipt') || validSdkReceipt(v.receipt));
  if (v.stage === 'capture') {
    const c = v.capture;
