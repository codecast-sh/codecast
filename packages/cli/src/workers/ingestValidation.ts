...
function* schema(value: unknown, job: IngestJob): Generator<void> {
  fields(value,['messages','bytesConsumed','fileSize','totalCount','maxRowId','model','signatures','receiptSignatures','messageTitles','handoffParents','metadata']);
  ...
  requireValue(job.client === 'cursor' ? Array.isArray(value.receiptSignatures) && value.receiptSignatures.length === value.messages.length : value.receiptSignatures === undefined);
  ...
  fields(meta,['slug','parentUuid','cwd','cliFlags','summaryTitle','headMessages','teamInfo','planTools','subagent','appServerHead','codex','forkRoot','completedReview','backupAttempted','title','parentSessionId','agentName','internal','turn','permissionPrompt','warnings']);
