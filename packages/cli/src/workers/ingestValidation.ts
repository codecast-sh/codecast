...
function* message(value: unknown): Generator<void> {
  ...
  if (value.usage !== undefined) {
    ...
    fields(usage,['input_tokens','output_tokens','cache_creation_input_tokens','cache_read_input_tokens']);
    for (const key of ['input_tokens','output_tokens']) requireValue(Number.isFinite(usage[key]) && usage[key] >= 0);
    for (const key of ['cache_creation_input_tokens','cache_read_input_tokens']) requireValue(usage[key] === undefined || Number.isFinite(usage[key]) && usage[key] >= 0);
  }
...
function* schema(value: unknown, job: IngestJob): Generator<void> {
  fields(value,['messages','bytesConsumed','fileSize','totalCount','maxRowId','model','signatures','receiptSignatures','receiptOccurrences','rawBubbleCount','messageTitles','handoffParents','metadata']);
  ...
  requireValue(job.client === 'cursorDb' ? Number.isSafeInteger(value.rawBubbleCount) && value.rawBubbleCount >= value.totalCount && value.rawBubbleCount <= INGEST_MAX_BYTES : value.rawBubbleCount === undefined);
  ...
  for (let i=0;i<value.messages.length;i++) {
    if (value.receiptOccurrences) {
      ...
      requireValue(Number.isSafeInteger(occurrence) && occurrence >= (job.client === 'claude' ? job.offset : 0) && occurrence < (job.client === 'cursorDb' ? value.rawBubbleCount : job.client === 'claude' ? job.offset + value.bytesConsumed : job.identity.size));
