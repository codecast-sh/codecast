...
export async function readCursorIngest(file: string, offset: number) {
  ...
  try {
    ...
    const messages = parseCursorChatData(row.value,true);
...
export async function readOpencodeIngest(file: string, sessionId: string) {
  ...
  try {
    ...
    return {content:assembleOpencodeRows(session,messages,parts,true) ?? '',cwd:session.directory ?? undefined,parentSessionId:lineage?.parent_id && OPENCODE_SESSION_ID_RE.test(lineage.parent_id) ? lineage.parent_id : undefined,agentName:lineage?.agent ?? undefined};
  } finally { db.close(); }
}
