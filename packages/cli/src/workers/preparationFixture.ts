...
if (process.argv.includes('_worker')) runWorker('ingest');
  ...
  try {
    ...
    console.log(JSON.stringify({ runtime: process.versions, rawHash: hash(content), size, pid, prepareMs: opened - start, finishMs: finished - opened, maxGap, maxPageBytes: prep.maxPageBytes, maxWorkerRss: prep.maxWorkerRss, parentRss: process.memoryUsage().rss, exact: true, admission: true, reused: true }));
