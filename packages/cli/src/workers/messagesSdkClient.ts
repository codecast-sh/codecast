...
import { validMessagesSdkPage, validMessagesSdkPayload, type MessagesSdkPage, type MessagesSdkPayload, type SdkReceipt } from './messagesSdkTypes.js';
...
export class MessagesSdkBatch {
  ...
  private readonly plan = randomUUID();
  private attempt?: SdkReceipt;
  ...
  private received?: SdkReceipt;
  get acceptedReceipt(): SdkReceipt | undefined { return this.received; }
  private constructor(private readonly host: WorkerHost, private readonly budget: PayloadBudget, private readonly messageCount: number, private readonly signal?: AbortSignal) {
  ...
  static async open(value: { messages: PreparedWireMessage[]; bytes: number[] }, conversationId: string, options: { host: WorkerHost; budget: PayloadBudget; signal?: AbortSignal }): Promise<MessagesSdkBatch> {
    ...
    const batch = new MessagesSdkBatch(options.host, options.budget, value.messages.length, options.signal ?? options.budget.signal);
    try {
  ...
  private async request(command: Command, onReceipt?: (receipt: SdkReceipt) => void): Promise<MessagesSdkPage> {
    this.current();
    ...
    if (page.stage === 'outcome' && page.receipt) {
      if (command.action !== 'waitOutcome') throw new WorkerUnavailable('unexpected SDK receipt');
      this.receiveReceipt(page.receipt, onReceipt);
    }
    this.current(); await yieldMain(); return page;
  }
  private receiveReceipt(receipt: SdkReceipt, onReceipt?: (receipt: SdkReceipt) => void): void {
    const expected = this.attempt;
    if (!expected || receipt.plan !== expected.plan || receipt.attempt !== expected.attempt || receipt.workerGeneration !== expected.workerGeneration || receipt.messageCount !== expected.messageCount) throw new WorkerUnavailable('messages SDK receipt mismatch');
    if (this.received) return;
    this.received = Object.freeze({ ...receipt });
    this.custody = 'accepted';
    onReceipt?.(this.received);
  }
  private begin(apiToken: string): Promise<MessagesSdkPage> {
    this.attempt = Object.freeze({ plan: this.plan, attempt: randomUUID(), workerGeneration: this.hostGeneration, messageCount: this.messageCount });
    return this.request({ action: 'begin', apiToken, receipt: this.attempt });
  }
  private async outcome(network?: { reference: string; error: unknown }, logger?: (log: Log) => void, onAccepted?: (value: unknown) => void): Promise<unknown> {
    ...
    if (value.kind === 'success') {
      if (!this.acceptedReceipt) throw new WorkerUnavailable('missing SDK acceptance receipt');
  ...
  async send(options: { getApiToken: () => string; dispatch: MessagesSdkDispatch; logger?: (log: Log) => void; onReceipt?: (receipt: SdkReceipt) => void; onAccepted?: (value: unknown) => void }): Promise<unknown> {
    this.current();
    ...
    let requestPull: Promise<void> | undefined;
        ...
        pull: controller => {
          ...
          requestPull = (async () => {
            try {
              ...
              if (page.done) { sent.finish(); requestEnded = true; }
              if (bytes.length) controller.enqueue(bytes);
              if (requestEnded) controller.close();
            } catch (error) { controller.error(error); this.cancellation.abort(error); }
          })();
          return requestPull;
        },
        cancel: () => { requestEnded = true; },
      }, { highWaterMark: 1 });
      requestBody = body;
      waiter = this.request({ action: 'waitOutcome' }, onReceipt);
