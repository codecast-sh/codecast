      ...
      for (const [lineIndex,line] of content.split('\n').entries()) {
        ...
        if (Array.isArray(message.content)) for (const [blockIndex,block] of message.content.entries()) {
          if (block.type === 'tool_use' && ['ExitPlanMode','TaskCreate','TaskUpdate'].includes(block.name)) {
            const occurrence = createHash('sha256').update(JSON.stringify([job.client,job.sessionId,job.file,job.identity.dev,job.identity.ino,job.identity.birthtimeMs,entry.uuid ?? [job.offset,lineIndex],blockIndex,block.id ?? null])).digest('hex');
