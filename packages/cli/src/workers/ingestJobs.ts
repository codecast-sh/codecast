...
import { readIngestWindow, cursorPassBoundary } from '../transcriptWindow.js';
...
export async function readIngestJob(job: IngestJob): Promise<IngestResult> {
  ...
  if (job.client === 'cursorDb') {
    ...
    if (['claude','cursor','codex'].includes(job.client)) {
      const window = await readIngestWindow(job.file,job.offset,Math.max(0,before.size-job.offset),job.client === 'cursor' ? {boundary:cursorPassBoundary} : undefined);
    ...
    if (job.client === 'claude') {
      ...
      const head = await optional(() => readPart(job.file,16384),meta.warnings,'head');
      if (head !== undefined) {
