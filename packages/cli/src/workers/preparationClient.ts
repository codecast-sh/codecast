...
import { PREPARATION_COMPUTE_MS, PREPARATION_LIFETIME_MS, validPreparationPayload, type PreparationPage, type PreparationPayload } from './preparationTypes.js';
...
export class MessagePreparation {
  ...
  peakWorkerRss = 0;
  maxPageBytes = 0;
  private constructor(private readonly host: WorkerHost, private readonly messageCount: number, private readonly signal?: AbortSignal) {
  ...
  static async open(messages: PreparationMessage[], origin: PreparationOrigin, options: { host?: WorkerHost; signal?: AbortSignal } = {}): Promise<MessagePreparation> {
    ...
    occupied.add(host);
    const session = new MessagePreparation(host, messages.length, options.signal);
    try {
  ...
  private async transaction<T>(run: () => Promise<T>): Promise<T> {
    ...
    try { this.current(); } catch (error) { await this.close(); throw error; }
    this.busy = true;
  ...
  private async request(command: Command): Promise<PreparationPage> {
    ...
    if (!validPreparationPayload(payload)) throw new WorkerUnavailable('invalid preparation request');
    this.maxPageBytes = Math.max(this.maxPageBytes, Buffer.byteLength(JSON.stringify(payload)));
    ...
    this.peakWorkerRss = Math.max(this.peakWorkerRss, page.peakRss);
  ...
  paths(index: number): Promise<string[]> {
    return this.transaction(async () => {
      const result = await this.read({ action: 'paths', index });
      if (!Array.isArray(result) || result.length > 8 || result.some(path => typeof path !== 'string')) throw new WorkerUnavailable('invalid preparation image paths');
  ...
  finish(): Promise<{ messages: PreparedWireMessage[]; bytes: number[] }> {
    return this.transaction(async () => {
      const result = await this.read({ action: 'finish' }) as { messages: PreparedWireMessage[]; bytes: number[] };
      if (!result || !Array.isArray(result.messages) || !Array.isArray(result.bytes) || result.messages.length !== this.messageCount || result.bytes.length !== this.messageCount) throw new WorkerUnavailable('invalid preparation output');
      for (let i = 0; i < result.messages.length; i++) {
        const msg = result.messages[i], bytes = result.bytes[i];
        if (!msg || !['user','assistant','system'].includes(msg.role) || typeof msg.content !== 'string' || !Number.isFinite(msg.timestamp) || !Number.isSafeInteger(bytes) || bytes < 0) throw new WorkerUnavailable('invalid prepared message');
