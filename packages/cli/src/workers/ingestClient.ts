...
import { readIngestJob, ingestIdentity, sameIngestFile, samePersistedFile, sameIngestSnapshot, ingestWalIdentity, sameIngestWal } from './ingestJobs.js';
...
export { ingestIdentity, sameIngestFile, samePersistedFile, sameIngestSnapshot };
...
async function rememberIngest(result: IngestResult, job: IngestJob, checkpoint: () => void): Promise<IngestResult> {
  ...
  for (let i=0;i<result.messages.length;i++) {
    ...
    const hash=createHash('sha256').update(JSON.stringify([job.client,job.sessionId,job.file,job.identity.dev,job.identity.ino,job.identity.birthtimeMs,msg.uuid ? ['uuid',msg.uuid.length] : result.receiptOccurrences ? ['source',result.receiptOccurrences[i]] : job.client === 'cursorDb' ? [job.offset+i] : [job.offset,i]]));
