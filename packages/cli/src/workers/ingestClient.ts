...
const ingestRecords = new WeakMap<object, {key:string;signature:string;file:string;identity:string;offset:number;client:string;title?:string}>();
...
async function rememberIngest(result: IngestResult, job: IngestJob): Promise<IngestResult> {
  ...
  for (let i=0;i<result.messages.length;i++) {
    ...
    ingestRecords.set(msg,{key,signature,title:result.messageTitles[i] ?? undefined,file:job.file,offset:job.offset,client:job.client,identity:JSON.stringify([job.identity.dev,job.identity.ino,job.identity.birthtimeMs])});
