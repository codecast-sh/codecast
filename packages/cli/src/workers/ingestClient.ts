...
import { readIngestJob, ingestIdentity, sameIngestFile, sameIngestSnapshot } from './ingestJobs.js';
...
export async function readTranscriptIngest(input: Omit<IngestJob,'identity' | 'generation'>, options: {signal?: AbortSignal; timeoutMs?: number} = {}): Promise<IngestResult> {
  const job: IngestJob = {...input,generation:randomUUID(),identity:ingestIdentity(await fs.promises.stat(input.file))};
  ...
  current();
  if (!host) return readIngestJob(job);
  let cursor: string | undefined, sequence = 0, hostGeneration: number | undefined;
  ...
  try {
    ...
    if (!result || !Array.isArray(result.messages) || !result.metadata || !Array.isArray(result.metadata.warnings) || !Number.isSafeInteger(result.bytesConsumed) || result.bytesConsumed < 0 || result.bytesConsumed > Math.max(0,job.identity.size-job.offset) || !Number.isSafeInteger(result.totalCount) || result.totalCount < 0) throw new Error('invalid ingest result');
    ...
    if (cursor && !host.state.closed && host.state.pid !== null && host.state.generation === hostGeneration) {
      await host.request('ingest',{action:'close',cursor,generation:job.generation,sequence},{timeoutMs:1000}).catch(() => {});
    }
  }
}
