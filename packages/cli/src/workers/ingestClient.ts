...
async function rememberIngest(result: IngestResult, job: IngestJob, checkpoint: () => void): Promise<IngestResult> {
  ...
  for (let i=0;i<result.messages.length;i++) {
    ...
    const key=JSON.stringify([job.client,job.sessionId,job.file,job.identity.dev,job.identity.ino,job.identity.birthtimeMs,msg.uuid ? ['uuid',msg.uuid] : job.client === 'cursorDb' ? [job.offset+i] : [job.offset,i]]);
