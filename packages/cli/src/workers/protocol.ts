...
export type WorkerFrame = { v: 1; kind: WorkerKind; type: "request" | "result" | "error" | "heartbeat" | "cancel"; id?: string; operation?: "ping" | "read" | "scan" | "ingest" | "prepare"; payload?: unknown; deadline?: number; result?: unknown; error?: string; pid?: number };
export function validateFrame(value: unknown): asserts value is WorkerFrame {
  ...
  if (f.type === "request" || f.type === "result") {
    if (f.operation !== "ping" && !(f.operation === "read" && f.kind === "probe") && !(f.operation === "scan" && f.kind === "scan") && !(f.operation === "ingest" && f.kind === "ingest") && !(f.operation === "prepare" && f.kind === "ingest")) throw new Error("invalid worker operation");
  }
  if (f.type === "request") {
    if (!Number.isSafeInteger(f.deadline) || f.deadline! <= 0) throw new Error("invalid deadline");
    if (f.operation === "prepare" ? !validPreparationPayload(f.payload) : f.operation === "ingest" ? !validIngestPayload(f.payload) : f.operation === "scan" ? !validScanPayload(f.payload) : f.operation === "read" ? !validProbePayload(f.payload) : f.payload !== null) throw new Error("invalid worker payload");
  }
  if (f.type === "result") {
    if (f.operation === "prepare" ? !validPreparationPage(f.result) : f.operation === "ingest" ? !validIngestPage(f.result) : f.operation === "scan" ? !validScanPage(f.result) : f.operation === "read" ? !validProbeResult(f.result) : f.result !== "pong") throw new Error("invalid worker result");
  }
