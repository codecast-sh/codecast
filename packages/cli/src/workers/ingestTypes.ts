...
export type IngestJob = { client: typeof INGEST_CLIENTS[number]; file: string; sessionId: string; generation: string; identity: IngestIdentity; offset: number; model?: string; modelKnown?: boolean; recoverBackup?: boolean };
...
export type IngestResult = {
  ...
  metadata: {
    ...
    planTools?: { name: string; input: Record<string, any> }[];
    ...
    internal?: boolean; turn?: 'active' | 'idle' | 'unknown';
    warnings: string[];
  };
};
...
export function validIngestJob(v: unknown): v is IngestJob {
  return object(v) && keys(v,['client','file','sessionId','generation','identity','offset','model','modelKnown','recoverBackup']) && INGEST_CLIENTS.includes(v.client) && text(v.file,4096) && text(v.sessionId,256) && validCursor(v.generation) && count(v.offset) && (v.model === undefined || text(v.model,4096)) && (v.modelKnown === undefined || typeof v.modelKnown === 'boolean') && (v.recoverBackup === undefined || typeof v.recoverBackup === 'boolean') && object(v.identity) && keys(v.identity,['dev','ino','birthtimeMs','mtimeMs','ctimeMs','size']) && ['dev','ino','birthtimeMs','mtimeMs','ctimeMs','size'].every(k => Number.isFinite(v.identity[k]) && v.identity[k] >= 0) && count(v.identity.size);
}
...
export function validIngestPage(v: unknown): v is IngestPage {
  if (!object(v) || !keys(v,['cursor','generation','sequence','tokens','done']) || !validCursor(v.cursor) || !validCursor(v.generation) || !count(v.sequence) || typeof v.done !== 'boolean' || !Array.isArray(v.tokens) || v.tokens.length > INGEST_PAGE_TOKENS) return false;
