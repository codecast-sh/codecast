...
test('Cursor database emission preserves filtered bubble ordinals, native facts and public output', () => {
  ...
  try {
    ...
    const first = parseCursorChatData(JSON.stringify(data),true,(message,source)=>observed.push({message,source}),count=>extents.push(count));
    expect(extents).toEqual([10]);
    ...
    expect(parseCursorChatData('{}',true,undefined,count=>extents.push(count))).toEqual([]);
    expect(extents).toEqual([10,0]);
