...
describe("runChain records the chain as a run (the-line.md L7)", () => {
  test("posts upsert, create, then running and completed per step, then the terminal status", async () => {
    ...
    expect(progress[0]).toEqual({ run_id: "run-1", current_node_id: "step-1", node_id: "step-1", node_status: "running" });
    expect(progress[1]).toEqual({ run_id: "run-1", current_node_id: "step-1", node_id: "step-1", node_status: "completed", outcome: "success", result_preview: "the plan" });
    expect(progress[2]).toMatchObject({ node_id: "step-2", node_status: "running" });
    expect(progress[3]).toMatchObject({ node_id: "step-2", node_status: "completed", outcome: "success" });
    expect(progress[3].result_preview).toHaveLength(800);
    expect(progress[4]).toEqual({ run_id: "run-1", current_node_id: "exit", node_id: "exit", node_status: "completed", run_status: "completed" });
