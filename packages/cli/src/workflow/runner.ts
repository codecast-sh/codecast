import { FOREIGN_TEXT_CAPS, capForeignText, escapeForeignControlChars, inlineForeignText, fromConvexAgentType, toConvexAgentType } from "@codecast/shared/contracts";
...
async function executeSessionNode(
  ...
  const spawned = await cliCall(options, "/cli/spawn", {
    ...
    agent_type: agent,
    ...(model ? { model } : {}),
    ...(worktreeName ? { isolated: true, worktree_name: worktreeName } : {}),
...
// The failure path for a bound task (the-line.md L4). Retries exhausted
// (max_visits) parks the task in review as blocked so a person decides; a
// killed or failed hand returns it to open. Either way a blocker comment says
// what happened, so the board explains itself without the run log.
async function returnTaskOnFailure(options: RunOptions, state: WorkflowRunState): Promise<void> {
  ...
  await cliCall(options, "/cli/work/comment", {
    ...
    text: `Workflow ${exhausted ? "stopped: retries exhausted" : "failed"} (${reason}); task ${exhausted ? "left in review as blocked" : "returned to open"}.${detail}`,
  });
}
