...
export function parseAnswerSpec(
  ...
  if (kind === "form") {
    ...
    for (const pair of formPairs) {
      ...
      values[key] = v === "true" ? true : v === "false" ? false : v !== "" && !isNaN(Number(v)) ? Number(v) : v;
    }
  ...
  program
    ...
    .action(async (question: string | undefined, rest: string[], options: any) => {
      ...
      if (sub === "show" || sub === "recommend" || sub === "answer" || sub === "escalate") {
        ...
        try {
          parsed = parseAnswerSpec(row.kind ?? "single", rest[1], options.form as string[], row.options.length);
        } catch (err) {
          fail(err instanceof Error ? err.message : String(err));
        }
        ...
        if (options.json) console.log(JSON.stringify(result, null, 2));
          ...
          console.log(fmt.muted("  The answer is delivered to the asking session as a message."));
        }
        return;
      }
