...
// The build id of the code the RUNNING daemon started from. Written by the
// daemon at boot beside daemon.version. It answers "did the daemon's code
// change", which the version string cannot: most releases touch the CLI only,
// and bouncing a daemon holding hundreds of live panes for those is pure loss.
const BUILD_FILE = path.join(CONFIG_DIR, "daemon.build");

/** The build id the running daemon started from, or null when it is unknown
 *  (no file, an older daemon, an unreadable file). Null must never suppress a
 *  restart, so every caller treats it as "cannot tell, do what you would have
 *  done". One function so that rule lives in one place. */
function readRunningBuildId(): string | null {
  try {
    const id = fs.readFileSync(BUILD_FILE, "utf-8").trim();
...
// After an Agent Features toggle: only the hooks that belong to a feature
// move, so turning one feature on never installs anything else.
async function syncFeatureHooks(config?: Record<string, any> | null): Promise<void> {
  const { syncFeatureHooks: sync } = await import("./harnessHooksInstall.js");
  sync(config ?? readConfig());
}
  ...
  if (isDaemonRunning()) {
    try {
      ...
      if (runningVersion && runningVersion !== cliVersion) {
        ...
        if (cliIsNewer) {
          // Veto, never a trigger. The version compare above still decides
          // WHETHER a bounce is on the table; the build id only cancels one
          // when the daemon is already running this exact code. Two worktrees
          // of this repo run the same CLI at different commits, so a build id
...
function stopDaemon(): void {
  ...
  // Sweep any other daemon off the process table. `pgrep -f daemon.ts` only
  // ever matched a source install, so a built-JS or binary daemon survived
  // every stop and kept running beside its replacement.
  //
  // Bounded much tighter than the default: `stop` and `restart` are typed by a
  // person, and `ps` on a loaded machine has run for 26s. The pid file SIGTERM
  // above already covers the normal case, so a `ps` that times out costs this
  // one invocation its split brain catch and nothing else.
  try {
    for (const orphanPid of findOtherDaemonPids(snapshotProcessTable({ timeout: 5_000 }))) {
      try {
  ...
  // The daemon drains its retry queue before it exits, so give it more room
  // than the old 5s: killing it here would skip the pid file release, the
  // daemon.version and daemon.build cleanup and the stop lifecycle log.
  for (const pid of pidsToWait) {
    if (!waitForPidExit(pid, DAEMON_STOP_SIGKILL_MS)) {
...
/**
 * Bounce the daemon after a self update, but only when the code it runs
 * actually changed.
 *
 * Most releases touch the CLI and not the daemon. Restarting for those costs a
 * daemon holding hundreds of live panes a full boot for nothing. So ask the
 * freshly installed executable for its daemon build id and compare it to the
 * id the running daemon stamped at boot. Restart when they differ, or when
 * either is unknown. An unknown id must never SUPPRESS a restart that the old
 * behavior would have made.
 *
 * The child runs with auto-update disabled so it cannot start its own update.
 */
function bounceDaemonIfBuildChanged(daemonWasRunning: boolean): boolean {
  if (!daemonWasRunning) return false;
  ...
  try {
    ...
    const r = spawnSync(executablePath, args, {
      encoding: "utf-8",
      timeout: 5000,
      env: { ...process.env, CODECAST_NO_AUTO_UPDATE: "1" },
    });
 ...
 *
 * `--disable` used to only write `<key>_enabled: false` and print "disabled",
 * leaving the section in CLAUDE.md — so the agent kept reading a capability the
 * user believed they had turned off. Removal is the other half of the promise.
 *
 * The shared "Referencing objects" section is deliberately left alone: several
 * snippets depend on it, and working out whether the last one just went away is
 * the kind of bookkeeping that gets it wrong. A stale reference section costs a
 * few lines of context; removing one another enabled snippet still needs breaks
 * that snippet.
 */
function removeSnippetSection(enabledKey: string): boolean {
  const spec = SECTION_BY_ENABLED_KEY[enabledKey];
...
async function installOrchestration(update = false): Promise<{ installed: boolean; updated: boolean }> {
  // The bundle is text compiled into the binary; loaded here, at install, so
  // every other cast invocation leaves it off the boot graph.
...
const vaultCmd = program
  ...
  .alias("vaults")
  .description(
    "Read and write local markdown vaults (the web app calls this Files)\n\n" +
    "A vault is a directory of markdown you registered. The daemon serves it\n" +
    "to the web app's Files page over its local 127.0.0.1 bridge; these\n" +
    "commands read and write the same files directly, so a note you change\n" +
    "here shows up in an open browser tab within a second. Registration is\n" +
    "per-device and the files never leave this machine.\n\n" +
    "Managing the list of vaults:\n" +
    "  cast vault add ~/notes\n" +
    "  cast vault add ~/work/wiki --name Wiki\n" +
    "  cast vault ls\n" +
    "  cast vault mirror ~/notes --on   # let other devices read it\n" +
    "  cast vault forget ~/notes        # unregister; the files stay\n\n" +
    "Working on notes — the loop an agent runs:\n" +
    "  cast vault search 'tag:health sleep'      # find the note\n" +
    "  cast vault cat Sleep                      # read it\n" +
    "  cast vault grep 'bed at' --path 'Areas/**'  # find the exact line\n" +
    "  cast vault edit Sleep --old 'bed at 11' --new 'bed at 10'\n" +
    "  cast vault links Sleep                    # what points here\n" +
    "  cast vault open Sleep                     # a link for the human\n\n" +
    "Paths: every command takes a vault-relative path OR the bare name a\n" +
    "[[wiki link]] would use, resolved by the same rules the app uses. A name\n" +
    "matching several notes errors and lists them.\n\n" +
    "Vaults: --vault <id|name|dir> picks one. Without it, commands use the\n" +
    "vault the current directory is inside — so inside a project you can just\n" +
    "say `cast vault cat <note>`. With a single registered vault anywhere,\n" +
    "that one is used.\n\n" +
    "Machine-readable: every read command takes --json.\n\n" +
    "Exit codes: 0 ok · 1 not found · 2 ambiguous (name or vault matched\n" +
    "several) · 3 the vault's directory is unreachable"
  )
  .showHelpAfterError(true);
...
vaultCmd
  ...
  .alias("list")
  .description(
    "List the vaults this machine can open\n\n" +
    "Two kinds appear here. VAULTS are directories you registered with\n" +
    "`cast vault add` — they are listed in full. PROJECTS are your own code\n" +
    "repositories, found automatically because they contain markdown; there\n" +
    "are usually hundreds, so only a few are shown unless you ask for --all.\n" +
    "Both open, scan and edit identically once you name one.\n\n" +
    "The row marked `← you are here` is the vault every other command uses\n" +
    "when you leave --vault off.\n\n" +
    "Examples:\n" +
    "  cast vault ls\n" +
    "  cast vault ls --all      # every discovered project too\n" +
    "  cast vault ls --json     # always complete, for scripts"
  )
  .option("--all", "List every discovered project, not just the first few")
  .option("--json", "Output as JSON")
  .action((options: any) => {
    const vaults = listVaults(CONFIG_DIR);
    // --json is the machine surface and is never truncated: a script asking for
    // the list wants the list, and paging it would be a silent wrong answer.
    if (options.json) {
      console.log(JSON.stringify({ vaults }));
      return;
    }
    if (vaults.length === 0) {
      console.log(`${c.dim}No vaults. Add one with: cast vault add ~/notes${c.reset}`);
      return;
    }

    const here = vaultForCwd(vaults);
    const registered = vaults.filter((v) => v.kind !== "project");
    const projects = vaults.filter((v) => v.kind === "project");
    const PROJECT_PREVIEW = 8;

    const row = (v: VaultInfo) => {
      const current = v.id === here?.id ? ` ${c.green}← you are here${c.reset}` : "";
      const notes = v.note_count === undefined ? "" : ` ${c.dim}${v.note_count} notes${c.reset}`;
      // "local only" on every row is noise once there are hundreds — the
      // interesting state is the opt-in one, so only mirroring is called out.
      const mirror = v.mirror ? ` ${c.green}mirrored${c.reset}` : "";
      const home = v.home ? ` ${c.dim}opens at ${v.home}/${c.reset}` : "";
      console.log(`  ${c.cyan}${v.id}${c.reset} ${v.name}${notes}${mirror}${home}${current}`);
      console.log(`    ${c.dim}${v.root}${c.reset}`);
    };

    if (registered.length) {
      console.log(`${c.bold}Vaults${c.reset} ${c.dim}— registered with cast vault add${c.reset}`);
      registered.forEach(row);
    }

    if (projects.length) {
      // A project the caller is standing in is the one row that must never be
      // truncated away: it is the default for every other command.
      const shown = options.all
        ? projects
        : (() => {
            const head = projects.slice(0, PROJECT_PREVIEW);
            return here && here.kind === "project" && !head.some((p) => p.id === here.id)
              ? [here, ...head.slice(0, PROJECT_PREVIEW - 1)]
              : head;
          })();
      console.log(
        `${registered.length ? "\n" : ""}${c.bold}Projects${c.reset} ` +
        `${c.dim}— found automatically, ${projects.length} total${c.reset}`,
      );
      shown.forEach(row);
      if (shown.length < projects.length) {
        console.log(`  ${c.dim}… ${projects.length - shown.length} more — see: cast vault ls --all${c.reset}`);
      }
    }

    if (!here) {
      console.log(`\n${c.dim}This directory is not inside any of them — name one with --vault.${c.reset}`);
    }
  });
...
/** Print a VaultCliError the way its exit code says, then leave. The candidate
 *  list is capped: codecast registers a vault per project, so "which one did
 *  you mean" can have a hundred answers and a wall of them helps nobody. */
function failVault(err: unknown): never {
  if (err instanceof VaultCliError) {
    console.error(`${c.red}${err.message}${c.reset}`);
    for (const candidate of err.candidates.slice(0, 10)) {
      console.error(`  ${c.dim}${candidate}${c.reset}`);
    }
    if (err.candidates.length > 10) {
      console.error(`  ${c.dim}… ${err.candidates.length - 10} more — see: cast vault ls${c.reset}`);
    }
 ...
 *
 *  Mints /files — the surface is called "Files" now. /vault stays routable in
 *  the web app forever: this function printed /vault?f=… links into sessions,
 *  notes and users' own markdown files for months, and those are on disk. */
function vaultNoteUrl(path: string, line?: number): string {
  const at = line ? `&l=${line}` : "";
  return `${WEB_URL}/files?f=${encodeURIComponent(path)}${at}`;
}
...
withVaultOption(
  ...
  runVault(async () => {
    ...
    let total = 0;
    const byPath = [...ctx.contents].sort((a, b) => (a[0] < b[0] ? -1 : a[0] > b[0] ? 1 : 0));
    for (const [notePath, text] of byPath) {
...
withVaultOption(
  ...
  runVault(async () => {
    ...
    console.log(`\n${c.dim}codecast objects (${report.objects.length})${c.reset}`);
    if (!report.objects.length) console.log(`  ${c.dim}none${c.reset}`);
    for (const o of report.objects) {
      console.log(
        `  ${c.dim}${String(o.line).padStart(5)}${c.reset}  ${c.cyan}${o.id}${c.reset} ${c.dim}${o.type}${c.reset}  ${o.text}`,
      );
    }
...
withVaultOption(
  vaultCmd
    ...
    .description(
      ...
      "The link carries the note's path, not which vault it is in — the app\n" +
      "opens it in whichever vault the reader has open. With several vaults\n" +
      "registered, say which one alongside the link.\n\n" +
      "Examples:\n" +
...
codexAccountsCmd
  ...
  .action(async (options: any) => {
    ...
    if (outcome.outcome === "reset") {
      console.log(`${c.dim}  sessions on ${label} can run again — nudge any that stopped mid-turn${c.reset}`);
    }
  });
...
program
  ...
  .description(
    ...
    "  CONTENT: work state by default (grouped NEEDS INPUT → DONE → WORKING → DORMANT → IDLE),\n" +
    "           or --messages for conversation messages\n" +
    "  LIVENESS: a one-shot snapshot, or -w to stream changes live\n\n" +
    "States answer WHO ACTS NEXT, matching the web inbox. NEEDS INPUT = a human must unblock it\n" +
    "(open question, permission prompt, dead with output, or a finished turn nobody classified).\n" +
    "DONE = the agent delivered (`cast state --status done`); review at leisure. WORKING = the\n" +
    "agent is producing now. DORMANT = a machine wakes it (a declared `cast state --status\n" +
    "dormant`, an open background task/Monitor, or an armed trigger into it) — nothing is yours\n" +
    "until then. IDLE = blank sessions. --json swaps any view to JSON.\n\n" +
    ...
    "flips to done (finished) or needs_input (blocked); dormant means parked, not blocked:\n" +
    '  cast spawn --label fleet "task A" "task B"\n' +
    "  cast sessions --label fleet -w --json   # → {\"event\":\"transition\",\"to\":\"done\",…}\n" +
  ...
  .option("-t, --team [name|id|personal]", "Show a team's sessions (default: the active team; personal = just yours)")
  ...
  .action(async (sessionIds, options) => {
    ...
    // it was dismissed from the inbox. They also go to the server (session_ids)
    // so a named row is hydrated past the recency window and a subagent row —
    // hidden from the top-level monitor — still answers when asked for by id.
    ...
    const requestedIds = ids.length > 0 ? ids : undefined;
    ...
    // `--team <name>` names a team; `--team personal` is the personal workspace
    // (your own sessions, the default), so it never enters team mode.
    const namedTeam = typeof options.team === "string" ? await readWorkspace(options.team) : null;
    if (namedTeam?.kind === "personal") options.team = false;
    const teamId = namedTeam?.kind === "team" ? namedTeam.teamId : undefined;
    ...
    const scopeLabel = teamMode
      ? (options.member ? `team: ${options.member}` : options.global ? "all teams" : namedTeam?.kind === "team" ? `team: ${workspaceLabel(namedTeam)}` : "team")
    ...
    const fetchSnapshot = async () => {
      ...
      if (!teamMode) {
        const response = await cliFetchRead(`${siteUrl}/cli/inbox`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ api_token: config.auth_token, show_all: showAll, state: options.state, label: options.label, project_path: labelProjectPath, limit, session_ids: requestedIds }),
        });
    ...
    if (!teamMode) {
      stateClient.onUpdate("conversations:inboxForCLI" as any,
        { api_token: config.auth_token, state: options.state, show_all: showAll, label: options.label, project_path: labelProjectPath, limit, session_ids: requestedIds },
        (result: any) => { if (result && !result.error) onSessions(result.sessions || []); });
    } else {
      const projectPath = options.global ? undefined : getRealCwd();
      stateClient.onUpdate("conversations:feedForCLI" as any,
        { api_token: config.auth_token, project_path: projectPath, team_id: teamId, member_name: options.member, mine_only: options.mine || undefined, state: options.state, label: options.label, limit },
...
program
  ...
  .action(async (ref, line, options) => {
    ...
    if (fromUrl) {
      ...
      // `owner/repo@sha`), names its own type. A bare Convex id names none, so
      // the server says which table it belongs to (a project, a plan, a doc);
      // anything else is a session.
      entityType = entityTypeFromId(rawId)
        ?? (looksLikeConvexId(rawId) && !options.type ? await cliPost("/cli/entities/type", { id: rawId }).catch(() => null) : null)
        ?? "session";
    }
    if (options.type) {
...
program
  ...
  .action(async (options) => {
    ...
    if (options.disable) {
      config.memory_enabled = false;
      writeConfig(config);
      removeSnippetSection("memory_enabled");
      console.log("Memory disabled. Snippet removed from this machine.");
  ...
  .action(async (snippetArg: string | undefined, options) => {
    ...
    if (snippetArg) {
      ...
      if (options.disable) {
        ...
        // A feature's hook goes with its section (HARNESS_HOOKS `feature`).
        await syncFeatureHooks(config);
...
program
  ...
  .action(async (rawPrompts: string[], options: any) => {
    ...
    const spawnerSession = ownSessionId(getRealCwd()) || undefined;
...
program
  ...
  .action(async (options) => {
    ...
    // The daemon stays up across the update. performUpdate only renames the
    // executable and the running daemon holds the old inode, so nothing needs
    // it down — and leaving it up removes the window where a failed update
    ...
    if (success) {
      ...
      if (bounceDaemonIfBuildChanged(daemonWasRunning)) {
        ...
        console.log(`Updated to v${available}; daemon code unchanged, left it running`);
      } else {
        console.log(`Updated to v${available}`);
      }
    } else {
...
// The `--on <event>` vocabulary lives in @codecast/shared/contracts so the CLI
// and the web trigger pages cannot drift apart again. `cast trigger add -h`
// generates its list from it, so adding a name there is the only edit needed.
const EVENT_SHORTHANDS = TRIGGER_EVENT_SHORTHANDS;
const EVENT_NAMES = TRIGGER_EVENT_NAMES.join(", ");

/**
 * The event filter a `--on` trigger arms, plus what it ended up scoped to so
...
async function buildEventFilter(
  ...
  if (opts.pr !== undefined) {
    if (!isPrTriggerEvent(eventName)) {
      console.error(`--pr only narrows a pull request event, and ${eventName} is not one.`);
      process.exit(1);
    }
    const number = Number(opts.pr);
    if (!Number.isInteger(number) || number <= 0) {
      console.error(`--pr wants a pull request number, got "${opts.pr}".`);
      process.exit(1);
    }
...
org
  ...
  .action(async (name: string, options: any) => {
    const ws = await writeWorkspace(options.team);
    const reports_to = options.reportsTo ? await resolveOrgTarget(options.reportsTo, ws) : undefined;
    const role = await cliPost("/cli/org/create", { ...workspaceArgs(ws), name, handle: options.handle, reports_to, charter: options.charter, from_session: callingSession() });
    if (options.json) { console.log(JSON.stringify(role, null, 2)); return; }
    console.log(`${c.green}✓${c.reset} created ${c.bold}${role.name}${c.reset} ${c.dim}@${role.handle} (${role.short_id}) in ${workspaceLabel(ws)}${c.reset}`);
  });
...
// tooling: without it a woken anchor is told to run a command that does not
...
function printChatMessage(row: any, authors?: Map<string, any>) {
  ...
  const status = row.agent_status && row.agent_status !== "done"
    ...
    : "";
  const body = row.deleted_at ? `${c.dim}(deleted)${c.reset}` : row.content;
  console.log(`  ${c.dim}${when}${c.reset} ${who}${status} ${c.dim}·${c.reset} ${String(row._id).slice(0, 10)}`);
...
chat
  ...
  .action(async (options: any) => {
    ...
    if (!result.channels?.length) {
      console.log(`${c.dim}No channels yet. Create one: cast chat new <name>${c.reset}`);
      return;
    }
    const rail = new Map<string, any>(
      (result.rail ?? []).map((r: any) => [String(r.channel_id), r]),
    );
    for (const channel of result.channels) {
      const row = rail.get(String(channel._id));
      const unread = row?.unread
        ? ` ${c.yellow}${row.unread}${row.unread_capped ? "+" : ""}${c.reset}`
        : "";
      const mentions = row?.unread_mentions ? ` ${c.red}@${row.unread_mentions}${c.reset}` : "";
      const muted = row?.notify_level === "none" ? ` ${c.dim}(muted)${c.reset}` : "";
      console.log(`  ${c.bold}#${channel.name}${c.reset}${unread}${mentions}${muted} ${c.dim}${channel._id}${c.reset}`);
    }
  });
...
chat
  .command("read")
  .description("Read a channel, newest last")
  ...
  .action(async (options: any) => {
    ...
    const authors = new Map<string, any>(
      (result.authors ?? []).map((a: any) => [String(a._id), a]),
    );
    for (const row of result.messages ?? []) printChatMessage(row, authors);
    if (result.has_more) {
      console.log(`${c.dim}  older above — cast chat read --channel ${options.channel} --cursor ${result.next_cursor}${c.reset}`);
    }
  });

chat
  .command("thread")
  .description("Read one thread: its root and its replies")
  .argument("<root_id>", "Thread root message id")
  .option("--cursor <cursor>", "Page further back, from a previous next_cursor")
  .option("--json", "Machine-readable output")
  .action(async (rootId: string, options: any) => {
    const result = await cliPost("/cli/chat/thread", {
      root_id: rootId, cursor: options.cursor,
    });
    if (options.json) {
      console.log(JSON.stringify(result, null, 2));
      return;
    ...
    printChatMessage(result.root, authors);
    for (const row of result.replies ?? []) printChatMessage(row, authors);
  });
...
chat
  ...
  .action(async (text: string, options: any) => {
    ...
    const result = await cliPost("/cli/chat/send", {
      channel_id: options.channel,
      content: body,
...
chat
  .command("archive")
  .description("Archive a channel (or bring one back with --restore)")
  .argument("<channel_id>", "Channel id (from cast chat channels)")
  .option("--restore", "Un-archive instead")
  .action(async (channelId: string, options: any) => {
    const result = await cliPost("/cli/chat/archive", {
      channel_id: channelId, archived: !options.restore,
    });
    console.log(`${c.green}✓${c.reset} ${result.archived ? "archived" : "restored"}`);
  });
...
chat
  ...
  .action(async (messageId: string) => {
    ...
    console.log(
      result.stopped
        ? `${c.green}✓${c.reset} stopped — the thread shows who stopped it`
        : `${c.dim}nothing in flight to stop${c.reset}`,
    );
  });
...
chat
  ...
  .action(async (query: string, options: any) => {
    ...
    for (const hit of result.results) {
      console.log(`  ${c.bold}#${hit.channel_name}${c.reset} ${c.dim}${hit._id}${c.reset}`);
...
chat
  .command("react")
  .description("Toggle your reaction on a message")
  .argument("<message_id>", "Message id")
  .argument("<emoji>", "One emoji")
  .action(async (messageId: string, emoji: string) => {
    const result = await cliPost("/cli/chat/react", { message_id: messageId, emoji });
    console.log(`${c.green}✓${c.reset} ${result.reacted ? "added" : "removed"} ${emoji}`);
  });

chat
  .command("mark-read")
  .description("Mark a channel read, which also cancels its queued phone pushes")
  .requiredOption("--channel <id>", "Channel id")
  .action(async (options: any) => {
    const result = await cliPost("/cli/chat/mark-read", { channel_id: options.channel });
    console.log(
...
work
  ...
  .option("--team <name|id|personal>", "Workspace to file it in: a team, or personal (default: the session's team, else the directory's mapping)")
...
work
  ...
  .action(async (options: any) => {
    ...
    if (options.disable) {
      ...
      writeConfig(config);
      // Same promise as `cast install tasks --disable`: section and task
      // reminder both come off.
      removeSnippetSection("work_enabled");
      await syncFeatureHooks(config);
...
async function resolveProjectId(ref: string, team?: string): Promise<string> {
  if (!ref || looksLikeConvexId(ref)) return ref;
  const projects = await cliPost("/cli/projects/list", team ? workspaceScope(await readWorkspace(team)) : {});
...
projectCmd
  ...
  .option("--team <name|id|personal>", "Workspace the title is matched in: a team, or personal")
  .action(async (ref: string, options: any) => {
    const id = await resolveProjectId(ref, options.team);
...
doc
  ...
  .action(async (id: string, range: string | undefined, options: any) => {
    ...
    // An explicit range continues by range (the next window of the same size);
    // a page continues by page number.
    const next = end >= total
      ? ""
      : range
        ? `cast doc show ${id} ${end + 1}:${end + (end - start + 1)}`
        : page < pages
          ? `cast doc show ${id} -p ${page + 1}`
          : "";
...
doc
  ...
  .action(async (id: string, pattern: string, options: any) => {
    ...
    for (const i of contextWindow(shown, ctxLines, lines.length)) {
      if (i === null) {
        console.log(`${c.dim}  ⋮${c.reset}`);
        continue;
      }
      const isHit = shown.includes(i);
      const num = String(i + 1).padStart(5);
      const text = isHit ? lines[i].replace(matcher.all, (m: string) => `${c.yellow}${m}${c.reset}`) : lines[i];
      console.log(`${isHit ? c.cyan : c.dim}${num}${c.reset}  ${text}`);
    }
...
plan
  ...
  .option("--team <name|id|personal>", "Workspace to list: a team, or personal (default: the directory's mapping)")
...
plan
  ...
  .action(async (planId: string) => {
    ...
    const p = result;
    const icon = PLAN_STATUS_ICONS[p.status] || "?";
    const progress = planProgressLabel(p);
...
program
  .command("cursor")
  .description(
    "Cursor chat sync: cast cursor on|off|status\n\n" +
    "On macOS, reading Cursor's data needs a one-time permission (\"codecast\n" +
    "would like to access data from other apps\"). `cast cursor on` restarts\n" +
    "the daemon so that prompt appears now, in context — never at login."
  )
  .argument("[action]", "on | off | status", "status")
  .action(async (action: string) => {
    const config = readConfig();
    if (!config) {
      console.error("Not configured. Run: cast auth");
      process.exit(1);
    }
    const printStatus = () => {
      const pref = config.cursor_sync ?? "auto";
      const access = readDaemonState()?.cursorAccess;
      console.log(`  setting: ${pref}`);
      if (process.platform === "darwin") {
        console.log(`  macOS app data access: ${access ?? "not yet requested"}`);
        if (access === "denied") {
          console.log("  Fix: System Settings → Privacy & Security → allow codecast access to app data");
          console.log("  (or grant Full Disk Access), then run `cast cursor on` again.");
        } else if (!access && pref !== "on") {
          console.log("  Enable with: cast cursor on");
        }
      }
    };
    if (action === "status") {
    ...
    config.cursor_sync = action;
    writeConfig(config);
    if (action === "off") {
      kickstartManagedDaemon();
      console.log("Cursor sync disabled.");
      return;
    }
    ...
    if (process.platform === "darwin" && priorAccess !== "granted") {
      ...
      console.log("Restarting the daemon to start Cursor sync...");
    }
    if (!kickstartManagedDaemon()) {
      ensureDaemonRunning();
    }
    if (process.platform !== "darwin") {
      console.log("Cursor sync enabled.");
      return;
    }
    // The daemon probes on boot and records the outcome; surface it here so
    // the user learns the result in the same terminal that asked for it.
    const deadline = Date.now() + 60_000;
    let access: "granted" | "denied" | undefined;
    while (Date.now() < deadline) {
      await new Promise((r) => setTimeout(r, 1000));
      access = readDaemonState()?.cursorAccess;
      if (access) break;
    }
    if (access === "granted") {
      console.log("Cursor sync enabled — access granted.");
    } else if (access === "denied") {
      console.log("macOS denied access. To fix:");
      console.log("  System Settings → Privacy & Security → allow codecast access to app data");
      console.log("  (or grant Full Disk Access), then run `cast cursor on` again.");
    } else {
      console.log("Still waiting on the permission dialog — run `cast cursor status` after answering it.");
    }
  });
...
if (!isStableContextFastPath && !isCredentialHelperFastPath(process.argv) && !process.env.CODECAST_NO_AUTO_UPDATE) autoUpdateSettled = checkForUpdates().then(async (available) => {
  ...
  // Same ordering as `cast update`: update first, then bounce the daemon only
  // if the new executable carries different daemon code.
  const daemonWasRunning = getDaemonPid() !== null;
