...
// The message is the useful part for a human at the CLI — strip the wrapper and
// the stack so guard refusals (close-guard, depth cap, cycle) read as advice.
function cleanConvexError(raw: any): string {
  let msg = String(raw);
  msg = msg.split(/\n\s*at\s/)[0];
  msg = msg.replace(/^\s*(Uncaught\s+)?(Convex)?Error:\s*/i, "");
