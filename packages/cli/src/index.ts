    ...
    try {
      ...
      if (runningVersion && runningVersion !== cliVersion) {
        ...
        if (cliIsNewer) {
          ...
          if (runningBuild && runningBuild === DAEMON_BUILD_ID) return;
...
function stopDaemon(): void {
  ...
  try {
    for (const orphanPid of findOtherDaemonPids(snapshotProcessTable())) {
      try {
...
function bounceDaemonIfBuildChanged(daemonWasRunning: boolean): boolean {
  ...
  if (runningBuild && installedBuild && runningBuild === installedBuild) return false;
