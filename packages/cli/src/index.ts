    ...
    try {
      ...
      if (runningVersion && runningVersion !== cliVersion) {
        ...
        if (cliIsNewer) {
          ...
          const runningBuild = fs.existsSync(BUILD_FILE)
            ? fs.readFileSync(BUILD_FILE, "utf-8").trim()
            : null;
...
function bounceDaemonIfBuildChanged(daemonWasRunning: boolean): boolean {
  ...
  try { runningBuild = fs.readFileSync(BUILD_FILE, "utf-8").trim(); } catch {}
  ...
  try {
    ...
    if (/^[0-9a-f]{6,}$/.test(out)) installedBuild = out;
  } catch {}
...
program.hook('preAction', (thisCommand, actionCommand) => {
  ...
  const internalCmds = ['start', 'stop', 'status', 'daemon', 'codecast', '_daemon', '_watchdog', '_build-id', 'auth', 'login', 'update'];
