...
async function slackPauseAction(chatRef: string, options: any, paused: boolean) {
  ...
  if (options.json) {
    console.log(JSON.stringify({ ...result, paused, team_id: ws.teamId ?? null }, null, 2));
    return;
  }
  console.log(
...
chatSlack
  ...
  .action(async (chatRef: string, options: any) => {
    ...
    if (options.json) {
      console.log(JSON.stringify({ ...result, link_id: link._id, team_id: ws.teamId ?? null }, null, 2));
      return;
    }
    console.log(
      result.removed
...
workflow
  ...
  .action(async (fileArg: string | undefined, options: any) => {
    ...
    if (!options.dryRun && apiToken) {
      ...
      const { nodes, edges } = graphToPushPayload(graph);
