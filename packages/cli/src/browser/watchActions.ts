 ...
 * one time code or a card. Those report one typing action per burst, with
 * no text, so not even the length gets out.
 */
...
export function actionObserverSource(): string {
  ...
  const S = { recent: [], moveAt: 0, wheelAt: 0, downAt: 0, typing: null, last: null, scrolled: new WeakMap() };
  W.__castWatch = S;
  S.scrolled.set(W, [W.scrollX || 0, W.scrollY || 0]);
  const push = (a) => {
    S.recent.push(a);
  ...
  // An engine that scrolls by script (scrollBy, scrollIntoView) sends no
  // wheel; the scroll event is what is left. Deltas come from the position
  // change of whatever scrolled, the cursor stays where it last was, and a
  // scroll the wheel already reported is not reported twice.
  W.addEventListener("scroll", (e) => {
    const now = Date.now();
    if (now - S.wheelAt < ${WHEEL_INTERVAL_MS * 3}) return;
    const t = e.target;
    const el = t && typeof t.scrollTop === "number" ? t : W;
    const pos = el === W ? [W.scrollX || 0, W.scrollY || 0] : [el.scrollLeft || 0, el.scrollTop || 0];
    const was = S.scrolled.get(el) || [0, 0];
    S.scrolled.set(el, pos);
    const dx = pos[0] - was[0], dy = pos[1] - was[1];
    if (!dx && !dy) return;
    S.wheelAt = now;
    const c = S.last || [W.innerWidth / 2, W.innerHeight / 2];
    push(raw("scroll", c[0], c[1], { dx, dy }));
  }, opts);
  W.addEventListener("input", (e) => {
    const el = e.target;
    const c = center(el) || [0, 0];
    const now = Date.now();
    const fresh = !S.typing || S.typing.el !== el || now - S.typing.at > ${TYPING_GAP_MS};
    if (fresh) S.typing = { el, text: "", at: now };
    S.typing.at = now;
    if (sensitive(el)) {
      // One report per burst: a frame per keystroke would count the secret's
      // characters out loud.
      if (fresh) push(raw("type", c[0], c[1], { secret: true }));
      return;
    }
