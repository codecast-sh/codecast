...
export function registerBrowserCommand(program: Command, deps: PublishDeps): void {
  ...
  const act = <T>(
    ...
    isPaneMode(opts, me())
      ? withDesktopPanePage({}, fn, me()).catch((err) => {
          // A pane the human closed is said once (desktopPane.ts clears the
          // memory and the sticky choice), and the verb goes on in the
          // session's ordinary browser, the same as the engine path (cliEngine
          // ctx). Anything else about the pane stops here.
          const unavailable = err instanceof DesktopPaneUnavailable ? err : null;
          if (!unavailable || unavailable.reason !== "closed") return die((err as Error).message, unavailable?.hint);
          console.log(`${fmt.warning("!")} ${unavailable.message} — continuing in the session's usual browser`);
          return actElsewhere({ ...opts, pane: false }, fn);
        })
      : actElsewhere(opts, fn);
  const actElsewhere = <T>(
    opts: { tab?: string; real?: boolean; clone?: boolean; pane?: boolean },
    fn: (page: PageSession, state: InstanceState, conn: CdpClient) => Promise<T>,
  ) =>
    isRealMode(opts, me())
  ...
  registerAdvancedClone(br, (next) => registerBrowserCommand(next, deps));
  ...
  targetFlags(br.command("start"))
    .description("Connect to the human's Chrome through the extension")
  ...
  targetFlags(br.command("stop"))
    ...
    .action(async (o: { wipe?: boolean; force?: boolean; real?: boolean }) => {
      if (isRealMode(o, me())) {
        if (o.wipe || o.force) die("Ordinary browser commands cannot stop or wipe a separate browser.");
        const tab = ownedRealTab(me());
        if (!tab) return console.log("this session has no tab to close");
        await inReal(async (_bridge, conn) => {
          await conn.send("Target.closeTarget", { targetId: tab });
  ...
  async function openReal(url: string, o: { newTab?: boolean; wait: boolean }): Promise<void> {
    ...
    await inReal(async (bridge, conn) => {
      ...
      if (!o.newTab && owned && targets.some((t) => t.targetId === owned)) {
        ...
        // `background: true` because this one lands in the human's OWN Chrome:
  ...
  targetFlags(br.command("open <url>"))
    .description("Open a URL in this session's tab")
    ...
    .action(async (id: string, o: { real?: boolean; clone?: boolean; show?: boolean }) => {
      if (isRealMode(o, me())) {
        await inReal(async (bridge, conn) => {
          ...
          if (o.show) await conn.send("Target.activateTarget", { targetId: target.targetId });
