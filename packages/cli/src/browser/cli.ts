    ...
    .action(async (o: any) => {
      await withPage(o, async (page) => {
        ...
        if (rec.lateBy > 500) {
          console.log(
            fmt.muted(
              `  (recorder installed ${(rec.lateBy / 1000).toFixed(1)}s into this page's life — ` +
                `anything logged before that is not here. \`cast browser reload\` captures from the start.)`,
            ),
          );
        }
    ...
    .action(async (o: any) => {
      await withPage(o, async (page) => {
        ...
        if (rec.lateBy > 500) {
          console.log(
            fmt.muted(
              `  (recorder installed ${(rec.lateBy / 1000).toFixed(1)}s in — earlier requests are missing. ` +
                `\`cast browser reload\` captures the whole load.)`,
            ),
          );
        }
