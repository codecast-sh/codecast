...
function worker(opts: { delayInitialRead?: boolean } = {}) {
  ...
  class Socket {
    ...
    static CONNECTING = 0;
    readyState = 0;
    sent: Array<{ nonce?: string; id?: number; ok?: boolean; tabs?: unknown[] }> = [];
    onopen!: () => Promise<void>;
  ...
  const chrome = {
    storage: {
      local: { get: async (key: string) => {
        ...
        if (opts.delayInitialRead && reads === 1) {
          const initial = config;
          return new Promise((resolve) => { finishInitialRead = () => resolve({ bridge: initial }); });
        }
  ...
  return {
    ...
    finishInitialRead: () => finishInitialRead(),
    stall: () => { stalled = true; },
    changeConfig: () => { config = { token: 'new-pairing', port: 41730 }; },
    message: (op: string) => message({ op }, {}, () => {}),
    async prove(duringProof?: () => Promise<void>, valid = true) {
      ...
      const welcome = sock.onmessage({ data: JSON.stringify({ op: 'welcome', proof: valid ? proof : 'invalid' }) });
      const pending = duringProof?.();
      await welcome;
      await pending;
      return sock;
    },
...
describe('extension connection recovery', () => {
  test('a command arriving during host verification runs after the proof succeeds', async () => {
    const w = worker();
    await settle();
    const sock = w.sockets[0]!;
    await w.prove(() => sock.onmessage({ data: JSON.stringify({ id: 12, op: 'tabs.list' }) }));
    expect(sock.sent).toContainEqual({ id: 12, ok: true, tabs: [] });
  });

  test('a command arriving during an invalid host proof is never executed', async () => {
    const w = worker();
    await settle();
    const sock = w.sockets[0]!;
    await w.prove(() => sock.onmessage({ data: JSON.stringify({ id: 12, op: 'tabs.list' }) }), false);
    expect(sock.sent).toHaveLength(1);
    expect(sock.readyState).toBe(3);
  });

  test('a late reply from an old connection cannot answer a new hosts request', async () => {
    const w = worker();
    await settle();
    const old = await w.prove();
    let finish = () => {};
    w.context.chrome.tabs.query = () => new Promise(resolve => { finish = () => resolve([]); });
    const pending = old.onmessage({ data: JSON.stringify({ id: 12, op: 'tabs.list' }) });
    await settle();
    w.message('reconnect');
    await settle();
    const current = await w.prove();
    finish();
    await pending;
    expect(current.sent.some(message => message.id === 12)).toBe(false);
  });
  ...
  test('a late read from before a settings change cannot replace the new cached pairing', async () => {
    const w = worker({ delayInitialRead: true });
    w.changeConfig();
    w.message('reconnect');
    await settle();
    const sock = await w.prove();
