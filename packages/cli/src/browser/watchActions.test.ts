...
describe("the page script", () => {
  ...
  test("never reads text out of a password or a one time code field", () => {
    ...
    w.fire("input", { target: password, data: "hunter2", inputType: "insertText" });
    expect(last(w)).toMatchObject({ kind: "type", secret: true });
    expect(JSON.stringify(last(w))).not.toContain("hunter2");
    // A burst into the secret field is one report, not one per keystroke:
    // the count of frames would spell out the secret's length.
    const before = w.reported.length;
    for (const ch of "hunter2") w.fire("input", { target: password, data: ch, inputType: "insertText" });
    expect(w.reported.length).toBe(before);
