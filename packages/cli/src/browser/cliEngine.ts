...
export async function runVerb(verb: string, args: string[], o: Ctx = ctx(), run: RunOptions = {}): Promise<number> {
  ...
  // substring of a URL; the engine wants its own `t<N>` label or a full id.
  if (verb === "tab" && args[0] && !/^(list|new|close|switch|--.*|t\d+)$/.test(args[0])) {
...
async function loginAsPerson(url: string | undefined, waitSeconds: number): Promise<number> {
  ...
  if (plan === "cooldown") {
    ...
    if (raised.ok) {
      console.log(`${OK} raised the agent browser on ${startHost} — a separate window from your Chrome`);
      const fresh = readState();
      if (fresh) writeState({ ...fresh, loginRaisedAt: Date.now() });
    } else {
      console.log(`${fmt.warning("!")} could not raise the window (${raised.reason}); it is the Chrome window titled by the page, behind your others`);
    }
  }
