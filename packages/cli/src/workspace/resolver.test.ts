...
import { AGENT_CONFIG_COPY_CANDIDATES, AGENT_CONFIG_DIR_MAX_FILES, withAgentConfigCopies } from "./resolver.js";
...
describe("withAgentConfigCopies", () => {
  ...
  test("tracked files stay out; untracked and gitignored ones ride along, one file each, beside a tracked sibling", () => {
    ...
    expect(copyOf(m)).toEqual([".env", ".claude/settings.local.json", ".claude/skills/b/SKILL.md", ".claude/skills/b/helper.py", "AGENTS.md", "CLAUDE.local.md"]);
    expect(copyOf(m)).not.toContain("CLAUDE.md");
    expect(copyOf(m)).not.toContain(".claude/skills/a/SKILL.md");
    expect(copyOf(m)).not.toContain(".mcp.json");
  });
  ...
  test("a directory candidate over the file cap is skipped with a warning naming it; other candidates still ship", () => {
    commit("README.md");
    for (let i = 0; i <= AGENT_CONFIG_DIR_MAX_FILES; i++) write(`.claude/skills/big/f${i}.md`);
    write(".claude/commands/c.md");
    const m = withAgentConfigCopies(emptyManifest(), repo, { isInputRoot: false, warn });
    expect(copyOf(m)).toEqual([".claude/commands/c.md"]);
    expect(warnings).toEqual([`agent config directory .claude/skills skipped: over ${AGENT_CONFIG_DIR_MAX_FILES} files or 8 MiB`]);
  });
  ...
  test("an input root lists every present candidate file without git; a non-repo root does the same", () => {
    ...
    expect(copyOf(m)).toEqual([".claude/skills/s/SKILL.md", "CLAUDE.local.md"]);
  ...
  test("noise under a candidate never ships; existing .env detection is unchanged", () => {
    ...
    expect(AGENT_CONFIG_COPY_CANDIDATES).not.toContain(".mcp.json");
  });
});
