...
async function acquireWorkspaceUnlocked(repoRoot: string, name: string, opts: AcquireOptions): Promise<AcquireResult> {
  ...
  const seedFields = seed ? { startPoint: seed.startPoint, seedBase: seed.seedBase } : {};
  ...
  const prepared = await withPortReservations(repoRoot, async (reservations) => {
    ...
    if (!opts.skipPool && !opts.branch && !opts.startPoint && !existing && !inputRoot) {
...
async function acquireRootCheckoutUnlocked(repoRoot: string, opts: RootCheckoutOptions): Promise<AcquireResult> {
  ...
  const { existing, portAlloc, initialEnv } = await withPortReservations(repoRoot, async (reservations) => {
    const existing = readState(repoRoot, name);
    const reservedPorts = new Set(reservations
      .filter((reservation) => reservation.repoRoot !== root || reservation.workspace.name !== name)
      .flatMap((reservation) => Object.values(reservation.workspace.ports)));
    const portAlloc = await allocatePorts(manifest, { startIndex: existing?.resourceIndex ?? 0, reservedPorts });
    const initialEnv = buildWorkspaceEnv(manifest, portAlloc.ports);
    if (inputRoot) initialEnv.CODECAST_WORKSPACE_INPUT_ROOT = inputRoot;
    writeState(repoRoot, {
      name,
