...
const child = spawn(${JSON.stringify(binary)}, [...process.argv.slice(2), "--stdio"], { env: { PATH: process.env.PATH, HOME: process.env.HOME, CODEX_HOME: ${JSON.stringify(home)} }, stdio: ["pipe", "pipe", "pipe"] });
...
`, { mode: 0o755 });
  ...
  const server = new CodexAppServer({ codexBinary: wrapper, log: line => process.stderr.write(line + "\n"), defaultPermissions: () => permissions });
  server.on("error", () => {});
  try {
