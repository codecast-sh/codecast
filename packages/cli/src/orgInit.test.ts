...
describe("buildOrgAnalyzerPrompt", () => {
  ...
  test("holds the review as a conversation: the rules, small proposals as cards, the spec, and no letter", () => {
    for (const mode of ["init", "review"] as const) {
      ...
      // Rounds 23 to 25: the records card shrank to a third while the runs read as much; the sentence is short, the card complete.
      expect(p).toContain("the one proposal that brings every record the evidence settles in line");
      expect(p).toContain("It begins with the content, never with a note about the turn");
