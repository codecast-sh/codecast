...
test("parent extraction refuses oversized marked bodies before inspecting their contents", () => {
  const oversized = `<session-message from="${parent}" subagent="true">\n${"x".repeat(256_000)}\n</session-message>`;
  expect(subagentPromptParent(oversized, knownParents)).toBeUndefined();
  expect(() => formatAgentPrompt(parent, "x".repeat(256_000), true)).toThrow("too large");
});
