...
// The turn end settle runs on every Stop hook idle, every transcript driven
// turn end and every pane reconcile. It used the default lookup tier, whose
// per miss probe stats every project dir on the loop, so each settle of a
// session with no transcript in the index paid a walk of the store (6 to 9s
// freezes on a busy disk, 2026-09-02).
describe("resolveTurnEndStatus", () => {
  test("a settle for a session missing from the index never probes the stores on the loop", async () => {
    const reports: string[] = [];
    setSlowSyncFsThresholdForTests(0);
    setSlowSyncSink((m) => reports.push(m));
    await ensureSessionFileIndex();
    // Created after the walk: the default tier finds it by probing every
    // project dir, and that probe reports at the zero threshold. The settle
    // must not.
    const fresh = "7d3b1e9a-0000-4000-8000-0000000000ee";
    fs.writeFileSync(
      path.join(tmpHome, ".claude", "projects", "-Users-x-proj-1", `${fresh}.jsonl`),
      JSON.stringify({ type: "user", cwd: "/tmp/proj" }) + "\n",
    );
    expect(resolveTurnEndStatus(fresh)).toBe("idle");
    expect(resolveTurnEndStatus(UNKNOWN)).toBe("idle");
    expect(resolveTurnEndStatus(CLAUDE)).toBe("idle");
    expect(reports.filter((m) => m.includes("probeRecent"))).toEqual([]);
    expect(reports.filter((m) => m.includes("walkDirsSync"))).toEqual([]);
