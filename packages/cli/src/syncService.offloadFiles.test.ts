...
describe("SyncService.offloadFiles", () => {
  it("uploads a sent file and hands back what the card needs", async () => {
    ...
    const messages = [{ files: [{ localPath: path, name: "report.pdf", toolUseId: "t1", caption: "sign page 2" }] }];
    await sync.offloadFiles(messages as any);
  ...
  it("keeps a file that is too large, and never uploads it", async () => {
    ...
    const messages = [{ files: [{ localPath: path, name: "huge.bin", toolUseId: "t1" }] }];
    await sync.offloadFiles(messages as any);
  ...
  it("keeps a file that has gone away", async () => {
    ...
    const messages = [{ files: [{ localPath: "/nope/gone.docx", name: "gone.docx", toolUseId: "t1" }] }];
    await sync.offloadFiles(messages as any);
  ...
  it("records a failed upload rather than dropping the file", async () => {
    ...
    const messages = [{ files: [{ localPath: path, name: "notes.txt", toolUseId: "t1" }] }];
    await sync.offloadFiles(messages as any);
  ...
  it("uploads one path once, however often a live turn re-materializes it", async () => {
    ...
    const first = [{ files: [{ localPath: path, name: "plan.md", toolUseId: "t1" }] }];
    const second = [{ files: [{ localPath: path, name: "plan.md", toolUseId: "t1" }] }];
    await sync.offloadFiles(first as any);
    await sync.offloadFiles(second as any);
  ...
  it("leaves an already-uploaded file alone", async () => {
    ...
    let uploads = 0;
    (sync as any).uploadBytes = async () => { uploads++; return "sid"; };
    const messages = [{ files: [{ name: "a.pdf", mediaType: "application/pdf", storageId: "sid-old", toolUseId: "t1" }] }];
    await sync.offloadFiles(messages as any);
    expect(uploads).toBe(0);
