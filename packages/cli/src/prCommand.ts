...
/** "3 green" / "1 red of 4" / "" when the PR has no checks. */
...
async function locate(
  ...
  const locator = buildPrLocator(ref, {
    session: deps.detectCurrentSessionId(),
    repository: opts.repo ?? local.repository,
    branch: local.branch,
  });
  if (opts.repo) locator.repository = opts.repo;
  return locator;
}
...
export function registerPrCommand(program: Command, deps: PublishDeps): void {
  ...
  pr.command("comment")
    ...
    .action(async (ref: string | undefined, text: string | undefined, options) => {
      // One argument means the ref was left out and this is the body: a
      // reference always parses, a comment almost never does.
      let refArg = ref;
      let body = text;
      ...
      console.log(`${c.green}ok${c.reset} commented on ${c.cyan}${result.repository}#${result.number}${c.reset}` +
