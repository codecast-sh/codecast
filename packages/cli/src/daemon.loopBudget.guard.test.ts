...
const ROWS: Row[] = [
  ...
  { file: D, name: "attemptHibernation", kind: "function", minLines: 35, mustContain: "inspectTarget" },
  { file: D, name: "inspectHibernationTarget", kind: "function", minLines: 20, mustContain: "snapshotProcessTableAsync" },
  { file: D, name: "parkHibernationTerminal", kind: "function", minLines: 25, mustContain: "boundary.revalidate" },
  { file: D, name: "flushHibernationStamp", kind: "function", minLines: 20, mustContain: "serializeSessionStatus" },
  { file: D, name: "runHeartbeatFlush", kind: "function", minLines: 20, mustContain: "heartbeatManagedSessionsBatch" },
  ...
  // (readConversationCache, findReapTranscript) are memoized single stats and
  // are shared with the reaper, which runs on the same tick.
  { file: D, name: "productionHibernationIo", find: "const productionHibernationIo", kind: "call", minLines: 30, mustContain: "tmuxExec" },
  { file: D, name: "sweepStaleTmuxServers", kind: "function", minLines: 10, mustContain: "staleTmuxServerKillPlan" },
  // Nested in main().
  ...
  { file: "cursorWatcher.ts", name: "checkWorkspaceForChanges", kind: "method", minLines: 10, mustContain: "observation" },
  { file: "cursorWatcher.ts", name: "readCursorRows", kind: "method", minLines: 5, mustContain: "collectScan" },
...
describe("daemon loop budget guard", () => {
  test("migrated ingest bodies cannot read or parse transcripts on the supervisor", () => {
    for (const {row,blocks} of extracted.filter(({row}) => row.name.endsWith("Pass") && row.mustContain === "await readTranscriptIngest")) {
      for (const block of blocks) for (const {line} of codeLines(block.text)) {
        expect(line, row.name).not.toMatch(/\b(readIngestWindow|readFileHead|readFileTailSync|parseTranscriptFor|parseSessionFile|parseCodexSessionFile|extractMessagesFromCursorDb|assembleOpencodeSession|recoverImagesFromBackup|readCodexModelBeforeOffset|generateTitleFromMessage|detectPermissionPrompt|codexSessionMetadataFor|resolveCodexForkRoot)\s*\(|JSON\.parse\s*\(|fs\.promises\.readFile\s*\(/);
      }
    }
  });
