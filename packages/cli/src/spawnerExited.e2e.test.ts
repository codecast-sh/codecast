...
test.skipIf(process.platform === "win32")("a spawned child that exits before its conversation exists still resolves its parent", async () => {
  ...
  try {
    await run(process.execPath, [join(import.meta.dir, "test-helpers/exitedSpawnFixture.ts"), dir], { env: { ...process.env, HOME: dir, NODE_ENV: "test" }, timeout: 20_000, killSignal: "SIGKILL" });
