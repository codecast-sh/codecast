...
describe("classifyStartedPane", () => {
  ...
  test("grok's y/n folder-trust dialog is trust, not booting", () => {
    ...
    expect(classifyStartedPane(GROK_TRUST_PANE, grokPrompt, grokReadiness, { alternateScreen: true })).toBe("trust");
    expect(classifyStartedPane(LAUNCH_ECHO + "\n" + GROK_TRUST_PANE, grokPrompt, grokReadiness, { alternateScreen: true })).toBe("trust");
  });
