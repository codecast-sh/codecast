...
// Whether a tmux-hosted agent has a turn running. The pane is the authority
// when it is unambiguous: a spinner is a turn, an empty prompt is not, whatever
// a lagging hook says. Dialog and unrecognized states fall back to the hook
// status, which is what names a permission prompt as "inside a turn". With no
// pane at all (an agent outside tmux) the hook status is all there is.
export function turnLooksActive(hookStatus: string | null | undefined, paneState: string | null | undefined): boolean {
  ...
  if (paneState === "idle" || paneState === "exited" || paneState === "starting") return false;
