...
//
// It is literally the gate's own test (the gate calls this function), which is
// what ct-49841 fixed: the two used to be separate code, the gate counting
...
describe("tmuxComposerHoldsPayload", () => {
  ...
  test("two copies at the prompt are not 'already there' (ct-49841)", () => {
    // The doubled composer ct-49753 refuses at the gate. Read as "already at
    // the prompt" it took the pre-paste short circuit straight to Enter, which
    // submits both copies as one message: the very outcome the gate's count
    // exists to prevent.
    expect(tmuxComposerHoldsPayload(BOX(`${PAYLOAD}${PAYLOAD}`), PAYLOAD)).toBe(false);
    expect(tmuxComposerHoldsPayload(BOX(`${PAYLOAD} ${PAYLOAD}`), PAYLOAD)).toBe(false);
  });

  test("a payload that repeats its own opening is still recognized once", () => {
    // Same opt-out the gate makes: such a payload reads as two copies of
    // itself, so counting would reject every delivery of it.
    const repeating = "z".repeat(120);
    expect(tmuxComposerHoldsPayload(BOX(repeating), repeating)).toBe(true);
  });
