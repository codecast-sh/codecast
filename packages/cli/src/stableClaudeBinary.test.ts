...
import { CLAUDE_VERSIONED_BINARY_RE, resolveClaudeInstall, stableClaudeBinary } from "./stableClaudeBinary.js";
...
describe("stableClaudeBinary", () => {
  ...
  test("resolveClaudeInstall follows the install symlink and skips missing candidates", () => {
    const { root, release } = rig();
    const source = release("2.1.241");
    const link = path.join(root, "bin", "claude");
    fs.mkdirSync(path.dirname(link));
    fs.symlinkSync(source, link);
    const notOnPath = () => null;
    expect(resolveClaudeInstall([path.join(root, "nope", "claude"), link], notOnPath)).toBe(fs.realpathSync(source));
    expect(resolveClaudeInstall([path.join(root, "nope", "claude")], notOnPath)).toBeNull();
    expect(resolveClaudeInstall([], () => link)).toBe(fs.realpathSync(source));
  });
