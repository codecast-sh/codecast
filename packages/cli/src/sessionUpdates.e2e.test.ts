...
function fixture() {
  ...
  const db = makeFakeDb({
    users: [{ _id: "owner" }],
    conversations: [
      { _id: "source", short_id: "jxsrc01", session_id: "native-thread", user_id: "owner", is_private: true },
      { _id: "source-2", short_id: "jxsrc02", session_id: "native-thread-2", user_id: "owner", is_private: true },
      { _id: "target", short_id: "jxtgt01", session_id: "target-thread", user_id: "owner", is_private: true, owner_device_id: "device", status: "completed" },
    ],
    managed_sessions: [], pending_messages: [], session_updates: [],
    session_decisions: [{ _id: "decision", conversation_id: "target", state: "pending", question: "Approve the change?" }],
  });
  ...
  const send = (body: string, requestId: string, from?: string, stdin?: string) =>
    run(["send", "jxtgt01", body, "--update", "--request-id", requestId, ...(from ? ["--from", from] : [])], stdin);
...
describe("updates through CLI, handlers, batching, claim, ACK and presentation", () => {
  test("multiple source sessions reach one envelope with exact bodies and an honest joined receipt", async () => {
    ...
    f.advance(500);
    await f.send(secondBody, requestIds[1], "jxsrc02");
    ...
    expect(f.db._tables.session_decisions[0].state).toBe("pending");
    expect(parseSessionUpdateBatch(pending.content)?.members).toEqual([
      { id: first._id, from: "jxsrc01", sent_at: createdAt, body: firstBody },
      { id: f.db._tables.session_updates[1]._id, from: "jxsrc02", sent_at: createdAt + 500, body: secondBody },
    ]);
  ...
  test("queued cancellation affects one member and ordinary sends bypass batching", async () => {
    ...
    await f.run(["send", "jxtgt01", "Ordinary send"]);
    expect(f.db._tables.pending_messages).toHaveLength(1);
    expect(f.db._tables.pending_messages[0].content).toBe('<session-message from="jxsrc01">\nOrdinary send\n</session-message>');
    await f.flushTimer();
