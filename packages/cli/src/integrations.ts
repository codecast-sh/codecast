...
export function registerIntegrationsCommand(program: Command, deps: IntegrationsDeps): void {
  ...
  integrations
    ...
    .action(async (providerRaw: string, options: any) => {
      ...
      const candidates: any[] = Array.isArray(result) ? result : result?.candidates ?? [];
      if (options.json) {
  ...
  integrations
    ...
    .action(async (providerRaw: string, ref: string, options: any) => {
      ...
      const result = await post("/cli/integrations/candidates", { provider });
      const pool: any[] = (Array.isArray(result) ? result : result?.candidates ?? []).filter((cand: any) => !options.kind || cand.kind === options.kind);
      ...
      console.log(`${c.green}ok${c.reset} Importing ${c.bold}${cand.name}${c.reset} ${c.dim}(${cand.kind})${c.reset} as ${c.cyan}${source.id ?? source._id}${c.reset}`);
      console.log(fmt.muted(`Pull it now with: cast integrations sync ${source.id ?? source._id}`));
    });
