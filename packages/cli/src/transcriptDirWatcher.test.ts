...
describe("transcriptDirWatcherConfig: dirFilter per client", () => {
  ...
  test("muse enters dated YYYY/MM/DD dirs and a session uuid dir only, never dot dirs", () => {
    ...
    // subagent nests (sessions/.../<uuid>/subagent/<subuuid>/session.jsonl)
    // refuse: the date-sharded shape check fails below the session dir.
    expect(
      cfg.extractSessionId(file("r", "2026", "09", "18", uuid, "subagent", uuid, "session.jsonl")),
    ).toBeNull();
  });
});
