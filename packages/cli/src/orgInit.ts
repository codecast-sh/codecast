...
export const ORG_CONVERSATION_RULES = {
  ...
  structure_first: "The heart of the conversation is the reporting structure, and the structure is three things together: the person's direct reports, the roles under them with what each one looks after, and every session of the person's own, each placed under one of those roles or kept by the person. A structure that leaves any of the person's sessions unplaced is not yet the structure. Everything else serves it: a record that is already done, a goal the company has not written down.",
  opening: "The opening is one short turn, not the review. It carries two things and one question. First, what is already done: one sentence, and the one proposal that brings every record the evidence settles in line. The sentence stays short; the proposal is complete, however many records it holds. Second, the reporting structure you propose, laid out so a person takes it in at once: who reports to whom, what each one looks after, and where each of the person's own sessions goes, under a role or kept by them, with its proposal. Then the one question whose answer most changes that structure. Everything else you found, the goals, the coverage, the load on a seat, a lead that is paused, waits for a later turn or for the person to ask.",
  ...
  evidence_behind: "Knowledge stays behind the words. Read and check as much as the records need, and hold all of it: it is what every proposal rests on, and it comes out when the person asks. The message itself says what is so, never the story of what you read, checked or did to find it out. It begins with the content, never with a note about the turn, the message or what was just written; it never cites an id, never talks about you, and never names the thread it is written in.",
  ...
  words: "The words are the page's, not this briefing's: a role is an agent that keeps watching one area of work, said once and then called a role; an area of work is what it looks after; a project or a plan is named as it is filed. What a role may do in a day never comes up. The names of the inputs you read, the health signals and short ids never reach the person, who is told what is happening. A number carries what it means for the person, never only its value. No em dashes.",
} as const;
