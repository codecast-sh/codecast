...
export const ORG_UNNAMED_ROLES_RULE = "A session older than a week with a standing purpose is a role that has not been named. No row of `sessions.long_running` is a role yet, whatever it reports to and however many projects its row shows it touched: the roles are `org.roles` and nothing else, and a session that reports to a role is that role's hand, not a role. The inputs list the long running sessions with the facts to judge them by (`sessions.long_running`): how long each has run, how many helper sessions it started, the routines that wake it, its pinned state, its first message and the projects it touched. A session that keeps returning to one area of work, that wakes on a routine, or that has started helpers by the hundred is managing that area already, whatever it was first opened for, and a routine that wakes it on a schedule to do the same job again is proof enough on its own, however small its spend and however few helpers it has started, when the job has no end the row names; a monitor kept on a finished fix, a watch with a date, and an experiment with a checkpoint are not standing jobs, whatever wakes them: each is a program at most, or nothing. A long session that is one large task or one feature build ends when the task ends, and is not a role, whatever its size: at most it is a program whose end is that plan or task. When it was last active is on its row: a session quiet for more than two weeks has ended, whatever its size, and is not a seat; the sessions that are roles are the ones still active this week. Propose each session that already works as a role as a role change whose seat is the session itself, so accepting names it rather than replaces it: nothing restarts, its history stays, and the sessions it started stay under it. Everything about the role is read from the session, not designed: its name is the session's title, its area is the projects its own row shows it touched, the ones it filed tasks in and is bound to, and a project it never touched does not enter its area because that project lacks a lead; it reports to the person who owns it, and its charter is drafted from its pinned state and its first message, written as the job it has been doing and never as today's status. Its daily limit is its measured daily use with headroom, stated with the number it came from and the days that number covers (`use_7d` on its row), so the limit never stops work that runs today. When its row carries no measured use, the limit is provisional: keep the default, say in the change and in the letter that it is a starting point nobody has measured and will be raised when it is hit, and never state a confident multiple of today's cost that rests on a guessed limit. Tell the person what naming changes and what it does not, and when the session is private, that a team's role can be seen by the team. Such a role is evidence of where work is managed, so read it before you size any new role over the same area. Naming keeps the reporting line the session has today, which is the person who runs it: who a session answers to is part of how it works, and the card promises that naming changes nothing about that. An area that a role already watches, or that a paused role used to watch, is not a reason to leave such a session unnamed and never a reason to fold it into that role's area in a finding: name it, and when you think it belongs under that role, say so as a separate move change in the same ask, in its own sentence with its reason, so the person can accept the name and skip the move. In a finding, name every row of `sessions.long_running` you did not propose, each with its reason in a few words, so the person can name one you left out and can see that none was skipped; a row that is neither proposed nor in that list is a row you did not read.";
