...
export function registerOrgInitCommands(program: Command, deps: OrgInitDeps): void {
  ...
  org
    .command("apply")
    .description("Walk a proposal's changes at a shell: accept, edit or skip each one (op-N); refused inside a session, where a person must decide. A decision stack (ds-N) from a template still applies as before.")
    .argument("<ref>", "A proposal (op-N) or, for templates that still use one, a decision stack (ds-N)")
    .option("--all", "Accept every remaining change in order without walking them (op-N only)")
