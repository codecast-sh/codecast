...
export const ORG_COVERAGE_RULE = "Every piece of work has a lead, and a paused lead is not one: `coverage` names it as `lead_paused` and counts the project as without a lead, so the proposal asks to resume or replace it as a change the person accepts or skips (a move that resumes it, or a role that replaces it and a retire), never as a finding alone, before anything else over that area. `coverage` says where things stand: the projects with work and their lead (the role the project names, or the one role whose area lists it), the roles that only watch, and the work outside any project (plans with open tasks and no project, tasks with neither, areas of a repository no project claims). Cover all of it from the initiative down, the projects of an active initiative first. Wrap the project that exists: a role's area is the project as it is filed, under its own name, and you never invent an area beside a project that already holds the work. A session already working as a role is the first candidate to lead what it touched. Some work is a question for the person, not a project or a role: a repository with many commits and few of this workspace's sessions is another company's work seen through a shared checkout, and a new repository one person alone works in, with no project and no task under it, is theirs to claim or not: neither a live site, a shipped version nor a plan filed for it settles that. Ask in one finding and propose nothing over it until they answer. Work outside any project that this workspace's own sessions and filed tasks show to be its business gets a project first, a `projects` create whose title says what the work is and whose path is where it lives, with `file` changes for the loose plans, and then a lead, in the same proposal and in that order. Aim at about one role per project, and depart from that only with a reason the change states: two small projects next to each other share one lead, or one large project is split along its plans, each side filed under its own project first. A lead watches work that sessions and commits carry: a person's own hand list (imported tickets, hiring, deals, filings) gets no role and one sentence saying why, and a tracker of one person's proposals shares the lead of the area they propose for. Count coverage after the records are in line: a project the evidence says is finished or untouched is paused and leaves the count. Alike leads, each wrapping one project, are one ask; a lead that departs from one per project is its own. After the headings, one coverage line gives before and after in counts, in this form: \"9 of 12 projects had a lead; after, 12 of 12, and 2 new projects hold work that had none\". Before is `coverage.with_lead` of `coverage.with_work`, corrected for the projects your own status changes pause; after is what stands once every change is accepted. When you stop short of full coverage, the line says how many are left and a finding says why.";
