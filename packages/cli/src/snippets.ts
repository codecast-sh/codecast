...
export function getSnippetTargets(): SnippetTarget[] {
  ...
  for (const descriptor of Object.values(AGENT_CLIENTS)) {
    ...
    const filePath = declared.replace(/^~(?=\/)/, home);
    const dirPath = path.dirname(filePath);

    // Presence gate: claude is the host CLI's own client and always gets its
    // file; any other client is included only when its config directory already
    // exists, which is the long-standing signal for "installed here". Without
    // it, every machine would grow an ~/.codex/AGENTS.md it never asked for.
    if (descriptor.id !== "claude" && !fs.existsSync(dirPath)) continue;

    targets.push({ filePath, dirPath, label: declared });
  }
