...
/** The sentinel's read is gated behind a front-app change, so this costs the
 *  daemon nothing — see InstanceState.computerRaisedAt. With no managed
 *  browser there is no state to patch and nothing to bounce, so skipping is
 *  correct rather than lossy. */
export function stampComputerRaise(): void {
  try {
    const fresh = readState();
    if (fresh) writeState({ ...fresh, computerRaisedAt: Date.now() });
  } catch {
    /* the raise still happens; the worst case is one bounced front-switch */
  }
}
...
export class ComputerClient {
  ...
  private async spawnHelper(): Promise<net.Socket> {
    ...
    try {
      ...
      child = spawn(launch.cmd, launch.args, { detached: true, stdio: "ignore" });
      child.unref();
    } catch (err) {
      fs.rmSync(socketDir, { recursive: true, force: true });
      throw asComputerError(err);
    ...
    const failure = watchLaunchFailure(child);
 ...
 *  launch fails fast with the real reason instead of timing out. */
function watchLaunchFailure(child: ChildProcess): { promise: Promise<never>; cleanup: () => void } {
  let cleanup = (): void => {};
  const promise = new Promise<never>((_resolve, reject) => {
    const onError = (err: Error) => reject(new ComputerError("accessibility_error", `the computer helper failed to start: ${err.message}`));
    const onExit = (code: number | null, signal: NodeJS.Signals | null) =>
      reject(
        new ComputerError(
          "accessibility_error",
          `the computer helper exited before connecting: ${typeof code === "number" ? `code ${code}` : `signal ${signal ?? "unknown"}`}`,
        ),
      );
