...
//
// The rule (design ct-49518 section 11.2): no verb raises a window.
// `--restore-window` is the only exception in the whole feature, and it is
// stamped through the focus sentinel's deliberate-raise path so a raise of the
// agent's own Chrome is not bounced a second later. Any other file that sets
// `restoreWindow` on a helper request is an unstamped, unasked-for front
// switch — the failure our focus memories exist to prevent.
//
...
const ALLOWED: Record<string, string> = {
  "computer/cli.ts": "declares --restore-window, the one flag in the feature that may take the human's screen",
  "computer/run.ts": "turns that flag into the helper request field, and nothing else here sets it",
