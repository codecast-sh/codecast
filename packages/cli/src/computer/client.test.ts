...
describe("ComputerClient", () => {
  test("launches a helper, records it, and destroys the token file once connected", async () => {
    isolatedHome();
  ...
  test("a second client reuses the running helper instead of launching another", async () => {
  ...
  test("two invocations racing to launch produce one helper", async () => {
    const home = isolatedHome();
    const log = path.join(home, "launches.log");
    withEnv({ FAKE_LAUNCH_LOG: log });
  ...
  test("a helper from another release is terminated and relaunched, not reused", async () => {
    const home = isolatedHome();
    const log = path.join(home, "launches.log");
  ...
  test("a helper on another protocol version fails as provider_incompatible", async () => {
    isolatedHome();
  ...
  test("a helper error keeps its code and carries the recovery", async () => {
    isolatedHome();
    withEnv({
  ...
  test("a code this CLI does not know reads as accessibility_error, not as success", async () => {
    isolatedHome();
  ...
  test("a request carrying the wrong token is refused", async () => {
    isolatedHome();
    const client = makeClient();
  ...
  test("a timed-out request invalidates the queue behind it", async () => {
    isolatedHome();
  ...
  test("a busy helper is reported busy, never killed", async () => {
    isolatedHome();
  ...
  test("a helper that dies on launch fails with its exit, not with a timeout", async () => {
    isolatedHome();
  ...
  test("terminate stops the helper and forgets it", async () => {
    isolatedHome();
    const client = makeClient();
    await client.capabilities();
  ...
  test("--restore-window stamps the deliberate raise before the request goes out", async () => {
    isolatedHome();
    const order: string[] = [];
    const client = makeClient({
