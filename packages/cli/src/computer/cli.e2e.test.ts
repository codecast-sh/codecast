...
const granted = withHelper ? test : test.skip;
...
describe("the verb group is registered and answers without a helper", () => {
  test("`cast computer --help` lists every verb the design names", () => {
  ...
  test("an invented verb fails instead of printing help to stdout", () => {
  ...
  test("a flag error is refused in-process, with the code and the recovery", () => {
  ...
  test("a build with no embedded helper says so rather than hanging or crashing", () => {
  ...
  test("permissions reports the missing helper instead of claiming a grant", () => {
    if (withHelper) return;
  ...
  test("--id only accepts the two grants that exist", () => {
