...
/**
 * Mark a raise this process is about to make as deliberate, so the daemon's
 * focus sentinel spares it instead of bouncing the window a second later.
 *
 * Why here rather than in the caller: both raise sites in the feature need it
 * (`--restore-window` through the client, `permissions --open-settings`
 * through the helper's setup window), and this file is already where a
 * short-lived `cast computer` process writes state the daemon reads.
 *
 * The sentinel's read is gated behind a front-app change, so this costs the
 * daemon nothing — see InstanceState.computerRaisedAt. With no managed browser
 * there is no state to patch and nothing to bounce, so skipping is correct
 * rather than lossy.
 */
