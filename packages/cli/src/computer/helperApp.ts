...
declare const CODECAST_MAC_COMPUTER_HELPER: string;
...
/** The embedded `.app` as a tar, or null on a build without one (Linux and
 *  Windows targets, and a Mac without swift). */
export function embeddedHelperPayload(): Buffer | null {
  if (typeof CODECAST_MAC_COMPUTER_HELPER !== "string" || !CODECAST_MAC_COMPUTER_HELPER) return null;
  return Buffer.from(CODECAST_MAC_COMPUTER_HELPER, "base64");
}
...
export function materializeHelperApp(opts: { version?: string } = {}): MaterializeResult {
  ...
  const payload = embeddedHelperPayload();
  if (!payload) {
...
export function helperAvailability(): HelperAvailability {
  ...
  return {
    embedded: embeddedHelperPayload() !== null,
    materialized,
    appPath,
    atFixedPath,
