 ...
 * token to this machine. Remote hosts get a token of their own through
 * mintForDevice (sd-242), and a machine whose device id changes (Migration
 * Assistant, a disk clone) heals itself (deviceTokenHeal.ts). Turning this off
 * again only stops new mints from binding; bound tokens keep working.
 */
export const REQUEST_DEVICE_BOUND_TOKENS = true;
 ...
 * `heal` starts the heal when the device moved; only the long lived daemon
 * and an ordinary cast command pass it. A hook or other hot path must not:
 * the pending mint would keep a process alive that should exit at once, and
 * the minted device it presents meanwhile is still accepted.
 */
export function bearerFromStored(stored: string, opts: { heal?: boolean } = {}): string {
  ...
  if (record?.from === fingerprint(secret)) {
    ...
    if (record.to && secretFromStored(record.to) !== secret) return bearerFromStored(record.to, opts);
  }
  ...
  if (minted !== current && opts.heal) startDeviceTokenHeal(secret, minted, current);
