...
import { SLOW_SYNC_FS_MS, reportSpawnTimeout, setSlowSyncFsThresholdForTests, setSlowSyncSink, timeSync, timeSyncFs } from "./slowSync.js";
...
describe("slow sync reporting", () => {
  ...
  it("a test threshold of zero makes every timed fs call report, and null restores the default", () => {
    const seen: string[] = [];
    setSlowSyncSink((m) => seen.push(m));
    try {
      setSlowSyncFsThresholdForTests(0);
      timeSyncFs("walkDirsSync", "/tmp/a", () => 1);
      setSlowSyncFsThresholdForTests(null);
      timeSyncFs("walkDirsSync", "/tmp/b", () => 1);
    } finally {
      setSlowSyncSink(null);
      setSlowSyncFsThresholdForTests(null);
    }
    expect(seen).toHaveLength(1);
    expect(seen[0]).toContain("/tmp/a");
  });

  it("reportSpawnTimeout names the killed child through the same sink", () => {
    const seen: string[] = [];
    setSlowSyncSink((m) => seen.push(m));
    try {
      reportSpawnTimeout("security", "find-generic-password -s x -w", 30_000);
    } finally {
      setSlowSyncSink(null);
    }
    expect(seen).toEqual(["[SPAWN-TIMEOUT] security killed after 30000ms: find-generic-password -s x -w"]);
  });
});
