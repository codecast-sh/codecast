...
// Tests lower the filesystem threshold to zero so a sync walk of a small
// fixture still reports: the report is then the proof that a call ran on the
// loop at all, not a measure of how long it took.
let slowSyncFsThresholdMs = SLOW_SYNC_FS_MS;
export function setSlowSyncFsThresholdForTests(ms: number | null): void {
  slowSyncFsThresholdMs = ms ?? SLOW_SYNC_FS_MS;
}

/**
 * A child the daemon had to kill on its timeout. Not a loop hold (the call was
 * async), but the same reader wants to know: a wedged keychain or launchctl
 * call otherwise fails quietly and the tick that made it stops for good.
 */
export function reportSpawnTimeout(name: string, detail: string, timeoutMs: number): void {
  try {
...
export function timeSyncFs<T>(name: string, detail: string | (() => string), fn: () => T): T {
  return timeSync("SLOW-SYNC-FS", slowSyncFsThresholdMs, name, detail, fn);
}
