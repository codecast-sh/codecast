...
const server = Bun.serve({
  port: 0,
  async fetch(req) {
    const body = (await req.json()) as { path: string; args: any[] };
    ...
    await Bun.sleep(250); // a real round trip, so concurrent readers overlap it
...
try {
  if (mode === "login") {
    ...
    out.bearer = bearerFromStored(JSON.parse(fs.readFileSync(file, "utf-8")).auth_token);
