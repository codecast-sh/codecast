...
describe("hibernationBlockReason", () => {
  ...
  for (const status of ["working", "thinking", "compacting"] as const) {
  ...
  test("a settle verdict does not block — parked work is exactly what to park", () => {
    for (const status of ["idle", "waiting", "dormant", "done", "connected"] as const) {
