...
export type FolderRow = {
  ...
  truncated: boolean;
  /** The synced count is the server's exact count for the folder, not the
   *  recent window's (which misses folders without recent sessions). */
  counted: boolean;
  first: number | null;
  last: number | null;
  /** Still a folder on this machine; null when this machine never saw it. */
  exists: boolean | null;
  /** Other listed folders inside this one: its synced count includes theirs. */
  nested: number;
  /** Sessions shared one by one, whatever the folder's rule says. */
  sharedByHand: number;
  /** Sessions kept private by hand: no rule shares them. */
  keptPrivate: number;
  syncing: boolean;
  sharing: Sharing;
};
...
export function buildFolderRows(
  ...
  for (const folder of paths) {
    ...
    rows.push({
      ...
      nested: 0,
      sharedByHand: e?.manually_shared ?? 0,
      keptPrivate: e?.hidden ?? 0,
      syncing: isSyncing(overview.settings, folder),
      sharing: sharingOf(overview.rules, folder, repository),
    });
  }
  for (const row of rows) row.nested = rows.filter((r) => r.path.startsWith(row.path + "/")).length;
  return rows.sort((a, b) => (b.last ?? 0) - (a.last ?? 0));
}

/** A folder worth a decision: it has synced sessions, or it is still on
 *  this machine. A deleted scratch folder that never synced is noise. */
export function isLive(row: FolderRow): boolean {
  return row.synced > 0 || row.exists !== false;
}
...
export type SyncPlan = {
  ...
  leftSyncAll: boolean;
  /** Folders left off that list because choosing them would include a
   *  folder being stopped: their own sessions stop syncing too. */
  alsoStopped: string[];
};
...
export function planSyncChange(settings: SyncSettings, change: SyncChange, known: string[]): SyncPlan {
  const none: SyncPlan = { next: null, stillCovered: [], leftSyncAll: false, alsoStopped: [] };
  ...
  // "Inside" the way the daemon reads a chosen folder (syncScope.ts): the
  // folder itself, or a path below it. A chosen "/" covers only itself there.
  const under = (f: string, d: string) => {
    const [a, b] = [path.resolve(f), path.resolve(d)];
    return a === b || a.startsWith(b + path.sep);
  };
  // A folder taken off the list takes the folders inside it along.
  const dropped = (f: string) => change.unsync.some((d) => under(f, d));
  const fromAll = settings.sync_mode === "all";
  const base = fromAll ? [...new Set(known)] : settings.sync_projects;
  // Building the list from "everything", a folder that contains a stopped one
  // cannot go on it: a chosen folder covers all of its subfolders.
  const alsoStopped = fromAll ? base.filter((f) => !dropped(f) && change.unsync.some((d) => d !== f && under(d, f))) : [];
  const kept = base.filter((f) => !dropped(f) && !alsoStopped.includes(f));
  ...
  return { next: changed ? after : null, stillCovered, leftSyncAll: fromAll, alsoStopped };
}
