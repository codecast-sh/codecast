...
import { buildFolderRows, parseSince, planSyncChange, prettyPath, resolveFolder, resolveTeam, sharingOf, type FolderRule, type Overview } from "./model.js";
...
describe("buildFolderRows", () => {
  ...
  test("exact server counts replace the recent window's count", () => {
    const rows = buildFolderRows(overview(), null, { [`${HOME}/src/app`]: { count: 1200, first_started_at: 100, last_started_at: 5_000, truncated: true } });
    expect(rows[0]).toMatchObject({ synced: 1200, truncated: true, first: 100, onMachine: null });
  });
...
describe("planSyncChange", () => {
  ...
  test("stopping one folder while everything syncs switches to every other known folder", () => {
    ...
    expect(plan).toEqual({ next: { sync_mode: "selected", sync_projects: [`${HOME}/src/app`, `${HOME}/src/notes`] }, stillCovered: [], leftSyncAll: true });
  });
