...
import { buildFolderRows, isLive, parseSince, planSyncChange, prettyPath, resolveFolder, resolveTeam, sharingOf, type FolderRule, type Overview } from "./model.js";
...
describe("planSyncChange", () => {
  const known = [`${HOME}/src/app`, `${HOME}/src/notes`, `${HOME}/scratch`];

  test("stopping one folder while everything syncs switches to every other known folder", () => {
    const plan = planSyncChange({ sync_mode: "all", sync_projects: [] }, { unsync: [`${HOME}/scratch`] }, known);
    expect(plan).toEqual({ next: { sync_mode: "selected", sync_projects: [`${HOME}/src/app`, `${HOME}/src/notes`] }, stillCovered: [], leftSyncAll: true, alsoStopped: [] });
  });

  test("from everything, a folder containing the stopped one stays off the list, and says so", () => {
    const plan = planSyncChange({ sync_mode: "all", sync_projects: [] }, { unsync: [`${HOME}/src/app`] }, [HOME, ...known]);
    expect(plan.next).toEqual({ sync_mode: "selected", sync_projects: [`${HOME}/src/notes`, `${HOME}/scratch`] });
    expect(plan.alsoStopped).toEqual([HOME]);
    expect(plan.stillCovered).toEqual([]);
  });

  test("stopping a folder stops the folders inside it too", () => {
    const plan = planSyncChange({ sync_mode: "all", sync_projects: [] }, { unsync: ["/private/tmp"] }, [...known, "/private/tmp", "/private/tmp/a", "/private/tmp/b/c"]);
    expect(plan.next).toEqual({ sync_mode: "selected", sync_projects: known });
    expect(plan.stillCovered).toEqual([]);
  });

  test("a chosen parent still covers a folder taken off the list", () => {
    const plan = planSyncChange({ sync_mode: "selected", sync_projects: [`${HOME}/src`] }, { unsync: [`${HOME}/src/app`] }, known);
    expect(plan.next).toBeNull();
    expect(plan.stillCovered).toEqual([{ folder: `${HOME}/src/app`, by: `${HOME}/src` }]);
  });

  test("syncing adds only folders not already covered; --all is a no-op when already on", () => {
    const settings = { sync_mode: "selected" as const, sync_projects: [`${HOME}/src`] };
    expect(planSyncChange(settings, { sync: [`${HOME}/src/app`] }, known).next).toBeNull();
    expect(planSyncChange(settings, { sync: [`${HOME}/scratch`] }, known).next).toEqual({ sync_mode: "selected", sync_projects: [`${HOME}/src`, `${HOME}/scratch`] });
    expect(planSyncChange({ sync_mode: "all", sync_projects: [] }, { all: true }, known).next).toBeNull();
    expect(planSyncChange(settings, { all: true }, known).next).toEqual({ sync_mode: "all" });
  });
});
