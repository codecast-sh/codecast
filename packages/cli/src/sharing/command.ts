...
async function loadState(deps: Deps, opts: { rescan?: boolean; exact?: boolean } = {}): Promise<State> {
  ...
  if (opts.exact) {
    ...
    // the ones missing, read what is there, and read the rest once more after
    // the server has had a moment to build them.
    await apiPost(deps, "/cli/sharing/stats", { paths: wanted.slice(0, 300) }, { exitOnError: false }).catch(() => {});
