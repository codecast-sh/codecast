...
async function loadState(deps: Deps, opts: { rescan?: boolean; exact?: boolean } = {}): Promise<State> {
  ...
  if (opts.exact) {
    const synced = rows.filter((r) => r.synced > 0).map((r) => r.path);
    // Counts are kept per folder on the server and built on request; ask for
    // any that are missing, and read the ones already there.
    void apiPost(deps, "/cli/sharing/stats", { paths: synced.slice(0, 200) }, { exitOnError: false }).catch(() => {});
    rows = buildFolderRows(overview, local, await exactCounts(deps, synced));
  }
...
function printOverview(state: State, opts: { all?: boolean }): void {
  ...
  const syncingCount = rows.filter((r) => r.syncing).length;
  ...
  if (settings.sync_mode === "all") {
    ...
    console.log(`  Only chosen folders upload (${settings.sync_projects.length} chosen). A folder used for the first time does not sync until it is added.`);
  }
  ...
  for (const team of teams) {
    const level = teamLevel(team);
    const pinned = (team.visibility_history?.length ?? 0) > 0 ? fmt.muted(" (older sessions stay at a lower level)") : "";
    console.log(`  ${fmt.accent(team.name)}  ${describeTeammates(team.member_count)}, level ${level.label}: they see ${level.sees}${pinned}`);
  }

  const shown = opts.all ? rows : rows.slice(0, 40);
  console.log(`\n${c.bold}Folders${c.reset} ${fmt.muted(`${rows.length} known, ${syncingCount} syncing, newest first${shown.length < rows.length ? `; showing ${shown.length}, --all for every one` : ""}`)}`);
  const width = Math.min(44, Math.max(12, ...shown.map((r) => prettyPath(r.path, home).length)));
  for (const r of shown) {
    const name = prettyPath(r.path, home);
    const counts = [
      r.onMachine != null ? `${r.onMachine} here` : null,
      r.synced > 0 ? `${r.synced}${r.truncated ? "+" : ""} synced` : "none synced",
    ].filter(Boolean).join(", ");
    const span = formatDateRange(r.first, r.last);
    const sync = r.syncing ? fmt.success("syncing") : fmt.warning("not syncing");
    const gone = r.exists === false ? fmt.muted(" (folder deleted)") : "";
    const repo = r.repository ? fmt.muted(` ${r.repository}`) : "";
    console.log(`  ${fmt.path(name.length > width ? "…" + name.slice(-(width - 1)) : name.padEnd(width))}  ${sync}  ${describeSharing(r.sharing, home)}  ${fmt.muted(`${counts}${span ? `, ${span}` : ""}`)}${repo}${gone}`);
  }
  console.log(`\n${fmt.muted("Change with: cast sharing sync | unsync | share | private | lock | team | session. cast sharing --help explains each.")}`);
}
...
settings page at once, and the daemon applies sync changes within a minute.`)
    .action(async (opts: { json?: boolean; all?: boolean; rescan?: boolean }) => {
      ...
      if (opts.json) {
        console.log(JSON.stringify({
          ...
          folders: (opts.all ? state.rows : state.rows.slice(0, 40)),
  ...
  sharing
    ...
    .action(async (ref: string, opts: { limit: string; json?: boolean }) => {
      ...
      const result = await apiPost(deps, "/cli/sharing/sessions", { path_prefixes: [folder], limit: Math.max(1, Number(opts.limit) || 25) }, { read: true });
      ...
      if (opts.json) {
        console.log(JSON.stringify({ folder, total: result.total, truncated: result.truncated, sessions: rows.map((r) => ({ ...r, visibility: visibility(r) })) }, null, 2));
        return;
      }
      const row = rowFor(state, folder);
      console.log(`${fmt.path(prettyPath(folder, state.home))}  ${formatSessionCount(result.total ?? rows.length, !!result.truncated)} on codecast${row ? `, ${describeSharing(row.sharing, state.home)}` : ""}`);
      for (const r of rows) {
      ...
      if (rows.length < (result.total ?? 0)) console.log(fmt.muted(`  … ${result.total - rows.length} older. -n for more.`));
    });
