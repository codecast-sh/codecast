...
A folder is a path (~/src/app), a repository (owner/app) or a folder name (app).
  ...
  cast sharing unsync ~/src/scratch            stop uploading it (synced sessions stay but lose its own share rule; --delete removes them)
...
settings page at once, and the daemon applies sync changes within a minute.`)
  ...
  sharing
    ...
    .action(async (refs: string[], opts: { delete?: boolean; json?: boolean }) => {
      ...
      if (asJson(opts)) {
        console.log(JSON.stringify({ changed: !!plan.next, settings: plan.next ?? state.overview.settings, still_syncing: plan.stillCovered, rules_removed: unruled, deleted }, null, 2));
        return;
      }
      for (const folder of folders) {
