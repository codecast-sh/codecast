...
function printOverview(state: State, opts: { all?: boolean }): void {
  ...
  if (settings.sync_mode !== "selected") {
    const excluded = settings.sync_excluded ?? [];
    console.log(excluded.length === 0
      ? `  Every folder on this machine uploads, new ones included.`
      : `  Every folder uploads, new ones included, except ${excluded.map((f) => prettyPath(f, home)).join(", ")} and the folders inside ${excluded.length === 1 ? "it" : "them"}.`);
  } else {
...
async function writeSync(deps: Deps, state: State, change: Parameters<typeof planSyncChange>[1]): Promise<ReturnType<typeof planSyncChange>> {
  const plan = planSyncChange(state.overview.settings, change);
...
Two settings decide what happens to a folder's sessions:
  sync     whether they upload to codecast at all. Everything uploads by default,
           new folders included; an unsync keeps one folder (and those inside it) off.
...
settings page at once, and the daemon applies sync changes within a minute.`)
  ...
  sharing
    ...
    .argument("[folders...]", "Paths, repositories or folder names")
    .option("--all", "Sync every folder, new ones included, and clear every exclusion")
    .option("--json", "Machine-readable output")
    .action(async (refs: string[], opts: { all?: boolean; json?: boolean }) => {
      ...
      if (asJson(opts)) {
        console.log(JSON.stringify({ changed: !!plan.next, settings: { ...state.overview.settings, ...plan.next }, still_off: plan.stillCovered }, null, 2));
        return;
      }
      const names = (list: string[]) => list.map((f) => prettyPath(f, state.home)).join(", ");
      for (const c of plan.stillCovered) console.log(`${fmt.warning("still off")}  ${fmt.path(prettyPath(c.folder, state.home))}: it is inside the excluded folder ${prettyPath(c.by, state.home)}. Sync that folder to lift it.`);
      const synced = folders.filter((f) => !plan.stillCovered.some((c) => c.folder === f));
      if (!plan.next) {
        if (asAll(opts) || synced.length > 0) console.log(`Already syncing ${asAll(opts) ? "every folder" : names(synced)}.`);
      ...
      else console.log(`${fmt.success("syncing")}  ${names(synced)}. Past sessions upload within a minute or two. Undo: cast sharing unsync ${synced.map((f) => quoteArg(prettyPath(f, state.home))).join(" ")}`);
    });
  ...
  sharing
    ...
    .action(async (refs: string[], opts: { delete?: boolean; dryRun?: boolean; json?: boolean }) => {
      ...
      const folders = folderArgs(state, refs);
      const plan = opts.dryRun ? planSyncChange(state.overview.settings, { unsync: folders }) : await writeSync(deps, state, { unsync: folders });
      const everything = state.overview.settings.sync_mode !== "selected";
      ...
      for (const folder of folders) {
        if (plan.stillCovered.some((c) => c.folder === folder)) continue;
        const sharing = rowFor(state, folder)?.sharing;
        const ownRule = !!sharing && sharing.kind !== "private" && !sharing.inherited;
        if (ownRule) unruled.push(folder);
      ...
      const outcome = (folder: string) => {
        const name = prettyPath(folder, state.home);
        const covered = plan.stillCovered.find((c) => c.folder === folder);
        if (covered) return `${name} ${opts.dryRun ? "would keep" : "keeps"} syncing: the chosen folder ${prettyPath(covered.by, state.home)} includes it. Unsync that folder and sync its other folders one by one.`;
        const row = rowFor(state, folder);
        const count = row ? formatSessionCount(row.synced, row.truncated) : "no sessions";
        const past = opts.delete
          ? opts.dryRun ? `Its ${count} would be deleted from codecast.` : `${formatSessionCount(deleted[folder] ?? 0)} deleted from codecast.`
          : row && row.synced > 0
            ? `The ${count} already synced ${opts.dryRun ? "would stay" : "stay"}${unruled.includes(folder) ? `, private again (its rule "${describeSharing(row.sharing, state.home)}" goes)` : ""}; --delete removes them.`
            : "";
        return `${name} ${opts.dryRun ? "would stop" : "stops"} uploading${everything ? ", and every other folder keeps syncing" : ""}. ${past}`.trim();
      };
      if (asJson(opts)) {
        console.log(JSON.stringify({
          dry_run: !!opts.dryRun,
          changed: !!plan.next,
          settings: { ...state.overview.settings, ...plan.next },
          still_syncing: plan.stillCovered,
          rules_removed: unruled,
          ...(opts.dryRun ? {} : { deleted }),
        }, null, 2));
        return;
      }
      for (const folder of folders) {
        const covered = plan.stillCovered.some((c) => c.folder === folder);
        console.log(`${opts.dryRun ? fmt.muted("dry run") : covered ? fmt.warning("still syncing") : fmt.success("not syncing")}  ${outcome(folder)}`);
        const row = rowFor(state, folder);
        if (!opts.dryRun && unruled.includes(folder) && row) console.log(fmt.muted(`  Its rule was: ${describeSharing(row.sharing, state.home)}. Undo: ${undoCommand(folder, row.sharing, state.home)}`));
      }
      const stopped = folders.filter((f) => !plan.stillCovered.some((c) => c.folder === f));
      if (!opts.dryRun && stopped.length > 0) console.log(fmt.muted(`Undo: cast sharing sync ${stopped.map((f) => quoteArg(prettyPath(f, state.home))).join(" ")}`));
    });
