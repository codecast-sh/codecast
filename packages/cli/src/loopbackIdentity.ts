...
//
// This module imports node builtins and the shared atomic write helper, and
// nothing else, on purpose: the CLI loads it for `rotate-token` and a future
// worker process loads it before the daemon exists, so it must not drag in ws
// or the daemon's module graph.
...
function generateTerminalToken(): string {
  return crypto.randomBytes(32).toString("hex");
}

// listen() takes anything, and bun folds a number outside the port range into
// 16 bits rather than refusing it: a saved port of 999999 binds 65535, and
// every reader then looks for the daemon where it is not. So a bad port is
// rejected here, not at the socket. The daemon's own saved port always comes
// from listen(0), which is always ephemeral and always above 1024.
function usablePort(port: unknown): number {
  if (typeof port !== "number" || !Number.isFinite(port)) return 0;
  const p = Math.floor(port);
  return p >= 1024 && p <= 65535 ? p : 0;
}
...
export function readIdentity(configDir: string): LoopbackIdentity | null {
  ...
  const id: LoopbackIdentity = { port: usablePort(parsed.port), token: parsed.token, created_at: createdAt };
  if (typeof parsed.pid === "number" && Number.isFinite(parsed.pid)) id.pid = parsed.pid;
  return id;
}

// atomicWriteFile publishes by rename and fchmods after create, so a crash
// during a write can never leave a torn identity file. A torn file would read
// as junk, and the next boot would mint a new token and port, which is the
// exact loss this module exists to prevent.
export function writeIdentity(configDir: string, id: LoopbackIdentity): void {
  atomicWriteFile(identityFile(configDir), JSON.stringify(id), { mode: 0o600 });
}

export function writeHookPort(configDir: string, port: number): void {
  atomicWriteFile(hookPortFile(configDir), String(port), { mode: 0o600 });
}
...
export interface ListenOptions {
  ...
  log: (msg: string) => void;
  /** Raised to the server log, so a machine that ends up with no loopback
   *  surface says so somewhere a person will look. */
  warn?: (msg: string) => void;
}

/**
 * Listen on the saved port, falling back to one the OS picks when something
 * else holds it, and record the port that won in both hook-port and the
 * identity file.
 *
 * ANY failure before the listen falls back, and exactly once. A saved port can
 * fail with EADDRINUSE, EACCES or EADDRNOTAVAIL depending on what took it and
 * how the file got there, and asking the OS instead recovers every one of
 * them. A second failure means the local interface is unusable and looping
 * would spin. Callers wait on the resolved promise rather than reading a port
 * that is still 0.
 *
 * Errors reach the executor two ways: as an "error" event, and as a throw out
 * of listen() itself, which is what node does for a port outside 0..65535.
 * Both take the same path, or a corrupt identity file would become an
 * unhandled rejection, and node aborts the process on one.
 */
export function listenOnSavedPort(server: Server, opts: ListenOptions): Promise<number> {
  ...
  return new Promise<number>((resolve) => {
    ...
    function onError(err: NodeJS.ErrnoException): void {
      ...
      if (!fellBack) {
        fellBack = true;
        warn(`Hook server could not bind port ${identity.port} (${err.code || err.message}), asking the OS for another`);
        tryListen(0);
        return;
      }
      warn(`Hook server listen failed, no loopback surface on this daemon: ${err.message}`);
    }
    ...
    tryListen(identity.port > 0 ? identity.port : 0);
  });
}
