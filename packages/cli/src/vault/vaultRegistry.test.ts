...
describe("config healing", () => {
  test("a duplicated row is collapsed rather than shown twice", () => {
    // Registration is a read-modify-write of a shared file, and a project vault
    // is registered by whichever request reaches it first — a scan and a WS
    // hello arriving together can both find the entry missing.
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vault-dupe-"));
    const cfgDir = path.join(dir, ".codecast");
    fs.mkdirSync(cfgDir, { recursive: true });
    const row = { id: vaultId(dir), root: dir, name: "dupe", added_at: 1 };
    fs.writeFileSync(
      path.join(cfgDir, "config.json"),
      JSON.stringify({ vaults: [row, { ...row, name: "dupe (second writer)" }] }),
    );
    const listed = registeredVaults(cfgDir);
