...
/**
 * The vault a command runs against: `--vault <id|name|dir>`, or the only
 * registered vault when there is exactly one.
 *
 * Defaulting matters more than it looks. Most people register one vault and
 * would have to repeat `--vault` on every call for no information gain; two or
 * more and a silent default would write to the wrong one, so that case errors
 * with the list instead of guessing.
 */
export function selectVault(configDir: string, spec?: string): VaultInfo {
  ...
  throw new VaultCliError(
    `${vaults.length} vaults registered — name one with --vault.`,
    VAULT_EXIT.ambiguous,
    vaults.map((v) => `${v.id}  ${v.name}  ${v.root}`),
  );
}
...
export function searchNotes(ctx: VaultCtx, queryText: string, limit = 20): SearchHit[] {
  ...
  for (const path of ctx.index.paths()) {
    ...
    hits.push({
      ...
      score,
      excerpt: firstMatchLine(ctx.contents.get(path) ?? body, terms, q.phrases),
    });
  }
