...
export class VaultWatchHub {
  ...
  /** Files currently known for a watched vault (test/diagnostic surface). */
  knownFiles(vaultId: string): string[] {
    return [...(this.entries.get(vaultId)?.files.keys() ?? [])].sort();
  }
  ...
  private entryFor(vault: VaultInfo): WatchEntry {
    ...
    // Prime the table before the watcher can fire, so pre-existing files are not
    // announced as adds to a client that already has them from its scan.
    void scanVault(vault.root)
      .then((files) => {
