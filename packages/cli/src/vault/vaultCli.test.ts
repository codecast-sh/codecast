...
describe("selectVault", () => {
  ...
  test("several vaults and a cwd in none of them is ambiguous, and lists them", async () => {
    const other = path.join(base, "wiki");
    fs.mkdirSync(other);
    addVault(configDir, notesDir);
    addVault(configDir, other, "Wiki");

    const err = await failure(() => selectVault(configDir, undefined, base));
    expect(err.exitCode).toBe(VAULT_EXIT.ambiguous);
    expect(err.candidates).toHaveLength(2);
  });

  test("with many vaults, the one holding the current directory wins", () => {
    const other = path.join(base, "wiki");
    fs.mkdirSync(path.join(notesDir, "Areas"), { recursive: true });
    fs.mkdirSync(other);
    const notes = addVault(configDir, notesDir);
    addVault(configDir, other, "Wiki");

    expect(selectVault(configDir, undefined, notesDir).id).toBe(notes.id);
    expect(selectVault(configDir, undefined, path.join(notesDir, "Areas")).id).toBe(notes.id);
  });

  test("a vault nested inside another resolves to the inner one", () => {
    const inner = path.join(notesDir, "Team");
    fs.mkdirSync(inner);
    addVault(configDir, notesDir);
    const nested = addVault(configDir, inner, "Team");
    expect(selectVault(configDir, undefined, inner).id).toBe(nested.id);
  });

  test("--vault still overrides the directory you are standing in", () => {
    const other = path.join(base, "wiki");
    fs.mkdirSync(other);
    addVault(configDir, notesDir);
    const wiki = addVault(configDir, other, "Wiki");
    expect(selectVault(configDir, "Wiki", notesDir).id).toBe(wiki.id);
  });
...
describe("vaultForCwd", () => {
  test("finds the vault a directory is inside, and null outside them all", () => {
    const other = path.join(base, "wiki");
    fs.mkdirSync(path.join(notesDir, "Areas"), { recursive: true });
    fs.mkdirSync(other);
    const notes = addVault(configDir, notesDir);
    const wiki = addVault(configDir, other, "Wiki");
    const vaults = [notes, wiki];

    expect(vaultForCwd(vaults, notesDir)?.id).toBe(notes.id);
    expect(vaultForCwd(vaults, path.join(notesDir, "Areas"))?.id).toBe(notes.id);
    expect(vaultForCwd(vaults, other)?.id).toBe(wiki.id);
    expect(vaultForCwd(vaults, base)).toBeNull();
  });

  test("a vault nested in another wins — the more specific answer", () => {
    const inner = path.join(notesDir, "Team");
    fs.mkdirSync(inner);
    const outer = addVault(configDir, notesDir);
    const nested = addVault(configDir, inner, "Team");
    expect(vaultForCwd([outer, nested], inner)?.id).toBe(nested.id);
  });
});
