...
if (process.env.PUSH_TEST_MODE === "hang") { setTimeout(() => {}, 600000); }
else if (process.env.PUSH_TEST_MODE === "refuse") { process.stdout.write('{"hash":"h","applied":[],"unchanged":0,"host_edited":[],"pruned":[],"errors":[],"refused":"other_device"}\\n'); process.exit(3); }
else if (process.env.PUSH_TEST_MODE === "old") { process.stderr.write("error: unknown command 'mirror-apply'\\n"); process.exit(1); }
else if (process.env.PUSH_TEST_MODE === "nocast") { process.stderr.write("bash: line 1: cast: command not found\\n"); process.exit(127); }
else if (process.env.PUSH_TEST_MODE === "fail") { process.stdout.write("SECRET_STDOUT"); process.stderr.write("boom: SECRET_STDERR_LINE\\n"); process.exit(7); }
else { process.stdout.write("install noise\\n" + JSON.stringify({ hash: process.env.PUSH_TEST_HASH || "h", applied: ["a"], unchanged: 1, host_edited: [], pruned: [], errors: [] }) + "\\n"); process.exit(0); }
`, { mode: 0o700 });
...
describe("pushMirrorToHostAsync", () => {
  test("runs ssh with sshBase args and the exact remote command, streams the bundle, parses the last JSON line", async () => {
    ...
    expect(argv).toEqual([...sshBase(host), "ubuntu@cloud-test.invalid", MIRROR_APPLY_COMMAND]);
...
describe("mirrorHomeToHost", () => {
  function deps(stamps: LocalMirrorStamps, pushes: string[], outcome: MirrorPushOutcome = { pushed: true, hash: "H1", result: { hash: "H1", applied: ["x", "y"], unchanged: 0, host_edited: [], pruned: [], errors: [] } }, stampReads: string[] = [], remoteHash: string | null = null): Partial<MirrorDeps> {
    return {
      ...
      readStamp: async (h) => { stampReads.push(hostKey(h)); return remoteHash ? { version: 1, hash: remoteHash, source_device_id: "d", source_user_id: "u", applied_at: "", files: {}, managed_roots: [] } : null; },
  ...
  test("skips ssh when the local stamp holds the hash; pushes otherwise; force overrides; failures are recorded", async () => {
    ...
    const pushes: string[] = [];
    const r = await mirrorHomeToHost(host, { deps: deps(stamps, pushes) });
  ...
  test("a refusal is final for that bundle: no re-upload on the next prepare/wake, force asks again, a new bundle asks again", async () => {
    const pushes: string[] = [];
    ...
    const again = await mirrorHomeToHost(host, { deps: refusing });
    expect(again).toMatchObject({ pushed: false, reason: "other_device", skipped: "refused earlier", hash: "H1" });
    expect(pushes).toHaveLength(1);
...
describe("runMirrorTick", () => {
  ...
  function tickDeps(opts: { hash?: string; stamps?: LocalMirrorStamps; remote?: Record<string, string | null>; outcome?: (h: RemoteHost) => MirrorPushOutcome; hosts?: RemoteHost[] } = {}) {
    ...
    const deps: Partial<MirrorDeps> = {
      ...
      readStamp: async (h) => { stampReads.push(hostKey(h)); const r = opts.remote?.[hostKey(h)]; return r ? { version: 1, hash: r, source_device_id: "d", source_user_id: "u", applied_at: "", files: {}, managed_roots: [] } : null; },
  ...
  test("a failed host is skipped on the fast tick until the local hash changes, retried on the periodic tick; failures log once per reason", async () => {
    ...
    expect(periodic.skipped).toEqual(["ubuntu@a.invalid"]);
    expect(t.pushes).toHaveLength(1);
...
describe("mirrorForPrepare", () => {
  test("cloud_mirror_enabled=false (boolean or string) attempts nothing; a throwing push still returns and logs", async () => {
    const calls: string[] = [];
    const lines: string[] = [];
    ...
    const failed = await mirrorForPrepare(host, "i-1", (m) => lines.push(m), { config: { user_id: "u" }, mirror });
    expect(failed).toBe("config mirror failed: ssh down");
    expect(lines).toEqual(["config mirror failed: ssh down"]);
    const ok = (async () => ({ pushed: true, hash: "abcdef0123", changed: 3, result: { hash: "abcdef0123", applied: ["a", "b", "c"], unchanged: 0, host_edited: ["h"], pruned: [], errors: [] } })) as any;
    expect(await mirrorForPrepare(host, "i-1", () => {}, { config: { user_id: "u" }, mirror: ok })).toBe("mirrored 3 changed config file(s) (abcdef01) — 1 host-edited kept");
    const unprov = (async () => ({ pushed: false, reason: "unprovisioned", changed: 0 })) as any;
    expect(await mirrorForPrepare(host, "i-1", () => {}, { config: { user_id: "u" }, mirror: unprov })).toBe("config mirror skipped: unprovisioned — cast hosts provision i-1");
    const earlier = (async () => ({ pushed: false, reason: "other_device", skipped: "refused earlier", changed: 0, hash: "H1" })) as any;
    expect(await mirrorForPrepare(host, "i-1", () => {}, { config: { user_id: "u" }, mirror: earlier })).toBe("config mirror skipped: other_device — cast hosts sync --take-over to take it over (not retried until the config changes)");
