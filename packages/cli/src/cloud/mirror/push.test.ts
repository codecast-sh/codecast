...
else if (process.env.PUSH_TEST_MODE === "kept") { fs.writeSync(1, JSON.stringify({ hash: "h", applied: [], unchanged: 3, host_edited: [".claude/projects/p/memory/MEMORY.md"], pruned: [], errors: [] }) + "\\n"); process.exit(1); }
...
describe("pushMirrorToHostAsync", () => {
  ...
  test("a host that kept its own edit answers exit 1 with a reply naming the file: a conflict, not a transport failure", async () => {
    process.env.PUSH_TEST_MODE = "kept";
    expect(await pushMirrorToHostAsync(host, bundle)).toMatchObject({ pushed: false, hash: "h", reason: ".claude/projects/p/memory/MEMORY.md: remote edit conflict" });
  });
...
describe("mirrorHomeToHost", () => {
  ...
  test("a host edit conflict is not retried by the fast tick until the laptop copy changes; the verify tick retries it", async () => {
    const pushes: string[] = [];
    const stamps: LocalMirrorStamps = {};
    let now = new Date("2026-09-06T00:00:00Z");
    const edited = { ...deps(stamps, pushes, { pushed: true, hash: "H1", result: { hash: "H1", applied: ["a"], unchanged: 0, host_edited: [".claude/projects/p/memory/MEMORY.md"], pruned: [], errors: [] } }), now: () => now };
    const first = await mirrorHomeToHost(host, { deps: edited });
    expect(first).toMatchObject({ pushed: false, reason: ".claude/projects/p/memory/MEMORY.md: remote edit conflict" });
    expect(stamps["ubuntu@cloud-test.invalid"]!.last_failure).toMatchObject({ hash: "H1" });
    now = new Date(now.getTime() + 6 * 60 * 60_000);
    expect(await mirrorHomeToHost(host, { onlyIfChanged: true, deps: edited })).toMatchObject({ pushed: false, skipped: "refused earlier" });
    expect(pushes).toHaveLength(1);
    await mirrorHomeToHost(host, { deps: edited });
    expect(pushes).toHaveLength(2);
    await mirrorHomeToHost(host, { onlyIfChanged: true, deps: { ...edited, build: async () => fakeBuild("H2") } });
    expect(pushes).toHaveLength(3);
  });
