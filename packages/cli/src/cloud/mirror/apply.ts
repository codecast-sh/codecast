...
function reconcileToml(previous: string, current: string, incoming: string, conflict: () => void): string {
  const parse = (text: string) => Object.fromEntries(splitTomlTables(text).map((table) => {
    ...
    for (const name of Object.keys(fields)) fields[name] = fields[name]!.trimEnd();
...
export function finalBytes(file: ParsedFile, current: Buffer | null, home: string, pinnedEnvKeys: readonly string[], previous?: StampFile, conflict: () => void = () => {}): Buffer {
  ...
  switch (file.kind) {
    ...
    case "codex-toml":
      return Buffer.from(mergeCodexToml(reconcileToml(previous?.source ?? "", cur ?? "", text, conflict), cur));
    case "gitconfig":
...
export async function applyMirrorBundle(bundle: ParsedBundle, opts: ApplyOptions): Promise<ApplyResult> {
  ...
  for (const file of bundle.files) {
    ...
    try {
      checkDestination(home, file.path);
      const current = readIfRegular(abs);
      const before = prev?.files[file.path]?.removed ? undefined : prev?.files[file.path];
      const project = bundle.header.project_roots?.some((root) => file.path.startsWith(`${root}/`));
      if (project && !before && current && !current.equals(finalBytes(file, current, home, pinned))) {
        result.host_edited.push(file.path);
        stampFiles[file.path] = { sha: file.sha256, written: sha256(finalBytes(file, current, home, pinned)), kind: file.kind, mode: file.mode, host_edited: true, ...(!VERBATIM_KINDS.includes(file.kind) ? { source: file.bytes.toString("utf8") } : {}) };
        continue;
      }
      ...
      const bytes = finalBytes(file, current, home, pinned, before, () => { conflicted = true; });
  ...
  if (prev) {
    ...
    for (const [rel, info] of Object.entries(prev.files)) {
      ...
      if (info.removed) { stampFiles[rel] = info; result.host_edited.push(rel); continue; }
      if (kind === "gitconfig") {
        try {
          if (info.source !== undefined && !finalBytes({ path: rel, kind, mode: info.mode, size: 0, sha256: "", bytes: Buffer.from(info.source) }, current, home, pinned, info, () => { result.host_edited.push(rel); }).equals(current)) {
            throw new Error("gitconfig mirror block changed remotely");
          }
          if (result.host_edited.includes(rel)) { stampFiles[rel] = info; continue; }
      ...
      if (!VERBATIM_KINDS.includes(kind)) {
        try {
          ...
          const bytes = finalBytes({ path: rel, kind, mode: info.mode, size: 0, sha256: sha256(empty), bytes: Buffer.from(empty) }, current, home, pinned, info, () => { conflicted = true; });
          if (!bytes.equals(current)) {
            if (bytes.toString().trim()) writeMirroredFile(home, rel, bytes, info.mode); else fs.unlinkSync(abs);
            result.pruned.push(rel);
          }
  ...
  for (const [rel, info] of Object.entries(stampFiles)) {
    ...
    try {
      ...
      if ((fs.statSync(path.join(home, rel)).mode & 0o777) !== Number.parseInt(info.mode, 8)) throw new Error("mode changed during refresh");
...
export function runHostRefresh(home: string): void {
  ...
  if (entry) {
    const result = spawnSync(process.execPath, [entry, "snippets-refresh"], {
