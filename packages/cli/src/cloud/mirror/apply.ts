  ...
  for (const [rel, info] of Object.entries(stampFiles)) {
    ...
    try {
      ...
      if (info.source !== undefined && info.kind) {
        ...
        finalBytes({ path: rel, kind: info.kind, mode: info.mode, size: bytes.length, sha256: sha256(bytes), bytes }, current, home, pinned, info, () => { throw new Error("mirrored content changed during refresh"); });
      }
