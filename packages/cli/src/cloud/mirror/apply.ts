...
export async function applyMirrorBundle(bundle: ParsedBundle, opts: ApplyOptions): Promise<ApplyResult> {
  ...
  for (const root of bundle.header.project_roots ?? []) {
    try {
  ...
  for (const file of bundle.files) {
    ...
    try {
      ...
      if (project && !before && current && !cleanTracked && !current.equals(finalBytes(file, current, home, pinned))) {
  ...
  if (prev) {
    ...
    for (const [rel, info] of Object.entries(prev.files)) {
      ...
      if (info.alias && manifest.has(info.alias)) continue;
      let dest = rel;
