...
const LEGACY_MAGIC = "CASTMIRROR1\n";
const MAX_HEADER_BYTES = 16 * 1024 * 1024;
export const MAX_MIRROR_BODY_BYTES = 256 * 1024 * 1024;
export const MAX_MIRROR_EXPANDED_BYTES = 1024 * 1024 * 1024;
...
export function buildMirrorBundle(entries: BundleInput[], meta: BuildMeta): BuiltBundle {
  if (entries.reduce((n, e) => n + e.bytes.length, 0) > MAX_MIRROR_EXPANDED_BYTES) throw new Error("mirror bundle exceeds 1 GiB expanded write limit");
  ...
  const files = sorted.map((e) => {
    ...
    const prior = unique.get(hash);
    if (prior && !prior.equals(e.bytes)) throw new Error("mirror body SHA collision or size mismatch");
    if (!prior) { unique.set(hash, e.bytes); uniqueSize += e.bytes.length; }
    return { path: e.path, kind: e.kind, mode: e.mode, size: e.bytes.length, sha256: hash };
  });
  if (uniqueSize > MAX_MIRROR_BODY_BYTES) throw new Error("mirror bundle exceeds 256 MiB unique body limit");
  ...
  const bytes = Buffer.concat([Buffer.from(MIRROR_MAGIC), len, headerBytes, ...unique.values()]);
...
export async function parseMirrorBundle(input: Buffer | AsyncIterable<Buffer | string>): Promise<ParsedBundle> {
  ...
  const prefix = bytes.subarray(0, magic.length).toString();
  if (bytes.length < magic.length + 4 || (prefix !== MIRROR_MAGIC && prefix !== LEGACY_MAGIC)) {
  ...
  if (!header || typeof header !== "object" || ![1, 2].includes(header.version) || (header.version === 1 ? LEGACY_MAGIC : MIRROR_MAGIC) !== prefix) {
  ...
  for (const f of header.files) {
    ...
    if (!Number.isSafeInteger(f.size) || f.size < 0) throw new Error(`bad size for ${f.path}`);
    expandedSize += f.size;
    if (expandedSize > MAX_MIRROR_EXPANDED_BYTES) throw new Error("mirror bundle exceeds 1 GiB expanded write limit");
    if (typeof f.sha256 !== "string" || !/^[0-9a-f]{64}$/.test(f.sha256)) throw new Error(`bad sha256 for ${f.path}`);
    let body = header.version === 2 ? unique.get(f.sha256) : undefined;
    if (body) {
      if (body.length !== f.size) throw new Error(`duplicate SHA body size mismatch for ${f.path}`);
    } else {
      if (offset + f.size > bytes.length) throw new Error(`truncated mirror bundle (${f.path})`);
      body = bytes.subarray(offset, offset + f.size);
      offset += f.size;
