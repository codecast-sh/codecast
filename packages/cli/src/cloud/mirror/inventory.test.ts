...
import {
  DEFAULT_EXCLUDES, MIRROR_DENYLIST, MIRROR_SIZE_CAP, MIRROR_SOURCES, collectMirrorFiles, globToRegExp, isDeniedPath, readGlobalGitConfig,
} from "./inventory";
...
describe("denylist", () => {
  ...
  test("a directory is only denied by a path entry, never by a name glob", () => {
    ...
    expect(isDeniedPath(".claude/projects", true)).toBe(false);
  });
...
describe("collectMirrorFiles", () => {
  test("every present source is collected with its kind; denied and codecast-owned paths never are, even when included", async () => {
    ...
    write(".gemini/commands/c.toml", 'prompt = "c"');
    ...
    const alternates = new Set([".opencode"]);
  ...
  test("a bundle over the cap is refused naming the biggest root", async () => {
    write(".claude/skills/big/blob.bin", Buffer.alloc(600));
    write(".codex/skills/big/blob.bin", Buffer.alloc(600));
    await expect(collectMirrorFiles({ home, hostHome, maxBytes: 1024 })).rejects.toThrow(/over the .* MiB cap — largest: \.claude \(/);
  });
  ...
  test("conditional global profiles resolve explicitly while the home bundle stays invariant across repos", async () => {
    ...
    const effective = readGlobalGitConfig(home, repo, { GIT_CONFIG_GLOBAL: path.join(home, ".gitconfig") });
    expect(effective).toContainEqual({ key: "pull.rebase", value: "true" });
    expect(effective).toContainEqual({ key: "alias.co", value: "checkout" });
    expect(text).not.toContain("rebase = true");
    expect(text).not.toContain("co = checkout");
  ...
  test("custom XDG config is read before the home config without reordering duplicate keys", () => {
    write("custom-config/git/config", "[alias]\n\tx = xdg-first\n\tx = xdg-last\n");
    write(".gitconfig", "[alias]\n\tx = home-last\n");
    const pairs = readGlobalGitConfig(home, undefined, { GIT_CONFIG_GLOBAL: undefined, XDG_CONFIG_HOME: path.join(home, "custom-config") });
    expect(pairs.filter((p) => p.key === "alias.x").map((p) => p.value)).toEqual(["xdg-first", "xdg-last", "home-last"]);
  });
