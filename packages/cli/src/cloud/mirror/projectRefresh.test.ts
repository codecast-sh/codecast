...
test("registrations follow stable host identity and retirement never recreates a deleted target", async () => {
  ...
  await registerProjectContext(host, sourceRoot, targetRoot, { file, hostId: "i-123" });
...
test("mirror process stop cancels delayed startup and joins an actual child's close", async () => {
  ...
  const running = startMirrorProcess({ shouldRun: () => true, log: () => {}, delayMs: 0, invocation });
  for (let n = 0; n < 200 && !fs.existsSync(marker); n++) await new Promise((r) => setTimeout(r, 10));
...
test("mirror process stop also joins descendants after an early direct-child exit", async () => {
  ...
  const running = startMirrorProcess({ shouldRun: () => true, log: () => {}, delayMs: 0, invocation: { command: process.execPath, args: [program] } });
  for (let n = 0; n < 200 && !fs.existsSync(marker); n++) await new Promise((r) => setTimeout(r, 10));
  expect(fs.existsSync(marker)).toBe(true);
