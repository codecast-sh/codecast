...
test("the daemon's statusline hook stays home: the settings keep the command, the file does not travel, and no reference is required", async () => {
  ...
  const settings = parseJsonLoose(inv.entries.find((e) => e.path === ".claude/settings.json")!.bytes.toString("utf8")) as { statusLine: { command: string } };
  expect(settings.statusLine.command).toBe("/home/u/.claude/hooks/codecast-statusline.sh");
});
