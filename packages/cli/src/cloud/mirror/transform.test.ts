...
describe("transformClaudeSettings", () => {
  ...
  test("drops auth/routing keys, bypass disabler, denied and secret env; keeps and remaps the rest; drops codecast hooks only", () => {
    ...
    for (const k of ["apiKeyHelper", "awsAuthRefresh", "awsCredentialExport", "otelHeadersHelper", "forceLoginMethod", "forceLoginOrgUUID", "sandbox", "enableAllProjectMcpServers", "enabledMcpjsonServers", "disabledMcpjsonServers"]) {
...
`;
  ...
  test("transformCodexToml drops [projects.*] and [mcp_servers.*] entirely, scrubs secrets, remaps, keeps unknown tables", () => {
    ...
    expect(scrubbed).toContain("[mcp_servers.linear]");
    expect(scrubbed).toContain("[mcp_servers.linear.env]");
    expect(scrubbed).toContain("api_key");
    expect(transformGrokToml).toBe(transformCodexToml);
  });

  test("transformTomlRemap keeps every non-MCP table and scrubs inside them", () => {
...
describe("JSON kinds", () => {
  test("transformGeminiSettings drops mcpServers and a nested authToken", () => {
    const { value, scrubbed } = transformGeminiSettings({ theme: "dark", mcpServers: { a: {} }, x: { authToken: "t", path: "/Users/ashot/p" } }, ctx);
    expect(value).toEqual({ theme: "dark", x: { path: "/home/ubuntu/p" } });
    expect(scrubbed).toEqual(["mcpServers", "x.authToken"]);
  });
  test("transformOpencodeJson drops mcp; parseJsonLoose reads JSONC", () => {
    const parsed = parseJsonLoose('{\n  // comment\n  "model": "x", /* c */ "mcp": {"a": 1},\n}');
    const { value } = transformOpencodeJson(parsed, ctx);
    expect(value).toEqual({ model: "x" });
  });
...
describe("transformByKind: instruction files", () => {
  test("claude-md / agents-md are section-stripped AND home-remapped; verbatim kinds are not", () => {
    ...
    expect(transformByKind("verbatim", Buffer.from("see /Users/ashot/x\n"), ctx).bytes.toString()).toBe("see /Users/ashot/x\n");
  });
});

describe("transformForHost (workspace staging)", () => {
  test("settings get the scrub/remap, codex config the table rules, everything else verbatim, unparseable → null", () => {
