...
import {
  ...
  composeInstructionFile, mergeClaudeSettings, mergeCodexHooks, mergeCodexToml, mergeGitconfig, readStamp, stripGitconfigBlock, withMirrorLock,
} from "./apply";
...
describe("merges", () => {
  ...
  test("mergeCodexToml keeps host [mcp_servers.*] tables (the laptop never ships its own) and drops the laptop's other tables the host had", () => {
    const hostToml = `${HOST_CODEX_TOML}\n[mcp_servers.linear]\ncommand = "npx"\n\n[mcp_servers.linear.env]\nX = "1"\n\n[tui]\ntheme = "host"\n`;
    const merged = mergeCodexToml('model = "gpt-5"\n', hostToml);
    expect(merged).toContain('[mcp_servers.linear]\ncommand = "npx"');
    expect(merged).toContain('[mcp_servers.linear.env]\nX = "1"');
    expect(merged).toContain('[projects."/home/ubuntu/work/codecast"]');
    expect(merged).not.toContain("[tui]");
    expect(mergeCodexToml(merged, hostToml)).toBe(merged);
  });
...
describe("applyMirrorBundle", () => {
  ...
  test("host-edited verbatim files are kept and reported on EVERY later apply — unrelated laptop changes included — until the laptop copy itself changes", async () => {
    seedHost();
    await apply(await bundleOf(laptopEntries()));
    ...
    expect(r5.host_edited).toEqual([]);
    expect(r5.applied).toEqual([skill]);
    expect(read(skill)).toBe("# laptop v2\n");
  ...
  test("merged kinds are not host-edit protected: a host edit to the user portion of CLAUDE.md or to settings.json is re-merged laptop over host", async () => {
    seedHost();
    await apply(await bundleOf(laptopEntries()));
    ...
    expect(r.applied).toEqual([".claude/CLAUDE.md"]);
    expect(read(".claude/CLAUDE.md")).toContain("Always run the tests.");
  });
  ...
  test("prune removes only stamped paths under managed roots, never a host-created or codecast-owned file", async () => {
    ...
    expect(readStamp(home)!.files[".claude/skills/mine/SKILL.md"]).toBeUndefined();
  ...
  test("prune never deletes a merged kind the host co-owns (codex trust + hooks, settings pins, CLAUDE.md sections); the gitconfig kind loses only its block", async () => {
    ...
    expect(r.pruned).toEqual([".gitconfig"]);
    expect(r.errors).toEqual([]);
    expect(read(".codex/config.toml")).toContain('[projects."/home/ubuntu/work/codecast"]');
    expect(read(".codex/config.toml")).toContain("hooks = true");
    ...
    const stamp = readStamp(home)!;
    for (const p of [".codex/config.toml", ".claude/settings.json", ".claude/CLAUDE.md", ".codex/hooks.json", ".gitconfig"]) expect(stamp.files[p], p).toBeUndefined();
