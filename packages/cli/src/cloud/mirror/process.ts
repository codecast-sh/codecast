...
export function mirrorProcessSnapshot(timeoutMs: number): Promise<MirrorProcessIdentity[]> {
  return new Promise((resolve, reject) => {
    execFile("/bin/ps", ["-ax", "-o", "pid=,ppid=,pgid=,uid=,lstart="], { encoding: "utf8", timeout: timeoutMs, maxBuffer: 4 * 1024 * 1024 }, (error, out) => {
...
export function startMirrorProcess(opts: {
  ...
  let active: { child: ChildProcess; closed: boolean; known: Map<number, MirrorProcessIdentity>; ready: Promise<void>; cleanup?: Promise<void> } | undefined;
  const schedule = (delay: number) => {
    if (stopped) return;
    timer = setTimeout(() => { void attempt(); }, delay);
    timer.unref();
  };
  const cleanup = (allocation: NonNullable<typeof active>): Promise<void> => {
    ...
    const retire = async () => {
      ...
      while (performance.now() < deadline) {
        if (!pid) {
          ...
          const rows = await snapshot(Math.max(1, Math.min(500, Math.ceil(deadline - performance.now()))));
  ...
  const attempt = async () => {
    ...
    try {
      ...
      const allocation = { child, closed: false, known: new Map<number, MirrorProcessIdentity>(), ready: Promise.resolve() } as NonNullable<typeof active>;
      active = allocation;
      ...
      allocation.ready = snapshot(500).then((rows) => {
      ...
      void allocation.ready.catch((error) => opts.log(`mirror identity acquisition failed: ${error.message}`));
...
export function watchMirrorParent(stop: () => Promise<void>, log: (message: string) => void, timeoutMs = 2_000): () => void {
  ...
  let lost = false;
  const retire = async () => {
    const rows = await mirrorProcessSnapshot(500);
