...
export async function buildHomeMirror(opts: BuildHomeMirrorOptions): Promise<HomeMirror> {
  ...
  const ctx = { fromHome: home, toHome: opts.hostHome, pathMappings: canonicalProjects.flatMap((p) => projectPathMappings(p, home, opts.hostHome)) };
  ...
  for (const e of inv.entries) {
    ...
    for (const project of projects) {
      ...
      if (e.path.startsWith(prefix)) add({ ...e, path: `.claude/projects/${claudeProjectDirName(project.targetRoot)}/memory/${e.path.slice(prefix.length)}` }, { fromHome: home, toHome: opts.hostHome, pathMappings: projectPathMappings(project, home, opts.hostHome) });
    }
  }
  const discoveries = new Map<string, Awaited<ReturnType<typeof collectProjectContextAsync>>>();
  for (const project of projects) {
    let discovered = discoveries.get(project.sourceRoot);
    if (!discovered) {
      ...
      discovered = await collectProjectContextAsync({ root: project.sourceRoot, home, includeTracked: true, includeAncestors: true, config: opts.config });
      discoveries.set(project.sourceRoot, discovered);
    }
...
export async function pushMirrorToHostAsync(host: RemoteHost, bundle: Buffer, opts: { timeoutMs?: number; signal?: AbortSignal; command?: string } = {}): Promise<MirrorPushOutcome> {
  const { code, stdout, stderr } = await mirrorSsh(host, opts.command ?? MIRROR_APPLY_COMMAND, { input: bundle, timeoutMs: opts.timeoutMs ?? MIRROR_PUSH_TIMEOUT_MS, signal: opts.signal });
...
export async function readRemoteMirrorStamp(host: RemoteHost, timeoutMs = 20_000, signal?: AbortSignal, command = MIRROR_APPLY_COMMAND.replace("--stdin", "--verify")): Promise<MirrorStamp | null> {
