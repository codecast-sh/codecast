...
import {
  ...
  contextReferences, isAccessError, isActiveConfig, isDefaultExcluded, isDeniedPath, isNativeBinary, isReferencedDirectory, matchesContextPattern,
} from "./discovery.js";
...
export async function collectMirrorFiles(opts: CollectOptions): Promise<Inventory> {
  ...
  const collect = async (initial: string) => {
  ...
  for (let index = 0; index < referenceQueue.length; index++) {
    ...
    for (const ref of contextReferences(text, path.join(home, entry.path), home)) {
      ...
      if (!rel || entries.has(rel) || collectedDirs.has(rel) || skippedPaths.has(rel)) continue;
      try {
        const resolved = await resolveEntry(rel);
        if (!resolved) { skip(rel, "referenced path absent or excluded"); continue; }
        if (resolved.stat.isDirectory() && !isReferencedDirectory(ref)) continue;
        await collect(rel);
      } catch (err) {
        if (isActiveConfig(entry.kind) || !isAccessError(err)) throw err;
...
export function readGlobalGitConfig(home: string, cwd?: string, env?: NodeJS.ProcessEnv): Array<{ key: string; value: string }> {
  ...
  const sources = gitEnv.GIT_CONFIG_GLOBAL ? [gitEnv.GIT_CONFIG_GLOBAL] : [path.join(home, ".gitconfig"), path.join(gitEnv.XDG_CONFIG_HOME!, "git/config")];
  if (!sources.some((file) => fs.lstatSync(file, { throwIfNoEntry: false }))) return [];
  const output = execFileSync("git", ["config", "--global", "--includes", "--list", "-z", "--show-origin"], {
    cwd: cwd ?? home, env: gitEnv, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: 30_000, maxBuffer: 8 * 1024 * 1024,
  });
  if (process.env.CLOUD_CONTEXT_TRACE) fs.appendFileSync(process.env.CLOUD_CONTEXT_TRACE, JSON.stringify({ home, output, env: gitEnv.GIT_CONFIG_GLOBAL }) + "\n");
