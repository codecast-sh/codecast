...
export async function registerProjectContext(host: RemoteHost, sourceRoot: string, targetRoot: string, opts: { file?: string; hostId?: string } = {}): Promise<void> {
  ...
  await withMirrorLock(path.join(path.dirname(file), "registrations-lock"), async () => {
    ...
    if (previous?.sourceRoot === sourceRoot && !previous.retired) return;
    ...
    if (previous) { previous.retired = false; previous.host = key; }
