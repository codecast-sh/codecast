 ...
 * logged: (1) agent logins, (2) host git readiness, (3) the
...
export async function prepareCloudHost(opts: {
  ...
  if (isCloudMirrorEnabled(readLocalConfig())) {
    ...
    await registerProjectContext(host, opts.localGitRoot, repoPath, { hostId: up.id, previousAddress: cloud.address });
 ...
 */
export async function acquireRemoteWorkspace(host: RemoteHost, repoPath: string, name: string, localGitRoot: string, opts: { onProgress?: Progress } = {}): Promise<RemoteWorkspace> {
  ...
  const workspace = parseAcquireOutput(name, result.stdout);
  if (isCloudMirrorEnabled(readLocalConfig())) {
    const { registerProjectContext } = await import("./mirror/projectRefresh.js");
    await registerProjectContext(host, localGitRoot, workspace.path);
    await mirrorForPrepare(host, host.address, opts.onProgress ?? (() => {}), { localGitRoot });
  }
  return workspace;
}
