...
describe("cast hosts keepalive", () => {
  ...
  test("uses the default config directory in an SSH shell without a remote flag", async () => {
    const { configDir, ...options } = fixture();
    const modulePath = path.join(import.meta.dir, "keepalive.ts");
    const script = `import { createHostKeepalive } from ${JSON.stringify(modulePath)}; console.log(JSON.stringify(createHostKeepalive("1", ${JSON.stringify(options)})));`;
    const { stdout } = await exec(process.execPath, ["--eval", script], {
      env: { ...process.env, CODECAST_DIR: configDir, CODECAST_REMOTE_DEVICE: "" },
    });
    const lease = JSON.parse(stdout);
    expect(fs.readdirSync(path.join(configDir, "host-keepalive"))).toEqual([lease.id]);
  });
