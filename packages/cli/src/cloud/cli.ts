...
export function registerCloudCommand(program: Command): void {
  ...
  cloud
    ...
    .action(async (opts: { stdin?: boolean; into?: string }) => {
      const { parseMirrorBundle } = await import("./mirror/bundle.js");
      const { applyMirrorBundle, applyStagingBundle, readStamp, withMirrorLock } = await import("./mirror/apply.js");
      ...
      try {
        ...
        process.exit(result.refused ? 3 : 0);
      } catch (err) {
        console.log(JSON.stringify({ error: err instanceof Error ? err.message : String(err) }));
        process.exit(1);
      }
