...
`;
  ...
  test("untracked project agent config reaches the host worktree through the manifest copy path; a tracked CLAUDE.md is untouched", () => {
...
`);
    ...
    refreshRemoteCheckout(host, laptop, remote);
    const ws = acquireRemoteWorkspace(host, remote, "cloud-agent", laptop);
...
describe("agent config staging", () => {
  ...
  test("stages .claude/settings.local.json scrubbed + remapped and CLAUDE.local.md verbatim, 0600, under the inputs dir", () => {
    gitInit(laptop);
    gitInit(remote);
    ...
    expect(fs.readFileSync(path.join(inputRoot, "CLAUDE.local.md"), "utf8")).toBe(`# mine at ${home}/x\n`);
  ...
  test("an unparseable .claude/settings.local.json is left out of the staging set AND reported through the caller's warn (stderr by default)", () => {
    gitInit(laptop);
    gitInit(remote);
    write(laptop, ".claude/settings.local.json", "{ not json");
    write(laptop, "CLAUDE.local.md", "# mine\n");
    const warnings: string[] = [];
    const inputRoot = stageCloudInputs(host, laptop, remote, "cloud-1", { warn: (m) => warnings.push(m) });
    expect(fs.existsSync(path.join(inputRoot, ".claude/settings.local.json"))).toBe(false);
    expect(fs.readFileSync(path.join(inputRoot, "CLAUDE.local.md"), "utf8")).toBe("# mine\n");
    expect(warnings).toEqual(["skipping .claude/settings.local.json: not parseable, so it cannot be scrubbed"]);
    // acquireRemoteWorkspace forwards its progress callback as the warn channel.
    const lines: string[] = [];
    process.env.CLOUD_TEST_ACQUIRE = JSON.stringify({ name: "cloud-2", path: "/x", branch: "b", ports: {}, created: true, state: "ready", contract: { ok: true, failures: [] } });
    acquireRemoteWorkspace(host, remote, "cloud-2", laptop, { onProgress: (m) => lines.push(m) });
    expect(lines).toContain("skipping .claude/settings.local.json: not parseable, so it cannot be scrubbed");
  });
