...
import { shq, ssh } from "../remote/session-move.js";
...
function usd(n: number, places: number): string {
  return `$${n.toFixed(places).replace(/(\.\d*?)0+$/, "$1").replace(/\.$/, "")}`;
}
...
export function estimateHostCost(input: {
  ...
  const disk = diskMonthlyUsd === null ? "disk size unknown" : `about ${usd(diskMonthlyUsd, 2)}/month disk`;
  ...
  if (input.state !== "running") {
    const line =
      diskMonthlyUsd === null
        ...
        : `asleep: about ${usd(diskMonthlyUsd, 2)}/month (disk only)`;
  ...
  const rate =
    hourlyUsd === null
      ...
      : `about ${usd(hourlyUsd, 4)}/hour running`;
...
async function collectHostReport(host: CloudHost, convex: Convex, convexError?: string): Promise<HostReport> {
  ...
  if (!deviceId) {
    if (!live) deviceNote = "device id unknown until it wakes";
      ...
      const learned = await guard(async () => {
        ...
        return learnHostDeviceId({ ...host, address: address ?? undefined }, toRemoteHost({ ...host, address: address ?? undefined }));
      });
  ...
  if (live && address) {
    const listed = await guard(() =>
      parseRemoteWorkspaceList(
        ssh({ ...toRemoteHost({ ...host, address }) }, REMOTE_WORKSPACE_LIST_SCRIPT, 60_000),
      ),
    );
