...
export interface HarnessOptions extends ShimOptions {
  ...
  jsonlPath?: string;
  /**
   * HOME for the pane. Default: a temp home of the harness's own, so a run
   * never writes inside the human's ~/.claude.
   *
   * Pass this process's HOME when the code under test has to FIND the
   * transcript by itself: the daemon's session file index is rooted at
   * `process.env.HOME` (inspectHibernationTarget checks that explicitly), so a
   * pane writing into a home of its own is invisible to it and every
   * hibernation candidate is rejected. A home supplied here is never deleted by
   * tearDown.
   */
  home?: string;
}
 ...
 * needs, on this process's private tmux server. The spawn itself lives in
 * tmuxPane.ts, which `cast bench` also uses — see the note at the top of that
 * file for why it must not import this one.
 */
...
export function spawnHarness(opts: HarnessOptions = {}): Harness {
  ...
  // deletes a directory inside the human's ~/.claude — unless the caller named
  // a home, which it does when the daemon has to find the transcript itself.
  const ownedHome = !opts.home;
  const homeDir = opts.home ?? fs.mkdtempSync(path.join(os.tmpdir(), "codecast-test-home-"));
  ...
  return {
    ...
    tearDown(): void {
      ...
      if (ownedHome) {
        ...
        // A borrowed home (the human's, usually): take only what we wrote.
        try {
...
// agent installed (every CI runner today) stays green — but only for a client
// that is genuinely absent. A probe that could not run throws instead of
// skipping; see binaryProbe.ts (ct-49770).
export { hasBinary };
