...
export interface HarnessOptions extends ShimOptions {
  /** Tmux session name override; the default appends a random suffix to the prefix. */
  tmuxSession?: string;
  /** Tmux runner override. The bench drives its own socket, so its panes never
   *  touch the private test server this file otherwise insists on. */
  runTmux?: typeof defaultTmuxRun;
  /** One spawn attempt and no liveness wait: the caller verifies the pane itself. */
  singleAttempt?: boolean;
  /** Fires the moment new-session returns 0, before any liveness check. */
  onCreated?: () => void;
...
// Module-level default runner. spawnHarness shadows this name with the
// caller's runner when one is supplied, so every pane call a fixture makes goes
// to the socket that fixture owns.
const tmuxRun = defaultTmuxRun;
...
export function spawnTmuxPane(opts: {
  ...
  // A caller with its own runner owns its own socket, so the private-server
  // assertion does not apply to it — it applies to the default runner, which is
  // the one that could otherwise land a pane on the machine's tmux.
  const tmuxRun = opts.runTmux ?? defaultTmuxRun;
  if (!opts.runTmux) assertIsolatedTmux();
  ...
  for (let attempt = 0; attempt < (opts.singleAttempt ? 1 : 3); attempt++) {
    ...
    if (r.status !== 0) {
      ...
      if (!opts.singleAttempt) tmuxRun(["kill-session", "-t", opts.session]);
      continue;
    }
    opts.onCreated?.();
