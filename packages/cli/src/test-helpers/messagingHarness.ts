...
const CLIENT_RECIPES: Record<MatrixClientId, ClientRecipe> = {
  ...
  // No clipboard env: the pair this recipe used to set (GROK_CLIPBOARD_NO_OSC,
  // GROK_CLIPBOARD_NO_DATA_CONTROL) never did anything — the first is not a grok
  // variable and the second is a Wayland copy route — and with them set an image
  // on the machine clipboard still landed in the injected message. What keeps
  // this pane clean is the delivery itself: grok is typed, never pasted
  // (ct-49607). Leaving the env out is what makes these cells prove that.
  grok: {
    launch: (ctx) => {
      fs.mkdirSync(ctx.homeDir, { recursive: true });
      fs.writeFileSync(path.join(ctx.homeDir, "config.toml"), [
