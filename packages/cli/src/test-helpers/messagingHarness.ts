...
// `defaultTmuxRun` is the same function under a second name: spawnHarness
// shadows `tmuxRun` with the caller's own client, so the module-scope default
// needs a name the shadow cannot capture.
import { tmuxRun, tmuxRun as defaultTmuxRun } from "../tmux.js";
...
export interface HarnessOptions extends ShimOptions {
  /** Exact tmux session name. Default: `${tmuxPrefix}-${uuid}`. */
  tmuxSession?: string;
  /** tmux client to spawn through. A caller that brings its own runs its own server. */
  runTmux?: typeof tmuxRun;
  /** Spawn once and report it created, leaving liveness and teardown to the caller. */
 ...
 * after exec looks healthy. `singleAttempt` opts out for a caller that owns the
 * pane's lifecycle itself (the bench fixture, which polls for the pane and
 * reaps it on its own server).
 */
export function spawnTmuxPane(opts: {
  ...
  /** tmux client to spawn through. Default: this process's private server. */
  run?: typeof tmuxRun;
  /** Spawn once, skip the liveness wait, and leave teardown to the caller. */
  singleAttempt?: boolean;
  onCreated?: () => void;
}): void {
  const run = opts.run ?? tmuxRun;
  // The guard keeps a pane off the machine's default server. A caller that
  // supplies its own client has already named the server it wants.
  if (!opts.run) assertIsolatedTmux();
  const alive = (): boolean => run(["has-session", "-t", opts.session]).status === 0;
  let lastErr = "";
  for (let attempt = 0; attempt < (opts.singleAttempt ? 1 : 3); attempt++) {
    ...
    if (r.status !== 0) {
      ...
      if (!opts.singleAttempt) run(["kill-session", "-t", opts.session]);
      continue;
    }
    opts.onCreated?.();
    if (opts.singleAttempt) return;
    ...
    run(["kill-session", "-t", opts.session]);
  }
  throw new Error(`tmux pane spawn failed after ${opts.singleAttempt ? 1 : 3} attempts: ${lastErr}`);
}
...
export function spawnHarness(opts: HarnessOptions = {}): Harness {
  ...
  spawnTmuxPane({
    ...
    env,
    run: opts.runTmux,
    singleAttempt: opts.singleAttempt,
    onCreated: opts.onCreated,
  });
  // A caller running its own server reaps its own panes; the prefix sweep here
  // only ever addresses this process's private one.
