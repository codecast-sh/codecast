...
export function spawnTmuxPane(opts: {
  ...
  /** tmux runner override (the bench fixture drives a scripted tmux). */
  runTmux?: typeof defaultTmuxRun;
  /** Spawn once and trust it, for fixtures whose tmux is scripted. */
  singleAttempt?: boolean;
  /** Fires after each tmux new-session that returned success. */
  onCreated?: () => void;
}): void {
  assertIsolatedTmux();
  const run = opts.runTmux ?? tmuxRun;
  const attempts = opts.singleAttempt ? 1 : 3;
