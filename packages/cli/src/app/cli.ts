...
/**
 * Wait until the app has booted and catch-up is quiet: #root has content (or
 * an error boundary took over), nothing in flight, no pending sends, and no
 * row apply for QUIET ms. Top-level await, not an async IIFE: under CDP's
 * replMode the IIFE's promise comes back as an empty object.
 */
function settleScript(timeoutMs: number, quietMs: number): string {
  return `const __castStarted = Date.now();
const __castDeadline = __castStarted + ${timeoutMs};
let __castQuietSince = null, __castLastSeq = -1, __castLast = null;
while (Date.now() < __castDeadline) {
  ...
  const init = booted && (s ? s.clientStateInitialized !== false : document.readyState === "complete");
  __castLast = { inflight, pending, init, booted, seq };
  const quiet = init && inflight === 0 && pending === 0 && seq === __castLastSeq;
  if (quiet) {
    if (__castQuietSince == null) __castQuietSince = Date.now();
    if (Date.now() - __castQuietSince >= ${quietMs}) break;
  } else {
    __castQuietSince = null;
  }
  __castLastSeq = seq;
  await new Promise((r) => setTimeout(r, 100));
}
({ settled: __castQuietSince != null && Date.now() - __castQuietSince >= ${quietMs}, waitedMs: Date.now() - __castStarted, ...(__castLast || { inflight: 0, pending: 0, init: false, booted: false, seq: 0 }) })`;
}
...
async function goto(what: string, o: TargetOpts & { timeout?: string; reload?: boolean; json?: boolean }, deps: PublishDeps): Promise<void> {
  ...
  const arrived = landed.startsWith(result.route);
...
async function asUser(who: string | undefined, o: TargetOpts & { restore?: boolean; json?: boolean }, deps: PublishDeps): Promise<void> {
  const target = await targetFor(o, deps);
  const cfg = cliConfig();
