...
describe("fenced daemon runtime rail", () => {
  ...
  test("a stale daemon boot cannot reach a driver or legacy fallback", async () => {
    const calls: string[] = [];
    const events: string[] = [];
    const logs: string[] = [];
    const runtime = new DaemonExecutionRuntime({
      transport: recoveringTransport(work(false), calls),
      ownerDeviceId: "device-1",
      daemonBootId: "boot-1",
      drivers: [fakeDriver(events)],
      journal: new InMemoryExecutionOperationJournal(),
    ...
    await runtime.processWorkUpdate(work(false));
    expect(events).toEqual([]);
    expect(calls).toEqual([]);
    expect(logs.join("\n")).toContain("daemon boot mismatch");
  });
