...
export async function loadNativeCatalog(opts: NativeCatalogOptions = {}): Promise<NativeCatalog> {
  ...
  const ignoredAvailable =
    list.available.length === 0 && fromFiles.some((p) => !installedIds.has(p.pluginId));

  const degraded =
    error ??
    (ignoredAvailable
      ? `${CLAUDE_BINARY} listed no installable plugins, so the catalog was read from the cloned marketplace files; upgrade ${CLAUDE_BINARY} to get install counts back`
      : undefined);
