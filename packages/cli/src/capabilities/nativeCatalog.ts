...
//
// The module is split in two, and the split is load bearing rather than tidy.
// Everything down to "Reading the disk" is pure: it takes a string or a parsed
// object and returns a shape, touching no node builtin. Convex has to parse the
// same bytes — the daemon submits raw observations and a mutation derives the
// surfaces (design §6.2) — and Convex cannot import this package. So the pure
// half is written to move to `packages/shared/contracts/` unchanged, exactly as
// `snippets.ts` is already split between a pure catalog there and the writers
// here. Keep new pure parsing above the line and new file or process work below
// it, and that move stays a file split with no logic in it.
//
...
export interface NativeCatalog {
  ...
  installed: NativeInstalledPlugin[];
  /**
   * The catalog: every plugin this machine can see, from the tool and from the
   * cloned marketplace files together. `installed` says which of them are
   * already here.
   *
   * The union is not tidiness. Claude Code drops a plugin from its own
   * `--available` list once it is installed and loadable at this scope —
   * verified, the tool offered 297 of the 300 entries in the cloned files and
   * the three it withheld were exactly the three it had loaded. Reporting the
   * tool's list alone would blank out precisely the plugins the user has.
   */
  available: NativePluginOffer[];
  marketplaces: NativeMarketplaceCatalog[];
  /** How the answer was obtained. `cli` still reads the cloned files for the
   *  fields the listing omits; `disk` means the tool never answered at all, and
   *  `none` means neither path found anything. */
  origin: "cli" | "disk" | "none";
  /** One short phrase naming what went wrong and what to do about it, written
   *  for a person to read. Present whenever the tool answered short. */
  degraded?: string;
}
...
/**
 * A plugin or marketplace name we are willing to build an id from.
 *
 * A `marketplace.json` is written by whoever publishes the marketplace and
 * cloned onto the user's disk by the plugin manager, so every name in it is
 * untrusted input. The id built from it lands in argv when the tool is driven
 * (`claude plugin details <id>`), where a leading `-` is read as a flag rather
 * than as a name — `--dangerously-skip-permissions` is a plugin name a file can
 * legally declare. There is no shell here, so this is about argument position,
 * not quoting.
 *
 * `@` is rejected for a different reason: it is the id separator, so a name
 * carrying one would resolve to a marketplace other than the one that declared
 * it. Everything else is allowed, including non-ASCII — dropping a legitimate
 * plugin from a 2,281-entry catalog costs more than it buys.
 */
export function isUsablePluginName(value: string): boolean {
  if (value.length === 0 || value.startsWith("-")) return false;
  if (/[\s@/\\]/.test(value)) return false;
  // Control characters are not names either. Written as a code point test so the
  // source stays plain ASCII rather than carrying the bytes it rejects.
  return ![...value].some((ch) => ch.charCodeAt(0) < 0x20 || ch.charCodeAt(0) === 0x7f);
}

/** `text()`, narrowed to names that may be composed into a plugin id. */
function pluginName(value: unknown): string | undefined {
  const trimmed = text(value);
  return trimmed && isUsablePluginName(trimmed) ? trimmed : undefined;
}
...
function parseAvailableRow(raw: unknown): NativePluginOffer | undefined {
  if (!isRecord(raw)) return undefined;
  const rawId = text(raw.pluginId);
  const fromId = rawId ? splitPluginId(rawId) : undefined;
  const name = pluginName(raw.name) ?? pluginName(fromId?.name);
  const marketplace = pluginName(raw.marketplaceName) ?? pluginName(fromId?.marketplace);
  ...
  return {
    // Composed from the two halves that passed `isUsablePluginName`, never
    // copied from the row: a `pluginId` with a second `@` in it would name a
    // marketplace other than the one the row claims.
...
function parsePluginSource(raw: unknown): NativePluginSource | undefined {
  ...
  if (asString) {
    // Usually a directory inside the marketplace repository, which carries no sha
    // of its own — the marketplace's own checkout is the pin.
    if (asString.startsWith(".") || asString.startsWith("/")) {
      return { kind: "path", raw: asString, path: asString };
    }
    // Only something with a scheme is reported as a url. A bare `owner/repo`
    // shorthand is kept verbatim under `other` rather than handed to a caller as
    // a url it would fail to open.
    if (asString.includes("://")) return { kind: "url", raw: asString, url: asString };
    return { kind: "other", raw: asString };
  }
  if (!isRecord(raw)) return undefined;
...
function parseCatalogPlugin(raw: unknown, marketplace: string): NativePluginOffer | undefined {
  if (!isRecord(raw)) return undefined;
  const name = pluginName(raw.name);
...
export function parseMarketplaceJson(
  ...
  // A file that declares a name we cannot compose an id from falls back to the
  // directory it was cloned into, which is what every plugin id on this machine
  // already uses.
  const name = pluginName(doc.name) ?? pluginName(fallbackName);
  if (!name) return null;

  const renames: Record<string, string | null> = {};
  if (isRecord(doc.renames)) {
    for (const [from, to] of Object.entries(doc.renames)) {
      // Anything that is not null and not a usable name is not a rename we can
      // act on, so it is dropped rather than stored as a value that would later
      // resolve to nonsense — or to an argv flag, since a resolved name is what
      // a caller asks the tool about.
      if (to === null) {
        renames[from] = null;
        continue;
      }
      const target = pluginName(to);
      if (target) renames[from] = target;
    }
  }
...
/** The tool's own "this plugin has no details" message. It arrives on stdout
 *  with exit 1, and it happens for plugins that ARE installed (verified:
 *  `ralph-loop@claude-plugins-official`), so it is an answer, not a failure. */
export function isPluginNotFound(stdout: string): boolean {
  return /^\s*Plugin\s+".*?"\s+not found/m.test(stdout.replace(/\r\n/g, "\n"));
}

/**
 * `claude plugin details <id>` output.
 *
 * Returns null when the tool did not describe a plugin. `readPluginDetails`
 * separates the two reasons that can happen; here it is one answer: nothing to
 * show.
 */
export function parsePluginDetails(stdout: string): PluginDetails | null {
  const body = stdout.replace(/\r\n/g, "\n");
  if (!body.trim()) return null;
  if (isPluginNotFound(body)) return null;
...
/**
 * Every plugin the cloned catalogs offer, once each.
 *
 * `seen` is carried in and mutated so the same run of ids serves both jobs the
 * caller has: keeping the tool's own offers first, and keeping a marketplace
 * file that lists a name twice from showing it twice.
 */
function catalogOffers(
  marketplaces: NativeMarketplaceCatalog[],
  seen: Set<string> = new Set(),
): NativePluginOffer[] {
  const out: NativePluginOffer[] = [];
...
export async function loadNativeCatalog(opts: NativeCatalogOptions = {}): Promise<NativeCatalog> {
  ...
  const fromDisk = (degraded?: string): NativeCatalog => {
    ...
    const available = catalogOffers(marketplaces);
    return {
  ...
  // Both lists empty means the tool told us nothing at all, so the disk answer —
  // which carries a phrase saying why — is strictly better. One list empty is
  // not a failure: no plugins installed is normal, and a missing `available` is
  // handled by the union below.
  if (list.available.length === 0 && list.installed.length === 0) {
    return fromDisk(error ?? `${CLAUDE_BINARY} returned no plugins`);
  }
  ...
  // Union in what the tool left out. It withholds anything already installed and
  // loadable, and an older binary that ignores `--available` withholds the lot,
  // so the tool's list on its own is a catalog with the user's own plugins cut
  // out of it.
  const fromFiles = catalogOffers(marketplaces, new Set(enriched.map((o) => o.pluginId)));

  // A withheld offer that is NOT installed here is the signature of a binary
  // that ignored `--available`: install counts are then missing everywhere, and
  // a person looking at a catalog with no counts deserves the reason.
  const installedIds = new Set(list.installed.map((p) => p.pluginId));
  const availableFlagIgnored =
    list.available.length === 0 && fromFiles.some((p) => !installedIds.has(p.pluginId));

  const degraded =
    error ??
    (availableFlagIgnored
      ? `${CLAUDE_BINARY} offered no plugins, so the catalog came from the cloned marketplace files; upgrade ${CLAUDE_BINARY} to see install counts`
      : undefined);

  return {
    installed: list.installed,
    available: [...enriched, ...fromFiles],
    marketplaces,
    origin: "cli",
    ...(degraded ? { degraded } : {}),
  };
...
export interface PluginDetailsResult {
  /** Null when no plugin was described — either the tool said "not found", or it
   *  never ran. `error` is what tells those apart. */
  details: PluginDetails | null;
  /** One short phrase naming what went wrong. Absent means the tool answered. */
  error?: string;
}

/**
 * The projected token cost of one plugin.
 *
 * Details being absent is normal: the tool reports "not found" for some plugins
 * that ARE installed (see `parsePluginDetails`). That is `{details: null}` with
 * no error, and a caller renders the plugin without a cost badge. A missing
 * binary or a timeout is `{details: null, error}` — the same null, a different
 * thing to say, and the cost figure is the one number this library exists to
 * show, so "no cost data" must not stand in for "we never asked".
 */
export async function readPluginDetails(
  pluginId: string,
  opts: NativeCatalogOptions = {},
): Promise<PluginDetailsResult> {
  // The id goes into argv. A marketplace file is publisher written, so a name
  // that would land there as a flag is refused at the boundary rather than
  // trusted to have been filtered upstream.
  const { name, marketplace } = splitPluginId(pluginId);
  if (!isUsablePluginName(name) || (marketplace !== undefined && !isUsablePluginName(marketplace))) {
    return { details: null, error: `refusing to ask about a plugin named ${JSON.stringify(pluginId)}` };
  }
  const { stdout, error } = await runClaude(["plugin", "details", pluginId], opts);
  const details = parsePluginDetails(stdout);
  if (details) return { details };
  // The tool prints its "not found" message on stdout and exits non-zero, so the
  // exit code alone would report a plugin without details as a fault. What it
  // printed is the better witness.
  if (isPluginNotFound(stdout)) return { details: null };
  return { details: null, error: error ?? `${CLAUDE_BINARY} printed no plugin details` };
}
