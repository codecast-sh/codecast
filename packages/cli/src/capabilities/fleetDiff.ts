...
import type { CapabilityKind, CapabilityScope } from "./inventory.js";
...
export interface FleetDiffRow {
  ...
  /** The kind of pin this row was compared on: the most precise kind any machine
   *  with it switched on reported. Undefined where nobody reported a pin. */
  pinKind?: PinKind;
...
const SCOPE_ORDER = ["local", "project", "user"] as const satisfies readonly CapabilityScope[];
 ...
 */
const KIND_ORDER = ["plugin", "marketplace", "mcp", "skill", "command", "subagent"] as const satisfies
  readonly FleetRowKind[];
...
type _EveryScopeIsRanked = Ranked<Exclude<CapabilityScope, (typeof SCOPE_ORDER)[number]>>;
