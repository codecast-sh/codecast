...
/** Rename profiles whose name still carries the old auto-derived form (the
 * email domain's org part: claude2@almostcandid.com → almostcandid, or
 * almostcandid-2) to the current derivation (the local part: claude2).
 * Hand-picked names don't match the old pattern and are left alone. Moves the
 * secret snapshot along with the index row; returns the renames performed. */
export function migrateLegacyProfileNames(): Array<{ from: string; to: string }> {
  const index = readProfileIndex();
  const renames: Array<{ from: string; to: string }> = [];
  for (const [name, meta] of Object.entries(index.profiles)) {
    if (!meta.email?.includes("@")) continue;
    const org = meta.email.split("@")[1]?.split(".")[0]?.toLowerCase().replace(/[^a-z0-9._-]/g, "");
    const legacy =
      !!org &&
