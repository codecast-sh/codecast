...
END {
  ...
  OFS = "\037"
  print str("session_id"), str("sessionId"), str("hook_event_name"), str("tool_name"), str("permission_mode"), str("notification_type"), str("source"), str("trigger"), str("transcript_path"), str("stop_hook_active"), str("command"), str("file_path"), str("pattern"), str("path"), str("url"), str("task"), str("plan")
}
`.trim();

/** Bash snippet: populate HOOK_* from $INPUT in one awk. */
export const HOOK_FIELDS_READ = `IFS=$'\\037' read -r HOOK_session_id HOOK_sessionId HOOK_hook_event_name HOOK_tool_name HOOK_permission_mode HOOK_notification_type HOOK_source HOOK_trigger HOOK_transcript_path HOOK_stop_hook_active HOOK_command HOOK_file_path HOOK_pattern HOOK_path HOOK_url HOOK_task HOOK_plan <<EOF
$(printf '%s' "$INPUT" | awk '
${HOOK_FIELDS_AWK}
')
EOF
