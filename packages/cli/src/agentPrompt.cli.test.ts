...
async function run(args:string[],config:Record<string,unknown>,env:Record<string,string>={}){
 ...
 if (env.ORIGIN_TEST_PANE) {
  const bin = join(dir, 'bin');
  mkdirSync(bin);
  writeFileSync(join(bin, 'tmux'), '#!/bin/sh\ncase "$*" in *"|"*) printf "%s\\n" "$ORIGIN_TEST_PANE";; *) exit 1;; esac\n');
...
test('nested launcher uses the current pane stamp or a fresh launch receipt, and refuses a foreign stamp', async () => {
 const config = {
...
test('conflicting inherited identities require an explicit sender', async () => {
 const config = { 'conversations.json': { native: parent, inherited: 'jx7297y2rfkpskb14mgbsjm5e58dv708' } };
 for (const inherited of ['inherited', 'stale']) {
  const env = { CODEX_THREAD_ID: 'native', CLAUDE_CODE_SESSION_ID: inherited };
  const ambiguous = await run([], config, env);
  expect(ambiguous.code).not.toBe(0);
  expect(ambiguous.out).toBe('');
  expect((await run(['--from', parent], config, env)).code).toBe(0);
 }
});
