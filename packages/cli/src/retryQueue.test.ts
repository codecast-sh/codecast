...
describe("RetryQueue", () => {
  ...
  it("replaces a queued partial with the newest same-UUID revision", () => {
    queue.add("addMessages", {
      conversationId: "c1",
      messages: [{
        messageUuid: "stream-1",
    ...
    const messages = queue.getPendingOperations()
      .flatMap((op) => op.params.messages as Array<any>);
  ...
  it("never lets a stale same-UUID retry replace a queued newer revision", () => {
    queue.add("addMessages", {
      conversationId: "c1",
      messages: [{
        messageUuid: "stream-1",
  ...
  it("keeps a newer successor when an older same-UUID write is in flight", async () => {
    const q = new RetryQueue({
      initialDelayMs: 10,
      maxDelayMs: 50,
      concurrency: 1,
    ...
    const firstStarted = new Promise<void>((resolve) => {
      q.setExecutor(async (op) => {
        const revision = (op.params.messages as Array<any>)[0].sourceRevision;
        if (revision === 10) {
          resolve();
    ...
    q.add("addMessages", {
      conversationId: "c1",
      messages: [{ messageUuid: "stream-1", sourceDeviceId: "device-a", sourceRevision: 10, content: "partial" }],
    });
    await firstStarted;
    q.add("addMessages", {
      conversationId: "c1",
      messages: [{ messageUuid: "stream-1", sourceDeviceId: "device-a", sourceRevision: 11, content: "complete" }],
    });

    const revisions = q.getPendingOperations()
      .flatMap((op) => op.params.messages as Array<any>)
      .map((message) => message.sourceRevision)
      .sort();
    expect(revisions).toEqual([10, 11]);

    releaseFirst();
    expect(await q.waitForCompletion(1000)).toBe(true);
    q.stop();
  });
  ...
  it("restores the newest revision when persisted same-UUID operations disagree", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "rq-revision-load-"));
    const persistPath = path.join(dir, "retry-queue.json");
    try {
      const op = (id: string, sourceRevision: number, content: string) => ({
        id,
        type: "addMessages",
        params: {
          conversationId: "hot",
        ...
        attempts: 0,
        nextRetryAt: Date.now(),
        createdAt: Date.now() + sourceRevision,
      });
      fs.writeFileSync(persistPath, JSON.stringify([
        op("newer", 11, "complete"),
        op("older-late-in-file", 10, "partial"),
      ]));

      const loaded = new RetryQueue({ persistPath, onLog: () => {} });
      const messages = loaded.getPendingOperations()
        .flatMap((queued) => queued.params.messages as Array<any>);
      expect(messages).toHaveLength(1);
      expect(messages[0]).toMatchObject({
        sourceRevision: 11,
        content: "complete",
