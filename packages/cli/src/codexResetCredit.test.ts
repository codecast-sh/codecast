...
describe("redeemCodexResetCredit", () => {
  ...
  it("refuses when only the WINDOWS moved — the credit would reset nothing new", async () => {
    const { fetchImpl, calls } = fakeBackend({});
    const offer = (await fetchCodexResetCredits(codexHome, { fetchImpl }))!;
    // The daemon's probe refreshed the meters while the human was deciding: the
    // window they were shown as pegged has since rolled on its own.
    const result = await redeemCodexResetCredit({
      account,
      codexHomeDir: codexHome,
      fetchImpl,
      readWindows: () => ({ session: { percent: 0 } }),
      expectedRevision: offerRevision(offer, { session: { percent: 100, resets_at: 5 } }),
      newKey: () => "key-1",
    });
    expect(result).toMatchObject({ status: "refused", reason: "stale_offer" });
...
// The in-process promise map cannot see another process. Two terminals — or the
// CLI while the daemon's auto path runs — each read the ledger, each find no
// pending attempt, and each spend a credit. Only a file lock stops that, so it
// is tested the only way that proves anything: two real processes against one
// real endpoint, racing for one offer.
describe("two processes redeeming the same offer", () => {
  it("spends exactly one credit; the loser reads the winner's ledger entry", async () => {
    let posts = 0;
    ...
    const server = Bun.serve({
      ...
      async fetch(req) {
        ...
        if (url.pathname.endsWith("/consume")) {
          posts++;
          seenIds.push((await req.json())?.redeem_request_id);
          // Slow enough that a second process without the lock would already be
          // past its own ledger read and committed to POSTing too.
          await new Promise((r) => setTimeout(r, 400));
          return Response.json({ code: "reset" });
        }
        // The balance the provider still reports; it does not drop the moment a
        // credit is spent, which is exactly why the LEDGER has to be the arbiter.
        await new Promise((r) => setTimeout(r, 300));
        return Response.json(OFFER_BODY);
      },
    });
    const origin = `http://127.0.0.1:${server.port}`;

    // The child rewrites the hardcoded chatgpt.com origin onto the test server,
    // so nothing in the module needs a test-only escape hatch.
    const child = path.join(sandbox, "redeem-child.ts");
    fs.writeFileSync(
      child,
      `const [origin, home, account] = process.argv.slice(2);\n` +
        `const mod = await import(${JSON.stringify(path.join(import.meta.dir, "codexResetCredit.ts"))});\n` +
        `const fetchImpl = (url, init) => fetch(String(url).replace("https://chatgpt.com", origin), init);\n` +
        `try {\n` +
        `  console.log(JSON.stringify(await mod.redeemCodexResetCredit({ account, codexHomeDir: home, fetchImpl })));\n` +
        `} catch (err) {\n` +
        `  console.log(JSON.stringify({ status: "threw", message: String(err) }));\n` +
        `}\n`,
    );

    const run = () =>
      new Promise<any>((resolve, reject) => {
        const proc = spawn(process.execPath, [child, origin, codexHome, "acct-1"], {
          env: { ...process.env, CODECAST_DIR: process.env.CODECAST_DIR, HOME: sandbox },
        });
        let out = "";
        ...
        proc.on("close", () => {
          try {
            ...
            reject(new Error(`child produced no result: ${out}`));
          }
        });
      });
    ...
    try {
      ...
      const attempts = readResetCreditLedger().attempts;
      expect(attempts).toHaveLength(1);
      expect(attempts[0]).toMatchObject({ account: "acct-1", state: "settled", outcome: "reset" });
      expect(attempts[0].key).toBe(seenIds[0]);

      // One spent it; the other, holding the lock afterwards, found that exact
      // offer already settled and refused before its own provider call.
      const spent = results.filter((r) => r?.status === "redeemed");
      const refused = results.filter((r) => r?.status === "refused");
      expect(spent).toHaveLength(1);
      expect(spent[0]).toMatchObject({ outcome: "reset", replayed: false });
      expect(refused).toHaveLength(1);
      expect(refused[0]).toMatchObject({ reason: "offer_spent" });
    } finally {
      server.stop(true);
    }
  });
