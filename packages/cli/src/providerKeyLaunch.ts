 ...
 *  on-device key store. Atomic (temp + rename) so a concurrent launch never reads a
 *  half-written file. Returns the file path when written, or null when nothing is
 *  managed (the default — clients then fall back to system auth). */
export function syncProviderKeyEnvFile(configDir: string): string | null {
  const env = providerKeyEnv(readProviderKeyStore(configDir));
