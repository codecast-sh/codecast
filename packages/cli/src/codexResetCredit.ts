...
function resetCreditLockPath(): string {
  return `${resetCreditLedgerPath()}.lock`;
}
...
// Two collisions, two guards, and neither substitutes for the other.
//
// Within one process, two callers for the same account should get ONE answer,
// so an in-flight promise is shared rather than raced (ct-49529).
//
// Across processes — two terminals, or the CLI while the daemon's auto path
// runs — the ledger's read-modify-write is the hazard: both read "no pending
// attempt", both mint an id, both POST, and the second write erases the first's
// record of a credit that is already gone. A file lock held around the whole
// critical section fixes that: the second process waits, then reads a ledger
// that already carries the first's pending or settled entry.
//
// The lock covers the LEDGER, not one account, because the file is rewritten
// whole — a write for one account would otherwise clobber another's.
const inFlight = new Map<string, Promise<RedeemResult>>();

// Long enough to sit through a redeem in front of us (a 15s read plus a 30s
// POST) with room to spare; the staleness reclaim in acquireFileLock handles a
// holder that was SIGKILLed mid-flight.
const LEDGER_LOCK_WAIT_MS = 120_000;
const LEDGER_LOCK_STALE_MS = 120_000;

/**
 * Spend one credit for `account`, or say why it refused to.
 *
 * The order matters and is the whole safety argument, and the ledger lock is
 * held across all of it so a second process sees a finished story, not a
 * half-written one:
...
export function redeemCodexResetCredit(opts: {
  ...
  const promise = (async () => {
    const release = await acquireFileLock(resetCreditLockPath(), {
      waitMs: LEDGER_LOCK_WAIT_MS,
      staleMs: LEDGER_LOCK_STALE_MS,
      describe: "cast accounts codex reset-credit",
    });
    try {
      return await redeemOnce(opts);
    } finally {
      release();
    }
  })().finally(() => {
