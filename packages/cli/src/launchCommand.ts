...
export function buildPrintArgs(input: PrintArgsInput): PrintArgsResult {
  ...
  if (agentType === "codex") {
    ...
    // `muse exec --session-id <uuid>` names the session to continue (the flag
    // doc says "use a specific session id"). No --last/--continue analog on
    // exec, so continueLast is dropped. Whether --session-id resumes live
    // history or just reuses the id is unverified against the CLI — the TUI
    // `muse resume <uuid>` path stays the authoritative resume.
