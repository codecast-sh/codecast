...
export function blankCodexRecoveryParams(
  data: { conversation: { agent_type?: string; model?: string; effort?: string }; messages: unknown[] },
  cwd: string | null,
  permissions: { approvalPolicy: ApprovalPolicy; sandbox: SandboxMode },
): ThreadStartParams | null {
  if (data.conversation.agent_type !== "codex" || data.messages.length > 0 || !cwd) return null;
