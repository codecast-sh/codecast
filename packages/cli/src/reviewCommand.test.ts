...
describe("cast review lifecycle", () => {
  test("add, ls, send, rm", async () => {
    ...
    // The confirmation names the place, not "Scope: file" for every note.
    expect(await run("add", "api.ts:5-9", "and here")).toContain("· Lines: 5-9");
    await run("rm", "3");
  ...
  test("committing the reviewed content does not fake a change", async () => {
    ...
    // The stamp is the content the reviewer read, so the same content on the
    // other side of a commit is still the same note.
    git(["commit", "-qam", "land it"], dir);
    expect(await run("ls")).not.toContain("[stale]");
  });
});
