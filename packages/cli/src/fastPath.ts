...
export function runFastPath(argv: string[]): boolean {
  ...
  if (argv[2] === "_boot-bytes") {
    ...
    // and nothing else — 1,499 bytes on bun 1.3.14. Without splitting bun
    // concatenates the whole CLI into it and the same probe reads 4,313,112,
    // which is what the release check refuses. Nothing outside a compiled
    // artifact can observe this: the chunking exists only in one.
