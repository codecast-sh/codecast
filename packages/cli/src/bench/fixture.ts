...
export function childProcess(command: string, args: string[], signal?: AbortSignal): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    ...
    let output = "";
    const child = execFile(command, args, { encoding: "utf8", signal, killSignal: "SIGKILL", timeout: 5000, maxBuffer: 4 * 1024 * 1024 }, (error, stdout) => {
      failure = error;
      output = stdout;
    });
...
export class BenchFixture {
  ...
  private allocationKey(f: Fixture) { return JSON.stringify([f.sessionId, f.tmuxSession, f.jsonlPath, f.registryPath]); }
  ...
  private async pane(f: Fixture, signal?: AbortSignal): Promise<string[] | null> {
    let raw: string;
    try { raw = await this.tmux(["display-message", "-p", "-t", `=${f.tmuxSession}:0.0`, "#{session_name}|#{session_id}|#{pane_id}|#{pane_pid}|#{pane_current_command}|#{session_windows}|#{window_panes}"], signal); }
    catch (e) {
  ...
  async append(f: Fixture, text: string, signal: AbortSignal) {
    ...
    const work = prior.then(async () => {
      ...
      await fs.appendFile(f.jsonlPath, line);
      signal.throwIfAborted();
      return true;
    });
  ...
  async cleanup(f: Fixture, signal: AbortSignal) {
    ...
    if (f.created) {
      ...
      for (;;) {
        signal.throwIfAborted();
        let alive = true;
        try { await childProcess("ps", ["-p", String(f.pid), "-o", "pid="], signal); }
        catch (e) { if ((e as { code?: number; stdout?: string }).code === 1 && !(e as { stdout?: string }).stdout?.trim()) alive = false; else throw e; }
        if (!alive && !await this.pane(f, signal)) break;
        await this.clock.sleep(25, signal);
      }
      const handle = await fs.open(f.jsonlPath, "r");
      try {
        let carry = "", count = 0;
  ...
  async finish() {
    ...
    if (this.projectDir) await fs.rmdir(this.projectDir).catch(e => { if (e.code !== "ENOENT") throw e; });
