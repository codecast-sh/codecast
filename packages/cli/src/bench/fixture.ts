...
export class BenchFixture {
  ...
  async prepare(signal: AbortSignal) {
    ...
    const project = await fs.lstat(this.projectDir);
    if (!project.isDirectory() || project.isSymbolicLink()) throw new Error("invalid fixture project directory");
    this.projectIdentity = `${project.dev}:${project.ino}`;
  ...
  private async removeDirectory(directory: string, identity: string, signal: AbortSignal) {
    signal.throwIfAborted();
    await this.options.beforeRemove?.(directory);
    signal.throwIfAborted();
    const stat = await fs.lstat(directory).catch(e => { if (e.code === "ENOENT") return null; throw e; });
    signal.throwIfAborted();
    if (!stat) return;
    if (!stat.isDirectory() || stat.isSymbolicLink() || `${stat.dev}:${stat.ino}` !== identity) throw new Error("fixture directory ownership changed");
  ...
  async finish(signal = new AbortController().signal) {
    ...
    const stat = await fs.lstat(this.scratch);
    signal.throwIfAborted();
    if (!stat.isDirectory() || stat.isSymbolicLink() || `${stat.dev}:${stat.ino}` !== this.scratchIdentity) throw new Error("scratch ownership changed");
    if (this.projectCreated) await this.removeDirectory(this.projectDir, this.projectIdentity, signal);
    ...
    await this.removeDirectory(this.scratch, this.scratchIdentity, signal);
  }
}
