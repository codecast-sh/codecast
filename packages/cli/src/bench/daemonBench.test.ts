...
for (const failed of [".md", ".json"]) test(`canonical report records ${failed} persistence failure`, async () => {
  const { f, deps, io, options, writes } = fixture(); const emitted: string[] = [], order: string[] = [];
  io.write = async (file, value) => { order.push(file); if (file.endsWith(failed)) throw new Error("private write failure"); writes.set(file, value); };
  const r = await f.clock.drive(runDaemonBench(deps, options, line => emitted.push(line), io));
  expect(order[0]).toEndWith(".md"); expect(order[1]).toEndWith(".json"); expect(r.acceptance.status).toBe("FAIL");
  const canonical = JSON.parse(failed === ".md" ? writes.get(r.outputs.json)! : emitted[emitted.length - 1]);
  expect(canonical).toEqual(r); expect(canonical.errors).toContain("report persistence failed");
  expect(canonical.observe.loopLag.records).toHaveLength(canonical.observe.loopLag.expected); expect(f.clock.timers.size).toBe(0);
});
