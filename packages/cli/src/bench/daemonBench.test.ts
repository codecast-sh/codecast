...
function fixture() {
  ...
  const io: Partial<DaemonBenchIO> = { identity: () => ({ port: 43210, token: "private-fake", reason: null }), context: async () => structuredClone(context), routes: f.io.routes, loadIO: f.io, logs: async () => ({ log: buildLogReport([], { sinceMs: 0, sleepWindows: [] }), windows: [] }), write: async (file, value) => { writes.set(file, value); } };
