...
// log() in daemon.ts writes `[ISO] ` then an optional `[LEVEL] ` tag. Only the
// four log levels count as a tag; `[LIFECYCLE]` and friends belong to the message.
const LINE_RE = /^\[(\d{4}-\d{2}-\d{2}T[^\]]+)\] (?:\[(DEBUG|INFO|WARN|ERROR)\] )?(.*)$/;
