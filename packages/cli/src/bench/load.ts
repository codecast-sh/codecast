...
export interface LoadResult {
  ...
  churn: { durationMs: number; intervalMs: number; linesAppended: number; expectedRounds: number; missedRounds: number; rounds: { scheduledAt: number; startedAt: number; finishedAt: number; lines: number }[] };
...
export type FixtureIO = Pick<BenchFixture, "scratch" | "fixtures" | "prepare" | "allocate" | "spawn" | "verify" | "mapping" | "append" | "pauseWrites" | "resumeWrites" | "hasToken" | "cleanup" | "finish">;
...
export async function exportedToken(post: LoadIO["post"], conversationId: string, token: string, role: "user" | "assistant", signal: AbortSignal) {
...
export async function ownedHookUrl(port: number, fixture: FixtureIO, f: Fixture, clock: BenchClock, signal: AbortSignal) {
  ...
  signal.throwIfAborted();
  const query = new URLSearchParams({ session_id: f.sessionId, status: "working", ts: String(Math.floor(clock.wall() / 1000)), transcript_path: f.jsonlPath });
...
export function measurementFailures(r: LoadResult): string[] {
  ...
  if (!window || r.roundTrips.some(t => [t.up, t.injected, t.echoed].some(l => {
    ...
    const overlappingChurn = r.churn.rounds.some(c => c.lines > 0 && c.startedAt <= l.finishedAt! && c.finishedAt >= l.startedAt!);
    const probesOverlap = [r.loopLag, r.hookStatus, r.termSessions].every(p => p?.records.some(s => s.outcome === "ok" && s.startedAt !== null && s.startedAt <= l.finishedAt! && s.finishedAt >= l.startedAt!));
    return !overlappingChurn || !probesOverlap;
...
export async function runLoadBench(deps: LoadDeps, opts: LoadOptions, say: (line: string) => void, overrides: Partial<LoadIO> = {}): Promise<LoadResult> {
            ...
            mappings.push((async () => {
              try {
                while (!f.conversationId) {
                  ...
                  if (id) { f.conversationId = id; start.mappedAt = clock.now(); start.outcome = "ok"; r.mapped++; break; }
                  await clock.sleep(opts.pollMs ?? 250, mappingWindow.signal);
                }
              } catch { start.outcome = outcomeFor(mappingWindow.signal); recordFailure("mapping", mappingWindow.signal, f.sessionId); }
              finally { await mappingWindow.close(); }
            })());
          } catch {
            start.outcome = outcomeFor(mappingWindow.signal); r.spawnErrors.push(f.sessionId); recordFailure("spawn", mappingWindow.signal, f.sessionId); await mappingWindow.close();
          }
        }
    ...
    await phase("measurement", async () => {
      ...
      const sample = async (f: Fixture, trip: RoundTrip) => {
        ...
        const leg = async (value: Leg, work: () => Promise<void>, startedAt = clock.now()) => {
          ...
          catch { value.outcome = outcomeFor(window.signal); value.reason = value.outcome; recordFailure("delivery", window.signal, f.sessionId); }
        ...
        await leg(trip.up, async () => {
          ...
          await poll(trip.up, () => exportedToken(io.post, f.conversationId!, upToken, "user", window.signal));
        });
        ...
        try {
          ...
          if (trip.injected.outcome === "ok") await leg(trip.echoed, () => poll(trip.echoed, () => exportedToken(io.post, f.conversationId!, downToken, "assistant", window.signal)), sentAt);
        ...
        finally { fixture.resumeWrites(f); }
  ...
  } catch { recordFailure("run", overall.signal); }
  finally {
    await overall.close();
      ...
      try {
        ...
        if (opts.keep) r.teardown.warnings.push("resources retained by --keep");
          ...
          for (const f of fixture.fixtures) {
            ...
            catch { r.teardown.warnings.push(`fixture ${f.sessionId} cleanup failed or ownership refused`); }
          }
          if (fixture.scratch && !r.teardown.warnings.length) {
            ...
            } catch { r.teardown.warnings.push("backend or file cleanup incomplete"); }
          }
