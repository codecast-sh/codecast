...
const build = require("node:crypto").createHash("sha256").update(require("node:fs").readFileSync(__filename)).digest("hex");
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    if (url.pathname === "/identity") return reply(200, { pid: process.pid, runtime: process.version, build });
