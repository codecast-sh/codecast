...
export function fakeLoad(clock = new FakeClock()) {
  ...
  const clean: string[] = [];
  const f: FixtureIO = {
    scratch: "/private-bench-run", fixtures: [],
    async prepare(signal) { touch("prepare"); signal.throwIfAborted(); },
    allocate() {
      const id = `fixture-${f.fixtures.length}`;
      const row: Fixture = { sessionId: id, tmuxSession: `tmux-${id}`, jsonlPath: `/private-bench-run/${id}.jsonl`, registryPath: `/private-bench-run/${id}.json`, created: false };
