...
describe("command groups stay off the boot graph", () => {
  ...
  test("index.ts, the CLI's command tree, statically imports no command group", () => {
    ...
    // Lifted from 190 to 205 when ct-49546 landed on a tree 35 commits newer
    // than the one it was measured on: the eager graph had gained 23 modules in
    // between, 14 of them `workers/*` reached through the scan client. None is
    // a command group, so the property this file guards still holds — see
    // ct-49758 for whether the worker graph belongs behind a dynamic import.
    expect(graph.nodes.size, "source files on index.ts's static graph").toBeLessThanOrEqual(205);
    expect(Math.round(graph.totalBytes / 1024), "KB of source on index.ts's static graph").toBeLessThanOrEqual(2800);
  }, GRAPH_WALK_TIMEOUT);
  ...
  test("the daemon carries no CLI command group either", () => {
    ...
    // Lifted from 240 for the same reason as index.ts's budget above.
