...
import { benchClock, runRouteProbes, MAX_BENCH_RECORDS, MAX_BENCH_TIMER_MS } from "./probes.js";
...
for (const durationMs of [NaN, Infinity, 0]) test(`invalid probe duration ${durationMs} sends nothing`, async () => {
  let sent = false;
  await expect(runLatencyProbe({ url: "http://private", durationMs, fetch: async () => { sent = true; return new Response("ok"); } })).rejects.toThrow("invalid probe bounds");
  expect(sent).toBe(false);
});

for (const options of [
  { intervalMs: Number.MIN_VALUE }, { intervalMs: 1000 / (MAX_BENCH_RECORDS + 1) },
  { startAt: Infinity }, { startAt: NaN }, { startAt: Number.MAX_VALUE },
  { durationMs: MAX_BENCH_TIMER_MS + 1 }, { maxInFlight: MAX_BENCH_RECORDS + 1 },
]) test(`computed probe bounds refuse ${Object.entries(options).map(([key, value]) => `${key}=${String(value)}`).join(",")} before transport or timer`, async () => {
  const clock = new FakeClock(); let calls = 0;
  clock.sleep = async () => { calls++; throw new Error("must not create a timer"); };
  await expect(runLatencyProbe({ url: "http://private", durationMs: 1000, ...options, clock, fetch: async () => { calls++; return new Response("ok"); } })).rejects.toThrow("invalid probe bounds");
  expect(calls).toBe(0);
});
