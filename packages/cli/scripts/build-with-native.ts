 ...
 *
 * Measured on bun 1.3.14, darwin-arm64, median of 11 runs (`cast bench boot
 * --binary`): `cast --help` 90 -> 50 ms CPU, `cast state` 80 -> 60 ms, `cast
 * bogus` 90 -> 60 ms. The same builds read 7,110,947 and 2,031 bytes from
 * `cast _boot-bytes`, which is the effect without the clock's 10 ms floor.
 *
