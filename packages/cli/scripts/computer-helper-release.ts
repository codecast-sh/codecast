...
import { COMPUTER_HELPER_BUNDLE_ID, COMPUTER_HELPER_BUNDLE_NAME, buildComputerHelper } from "./build-with-native.ts";
import { gradeAssessment } from "../src/computer/helperApp.ts";
 ...
 */
export function verifyHelperBundle(tar: string, expect: HelperIdentityExpectations = {}): { bundleId: string; version: string; authority: string } {
  ...
  try {
    ...
    return { bundleId, version, authority };
  } finally {
    fs.rmSync(stage, { recursive: true, force: true });
  }
}
...
if (import.meta.main) {
  ...
  if (command === "build") {
    ...
    if (!target) throw new Error("usage: computer-helper-release.ts verify <binaries-dir> [--identity <authority>] [--team <id>] [--version <v>]");
    const record = verifyRelease(target, { identity: flag("identity"), team: flag("team"), version: flag("version") });
