...
app.whenReady().then(() => {
  ...
  session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
    ...
    // Prod, the mkcert dev host, and any loopback (a Vite port, a worktree's
    // dev server) — all of them are this app's own code.
    const loopback = host === "localhost" || host === "127.0.0.1" || host === "[::1]";
    callback(host === "codecast.sh" || host === LOCAL_DEV_HOST || loopback);
  });
