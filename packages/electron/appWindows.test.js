...
test("sectionForRoute lights the tab the path sits under", () => {
  ...
  assert.equal(sectionForRoute("chat", "/chat/ch1").label, "Messages");
  assert.equal(sectionForRoute("chat", "/community/room"), null);
  // Plans read as documents: the Docs tab stays lit on a plan.
  assert.equal(sectionForRoute("work", "/plans/pl-1").label, "Docs");
