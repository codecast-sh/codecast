...
function findWindow(rig, arg) {
  return rig.windows.find((w) =>
    (w.options.webPreferences?.additionalArguments ?? []).includes(arg),
  );
}

test("no ring, no window — and the first ring builds one", () => {
  // An always-on-top transparent window with a subscription behind it is real
  // cost, and most launches never see a ring.
  const rig = loadShell();
  assert.equal(findWindow(rig, "--call-ring-window"), undefined);
...
test("a second ring finds the window already up", () => {
...
test("only the ring window may drive its own window", () => {
  ...
  const call = findWindow(rig, "--call-panel-window");
...
test("the accept reaches the call window, and names the invite", () => {
  const rig = loadShell();
  const { sender } = openRingWindow(rig);
  rig.handlers.get("call-ring-answer")(sender, "inv-7", "dm:a:b");
  const call = findWindow(rig, "--call-panel-window");
  const sent = [];
