...
import { HUDDLE_DIGEST_CLIENT_ID_PREFIX, TRANSCRIBE_LANGUAGE, isWrongScriptTranscript, parseRoomKey } from "@codecast/shared/contracts";
    ...
    if (apiKey) {
      try {
        ...
        if (audio) {
          ...
          if (isWrongScriptTranscript(content)) content = "";
        }
      } catch (err) {
