...
async function wakeMentionedParties(
  ...
  const selfSession = opts.message.origin_session_id;
  ...
  for (const role of opts.roles) {
    ...
    try {
      // TODO(W1 outbox): enqueue an immediate role_wake_outbox row (cause
      // "chat_mention", ref the message) instead of delivering straight to the
      // anchor, once orgWakes/orgEvents lands (org-roles-standing.md T3).
      await deliverToAnchor(ctx, anchor._id, text, `chat-mention:${opts.message._id}:${role._id}`);
      out.roles++;
    } catch {
      out.skipped.push(`delivery_failed:${role.handle}`);
    }
  }
...
export const linesSince = query({
  ...
  handler: async (ctx, args) => {
    ...
    for (const channelId of args.channel_ids.slice(0, 20)) {
      const channel = await readChannel(ctx, userId, channelId);
      if (!channel || channel.archived_at) continue;
      const rows = await ctx.db
        .query("chat_messages")
        .withIndex("by_channel_created", (q: any) =>
          q.eq("channel_id", channelId).gt("created_at", args.since))
        .order("desc")
        .take(scan);
