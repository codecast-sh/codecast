...
// WITH history into a client whose transcript codecast cannot rebuild (pi,
// cursor, opencode; grok until its bundle writer landed) used to stamp the
...
describe("switchSessionAgent refuses agents that cannot rebuild history", () => {
  test("pi on a session with messages: no patch, no divider, no daemon command", async () => {
    const db = seedConv();
    await expect(
      (switchSessionAgent as any)._handler(ctxFor(db), { conversation_id: CONV, agent_type: "pi" }),
    ).rejects.toThrow(/pi cannot take over/);
  ...
  test("pi on a blank session is a relaunch and goes through", async () => {
    const db = seedConv({
      conversations: [{
        _id: CONV,
        user_id: USER,
    ...
    const conv = db._tables.conversations.find((r: any) => r._id === CONV);
    expect(conv.agent_type).toBe("pi");
  });
});
