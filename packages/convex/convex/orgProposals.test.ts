...
describe("orgProposals asks (S19)", () => {
  ...
  test("create stores the asks; get and list return them; a proposal without asks derives them on read", async () => {
    ...
    expect((await readProposal(ctxOf(db), ME as any, plain.short_id)).asks.map((a: any) => [a.title, a.seqs])).toEqual([["Retire the agent @growth", [1]], ["1 smaller change: filing, goals and settings", [2]]]);
  });
