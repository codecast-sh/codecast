...
async function sayIntoSession(
  ...
  authUserId: Id<"users">,
  conversation: any,
  line: { content: string; client_id: string; sender: any | null; human: boolean },
): Promise<"told" | "deferred"> {
...
// standing session gets one line, not two.
async function writeEscalationDividers(
  ctx: { db: any },
  authUserId: Id<"users">,
  opts: { conversation: any; standing: any | null; caller: any | null; role: any; person: any; move: "handed" | "direct" | "back"; by: "role" | "person"; line: string; at: number },
): Promise<void> {
...
export async function performEscalateSession(
  ...
  return {
    ...
    notify: changed && actor.kind === "role"
      ? { conversation_id: reachedRow._id, title: direct ? `@${role.handle} put a session in front of you` : `@${role.handle} needs you`, message: direct ? `@${role.handle}: ${title} — ${line}` : `@${role.handle}: ${line} (${title})` }
      : null,
  };
}
