...
import { recordGitEvent } from "./gitEvents";
import {
  patchPullRequest,
...
/** Replace a check by its identity (GitHub's id when we have one, else name). */
function upsertCheck(existing: CheckEntry[] | undefined, entry: CheckEntry): CheckEntry[] {
  const key = entry.external_id ?? entry.name;
  const rest = (existing ?? []).filter((c) => (c.external_id ?? c.name) !== key);
  return [...rest, entry];
}

/** Fire a derived PR trigger — the names agents write in an event filter. */
async function fireTrigger(ctx: any, eventType: string, pr: Doc<"pull_requests">) {
...
export const storeWebhookEvent = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // triggers armed before the derived names existed.
...
export const processReviewEvent = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const repository: string = payload.repository?.full_name ?? prPayload.base?.repo?.full_name;
    ...
    if (worthWaking && event.action !== "dismissed") {
      await wakeShepherd(ctx, updated._id, "review_submitted", `${author} ${stateWords}`);
    }
...
async function ingestReviewComment(ctx: any, payload: any): Promise<boolean> {
  ...
  const repository: string = payload.repository?.full_name ?? pullRequest.base?.repo?.full_name;
  ...
  const commentId = await ctx.db.insert("review_comments", {
    ...
    line_end: comment.start_line ? comment.line ?? comment.original_line : undefined,
    side: comment.side,
  ...
  const where = comment.path ? `${comment.path}${comment.line ? `:${comment.line}` : ""}` : "the pull request";
  ...
  await recordGitEvent(ctx, {
    ...
    conversation_id: pr.shepherd_conversation_id,
    task_ids: pr.task_ids,
    meta: { file_path: comment.path, line_number: comment.line ?? comment.original_line },
...
export const processCheckSuiteEvent = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    let applied = 0;
    for (const pr of prs) {
      if ((pr.checks ?? []).length > 0) continue;
      const entry: CheckEntry = {
