...
/**
 * The dedupe key for one delivery (S1.4). Linear has no delivery-id header:
 * `webhookId` identifies the SUBSCRIPTION, so it only becomes a delivery key
 * once `webhookTimestamp` is folded in. The type+entity fallback covers older
 * payloads that omit webhookId entirely.
 */
