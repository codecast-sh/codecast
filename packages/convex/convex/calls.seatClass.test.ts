...
describe("getTeamMembers carries the seat class", () => {
  ...
  test("a viewer the room does not admit gets the bare boolean and nothing else", async () => {
    // A teammate is admitted to any unlocked live room (callRooms.openRoomDoor),
    // DM member or not, so an open DM tells Cy everything. A lock is what
    // shuts the door on her while Ann and Bob keep their seats by membership.
    const rows = tables();
    seat(rows, ANN, DM);
    seat(rows, BOB, DM, { walkie_joined_at: NOW });
    rows.call_room_state.push({ _id: "rs1", room_key: DM, team_id: T1, locked: true, locked_by: ANN, updated_at: NOW });
    ...
    expect(bob.walkie_joined_at).toBeUndefined();
    // The lock shut the door on Cy alone: Ann still reads the whole seat.
    expect(await rosterRow(rows, ANN, BOB)).toMatchObject({ in_room_key: DM, seat: "call" });
  });
