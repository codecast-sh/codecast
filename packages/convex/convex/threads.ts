...
import { THREAD_BADGE_SCAN, taskThreadParticipants, threadKindValidator, type ThreadKind } from "./threadReads";
...
type PageCache = {
  ...
  taskParticipants: Map<string, Id<"users">[]>;
...
const taskKind: ThreadKindResolver = {
  ...
  // viewer is a participant under taskThreadParticipants' human-act rule, or
  // wrote in the thread themself (touchThread files the actor even when the
  // rule would not). Rows filed under the older identity rule (the owner of
  // every agent task) drop here until the sweep purges them, and stay out
  // whatever an old client files.
  async access(ctx, userId, row, cache) {
    const task = await taskFor(ctx, row, cache);
    if (!task || !(await canAccessTask(ctx, userId, task))) return false;
    const participants = await memo(cache.taskParticipants, row.root_key, () => taskThreadParticipants(ctx, task));
    if (participants.some((id) => String(id) === String(userId))) return true;
