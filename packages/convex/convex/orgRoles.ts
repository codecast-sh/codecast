...
export async function performUpdateRole(
  ...
  const role = await requireRole(ctx, userId, args.role_id, "admin");
  if (args.review_backend !== undefined) {
    await ctx.db.patch(role._id, { review_backend: normalizeBackend(args.review_backend), updated_at: Date.now() });
  }
...
export async function performProvisionRole(
  ...
  await ctx.db.patch(role._id, patch);
  return { ...provisioned, role_id: role._id, short_id: role.short_id, handle: role.handle };
}
