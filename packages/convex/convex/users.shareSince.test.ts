...
describe("the backfill under a lock", () => {
  test("closes rule shares and leaves the owner's hand shares alone", async () => {
    const db = makeFakeDb({
      users: [{ _id: OWNER }],
      teams: [{ _id: TEAM }],
      team_memberships: [{ _id: "m1", user_id: OWNER, team_id: TEAM, role: "member", visibility: "full" }],
      directory_team_mappings: [{ _id: "dm1", user_id: OWNER, path_prefix: "/repo", auto_share: false, private: true }],
      conversations: [
        { _id: "c_rule", user_id: OWNER, git_root: "/repo", started_at: T, is_private: false, auto_shared: true, team_id: TEAM },
        { _id: "c_manual", user_id: OWNER, git_root: "/repo", started_at: T, is_private: false, team_id: TEAM },
      ],
      tasks: [], docs: [], plans: [], projects: [],
    });
    const ctx = ctxWith(db);
    await run(backfillDirectoryTeamMappingConversations, ctx, { user_id: OWNER, path_prefix: "/repo", source: "git_root" });
    const row = (id: string) => db._tables.conversations.find((c: any) => c._id === id);
    expect(row("c_rule")).toMatchObject({ is_private: true });
    expect(row("c_rule").auto_shared).toBeFalsy();
    expect(row("c_rule").team_id).toBeUndefined();
    expect(row("c_manual")).toMatchObject({ is_private: false, team_id: TEAM });
  });
});
  ...
  const db = () =>
    makeFakeDb({
      ...
      conversations: [
        ...
        // The path indexes order by started_at; this fake orders by updated_at,
        // so the rows' updated_at follow their start.
        { _id: "c4", user_id: OWNER, git_root: "/repo", started_at: T - 20, updated_at: T - 20, title: "manual", is_private: false, team_id: TEAM },
        { _id: "c5", user_id: OWNER, git_root: "/repo", started_at: NaN, updated_at: T - 30, title: "nan", is_private: true },
      ],
    });
...
describe("upsertDirectoryMapping addresses the repository, not the checkout", () => {
  ...
  test("a lock replaces the repository's share rule and needs no team", async () => {
    const db = base();
    const ctx = ctxWith(db);
    const r = await upsertDirectoryMapping(ctx, OWNER, { path_prefix: "/b", private: true });
    expect(r).toMatchObject({ action: "updated", share_since: null });
    const rules = db._tables.directory_team_mappings;
    expect(rules.map((m: any) => m.path_prefix)).toEqual(["/a"]);
    expect(rules[0]).toMatchObject({ auto_share: false, private: true, repository: KEY });
    expect(rules[0].team_id).toBeUndefined();
    expect(rules[0].share_since).toBeUndefined();
    // A later share from any checkout lifts the lock.
    const again = await upsertDirectoryMapping(ctx, OWNER, { path_prefix: "/b", team_id: TEAM_B });
    expect(again).toMatchObject({ action: "updated" });
    expect(db._tables.directory_team_mappings[0]).toMatchObject({ team_id: TEAM_B, auto_share: true });
    expect(db._tables.directory_team_mappings[0].private).toBeUndefined();
  });

  test("a lock on a folder with no rule and no sessions is created as is", async () => {
    const db = base();
    const ctx = ctxWith(db);
    const r = await upsertDirectoryMapping(ctx, OWNER, { path_prefix: "/health", private: true });
    expect(r).toMatchObject({ action: "created" });
    const row = db._tables.directory_team_mappings.find((m: any) => m.path_prefix === "/health");
    expect(row).toMatchObject({ user_id: OWNER, private: true, auto_share: false });
