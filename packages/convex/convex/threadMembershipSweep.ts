...
//
// A third step clears the assignee an agent's `cast task start` self-claim
// left on agent-origin tasks (ct-44870): isOnHumanBoard counts ANY assignee,
// so those claims leak the owner's own agent bookkeeping onto the human
// /tasks board. A task loses its assignee when the assignee is the owner, the
// task is not a human task, and no via:"human" assignee subscription row says
// a person assigned it on purpose (the web self-assign path leaves one; the
// CLI claim leaves none). ct-44836 fixed the CLI going forward; this cleans
// history. Run it on its own:
//
//   npx convex run threadMembershipSweep:sweepAssignees '{}'                 # dry run
//   npx convex run threadMembershipSweep:sweepAssignees '{"dryRun": false}'  # clear
//
  ...
  handler: async (ctx, args) => {
    ...
    if (args.step === "assignees") {
      ...
      for (const task of page.page) {
        if (!task.assignee || String(task.assignee) !== String(task.user_id)) continue;
        selfAssigned++;
        if (isHumanTask(task)) { keptHuman++; continue; }
        // An explicit web self-assign records intent as a via:"human" assignee
        // subscription row; the CLI self-claim records none.
        const taskSubs = await ctx.db
          .query("entity_subscriptions")
          .withIndex("by_entity", (q: any) =>
            q.eq("entity_type", "task").eq("entity_id", String(task._id)))
          .collect();
        const explicit = taskSubs.some((s) =>
          s.reason === "assignee" &&
          String(s.user_id) === String(task.user_id) &&
          s.via === "human");
        if (explicit) { keptExplicit++; continue; }
        cleared++;
...
async function runStep(
  ...
  step: "subscriptions" | "threadReads" | "assignees",
  dryRun: boolean,
) {
  const totals: any = {};
...
export const sweep = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const subscriptions = await runStep(ctx, "subscriptions", dryRun);
    const threadReads = await runStep(ctx, "threadReads", dryRun);
    return { dryRun, subscriptions, threadReads };
  },
});

export const sweepAssignees = internalAction({
  args: { dryRun: v.optional(v.boolean()) },
  handler: async (ctx, args) => {
    const dryRun = args.dryRun !== false;
    const assignees = await runStep(ctx, "assignees", dryRun);
    return { dryRun, assignees };
  },
});
