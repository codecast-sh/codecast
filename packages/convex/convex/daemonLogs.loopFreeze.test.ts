// The loop-freeze alert must fire once per incident and then stay quiet. Two
// traps live in the decision: a machine already alerted on keeps beating the
// same hour total every 5 minutes, and an OLD daemon that stopped reporting
// leaves its last value frozen on the device row forever. Both are handled by
// comparing hour totals rather than only timing out, so a value that never
// grows can alert at most once.
