...
// handled by the handler clearing the stamp the moment the hour falls back
// under the bar.
...
describe("checkDeviceLoopFreeze", () => {
  ...
  test("a machine that recovers alerts again on its next, smaller incident", async () => {
    ...
    // The incident ends: the freezes roll out of the trailing hour.
    rows[1].loop_freeze_1h_ms = 60_000;
    expect((await handler(ctx, {})).notified).toBe(0);
    // Recovery clears the stamp, so the next incident is judged on its own.
