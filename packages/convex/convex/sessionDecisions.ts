...
import { pickAnsweredDecision, formatDecisionAnswer } from "@codecast/shared/contracts";
...
export async function roleOfConversation(ctx: Ctx, conversation: any): Promise<Doc<"org_roles"> | null> {
  const roleId = conversation?.org_role_id ?? conversation?.standing_role_id;
...
export async function refreshHolder(ctx: Ctx, row: DecisionRow, now: number): Promise<void> {
  const roleIds = (row.hops ?? []).filter((h) => !h.note?.startsWith("skipped")).map((h) => h.role_id);
  const delegate = await stackDelegateRoleId(ctx, row.stack_id);
  if (delegate) roleIds.push(delegate);
...
// Wake each active ladder role once. A role without a standing session gets
// the hop recorded and nothing else (W1's outbox will carry these later).
async function wakeLadder(ctx: Ctx, roles: Doc<"org_roles">[], row: { _id: Id<"session_decisions">; short_id?: string; question: string }) {
  ...
  for (const role of roles) {
    ...
    try {
      await deliverToAnchor(
        ctx,
        role.anchor_id,
        `<decision-ask ${row.short_id ?? row._id}>${row.question}</decision-ask>\nA decision on your ladder. Read it with \`cast decide show ${row.short_id ?? row._id}\`, then \`cast decide recommend ${row.short_id ?? row._id} <n>\` within 5 minutes, or \`cast decide escalate\`.`,
        `decision-ask:${row._id}:${role._id}`,
      );
      woken.push(role._id);
    } catch {
      // No session yet, or decommissioned: the hop stays, the wake is skipped.
    }
  }
  return woken;
}
...
export function normalizeVerdict(row: DecisionRow, raw: Verdict): Verdict | { error: string } {
  ...
  for (const f of fields) {
    ...
    if (f.type === "select" && f.options && !f.options.includes(String(values[f.key]))) {
...
): Promise<{ already_resolved: boolean; answer_label?: string }> {
...
export async function askCore(ctx: Ctx, auth: { userId: Id<"users"> }, args: AskArgs): Promise<any> {
  ...
  if (existing) {
    ...
    return { id: existing._id, short_id: existing.short_id, updated: true, category, category_pinned: pinned, other_open: staleOpen(existing._id) };
  }
  ...
  if (stack) {
    await ctx.db.patch(stack._id, { decision_ids: [...stack.decision_ids, id], updated_at: now });
  }
  const woken = await wakeLadder(ctx, ladder.activeRoles, { _id: id, short_id, question: args.question });
...
export const listForSession = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.stack) {
      const stack = await findStack(ctx, args.stack);
      if (!stack) return { error: `Stack not found: ${args.stack}` };
    ...
    if (args.task) {
      const task = await findTask(ctx, args.task);
      if (!task) return { error: `Task not found: ${args.task}` };
      for (const status of ["pending", "answered", "dismissed", "withdrawn"] as const) {
        rows.push(
          ...(await ctx.db
            .query("session_decisions")
            .withIndex("by_task", (q) => q.eq("task_id", task._id).eq("status", status))
            .collect())
        );
      }
    } else {
      for (const status of ["pending", "answered", "dismissed", "withdrawn"] as const) {
        rows.push(
          ...(await ctx.db
            .query("session_decisions")
            .withIndex("by_conversation_status", (q) => q.eq("conversation_id", conversation._id).eq("status", status))
            .collect())
        );
      }
    }
...
export const escalate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // A role that escalates gives up its hold: the people answer.
    const patch: any = { hops, updated_at: now };
    if (row.holder?.kind === "role" && row.holder.id === String(found.role._id)) {
      const person = (row.asked_user_ids ?? [row.user_id])[0];
      patch.holder = { kind: "user", id: String(person) };
      patch.holder_key = `user:${person}`;
    }
...
// Who the token's caller is on this decision: a person in asked_user_ids (a
// human answer, always allowed and always wins), or the holder role speaking
// from its session under a live grant.
async function answererFor(
  ...
  // Prefer the role when the caller's session speaks as the holder role: the
  // grant is what the answer is recorded under, so the streak and the
  // "Handled without you" list stay honest even when the host is a person.
  if (sessionId && row.holder?.kind === "role") {
    const conversation = await ctx.db
      .query("conversations")
      .withIndex("by_session_id", (q: any) => q.eq("session_id", sessionId))
      .first();
    const role = conversation ? await roleOfConversation(ctx, conversation) : null;
    if (role && String(role._id) === row.holder.id && conversation.user_id.toString() === auth.userId.toString()) {
  ...
  return { error: "Not a holder of this decision: only the people it was asked of, or the holder role from its session, may answer" };
}
...
export async function agreementHistory(
  ...
  const rows: DecisionRow[] = await ctx.db
    .query("session_decisions")
    .withIndex("by_status_created", (q: any) => q.eq("status", "answered").gte("created_at", now - HISTORY_WINDOW_MS))
    .collect();
  const askers = new Set<string>();
  let agreements = 0;
  for (const r of rows) {
    if (r.category !== category) continue;
    if (!((r as any).scope_keys ?? []).includes(scopeKey)) continue;
    if (r.answered_by?.kind !== "user") continue;
    const hop = (r.hops ?? []).find((h) => h.role_id === roleId);
    if (!hop || hop.recommendation === undefined || hop.recommendation !== r.answer_index) continue;
    agreements += 1;
    askers.add(String(r.conversation_id));
  }
  return { agreements, askers: askers.size, eligible: agreements >= GRANT_RULE.agreements && askers.size >= GRANT_RULE.askers };
}
...
export async function grantCore(
  ...
  for (const r of pending) {
    if (r.category !== args.category || !((r as any).scope_keys ?? []).includes(args.scope_key)) continue;
    await refreshHolder(ctx, r, now);
  }
  return { id, already_granted: false, expires_at: now + GRANT_TTL_MS, forced: !history.eligible };
}

// Human only: the card's "Let this role answer questions like this here", or
// `cast stack delegate`. Auth'd web user or an api token; a session cannot
...
export const grant = mutation({
  ...
  handler: async (ctx, args) => {
    const userId = args.api_token ? (await verifyApiToken(ctx, args.api_token))?.userId : await getAuthUserId(ctx);
    if (!userId) return { error: "Unauthorized" };
    const { api_token: _t, ...rest } = args;
    return grantCore(ctx, userId, rest);
  },
});

export const revoke = mutation({
  args: { api_token: v.optional(v.string()), grant_id: v.id("decision_grants"), reason: v.optional(v.string()) },
  handler: async (ctx, args) => {
    const userId = args.api_token ? (await verifyApiToken(ctx, args.api_token))?.userId : await getAuthUserId(ctx);
    if (!userId) return { error: "Unauthorized" };
...
// conversation, and anyone who can access a role on its ladder (the role's
// host reading what its seat was asked).
async function userMayRead(ctx: Ctx, userId: Id<"users">, row: DecisionRow): Promise<boolean> {
  const people = (row.asked_user_ids ?? [row.user_id]).map(String);
  ...
  for (const hop of row.hops ?? []) {
    ...
    if (role && (await userCanAccessRole(ctx, userId, role))) return true;
  }
  return false;
}
  ...
  if (row.status === "answered" && row.answered_by?.kind === "user" && row.category && !isHumanOnlyCategory(row.category)) {
    ...
    for (const hop of row.hops ?? []) {
      ...
      for (const scopeKey of scopeKeys) {
        const h = await agreementHistory(ctx, hop.role_id, row.category, scopeKey, Date.now());
        if (h.eligible && !(await activeGrantFor(ctx, [hop.role_id], row.category, [scopeKey], Date.now()))) {
          const role = await ctx.db.get(hop.role_id);
...
export const listForRole = query({
  ...
  handler: async (ctx, args) => {
    ...
    const recent: DecisionRow[] = await ctx.db
      .query("session_decisions")
      .withIndex("by_status_created", (q) => q.eq("status", "pending").gte("created_at", Date.now() - HISTORY_WINDOW_MS))
      .collect();
    for (const r of recent) {
      if ((r.hops ?? []).some((h) => h.role_id === args.role_id)) out.set(String(r._id), r);
    }
    return Array.from(out.values()).map((r) => ({ ...r, held_by_role: r.holder_key === `role:${args.role_id}` }));
  },
});
