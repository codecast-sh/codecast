...
describe("orgWakes.buildFrame", () => {
  ...
  test("Your scope now reads back where each project stands from the brief, with the line's age (F5.2)", () => {
    const DAY = 86_400_000;
    const today = new Date(NOW).toISOString().slice(0, 10);
    const tenAgo = new Date(NOW - 10 * DAY).toISOString().slice(0, 10);
    const f = buildFrame(base([{ kind: "fold", cause: "x" }], {
      facts: { ...facts, scope: { ...facts.scope, projects: [{ id: "p1", title: "Infrastructure", short_id: "pj-1" }, { id: "p2", title: "Billing" }, { id: "p3", title: "Docs" }] } },
      brief: { content: `state line\n\n## Where it stands\n- Infrastructure: Postgres move is half done; the deploy waits on a review. (${today})\n- Billing: invoices ship again since Tuesday (${tenAgo})\n\n## Goals: Me\n1. A goal` },
    }));
    const section = f.text.slice(f.text.indexOf("## Your scope now"), f.text.indexOf("## Your sessions"));
    expect(section).toContain("Where it stands (your brief; a person reads these on your page):");
    expect(section).toContain("- Infrastructure: Postgres move is half done; the deploy waits on a review. (written today)");
    expect(section).toContain("- Billing: invoices ship again since Tuesday (written 10 days ago, older than a week)");
...
describe("a hand's briefing and the anchor's roles section (review: coherence, day scenario)", () => {
  ...
  test("the charter template carries the routing rule so a restart frame re-reads it", () => {
    ...
    expect(ROLE_RULES).toHaveLength(8);
    expect(charter).toContain("8. Where it stands is one sentence per project");
