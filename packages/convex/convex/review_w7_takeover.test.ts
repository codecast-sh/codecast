...
import { performCreateRole, performRetireRole, performSetProjectLead } from "./orgRoles";
...
describe("review: the takeover moves a session it should not", () => {
  ...
  test("naming a project's lead offers the person's one edit (leave the sessions) like every other scope gain", async () => {
    // performCoverProjects calls takeOverSessions with no `leave` option and
    // no dry count first: one click on the project page moves up to 100
    // sessions out of the person's needs input, and nothing can say no.
    const db = fixtures([conv(1), conv(2)]);
    const ctx = ctxOf(db);
    // A role with some other scope, so it does not look after the whole workspace.
    const r = await performCreateRole(ctx, ME as any, { name: "Growth", handle: "growth", team_id: TEAM, scope: { project_ids: ["projects_q" as any], plan_ids: [] } });
    const args: any = { project_id: P, role_id: String(r._id), leave_sessions: true };
    const res = await performSetProjectLead(ctx, ME as any, args);
