...
import { canAccessConversation, requireAccessibleSteeringItem, requireTeamMembership } from "./lib/access";
import {
  linkWorkspace,
...
async function createProposal(ctx: any, args: any) {
  ...
  const team_id = conversation?.team_id ?? await resolveCreateTeamId(ctx, userId, { team_id: args.team_id, workspace: args.workspace });
  const now = Date.now();
...
export const webApply = mutation({ args: { id: v.id("steering_proposals") }, handler: async (ctx, args) => applyProposal(ctx, await apiUser(ctx), await ctx.db.get(args.id)) });
