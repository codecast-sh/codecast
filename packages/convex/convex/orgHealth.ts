...
import { capacity, capacityFlags, type HealthFlag } from "@codecast/shared/contracts/orgCapacity";
...
/** A role's last scope event and its wake log over the week. The idle rule
 *  is the model's `idle_days`; `wholeWorkspaceLatest` is the newest event a
 *  whole workspace role saw, since its scope resolves to no rows. */
export async function roleActivity(ctx: Ctx, userId: Id<"users">, role: any, now: number, opts: { wholeWorkspaceLatest?: number | null } = {}): Promise<RoleActivity> {
  ...
  if (opts.wholeWorkspaceLatest !== undefined && isWholeWorkspace(scopeIds(role.scope ?? { project_ids: [], plan_ids: [] }))) {
  ...
  for (const w of wakes) {
    ...
    if (w.status === "dropped") dropped++; else delivered++;
  ...
  return {
    ...
    idle_days: lastScopeEventAt ? Math.floor((now - lastScopeEventAt) / D) : null,
    idle: !lastScopeEventAt || now - lastScopeEventAt > capacity("idle_days") * D,
    wakes_7d: { total: wakes.length - held, delivered, dropped, held, days_at_cap: daysAtCap, cap_hit_days: capHitDays, by_day: Object.fromEntries(byDay) },
  };
}
...
export async function computeOrgHealth(ctx: Ctx, userId: Id<"users">, teamId: Id<"teams"> | undefined, now: number) {
  ...
  // workspace scope is every row read.
  const itemsOf = (role: any) => {
    const ids = scopeIds(role.scope ?? { project_ids: [], plan_ids: [] });
    if (isWholeWorkspace(ids)) return { tasks, plans, projectIds: new Set(projects.map((p) => String(p._id))) };
  ...
  for (const status of ["pending", "answered"]) {
    const rows: any[] = await ctx.db.query("session_decisions").withIndex("by_status_created", (q: any) => q.eq("status", status).gte("created_at", cut7)).order("desc").take(HEALTH_CAPS.decisions);
    for (const d of rows) {
      for (const hop of d.hops ?? []) {
        const rid = String(hop.role_id);
        if (!roleById.has(rid)) continue;
        bump(decisionsOf, rid);
        if (hop.recommendation !== undefined) latency.set(rid, [...(latency.get(rid) ?? []), (hop.at - d.created_at) / 60_000]);
        if (typeof hop.note === "string" && hop.note.startsWith("escalated")) bump(escalations, rid);
      }
    }
  }
  ...
  let latestAnywhere: number | null = null;
  for (const t of tasks) latestAnywhere = Math.max(latestAnywhere ?? 0, t.updated_at ?? 0) || latestAnywhere;
  for (const p of plans) latestAnywhere = Math.max(latestAnywhere ?? 0, p.updated_at ?? 0) || latestAnywhere;
  for (const { session } of scan.sessions.values()) latestAnywhere = Math.max(latestAnywhere ?? 0, session.updated_at ?? 0) || latestAnywhere;
  ...
  for (const role of roles) {
    ...
    const flags: HealthFlag[] = capacityFlags({
      ...
      age_days: Math.floor((now - (role.created_at ?? now)) / D),
  ...
  // ── Company ─────────────────────────────────────────────────────────────
  // A project is owned when a role names it as owner, or when a role's
  // scope includes it (directly, through one of its plans, or because a role
  ...
  for (const r of roles) {
    ...
    for (const id of ids.project_ids) covered.add(id);
    for (const id of ids.plan_ids) { const pr = planProjectOf.get(id); if (pr) covered.add(pr); }
  }
  const liveProjects = projects.filter((p) => p.status !== "done");
  const unowned_projects = wholeWorkspaceRoles > 0 ? [] : liveProjects.filter((p) => !p.owner_role_id && !covered.has(String(p._id))).map((p) => ({ id: String(p._id), title: p.title }));
  const company = {
    unowned_projects,
  ...
  return {
    ...
    truncated: { sessions: scan.truncated, tasks: tasks.length >= HEALTH_CAPS.tasks, plans: plans.length >= HEALTH_CAPS.plans, projects: projects.length >= HEALTH_CAPS.projects },
    generated_at: now,
  };
}
