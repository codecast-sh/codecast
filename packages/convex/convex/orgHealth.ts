...
import { collectOrgSessions, computeScopeFeed, requireWorkspaceCaller, resolveScope, type OrgScan, type ResolvedScope } from "./org";
...
export async function roleActivity(ctx: Ctx, userId: Id<"users">, role: any, now: number, opts: { wholeWorkspaceLatest: number | null; resolved?: ResolvedScope | null }): Promise<RoleActivity> {
...
export async function computeOrgHealth(ctx: Ctx, userId: Id<"users">, teamId: Id<"teams"> | undefined, now: number, opts?: { decisions?: HealthDecisions }) {
  const cut7 = now - HEALTH_WINDOW_7D_MS;
  ...
  const [projects, plans, work] = await Promise.all([
    ...
    readWorkTasks(ctx, userId, teamId, { perStatus: HEALTH_CAPS.tasks, updatedSince: cut7, recentCap: HEALTH_CAPS.recent_tasks }),
  ]);
  ...
  // under either, read by index so a workspace past the task cap still counts
  // every row. A whole workspace role holds the remainder: what no narrower
  // role covers, which is the work it exists to find an owner for; that read
  // is over the capped workspace fetch, and `counted` says so.
  ...
  const itemsOf = async (role: any) => {
    ...
    if (resolved) {
      const active = resolved.tasks.filter(isActiveTask);
      return {
        tasks: active,
        plans: resolved.plans,
        projectIds: new Set(resolved.projects.map((p) => String(p._id))),
        resolved,
        counted: {
          rule: "scope" as const,
          projects: resolved.projects.length, plans: resolved.plans.length, tasks: active.length,
          complete: true,
          note: `${resolved.projects.length} projects and ${resolved.plans.length} plans in scope (${ids.plan_ids.length} named, the rest under the projects) and every task filed under either: ${active.length} tasks, read by index, the same rows the scope feed and the task list show${resolved.tasks.length > active.length ? ` (${resolved.tasks.length - active.length} mined suggestions left out)` : ""}`,
        },
      };
    }
    ...
    return { ...items, resolved: null as ResolvedScope | null, counted: { rule: "scope" as const, projects: projectIds.size, plans: planIds.size, tasks: items.tasks.length, complete: !tasksTruncated, note: `${projectIds.size} projects and ${planIds.size} plans in scope over the workspace fetch${tasksTruncated ? " (a floor: the workspace has more tasks than the cap)" : ""}` } };
  };
  ...
  for (const role of roles) {
    ...
    let done7 = 0, stalls = 0, changed7 = 0;
    ...
    const activity = await roleActivity(ctx, userId, role, now, { wholeWorkspaceLatest: latestAnywhere, resolved: items.resolved });
    const decisions7 = dec.decisionsOf[rid] ?? 0;
    // The load: what reached the seat this week and asked for its attention.
    const load: RoleLoad = {
      items_per_day: changed7 / 7,
      ...
      open_stalls: stalls + handoffs.blocked + handoffs.needs_context,
    ...
    const flow = {
      ...
      /** Distinct tasks and plans in scope that changed this week (load.items_per_day × 7). */
...
export const healthCorePart = query({
  args: { api_token: v.optional(v.string()), team_id: v.optional(v.id("teams")), now: v.optional(v.number()), decisions: v.optional(v.any()) },
  handler: async (ctx, args): Promise<any> => {
    const userId = await requireWorkspaceCaller(ctx, args.api_token, args.team_id);
    if (!userId) return null;
    return computeOrgHealth(ctx, userId, args.team_id, args.now ?? Date.now(), { decisions: args.decisions as HealthDecisions | undefined });
  },
});
...
export const healthReport = action({
  ...
  handler: async (ctx, args): Promise<any> => {
    const now = Date.now();
    const decisions = await ctx.runQuery((api as any).orgHealth.healthDecisionsPart, { ...args, now });
    if (decisions === null) return null;
    return ctx.runQuery((api as any).orgHealth.healthCorePart, { ...args, now, decisions });
  },
});
