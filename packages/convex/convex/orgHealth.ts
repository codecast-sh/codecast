...
import { capacity, capacityFlags, type HealthFlag, isOverloaded, overloadRatio } from "@codecast/shared/contracts/orgCapacity";
...
export async function computeOrgHealth(ctx: Ctx, userId: Id<"users">, teamId: Id<"teams"> | undefined, now: number) {
  ...
  for (const role of roles) {
    ...
    roleRows.push({
      role_id: role._id,
      // The busiest load count over its threshold (orgCapacity.overloadRatio);
