...
export async function readActivityCommits(ctx: Ctx, repos: string[], since: number, owner: CommitOwner, perRepoCap = ACTIVITY_COMMITS_PER_REPO): Promise<ActivityCommit[]> {
  ...
  for (const repo of repos) {
    ...
    for (const c of rows) if (await ours(c)) out.push({ repository: c.repository, timestamp: c.timestamp, author_name: c.author_name, author_email: c.author_email, files: c.files ?? null, task_ids: (c.task_ids ?? []).map((id: any) => String(id)), branch: c.branch ?? null, sha: c.sha ?? null, message: firstLine(c.message) });
  }
  return out;
}
