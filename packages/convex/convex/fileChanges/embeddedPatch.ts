...
export function getToolPatchInputs(tool: { name: string; input: string }): readonly string[] {
  const cached = patchInputsByTool.get(tool);
  if (cached && cached.name === tool.name && cached.input === tool.input) return cached.patches;
  const patches = Object.freeze(readToolPatchInputs(tool));
  patchInputsByTool.set(tool, { name: tool.name, input: tool.input, patches });
  return patches;
}

function readToolPatchInputs(tool: { name: string; input: string }): string[] {
