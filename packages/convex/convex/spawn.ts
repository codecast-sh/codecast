...
export const createSessionFromCli = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const handTask = roleGate && spawner?.active_task_id ? await ctx.db.get(spawner.active_task_id) : null;
    const handProject = handTask?.project_id ? await ctx.db.get(handTask.project_id) : null;
