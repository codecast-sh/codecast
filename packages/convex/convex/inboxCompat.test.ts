...
describe("un-flagged listInboxSessions payload — golden shape", () => {
  ...
  test("row key set is pinned", async () => {
    ...
    expect(Object.keys(row).sort()).toEqual([
      ...
      "awaiting_input", "daemon_alive_until", "effort", "forked_from", "git_branch", "git_root",
      "has_pending", "icon", "icon_color", "idle_summary", "image_preview_url",
      "implementation_session", "inbox_dismissed_at", "inbox_dormant_at", "inbox_killed_at",
      "inbox_pinned_at", "inbox_stash_hidden", "inbox_stashed_at", "is_anchor", "is_connected",
      "is_deferred", "is_dormant", "is_favorite", "is_idle", "is_pinned", "is_private",
      ...
      "pending_api_error_at", "pending_api_error_kind", "permission_mode", "producing_until",
      "project_path", "session_error", "session_id", "settle_verdict", "spawned_by_conversation_id",
      "started_at", "subtitle", "team_id", "thread_state", "thread_state_at",
