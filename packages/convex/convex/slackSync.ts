...
import { linkReceivesInbound, linkSendsOutbound, outboundSkipReason } from "./lib/slackOutbound";
...
export const LINK_DEFAULTS: Link["options"] = {
  threads: true,
  reactions: true,
  edits: true,
  files: true,
...
async function slackApi(token: string, method: string, params: Record<string, unknown> = {}): Promise<SlackResp> {
  // JSON only for the write methods that accept it (needed for `blocks`);
  // form encoding for everything else, including chat.getPermalink, which is
  // a read and rejects a JSON body.
  const json = /^(chat\.(postMessage|update|delete)|reactions\.(add|remove))$/.test(method);
  ...
  try {
    ...
    const data = (await resp.json()) as SlackResp;
    return data;
  } catch (error) {
...
export async function installationForTeam(ctx: ReadCtx, teamId: Id<"teams">): Promise<Install | null> {
  return await ctx.db
...
async function linkByChatChannel(ctx: ReadCtx, chatChannelId: Id<"chat_channels">): Promise<Link | null> {
  return await ctx.db
    .query("slack_channel_links")
...
function mergeOptions(base: Link["options"], patch?: Partial<Link["options"]> | null): Link["options"] {
  const out = { ...base };
  for (const [k, val] of Object.entries(patch ?? {})) {
    if (typeof val === "boolean") (out as any)[k] = val;
  }
  return out;
}
...
export const resolveLinkAuth = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await linkByChatChannel(ctx, channel._id);
    return {
      ok: true as const,
      user_id: userId,
...
export const commitLink = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const byChat = await linkByChatChannel(ctx, channel._id);
    if (byChat) {
    ...
    await ensureBridgeUser(ctx, install);
    const now = Date.now();
    const id = await ctx.db.insert("slack_channel_links", {
      ...
      options: mergeOptions(LINK_DEFAULTS, args.options),
      since_ts: args.since_ts,
      created_by: userId,
      created_at: now,
      updated_at: now,
    });
    // One notice in chat so the room knows where the new faces come from. Kept
    // out of Slack: the Slack side gets its own notice from the action.
    const actor = await ctx.db.get(userId);
    const bridge = install.bridge_user_id ?? (await ensureBridgeUser(ctx, install));
    const dir = args.direction ?? "both";
    const flow = dir === "both" ? "both ways" : dir === "slack_to_codecast" ? "from Slack only" : "to Slack only";
    await postChatMessage(ctx, {
      ...
      attachments: [],
      clientId: `${NOTICE_CLIENT_ID_PREFIX}${id}:linked`,
...
export const linkChannel = action({
  ...
  handler: async (ctx, args): Promise<{ ok: boolean; error?: string; link_id?: Id<"slack_channel_links"> }> => {
    ...
    const dir = args.direction ?? "both";
    const flow = dir === "both"
      ? "Messages here appear there and replies come back."
      : dir === "slack_to_codecast"
        ? "Messages here appear there."
        : "Messages from codecast appear here.";
...
export const updateLink = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.options) patch.options = mergeOptions(link.options, args.options);
...
export const unlinkChannel = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (install) {
      const bridge = await ensureBridgeUser(ctx, install);
      await postChatMessage(ctx, {
        channel,
        root: null,
        authorId: bridge,
        ...
        attachments: [],
        clientId: `${NOTICE_CLIENT_ID_PREFIX}${link._id}:unlinked`,
        externalAuthor: { name: "Slack", is_bot: true },
        syncLocalOnly: true,
      });
...
export const shareMessageToSlack = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const link = await linkByChatChannel(ctx, channel._id);
      ...
      if (link && (!link.last_error_at || now - link.last_error_at > 3_600_000)) {
        ...
        if (channel && install && !channel.archived_at) {
          ...
          await postChatMessage(ctx, {
            ...
            attachments: [],
            clientId: `${NOTICE_CLIENT_ID_PREFIX}${link._id}:failed:${Math.floor(now / 3_600_000)}`,
            externalAuthor: { name: "Slack", is_bot: true },
            syncLocalOnly: true,
          });
        }
...
async function buildInboundResolver(
  ...
  return {
    user: (id) => {
      ...
      if (!p) return null;
      const handle = p.codecast_user_id && link.options.match_people_by_email ? handles[p.codecast_user_id] ?? null : null;
      return { handle, name: p.name };
    },
  };
}
...
async function mirrorMessage(
  ...
  let externalAuthor: Doc<"chat_messages">["external_author"] | undefined;
  if (msg.user && !isBot) {
    const person = await resolvePerson(ctx, install, msg.user);
    if (person.codecast_user_id && link.options.match_people_by_email) {
      authorUserId = person.codecast_user_id;
    } else {
      externalAuthor = { name: person.name, handle: person.handle ?? undefined, avatar_url: person.avatar_url ?? undefined, is_bot: person.is_bot || undefined };
    }
  } else {
  ...
  if (content.length > MAX_CHAT_CONTENT) content = `${content.slice(0, MAX_CHAT_CONTENT - 2)}…`;
  ...
  handler: async (ctx, args): Promise<void> => {
    ...
    try {
      ...
      if (type === "message") {
        ...
        const label = who?.name ?? "Someone";
        const line = sub === "channel_join" ? `${label} joined the Slack channel`
          : sub === "channel_leave" ? `${label} left the Slack channel`
          : sub === "channel_topic" ? `${label} set the Slack topic: ${slackToMarkdown(String(event.topic ?? ""), { user: () => null })}`
          : sub === "channel_purpose" ? `${label} set the Slack purpose: ${slackToMarkdown(String(event.purpose ?? ""), { user: () => null })}`
          : sub === "channel_name" ? `${label} renamed the Slack channel to #${event.name ?? ""}`
          : sub === "pinned_item" ? `${label} pinned a message in Slack`
          : null;
        ...
        const res = await ctx.runMutation(internal.slackSync.applyInboundMessage, {
          ...
          author_user_id: who?.codecast_user_id && link.options.match_people_by_email ? who.codecast_user_id : undefined,
          external_author: who && !(who.codecast_user_id && link.options.match_people_by_email)
            ? { name: who.name, handle: who.handle ?? undefined, avatar_url: who.avatar_url ?? undefined }
            : undefined,
          content: `*${line}*`,
          attachments: [],
          live: false,
      ...
      if (type === "reaction_added" || type === "reaction_removed") {
        ...
        const res = await ctx.runMutation(internal.slackSync.applyInboundReaction, {
          ...
          user_id: person.codecast_user_id && link.options.match_people_by_email ? person.codecast_user_id : undefined,
        });
      ...
      if (type === "member_joined_channel" || type === "member_left_channel") {
        ...
        const res = await ctx.runMutation(internal.slackSync.applyInboundMessage, {
          ...
          author_user_id: who.codecast_user_id && link.options.match_people_by_email ? who.codecast_user_id : undefined,
          external_author: who.codecast_user_id && link.options.match_people_by_email
            ? undefined
            : { name: who.name, handle: who.handle ?? undefined, avatar_url: who.avatar_url ?? undefined },
...
export const applyInboundEdit = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const content = args.content.length > MAX_CHAT_CONTENT ? `${args.content.slice(0, MAX_CHAT_CONTENT - 2)}…` : args.content;
...
export const applyInboundDelete = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const reactions = await ctx.db
      ...
      .withIndex("by_message", (q: any) => q.eq("message_id", row._id))
      .collect();
...
export const pushContext = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const link = await linkByChatChannel(ctx, message.channel_id);
    if (!link) return null;
    const install = await ctx.db.get(link.installation_id);
    ...
    return {
      ...
      author: {
        ...
        isAgent: message.author_kind === "agent" || message.origin === "agent",
        via,
      },
...
function composeOutbound(c: NonNullable<Awaited<ReturnType<typeof pushContextHandler>>>) {
...
// Type helper only: lets composeOutbound name the query's return shape.
const pushContextHandler = async (_ctx: QueryCtx, _args: { message_id: Id<"chat_messages"> }) =>
  null as unknown as {
    message: Doc<"chat_messages">; link: Link; install: Install; channel: Doc<"chat_channels">;
    root_ts: string | null; author: { name: string; image: string | null; isAgent: boolean; via: string | null };
    handle_to_slack: Record<string, string>; attachments: Array<{ url: string; name: string | null; mime: string | null }>;
  } | null;
...
export const pushMessage = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!resp.ok) {
      await ctx.runMutation(internal.slackSync.recordOutboundFailure, { link_id: c.link._id, error: String(resp.error ?? "unknown") });
      return;
    }
...
export const pushEdit = internalAction({
  ...
  handler: async (ctx, args) => {
    const c = await ctx.runQuery(internal.slackSync.pushContext, { message_id: args.message_id });
    if (!c || !c.message.external || c.message.external.direction !== "outbound") return;
    if (!linkSendsOutbound(c.link) || !c.link.options.edits) return;
    const { text, blocks } = composeOutbound(c);
    ...
    if (!resp.ok) await ctx.runMutation(internal.slackSync.recordOutboundFailure, { link_id: c.link._id, error: `edit: ${resp.error}` });
  },
});
...
export const pushDelete = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (!resp.ok && resp.error !== "message_not_found") {
      await ctx.runMutation(internal.slackSync.recordOutboundFailure, { link_id: c.link._id, error: `delete: ${resp.error}` });
    }
  },
});
...
export const pushReaction = internalAction({
  ...
  handler: async (ctx, args) => {
    const c = await ctx.runQuery(internal.slackSync.pushContext, { message_id: args.message_id });
    if (!c || !c.message.external) return;
    if (!linkSendsOutbound(c.link) || !c.link.options.reactions) return;
    ...
    if (!resp.ok && resp.error !== "already_reacted" && resp.error !== "no_reaction" && resp.error !== "invalid_name") {
      await ctx.runMutation(internal.slackSync.recordOutboundFailure, { link_id: c.link._id, error: `reaction: ${resp.error}` });
    }
  },
});
