...
// An import that has not moved in this long lost its action (a crash outside
// the per line guard, a deploy mid run). The hourly sweep marks it stopped so
// the dialog can offer to run it again.
const BACKFILL_STALL_MS = 15 * 60_000;
...
export const updateLink = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (restartImport) {
      if (link.backfill?.status === "running") chatFail("INVALID", "The history import is still running");
      const now = Date.now();
      patch.backfill = {
...
export const mapSlackPerson = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(row._id, { codecast_user_id: args.codecast_user_id ?? undefined, mapped_by: "manual" });
    await ctx.scheduler.runAfter(0, internal.slackSync.reattributeSlackPerson, {
      team_id: args.team_id, slack_user_id: args.slack_user_id, link_index: 0,
    });
...
export const sweepSyncEvents = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const link of running) {
      const beat = link.backfill?.heartbeat_at ?? link.backfill?.started_at ?? 0;
      if (link.backfill?.status === "running" && beat < now - BACKFILL_STALL_MS) {
        await ctx.db.patch(link._id, {
