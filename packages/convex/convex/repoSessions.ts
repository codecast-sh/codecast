...
export async function gatherRepoSessions(ctx: { db: any }, viewer: BlameViewer, repository: string): Promise<RepoSessionRow[]> {
  ...
  for (const [userId, roots] of publishers) {
    for (const root of roots) {
      ...
      for (const conv of inRoot) if (repositoryFromRemote(conv.git_remote_url) === repo) consider(conv);
    }
    ...
    const pinned: Doc<"conversations">[] = await ctx.db
      ...
      .order("desc")
      .take(MAX_SESSIONS_PER_CHECKOUT);
    for (const conv of pinned) if (repositoryFromRemote(conv.git_remote_url) === repo) consider(conv);
  }
  ...
  return rows.sort((a, b) => b.updated_at - a.updated_at).slice(0, MAX_ROWS);
}
