...
export async function buildProjectTimeline(
  ...
  // Tasks: creation off the row; transitions and comments from the per-task
  // tables, capped to the most recently touched rows.
  ...
  for (const t of fanout) {
    ...
    for (const h of history) {
      ...
      if (h.new_value === "done" || h.new_value === "dropped") sawTerminal = true;
      push({
        ts: h.created_at,
        type: "task_status",
    ...
    // Older rows predate task_history; synthesize the close so finished work
    // still shows up when and as what it finished.
    if (!sawTerminal && (t.status === "done" || t.status === "dropped") && t.closed_at) {
      push({
        ts: t.closed_at,
        type: "task_status",
        actor_kind: "system",
...
export const post = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const fromAgent = !!args.conversation_id;
    return insertUpdate(ctx, project, {
      user_id: auth.userId,
      author: user?.name || user?.email || "unknown",
