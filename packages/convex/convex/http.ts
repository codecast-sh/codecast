...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    try {
      const body = await request.json();
      const { api_token, version, platform, pid, autostart_enabled, has_tmux, local_project_roots, pending_sync_count, oldest_pending_ms, pending_sync_messages, pending_sync_conversations, device_id, device_label, is_remote_device, cc_accounts, provider_key_pubkey, managed_provider_ids, settings, model_inventory } = body;
