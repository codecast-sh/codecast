...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    try {
      const body = await request.json();
      const { api_token, command_id, boot_id, device_id, release } = body;

      if (!api_token || !command_id || !boot_id) {
        return new Response(JSON.stringify({ error: "Missing required fields" }), {
          status: 400,
          headers: { "Content-Type": "application/json", ...corsHeaders },
        });
      ...
      const res = await ctx.runMutation(api.users.claimDaemonCommand, {
        api_token,
        command_id,
        boot_id,
        // The device scopes the lease: an untargeted command is meant for every
        // machine, so only two daemons on the same device may contend for it.
        device_id: typeof device_id === "string" ? device_id : undefined,
        release: release === true ? true : undefined,
      });
