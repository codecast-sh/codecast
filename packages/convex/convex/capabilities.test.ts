...
describe("reportInventory", () => {
  ...
  test("a machine that lost MORE entries to truncation is a change, not a resend", async () => {
    ...
    // The extra 500 all sort AFTER the kept prefix, so the stored bytes are
    // byte-identical and only the count of what fell off has moved.
...
describe("foldFleet", () => {
  ...
  test("an inventory missing its arrays entirely is an empty column", () => {
    const folded = foldFleet(devices("a", "b"), new Map([
      ["a", [{} as unknown as NormalizedInventory]],
      ["b", [inv([SKILL])]],
    ]));
