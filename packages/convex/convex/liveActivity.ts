...
export const currentState = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      enabled,
      state,
      quiet: isLiveActivityQuiet(state),
...
export async function performLiveActivityRefresh(
  ...
  switch (decision.action) {
    ...
    case "wait": {
      await scheduleLiveActivityRefresh(ctx, userId, { delayMs: decision.at - now });
      return { action: "wait" };
    }
    case "none":
      break;
  ...
  if (running) {
    ...
    await scheduleLiveActivityRefresh(ctx, userId, { delayMs: at - now });
  }
