...
describe("authorizeRoom invite grant", () => {
  ...
  test("a grant from an earlier huddle does not open a later one", async () => {
    // Everyone in the room now joined AFTER the ring was answered: a new
    // huddle in the same room, to which ub was never invited.
    const later = seed({
      call_members: [
        { _id: "cm-ua", room_key: key, team_id: TEAM, user_id: "ua", joined_at: now - 5_000, last_seen: now - 1000 },
      ],
      call_invites: [
        { _id: "inv1", room_key: key, team_id: TEAM, from_user: "ua", to_user: "ub", status: "accepted", created_at: now - 3_600_000, responded_at: now - 3_500_000 },
      ],
    });
    expect((await authorizeRoom(later, "ub" as any, key)).ok).toBe(false);
  });
