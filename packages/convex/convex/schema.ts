...
export default defineSchema({
  ...
  daemon_commands: defineTable({
    ...
    // Exclusive lease. Two daemons can share one device (a launchd instance and
    // a self spawned one, or both sides of an upgrade) and both poll this queue,
    // so without a claim both execute every command. The first writer wins and
    // holds for CLAIM_GRACE_MS (lib/daemonCommandClaim.ts); a claimer that died
    // holding one releases it when the lease lapses.
