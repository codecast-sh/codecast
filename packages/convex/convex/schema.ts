...
export default defineSchema({
  ...
  agent_tasks: defineTable({
    ...
    // Set by `cast trigger run` and the web Run now button, consumed by the
    // daemon's claim: the firing this trigger is armed for was asked for by a
    // person, so the precheck gate does not apply to it. Absent means the
    // schedule armed the firing.
    ...
    // Which source fired the newest run (TriggerFiringSource), stamped at
    // claim. Spawned runs are recorded as conversations and injected ones as
    // messages, and neither carries a source of its own — this is where "that
    // run was manual, which is why the gate did not run" is written down.
  ...
  agent_task_precheck_skips: defineTable({
    ...
    reason: v.string(),
    // The firing the gate refused (TriggerFiringSource). Only "scheduled" and
    // "recurring" can appear: manual and event firings are never gated.
    source: v.string(),
    created_at: v.number(),
