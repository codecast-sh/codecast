...
// A member's level of team visibility and one segment of its time split
// (teamVisibility.ts). Shared by team_memberships and the deleted-team roster.
const teamVisibilityLevelValue = v.union(
  v.literal("hidden"),
  v.literal("activity"),
  v.literal("summary"),
  v.literal("full")
);
const teamVisibilityLevel = v.optional(teamVisibilityLevelValue);
const visibilitySegment = v.object({
  before: v.number(),
  visibility: teamVisibilityLevelValue,
});
...
export default defineSchema({
  ...
  users: defineTable({
    ...
    notification_preferences: v.optional(v.object({
      ...
      // Which agent comments on tasks reach this person's Threads inbox
      // (threadReads.ts taskCommentIsNews). Absent reads as "needs_person":
      // blockers, reviews and @mentions; progress and notes are the board's.
      task_agent_comments: v.optional(v.union(
        v.literal("all"),
  ...
  teams: defineTable({
    ...
    deleted_members: v.optional(v.array(v.object({
      ...
      visibility_history: v.optional(v.array(visibilitySegment)),
    }))),
  })
  ...
  team_memberships: defineTable({
    ...
    joined_at: v.number(),
    visibility: teamVisibilityLevel,
    // "Share in full going forward": sessions STARTED before a segment's
    // `before` are read at that segment's level; everything after the last
    // boundary at `visibility`. Ascending by `before`; `visibility` is always
    // the highest level (teamVisibility.ts nextMembershipVisibility).
    visibility_history: v.optional(v.array(visibilitySegment)),
  })
  ...
  agent_tasks: defineTable({
    ...
    // A run parked at a usage limit keeps its session: the next firing
    // resumes it (`claude --resume`) instead of starting a fresh one, so one
    // firing stays one conversation and the agent continues where it stopped.
    // Set by agentTasks.parkRunAtLimit, consumed by the claim.
    parked_run_session_uuid: v.optional(v.string()),
