...
/** Height of a task's subtree: 0 for a leaf, 1 with children, 2 with grandchildren. Node-capped. */
async function taskSubtreeHeight(ctx: any, taskId: Id<"tasks">, maxNodes = 200): Promise<number> {
  let height = 0;
...
/** Cascade-close the subtree ids guardParentClose returned. */
async function cascadeClose(ctx: any, ids: Id<"tasks">[], newStatus: string, userId: Id<"users">) {
  const now = Date.now();
  for (const id of ids) {
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    // (CLI --subtasks) lifts the rule.
    if (args.ready) {
      const statusById = new Map<string, string>(tasks.map((t: any) => [String(t._id), t.status]));
      tasks = tasks.filter((t: any) => {
        if (t.status !== "open") return false;
        if (t.parent_id && !args.include_subtasks) {
          const parentStatus = statusById.get(String(t.parent_id));
          if (parentStatus === "in_progress" || parentStatus === "in_review") return false;
        }
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.plan_id) {
      ...
      const willBeSubtask = updates.parent_id !== undefined ? !!updates.parent_id : !!task.parent_id;
      const taskIds = plan.task_ids || [];
    ...
    if (cascadeIds.length > 0) await cascadeClose(ctx, cascadeIds, args.status!, auth.userId);
    await rollUpParentStart(ctx, task, args.status);
...
export const webList = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.ready) {
      ...
      // parent are that session's decomposition, not up-for-grabs work.
      const statusById = new Map<string, string>(tasks.map((t: any) => [String(t._id), t.status]));
...
export const webUpdate = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(task._id, updates);
    if (cascadeIds.length > 0) await cascadeClose(ctx, cascadeIds, args.status!, userId);
    await rollUpParentStart(ctx, task, args.status);
