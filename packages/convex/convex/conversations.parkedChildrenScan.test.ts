...
describe("parked rows skip the per-row children scan", () => {
  ...
  test("per-row reference reads are shared across rows: one get per distinct id", async () => {
    const db = makeFakeDb({
      users: [
        { _id: ME, name: "Me", email: "me@example.com" },
        { _id: "users_bot", name: "Anchor", email: "bot@example.com" },
      ],
      plans: [{ _id: "plans_p", short_id: "pl-1", title: "The plan", status: "active" }],
      conversations: [1, 2, 3].map((i) => ({
        _id: `conversations_${i}`, user_id: ME, status: "active", updated_at: NOW - i, started_at: NOW - H, message_count: 0,
        active_plan_id: "plans_p", acting_user_id: "users_bot",
      })),
      session_owners: [],
      managed_sessions: [],
      messages: [],
    ...
    db.get = (id: any) => { gets.push(String(id)); return origGet(id); };
    const { sessions } = await computeInboxSessions({ db }, ME as any, { show_all: false });
    expect(sessions.length).toBe(3);
    for (const s of sessions) {
      expect(s.active_plan?.short_id).toBe("pl-1");
      expect(s.author_name).toBe("Anchor");
    }
    expect(gets.filter((id) => id === "plans_p").length).toBe(1);
    expect(gets.filter((id) => id === "users_bot").length).toBe(1);
  });
