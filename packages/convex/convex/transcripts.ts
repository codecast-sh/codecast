...
export const mintAsrToken = action({
  ...
  > => {
    ...
    const mint = async (session: ReturnType<typeof asrTranscriptionSession>) => {
      ...
      if (!resp.ok) {
        ...
        return { error: `ASR session mint failed (${resp.status})` as const };
      }
      const data = (await resp.json()) as { value?: string; client_secret?: { value?: string } };
      const secret = data.value ?? data.client_secret?.value;
      if (!secret) return { error: "ASR session mint returned no secret" as const };
