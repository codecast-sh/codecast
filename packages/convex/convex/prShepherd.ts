...
export type WakePromptInput = {
  ...
  reason: string;
  detail?: string;
  /** Reasons that also piled up since the last delivered wake, if any. */
...
/**
 * Which reason leads the prompt when several piled up.
 *
 * Events arrive in the order GitHub happens to send them, and the last one is
 * rarely the one that matters most: a wake that says "the checks went green"
 * while the branch is in conflict points the shepherd at the wrong problem.
 * Most urgent first, and everything the agent can still act on ranks above
 * news it can only acknowledge.
 */
const WAKE_SEVERITY = [
  "conflict",
  "check_failed",
...
export function pickWakeReason(reasons: string[]): { headline: string; others: string[] } {
  ...
  if (seen.length === 0) return { headline: "bound", others: [] };

  const terminal = [...seen].reverse().find((r) => TERMINAL_REASONS.has(r));
  const headline =
    terminal ??
    [...seen].sort((a, b) => {
      const ai = WAKE_SEVERITY.indexOf(a);
      const bi = WAKE_SEVERITY.indexOf(b);
      // A reason with no ranking is ordinary news; it never outranks a ranked one.
      return (ai < 0 ? WAKE_SEVERITY.length : ai) - (bi < 0 ? WAKE_SEVERITY.length : bi);
    })[0];

  const others: string[] = [];
  for (const r of seen) if (r !== headline && !others.includes(r)) others.push(r);
...
export function buildWakePrompt(input: WakePromptInput): string {
  ...
  const others = input.other_reasons ?? [];
  if (others.length) {
    lines.push(`Also since the last wake: ${others.map((r) => REASON_TEXT[r] ?? r).join("; ")}.`);
  }
  ...
  if (failing.length) {
    ...
    for (const check of failing.slice(0, 12)) {
      lines.push(`- ${checkLabel(check)}: ${check.conclusion}${check.url ? ` — ${check.url}` : ""}`);
    }
  }
...
/**
 * The review threads still waiting on the shepherd.
 *
 * A review comment is not a note, it is one turn in a conversation, so asking
 * "is this comment resolved" is the wrong question. It reads every reply the
 * session already sent as another thing to answer, because the session replies
 * through gh as the pull request author and that sets no flag anywhere.
 *
 * A thread is outstanding when nobody has resolved it AND the last word in it
 * is not the author's. The comment shown is the one that OPENED the thread,
 * because that is the ask; the reply that answered it is not news.
 */
function outstandingThreads(
  comments: any[],
  prAuthor: string | undefined,
): Array<{ author: string; file_path?: string; line_number?: number; content: string; url?: string }> {
  const threads = new Map<string, any[]>();
  for (const c of comments) {
    // in_reply_to_id names the comment that opened the thread; a comment that
    // opened one is its own root. Rows from before threads were recorded have
    // neither and stand alone, which is how they behaved before.
    const root = String(c.github_in_reply_to_id ?? c.github_comment_id ?? c._id);
    const bucket = threads.get(root);
    if (bucket) bucket.push(c);
    else threads.set(root, [c]);
  }

  // A reply we mirrored to GitHub is the author speaking, whatever GitHub calls
  // the account it went out under.
  const isAuthor = (c: any) =>
    !!c.codecast_origin || (!!prAuthor && c.author_github_username === prAuthor);

  const out: any[] = [];
  for (const bucket of threads.values()) {
    const ordered = [...bucket].sort((a, b) => a.created_at - b.created_at);
    if (ordered.some((c) => c.resolved || c.resolved_at)) continue;
    if (isAuthor(ordered[ordered.length - 1])) continue;

    // The ask is the first thing in the thread that the author did not write.
    const ask = ordered.find((c) => !isAuthor(c)) ?? ordered[0];
    out.push({
      author: ask.author_github_username ?? "someone",
      file_path: ask.file_path,
      line_number: ask.line_number,
      content: ask.content,
      url: ask.html_url,
...
async function gatherWakeContext(ctx: Ctx, pr: PR) {
  ...
  return {
    comments: outstandingThreads(comments, pr.author_github_username),
    reviews: reviews
...
export async function wakeShepherd(
  ...
  const { headline, others } = pickWakeReason(pending);

  // Re-read immediately before building. Everything above may have written to
  // the row, and a prompt that describes a head commit the branch has already
  // moved past sends the shepherd after the wrong commit.
  const fresh = ((await ctx.db.get(prId)) as PR | null) ?? pr;
  const context = await gatherWakeContext(ctx, fresh);
  const prompt = buildWakePrompt({
    pr: fresh,
    ...context,
    reason: headline,
    // The detail belongs to the reason that was passed in. It only describes
    // the headline when they are the same event.
    detail: headline === reason ? detail : undefined,
  ...
  await ctx.db.patch(prId, {
    ...
    shepherd_last_wake_reason: headline,
    shepherd_wake_count: (pr.shepherd_wake_count ?? 0) + 1,
    // Handed over, so the slate is clean for whatever arrives next.
    shepherd_pending_reasons: [],
  });
...
export const applyMergeState = internalMutation({
  ...
  handler: async (ctx, args): Promise<{ retry: boolean }> => {
    ...
    if (result.stateChanged) {
      ...
      // The prompt already spells out what each state means, so a detail that
      // repeats it reads as a stutter ("...merges cleanly: ...merges cleanly").
      // Say something the sentence does not already carry, or say nothing.
      const detail =
        state === "conflicts"
          ? result.pr.base_ref
            ? `against ${result.pr.base_ref}`
            : undefined
          : state === "behind"
            ? `${result.pr.behind_by ?? 0} commit${result.pr.behind_by === 1 ? "" : "s"} behind ${result.pr.base_ref ?? "the base branch"}`
            : undefined;
