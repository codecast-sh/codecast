...
// request" rather than a redacted row. And a session binding is own-only: you
// may point your own agent at a pull request, never somebody else's.
import { describe, expect, test } from "bun:test";
import { makeFakeDb } from "./testDb";
import { hashToken } from "./apiTokens";
import { ls, show, shepherd, resolve, watchPRs, foldShepherdState, countChecks } from "./prCli";
...
describe("the shepherd binding", () => {
  test("on binds the session, off releases it", async () => {
    ...
    expect(testCtx.db._patched.at(-1)).toMatchObject({ _id: "conv_mine", patch: { pr_status: undefined } });
  });
...
describe("the folded shepherd state", () => {
  const base = { state: "open" } as any;

  test("the worst thing that is true wins", () => {
    expect(foldShepherdState({ ...base, state: "merged" })).toBe("merged");
    expect(foldShepherdState({ ...base, mergeable_state: "dirty" })).toBe("conflicts");
    expect(foldShepherdState({ ...base, behind_by: 3 })).toBe("behind");
    expect(foldShepherdState({ ...base, checks_state: "failure" })).toBe("ci_red");
    expect(foldShepherdState({ ...base, checks_state: "pending" })).toBe("ci_pending");
    expect(foldShepherdState({ ...base, review_decision: "changes_requested" })).toBe("changes_requested");
    expect(foldShepherdState({ ...base, unresolved_review_count: 2 })).toBe("changes_requested");
    expect(foldShepherdState({ ...base, review_decision: "approved" })).toBe("approved");
    expect(foldShepherdState(base)).toBe("ready");
  });

  test("a conflict outranks a red build", () => {
    expect(foldShepherdState({ ...base, mergeable_state: "dirty", checks_state: "failure" })).toBe("conflicts");
  });
});
...
describe("counting checks", () => {
  test("a check that has not concluded is still running", () => {
    ...
    } as any)).toEqual({ green: 1, red: 1, pending: 1 });
  });

  test("no checks at all is not a failure", () => {
    expect(countChecks({ checks: undefined } as any)).toEqual({ green: 0, red: 0, pending: 0 });
  });
});
