...
// chapters | play-button). Events: "chapterchange" ({ index, title }) and
...
export const CAST_PLAYER_JS = String.raw`(function () {
  ...
  var CSS = [
    ...
    ".shade{position:absolute;left:0;right:0;bottom:0;height:46%;background:linear-gradient(to top,rgba(0,0,0,.66),rgba(0,0,0,.22) 55%,rgba(0,0,0,0));pointer-events:none;transition:opacity .4s ease}",
  ...
  P._probe = function () {
    var self = this, k = 0;
    var next = function () {
      while (k < self.ch.length && self.ch[k].duration) k++;
      if (k >= self.ch.length) {
        self._layout();
      ...
      var done = function () {
        if (isFinite(probe.duration)) self.ch[k].duration = probe.duration;
        probe.removeAttribute("src"); probe.load();
        k++; self._layout(); self._sync(); next();
      };
      probe.addEventListener("loadedmetadata", done, { once: true });
      probe.addEventListener("error", function () { k++; next(); }, { once: true });
      probe.src = self.ch[k].src;
    };
    next();
  };
