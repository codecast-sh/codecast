// The one decision point for "does this chat write go to Slack?". chat.ts calls
// it after a message lands, an edit or delete is applied, or a reaction toggles;
// it reads the channel's link and the link's controls and schedules the push
// action or does nothing. It lives apart from slackSync.ts so chat.ts can import
// it without a cycle (slackSync imports chat's insert helpers).
...
export function linkSendsOutbound(link: SlackLink): boolean {
  return !link.paused && (link.direction === "both" || link.direction === "codecast_to_slack");
}

export function linkReceivesInbound(link: SlackLink): boolean {
