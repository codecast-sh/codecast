...
// The conservative read: what a keyword scan of the question and the options
// finds. Returns a protected category or `limit` when a phrase pins it, else
// null (the text does not force a category).
export function pinnedCategory(input: ClassifiableDecision): DecisionCategory | null {
  const parts = [input.question, ...input.options.flatMap((o) => [o.label, o.description ?? "", o.body_md ?? "", o.cost ?? "", o.risk ?? ""])];
