 ...
 */
export async function claimRefreshOn(db: any, id: any, expectedEnc: string, now: number): Promise<ClaimResult> {
  const row = await db.get(id);
  if (!row) return { ok: false, reason: "gone" };
  if (stampOf(row) !== expectedEnc) return { ok: false, reason: "superseded" };
 ...
 */
export async function writeRefreshOutcomeOn(db: any, id: any, args: OutcomeArgs): Promise<WriteResult> {
  const row = await db.get(id);
  if (!row) return { ok: false, reason: "gone" };
  if (row.refresh_lease_id !== args.lease || stampOf(row) !== args.expected_enc) return { ok: false, reason: "superseded" };
