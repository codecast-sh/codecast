...
export async function visibleInTeamList(
  ...
  const uid = String(userId);
  if (String(row.user_id) === uid) return true;
  if (table === "tasks" && row.assignee && String(row.assignee) === uid) return true;
  if (typeof row.workspace === "string" && row.workspace) return row.workspace === `team:${String(teamId)}`;
