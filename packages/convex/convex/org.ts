 ...
 *  ended. One reader, so the wake frame and org.health count the same wait. */
export function waitingSinceOf(state: string, raw: any): number | null {
  if (state !== "needs_input") return null;
  const pin = stateOf(raw);
  return (pin.state_status === "blocked" && pin.state_at) || raw.updated_at || null;
}
