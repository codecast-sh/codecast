...
export async function addToStackCore(ctx: Ctx, userId: Id<"users">, stackRef: string, decisionRef: string) {
  ...
  await ctx.db.patch(row._id, { stack_id: stack._id, scope_keys, updated_at: now });
...
export const removeFromStack = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if ("error" in found) return found;
    const row = await findDecision(ctx, args.decision);
    if (!row) return { error: `Decision not found: ${args.decision}` };
    const now = Date.now();
    ...
    await ctx.db.patch(row._id, { stack_id: undefined, scope_keys, updated_at: now });
...
export async function applyAutoDefaultsCore(ctx: Ctx, now: number): Promise<{ answered: number }> {
  ...
  for (const stack of stacks) {
    ...
    for (const m of await membersOf(ctx, stack)) {
      ...
      if (m.created_at + ms > now) continue;
