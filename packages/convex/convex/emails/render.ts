// Transactional email renderer. Pure module — no Convex imports — so templates
// render identically in the backend, in tests, and in the preview harness.
//
...
// - The wordmark is text (coral play glyph + "codecast"), never an image:
//   it survives image blocking and flips correctly in dark mode.

// ---------------------------------------------------------------------------
// Brand
// ---------------------------------------------------------------------------

/** Solarized-warm light palette, mirrored from packages/web globals.css. */
export const palette = {
  // Light (default) theme
  bodyBg: "#eee8d5", // solarized base2 — page ground behind the card
  cardBg: "#fdf6e3", // solarized base3 — warm card
  border: "#d9d2bc",
  text: "#002b36",
  textMuted: "#586e75",
  textDim: "#839496",
  insetBg: "#e7e0ca", // code box fill
  insetBorder: "#c9c2a9",
  accent: "#e86c5d", // coral — the logo's play arrow
  accentDark: "#c2543f", // coral darkened for 4.5:1 text contrast on cream
  buttonText: "#ffffff",
  // The terminal block is dark in BOTH themes — a terminal is dark. This also
  // makes it the one element that needs no dark-mode override.
  termBg: "#002b36",
  termText: "#eee8d5",
  termPrompt: "#859900", // solarized green prompt
  termMuted: "#586e75",
  ...
  dark: {
    ...
    insetBorder: "#0e4a5a",
    accent: "#e86c5d",
    accentLight: "#f08b7e", // links need to lighten, not darken, on dark
  },
} as const;

/** JetBrains Mono is the product's face; the stack degrades to system monos. */
...
export interface EmailDef {
  ...
  preheader: string;
  /** Small coral overline above the heading, e.g. "SECURITY". */
  eyebrow?: string;
  heading: string;
  blocks: EmailBlock[];
...
export function renderEmail(def: EmailDef): RenderedEmail {
          ...
          <tr>
            <td style="padding:0 6px 14px 6px;">
              <a href="${BRAND.url}" target="_blank" style="text-decoration:none;">
                <span style="font-family:${monoStack};font-size:17px;font-weight:700;color:${palette.accent};">&#9656;</span>
                <span class="em-wordmark" style="font-family:${monoStack};font-size:17px;font-weight:700;letter-spacing:0.5px;color:${palette.text};">codecast</span>
              </a>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:22px 6px 0 6px;">
              ...
              <p class="em-dim" style="margin:0;font-family:${monoStack};font-size:11px;line-height:1.7;color:${palette.textDim};">
                <a href="${BRAND.url}" class="em-dim" style="color:${palette.textDim};text-decoration:underline;">codecast.sh</a>
                &nbsp;&middot;&nbsp; ${escapeHtml(BRAND.tagline.toLowerCase())}
                &nbsp;&middot;&nbsp; &copy; ${year} ${BRAND.name}
              </p>
            </td>
          </tr>
