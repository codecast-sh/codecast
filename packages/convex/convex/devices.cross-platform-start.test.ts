...
test("a Codex start and its ownership queue for the offline checkout holder instead of an incompatible live runtime", async () => {
  ...
  const db = makeFakeDb({
    users: [{ _id: user }],
    conversations: [{ _id: conversation, user_id: user, agent_type: "codex", project_path: path, owner_device_id: "runtime" }],
    devices: [
  ...
  await enqueueStartSession({ db }, user, { conversationId: conversation, agentType: "codex", targetDeviceId: "runtime" });
