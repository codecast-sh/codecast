...
export const submitComments = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // without the page. Two principals are past the wall rather than in front
    // of it: the owner_key holder, and a signed-in account that can already
    // open this page in the app. An identity token alone is not one of them —
