...
describe("org.health", () => {
  test("an overloaded role and an idle one, each with its signals and flags", async () => {
    ...
    expect(growth.spend.wakes_7d_avg).toBeCloseTo(5 / 7);
    expect(growth.flow).toMatchObject({
      decisions_7d: 3, median_recommend_min: 12, escalations_7d: 0, frames_dropped_7d: 1, done_7d: 2,
    ...
    expect(codes(r.company.flags)).toEqual(["no_charter", "no_charter", "no_charter", "unowned", "unowned"]);
    expect(r.truncated).toEqual({ sessions: false, tasks: false, plans: false, projects: false });
    expect(r.generated_at).toBe(NOW);
  });

  test("an owner role on a project counts as ownership; a whole workspace role covers everything", async () => {
    ...
    const whole = await computeOrgHealth(ctxOf(fixtures({
      org_roles: [{ _id: "org_roles_ceo", short_id: "or-9", scope_type: "team", team_id: TEAM, host_user_id: ME, name: "Chief of Staff", handle: "chief-of-staff", scope: { project_ids: [], plan_ids: [] }, reports_to: { kind: "user", user_id: ME }, status: "active", charter: "x", created_by: ME, created_at: NOW - 30 * D, updated_at: 1 }],
    })), ME as any, TEAM, NOW);
    expect(whole.company.unowned_projects).toEqual([]);
    const cos = whole.roles[0];
    // The whole workspace: every task and plan; its idle clock is the newest event anywhere.
    expect(cos.load).toMatchObject({ open_tasks: 28, in_flight: 9, active_plans: 1 });
    expect(cos.idle_days).toBe(0);
    expect(codes(cos.flags)).toEqual(["overloaded", "review_stall"]);
  });
  ...
  test("roleActivity reads the wake log the way the analyzer inputs do", async () => {
    ...
    const a = await roleActivity(ctxOf(db), ME as any, growth, NOW);
    expect(a.wakes_7d).toMatchObject({ total: 5, delivered: 4, dropped: 1, held: 1, days_at_cap: 1, cap_hit_days: 2 });
