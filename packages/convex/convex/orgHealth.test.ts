...
function fixtures(extra: Record<string, any[]> = {}) {
  ...
  return makeFakeDb({
    ...
    tasks: [
      ...
      // Orphan has work of its own, so it is unowned; an empty project would not be.
      task("ct-orphan", { project_id: R }),
...
describe("org.health", () => {
  ...
  test("an owner role on a project counts as ownership; a whole workspace role holds the remainder and owns nothing", async () => {
    ...
    expect(cos.flow.items_changed_7d).toBe(6);
    expect(cos.counted).toMatchObject({ rule: "remainder", projects: 1, plans: 2, tasks: 4, complete: true });
...
describe("org.health company flags read work, not rows", () => {
  test("a paused project and an empty active project are neither unowned nor uncharted; an abandoned plan is not unfiled work", async () => {
    const db = fixtures({
      projects: [
        { _id: P, user_id: ME, team_id: TEAM, workspace: WS, short_id: "pr-1", title: "Growth", goal: "Bring users in", status: "active", project_path: "/repo/growth", created_at: 1, updated_at: NOW - 10 * H },
        { _id: Q, user_id: ME, team_id: TEAM, workspace: WS, short_id: "pr-2", title: "Billing", status: "active", project_path: "/repo/billing", created_at: 1, updated_at: NOW - 30 * D },
        { _id: R, user_id: ME, team_id: TEAM, workspace: WS, short_id: "pr-3", title: "Orphan", status: "active", created_at: 1, updated_at: NOW - 10 * H },
        { _id: "projects_paused", user_id: ME, team_id: TEAM, workspace: WS, short_id: "pr-4", title: "Parked", status: "paused", created_at: 1, updated_at: NOW },
        { _id: "projects_import", user_id: ME, team_id: TEAM, workspace: WS, short_id: "pr-5", title: "Linear import", status: "active", created_at: 1, updated_at: NOW },
      ],
      plans: [
        { _id: "plans_orphan", user_id: ME, team_id: TEAM, workspace: WS, project_id: R, short_id: "pl-7", title: "Orphan work", status: "active", goal: "g", created_at: 1, updated_at: NOW },
        { _id: "plans_abandoned", user_id: ME, team_id: TEAM, workspace: WS, short_id: "pl-8", title: "Gave up", status: "abandoned", goal: "g", created_at: 1, updated_at: NOW },
      ],
      tasks: [task("ct-gave-up", { project_id: undefined, plan_id: "plans_abandoned" })],
    });
    const r = await computeOrgHealth(ctxOf(db), ME as any, TEAM, NOW);
    // Orphan has an open plan and no owner: unowned. The empty import and the paused line are not.
    expect(r.company.unowned_projects).toEqual([{ id: R, title: "Orphan" }]);
