...
export async function applyHandoffLink(
  ...
  if (source.active_task_id) {
    ...
    if (task) {
      const ids = task.conversation_ids || [];
      if (!ids.some((id: any) => String(id) === String(child._id))) {
        await ctx.db.patch(task._id, { conversation_ids: [...ids, child._id] });
      }
      await linkConversationToEntityBestEffort(ctx, userId, {
        entityType: "task",
        entityId: String(task._id),
        conversationId: child._id,
        relationship: "work",
      });
    }
  }
  if (source.active_plan_id) {
    ...
    if (plan) {
      ...
      if (!ids.some((id: any) => String(id) === String(child._id))) {
        await ctx.db.patch(plan._id, { session_ids: [...ids, child._id], current_session_id: child._id, updated_at: Date.now() });
      } else {
        await ctx.db.patch(plan._id, { current_session_id: child._id, updated_at: Date.now() });
      }
      await linkConversationToEntityBestEffort(ctx, userId, {
        entityType: "plan",
