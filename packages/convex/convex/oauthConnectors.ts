...
export const claimRefresh = internalMutation({
  ...
  handler: async (ctx, args) => await claimRefreshOn(ctx.db, args.installation_id, args.expected_enc, args.now),
});
...
export const updateStoredTokens = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    return await writeRefreshOutcomeOn(ctx.db, installation_id, outcome);
  },
});
