...
/**
 * Persist the outcome of one refresh.
 *
 * A success is governed by the access ciphertext the claimant read: every
 * refresh and every reconnect writes a fresh ciphertext (random IV), so a
 * mismatch means newer credentials landed while the claimant awaited the
 * provider, and the older pair is discarded. A success that does pass is
 * the newest pair whoever holds the lease, so it also releases the lease.
 *
 * A failure is governed by the lease id: only the current claimant may park
 * the connection. A late failure from an expired claim, or one that arrives
 * after a reconnect (which clears the lease), is refused and cannot stamp
 * last_error over credentials that work. A deleted row is reported, never
 * recreated.
 */
export const updateStoredTokens = internalMutation({
  ...
  handler: async (ctx, args): Promise<{ ok: boolean; reason?: "gone" | "superseded" }> => {
    ...
    if (row.refresh_lease_id !== args.lease) return { ok: false, reason: "superseded" };
 ...
 * compare and swap (updateStoredTokens); when a write is refused the caller
 * re-reads and returns what the row holds now, never the pair it fetched:
 * a disconnected connection yields no credentials, a superseded refresh
 * yields the newer ones.
 */
export const getFreshAccessTokenForTeam = internalAction({
  ...
  handler: async (ctx, args): Promise<{ ok: boolean; token?: string; error?: string }> => {
    ...
    if (!refreshToken) {
      ...
      await ctx.runMutation(internal.oauthConnectors.updateStoredTokens, { installation_id: row._id, expected_enc: row.access_token_enc, lease });
      return await tokenOf(row);
    }
    const fail = async (error: string) => {
      ...
      if (stamped.ok) return { ok: false, error };
      if (stamped.reason === "gone") return noConnection;
      // Not ours to park any more: a reconnect or a newer claimant owns the
      // row. Their credentials are the answer, so wait for them.
      const landed = await waitForOther(row!.access_token_enc);
    ...
    if (written.ok) return { ok: true, token: tok.access_token };
    // Refused: the row moved on while we talked to the provider. What it
    // holds now is the truth; the pair we fetched is never handed out.
    return written.reason === "gone" ? noConnection : await current();
  },
});
