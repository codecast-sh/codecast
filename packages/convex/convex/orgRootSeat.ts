...
export type RootSeatRow = {
  ...
  /** The run was told to seat other workspaces; this one waits. */
  held?: boolean;
};
...
// `only` names the anchor rows to seat (the ids the plan prints); the rest
// of the plan is reported with `held` and left as it is, so a person can
// seat their own workspaces and leave other people's for them to hire.
export async function performSeatRootRoles(ctx: any, dryRun: boolean, only?: ReadonlySet<string>): Promise<{ dry_run: boolean; rows: RootSeatRow[]; seated: number }> {
  const rows = await planRootSeats(ctx);
  let seated = 0;
  for (const row of rows) {
    if (only && ROOT_SEAT_ACTIONS_THAT_SEAT.has(row.action) && !only.has(String(row.anchor_id))) row.held = true;
  }
  if (!dryRun) {
    for (const row of rows) {
      if (!ROOT_SEAT_ACTIONS_THAT_SEAT.has(row.action) || row.held) continue;
...
// Run once: `npx convex run orgRootSeat:seatRootRoles '{"dry_run":true}'`, then
// without. `anchor_ids` seats only those rows (from the plan's `anchor_id`).
export const seatRootRoles = internalMutation({
  args: { dry_run: v.optional(v.boolean()), anchor_ids: v.optional(v.array(v.string())) },
  handler: async (ctx, args) => performSeatRootRoles(ctx, !!args.dry_run, args.anchor_ids ? new Set(args.anchor_ids) : undefined),
});
