...
describe("getUserTasks visibility", () => {
  ...
  test("a teammate sees team-tagged tasks but never the owner's personal task", async () => {
    // canAccessTask gates on the RAW team_id (unlike canAccessDoc, which uses
    // effectiveTeamForResource), so a team-tagged task linked to a private
    // conversation is still team-visible here. The important guarantee this
    // encodes: a teammate never sees the owner's PERSONAL (teamless) task.
    const ids = (await tasks(MEMBER)).map((t: any) => t._id).sort();
    expect(ids).toEqual(["task_private", "task_team"]);
    expect(ids).not.toContain("task_personal");
  });
