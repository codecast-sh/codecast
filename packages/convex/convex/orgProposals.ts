...
// it, and the proposal (and its queue card) waits for that.
const DECIDABLE = new Set(["proposed", "failed"]);
async function resolveIfDone(ctx: Ctx, proposal: ProposalRow, now: number): Promise<boolean> {
  const open = (await changesOf(ctx, proposal._id)).some((c) => DECIDABLE.has(c.status) || c.status === "accepted");
...
export async function performDecideChange(
  ...
  if (!DECIDABLE.has(change.status)) throw new Error(`${proposal.short_id}#${change.seq} is already ${change.status}`);
  const now = Date.now();
  let out: any;
...
export async function performAcceptAll(ctx: Ctx, userId: Id<"users">, args: { proposal: string; from_session?: string; api_token?: string; provision?: boolean }): Promise<any> {
  await refuseUnlessHumanDecider(ctx, args);
  ...
  if (proposal.status !== "open") throw new Error(`${proposal.short_id} is ${proposal.status}`);
  const now = Date.now();
  const pending = orderForApply((await changesOf(ctx, proposal._id)).filter((c) => DECIDABLE.has(c.status)));
  const results = [];
  for (const change of pending) results.push(await acceptOne(ctx, userId, proposal, change, undefined, now, args.provision ?? true));
  await ctx.db.patch(proposal._id, { updated_at: now });
  const resolved = await resolveIfDone(ctx, proposal, now);
