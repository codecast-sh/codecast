...
describe("performListOwners", () => {
  ...
  test("an unowned session a person started lists them as the owner", async () => {
    const db = fixtures();
    const result = await performListOwners({ db }, ASHOT as any, "jx3abcd");
    expect(result?.owners.map((o) => o.user_id)).toEqual([ASHOT]);
    expect(result?.owners[0]?.added_by).toBe(ASHOT);
  });

  test("an unowned bot-run session lists no owner", async () => {
    const db = fixtures();
    const result = await performListOwners({ db }, ASHOT as any, "jx1abcd");
