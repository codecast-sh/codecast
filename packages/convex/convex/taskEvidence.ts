...
// The sessions whose work the task is: every conversation the task links.
// Linked sessions stay linked after the task closes (active_task_id is
// cleared then), so evidence survives the move to done.
const linkedConversationIds = (task: any): string[] => (task.conversation_ids ?? []).map(String);
...
export async function computeTaskEvidence(ctx: Ctx, viewerId: Id<"users">, task: any): Promise<TaskEvidence> {
  ...
  for (const cid of linkedConversationIds(task)) {
