...
import { AGENT_MODEL_CONFIG, AGENT_CLIENTS, modelAgentKey, fromConvexAgentType, toConvexAgentType, normalizeThreadState, parseThreadStateStatus, formatAgentSwitchNotice, findModelOption, canSessionBecomeAgent, agentForksFromAnyMessage, agentForksNatively } from "@codecast/shared/contracts";
...
export const forkFromMessage = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // state — so it launches immediately the same way; the daemon skips the
    // deferred resume for a fork that already exists locally.
