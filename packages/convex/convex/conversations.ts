...
export async function scanInboxConversations(
  ...
  // Assignment moves the session into the owners' inboxes. The recent /
  // pin / stash windows are keyed on the runner (`user_id`), so without this
  // drop a session handed to a teammate stays in the host's QUESTIONS and
  // Needs Input while also appearing in the assignee's owned window.
