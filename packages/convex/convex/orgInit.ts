...
async function longRunningSessions(ctx: Ctx, scan: OrgScanResult, now: number, work: AnalysisHandoff) {
  ...
  for (const { session, raw } of old.slice(0, ANALYSIS_CAPS.long_running)) {
    ...
    if (raw.project_path && projectByPath.has(raw.project_path)) touched.add(projectByPath.get(raw.project_path)!.id);
    if (task?.project_id) touched.add(String(task.project_id));
    for (const planId of new Set([raw.active_plan_id, task?.plan_id, ...(raw.plan_ids ?? [])].filter(Boolean).map(String))) { const plan = await ctx.db.get(planId as Id<"plans">); if (plan?.project_id) touched.add(String(plan.project_id)); }
    ...
    rows.push({
      ...
      projects: Array.from(touched).map((id) => projectById.get(id)).filter(Boolean).map((p) => ({ short_id: p!.short_id, title: p!.title })),
