...
try {
  ...
  // The recipes read the roster: both pages have to hold it first.
  const roster = `(async () => { for (let i = 0; i < 600; i++) { const st = window.__inboxStore?.getState(); if (st?.currentUser?._id && (st.teamMembers ?? []).length > 1) return st.teamMembers.length; await new Promise((r) => setTimeout(r, 100)); } throw new Error("no roster"); })()`;
  ...
  for (const name of names) {
    ...
    try {
      row.push(await float.screenshot(`${DIR}/state-${name}-float.png`, '.face-row[data-density="float"], .face-row[data-density="float"] .face-row-below'));
    } catch (e) {
