...
import { MESSAGE_PAGE_SIZE, useInboxStore } from "../inboxStore";
...
// The web cache coalesces writes; read the row once the newest write landed.
async function settled(firstId: string) {
  const deadline = Date.now() + 10_000;
  for (;;) {
    const cached = await loadConversationMessages(CONV);
    if (cached?.messages[0]?._id === firstId) return cached;
