  ...
  private async commandHasDurableCoverage(
    ...
    switch (requirement.kind) {
      ...
      case "command-id": {
        ...
        const view = await this.db.views.get(requirement.viewKey);
        if (!view || view.contractId !== requirement.contractId) return false;
        // Proof #3 (design §11.4): the owning source carries this command's
        // id. A log-ts view echoes the caller's recently-receipted command
        // ids from the SAME query snapshot, so inclusion proves the result
        // contains the write.
        if (view.coverage.kind === "log-ts") {
          return view.coverage.commandIds?.includes(command.id) ?? false;
        }
        return view.coverage.kind === "command-ids" &&
