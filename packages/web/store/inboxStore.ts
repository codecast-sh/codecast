...
function applyGestureInDraft(draft: any, msg: GestureMessage) {
  ...
  const forget = (ids: string[], ts: number, scope: "session-row" | "all" = "all") => {
    for (const fid of ids) {
      // Mirror ALL of pruneGhostSessions' delete guards, not just the first.
...
export const useInboxStore = create<InboxStoreState>(
  ...
  killSessions: action(function (this: Draft, ids: string[]) {
    if (ids.length === 0) return;
    soundKill();
    // "Kill all" is ONE user gesture, so its per-row cascades are merged into a
    // single bridge broadcast (the sound plays once for the same reason).
    const merged = { hidden: [] as string[], forgotten: [] as string[], ts: 0 };
    for (const id of ids) {
      const r = hideSessionInDraft(this, id, "kill");
      merged.hidden.push(...r.hidden);
      merged.forgotten.push(...r.forgotten);
      merged.ts = Math.max(merged.ts, r.ts);
    }
