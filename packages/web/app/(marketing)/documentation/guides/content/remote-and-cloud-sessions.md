...
The operating system is detected automatically. AWS profiles are saved with the host and reused for wake and sleep. Mac setup needs Homebrew and passwordless sudo on the SSH account. On a shared Mac, add `--service-user codecast` to create a separate login with the bootstrap account's authorized SSH keys and passwordless sudo. Existing accounts and their agents keep running. Setup refuses to replace linked agent configuration directories.
