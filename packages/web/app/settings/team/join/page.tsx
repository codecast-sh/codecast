...
export default function JoinTeamPage() {
  ...
  const { selectedPaths, toggle, selectedList, includePast, setIncludePast } = useWorkspaceSelection(data, teamId);
  ...
  const finishSetup = (paths: Record<string, boolean>) => {
    ...
    if (id) {
      save({ teamId: id, visibility, selectedPaths: paths, allProjects: data.allProjects, includePast })
        .then(({ mapped }) => {
  ...
  return (
    <TeamFlowShell
      ...
      continueLabel={shareActionLabel(impact, includePast) ?? "Open team"}
      enterAdvances
    >
