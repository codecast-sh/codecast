...
export default function SyncPage() {
  ...
  const menuCurrent = (row: ShareRow): ShareMenuCurrent => {
    ...
    if (lock) return { locked: true, inherited: lock.inherited };
  ...
  const handleAddProject = async () => {
    ...
    if (!projectPath.startsWith("/") && !projectPath.startsWith("~")) {
      toast.error("Type an absolute path", { description: "Like /Users/you/src/app or ~/notes." });
      return;
    }
    setNewProject("");
  ...
  return (
    <SettingsPanel>
      ...
      <SettingsSection
        ...
        description={
          hasTeams
            ? "Every repository and folder is private to you until you share it with a team, and anything not listed here is private too. A share covers every checkout and worktree of the repository. Each team's level sets how much teammates see of what you share with it. Lock a folder to keep every rule off it."
        ...
        {editMode && (
          <div className="flex items-center gap-2 px-4 py-2.5 sm:px-5">
            <Input
              ...
              placeholder="/path/to/folder — sessions or none, its rule applies from the first one"
        ...
        {sharingGroups.length > 0 ? (
          ...
          <div className="px-4 py-8 text-center text-sm text-sol-text-muted">
            {searchQuery ? (
              ...
              <p>Nothing listed yet, and that means every session is private to you.</p>
            )}
            {!searchQuery && (
              <p className="mt-1 text-xs text-sol-text-dim">
                Folders appear here as sessions sync. To set a rule before the first one, add the path above.
              </p>
            )}
          </div>
        )}
