...
export function TaskListContent({ projectId, scope }: { projectId?: string; scope?: TaskListScope } = {}) {
  ...
  // filter). `status` holds it: "" is the default (not done), "all" is
  ...
  // answers people want without thinking — everything, the open work, the
  ...
  const notDoneValue = useMemo(
    () => canonicalStatuses(statusOptionList.filter((st) => NOT_DONE.includes(st.category as TaskStatus)).map((st) => st.id)),
  ...
  const statusFilterValue = useMemo(() => {
    ...
    if (!statusFilter) return notDoneValue;
    return canonicalStatuses(statusFilter.split(","));
  }, [statusFilter, notDoneValue, canonicalStatuses]);
  ...
  // on the "Not done" pill rather than on an identical-looking custom set.
  const setStatusSelection = useCallback((v: string) => {
    ...
    else if (picked === notDoneValue) setStatusFilter("");
    else setStatusFilter(picked);
  }, [canonicalStatuses, statusOptionList, notDoneValue, setStatusFilter]);
    ...
    <GenericListView<TaskItem>
          ...
          tabs={[
            ...
            { key: "", label: "Not done", count: taskCounts.active, icon: Activity,
              title: "Not finished: " + statusOptionList.filter((st) => NOT_DONE.includes(st.category as TaskStatus)).map((st) => st.name).join(", ") },
            { key: doneValue || "done", label: "Done", count: taskCounts.done || 0, icon: STATUS_CONFIG.done.icon,
              title: "Finished work" },
            { key: "all", label: "All", count: taskCounts.all, icon: Layers, title: "Every status, including Done and Dropped" },
          ]}
