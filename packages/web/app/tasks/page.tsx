...
import { TaskDetailContent, CloseGuardDialog } from "./[id]/page";
...
export function TaskRow({ task, state, triageMode, onTriage, indent = 0, descendantCount = 0, hiddenDescendantCount = 0, progress, collapsed = false, onToggleCollapse, parentChip }: {
  task: TaskItem;
  state: ItemRowState;
  triageMode?: boolean;
  ...
  indent?: number;
  /**
   * Live subtasks below this one at any depth, counted across the whole
   * workspace rather than this view — so a parent still reports the work
   * under it when a filter hides those rows.
   */
  descendantCount?: number;
  /** How many of those the current view isn't rendering (agent-internal ones, usually). */
    ...
    <>
      ...
      {progress && progress.total > 0 && (
        <button
          ...
          className={`flex items-center gap-1 text-[10px] px-1.5 py-0 rounded border flex-shrink-0 font-mono cq-hide-compact transition-colors ${
            hiddenDescendantCount > 0
              ? "border-sol-cyan/30 text-sol-cyan/80 hover:bg-sol-cyan/10"
              : "border-sol-border/40 text-sol-text-dim hover:text-sol-text"
          }`}
          title={
...
export function TaskListContent() {
  ...
  // Close-guard state for the list surfaces (kanban drag, row status). The
  // detail page owns its own; this one covers list-level terminal moves.
  ...
  return (
    ...
    <GenericListView<TaskItem>
          ...
          customContent={viewMode === "kanban" ? ({ openPaletteForItems }) => (
            <KanbanView
              ...
              onStatusChange={(task, newStatus) => {
                // Terminal moves go through the close gateway \u2014 a kanban drag
                // must not bypass the open-subtasks guard.
                if (newStatus === "done" || newStatus === "dropped") {
                  const res = closeTaskWithGuard(task.short_id, newStatus);
                  if (res.needsConfirm) { setCloseGuard({ shortId: task.short_id, status: newStatus, open: res.openSubtasks }); return; }
                } else {
        ...
        {closeGuard && (
          <CloseGuardDialog
            guard={closeGuard}
            onResolve={(resolution) => { closeTaskWithGuard(closeGuard.shortId, closeGuard.status, resolution); setCloseGuard(null); }}
