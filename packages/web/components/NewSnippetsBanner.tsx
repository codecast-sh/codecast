...
import { SNIPPET_CATALOG, type SnippetDescriptor } from "@codecast/shared/contracts";
import { api } from "@codecast/convex/convex/_generated/api";
import { useInboxStore } from "../store/inboxStore";
import { useCurrentUser } from "../hooks/useCurrentUser";
import { useDevices } from "./DeviceBadge";
import { isRecentlyShipped, newSnippetsFor, snippetEnabledOn, snippetIntroKey } from "../lib/newSnippets";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function NewSnippetsBanner() {
  ...
  // TEMP VERIFY: force-show via localStorage.debugNewSnippets — remove before landing.
  const debugAll =
    typeof window !== "undefined" && !!window.localStorage.getItem("debugNewSnippets");
  const fresh = useMemo(
    () =>
      debugAll
        ? SNIPPET_CATALOG.filter((s) => isRecentlyShipped(s))
        : newSnippetsFor({
            userCreatedAt: typeof user?._creationTime === "number" ? user._creationTime : undefined,
            devices,
            dismissed,
          }),
    [debugAll, user?._creationTime, devices, dismissed],
  );
        ...
        <DialogContent className="max-w-xl max-h-[85vh] overflow-y-auto bg-sol-card border-sol-border">
          ...
          <div className="flex items-center justify-between gap-3 pt-1">
            <button
              ...
              className="text-xs text-sol-base1 hover:text-sol-text transition-colors inline-flex items-center gap-1"
            >
              Adjust anytime in Settings → Agent Features
              <ArrowRight className="w-3 h-3" />
            </button>
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-sol-text-dim">
                {onlineDevices.length === 0
                  ...
                  : onlineDevices.length === 1
                    ? "Applies to your online machine"
                    : `Applies to ${onlineDevices.length} online machines`}
              </span>
              <button
                onClick={() => {
                  stampAll();
