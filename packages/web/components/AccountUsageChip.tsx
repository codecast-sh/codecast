...
// opens the full panel: every saved account grouped by email (Claude and
// Codex rows under one header, active group first), the auto-switch toggle,
// and the path to Settings.
...
export function AccountUsageChip() {
  ...
  // Panel list: accounts grouped by email — the same login usually exists on
  // both providers, so one email header covers its Claude and Codex rows.
  // Groups holding an active account sort first; within a group the active
  // row leads (stable sort keeps Claude before Codex otherwise).
  type AccountEntry = { provider: "claude" | "codex"; p: ProfileRow; isActive: boolean };
  const byEmail = new Map<string, AccountEntry[]>();
  const addEntry = (key: string, e: AccountEntry) => byEmail.set(key, [...(byEmail.get(key) ?? []), e]);
  for (const p of profiles) addEntry(p.email ?? p.name, { provider: "claude", p, isActive: p === active });
  for (const p of codexProfiles) addEntry(p.email ?? p.name, { provider: "codex", p, isActive: p === activeCodex });
  const accountGroups = [...byEmail.entries()]
    .map(([email, entries]) => ({
      email,
      entries: [...entries].sort((a, b) => Number(b.isActive) - Number(a.isActive)),
      hasActive: entries.some((e) => e.isActive),
    }))
