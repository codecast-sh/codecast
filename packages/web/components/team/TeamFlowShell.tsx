    ...
    <div className="tf-root flex flex-col sm:flex-row gap-6 sm:gap-10" style={style}>
      <aside className="sm:w-40 shrink-0 tf-reveal" style={{ "--tf-i": 0 } as CSSProperties}>
        <div className="flex sm:flex-col items-center sm:items-start gap-3 sm:gap-2 mb-4 sm:mb-6">
          <TeamCrest icon={crest.icon} color={crest.color} size="lg" />
          <div className="min-w-0">
            <div className="text-[11px] uppercase tracking-wider text-sol-text-dim">{eyebrow}</div>
      ...
      <section className="flex-1 min-w-0 flex flex-col" key={stepIndex}>
        <header className="tf-reveal mb-5" style={{ "--tf-i": 1 } as CSSProperties}>
          <h1 ref={headingRef} tabIndex={-1} className="text-xl font-semibold text-sol-text focus:outline-none">
            {heading}
          </h1>
          {description && <p className="mt-1 text-sm text-sol-text-dim">{description}</p>}
        </header>
        ...
        >
          <button
            type="button"
            onClick={onBack}
            className="tf-ghost inline-flex items-center gap-1.5 h-9 px-3 rounded-md text-sm text-sol-text-dim hover:text-sol-text hover:bg-sol-bg-alt focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--team-flow-accent)]"
          >
            {backLabel}
