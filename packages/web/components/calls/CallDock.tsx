...
function ExpandedPanel({
  ...
  const videoTiles = tiles.filter((t) => t.cameraTrack || t.screenTrack);
    ...
    <div className="border-b border-sol-border px-3 py-2.5">
      ...
      <div className="flex items-center gap-1.5">
        ...
        {parsed?.kind === "session" && (
          <button
            onClick={openAnchor}
            className="rounded-md p-1.5 text-sol-text-muted transition-colors hover:bg-sol-base02"
...
function TileGrid({
  ...
  const shown = focused ? tiles.filter((t) => t.identity + tileKind(t) === focused) : tiles;
  return (
    <div
    ...
    >
      {shown.map((t) => (
        <VideoTile
          key={t.identity + tileKind(t)}
          tile={t}
          speaking={speaking.has(t.identity)}
          onClick={() =>
            setFocused((f) => (f === t.identity + tileKind(t) ? null : t.identity + tileKind(t)))
          }
        />
      ))}
    </div>
...
function tileKind(t: ParticipantTile): string {
  return t.screenTrack ? ":screen" : ":cam";
}
...
function VideoTile({
  ...
  const track = tile.screenTrack ?? tile.cameraTrack;
  useEffect(() => {
  ...
  return (
    <button
      ...
      title={`${tile.name}${tile.screenTrack ? " (screen)" : ""} — click to focus`}
    >
      <video
        ref={ref}
        autoPlay
