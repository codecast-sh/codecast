    ...
    >
      ...
      {showHead && (
        <div className={`rt-head${quietHead ? " rt-head-quiet" : ""}`}>
          ...
          {/* On the stage the listening line shares the header's last row with
              the switch: that row exists whenever an agent is in the room, so
              the line costs the list no height. */}
          {listening && surface === "stage" && (
            <span className="rt-listening rt-head-listening" title={listeningText}>
              ...
              <span className="truncate">{listeningText}</span>
            </span>
          )}
