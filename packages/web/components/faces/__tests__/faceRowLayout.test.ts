...
import { FACE_ROW_METRICS, LINK_PULL, faceRowSize, faceRowWidth, floatingRowSize, FLOAT_CHROME_BAND } from "../FaceRow";
...
describe("the float", () => {
  test("64px faces inside the call circles' margin, and the name row on hover", () => {
    ...
    expect(faceRowSize("float", 2, 1, false)).toEqual({
      ...
      height: 2 * FACES_PADDING + FLOAT_CHROME_BAND + 64,
    });
    expect(faceRowSize("float", 2, 1, true).height).toBe(2 * FACES_PADDING + FLOAT_CHROME_BAND + 64 + ROW_GAP + NAME_HEIGHT);
    // The empty window is its margin and nothing else.
    expect(faceRowSize("float", 0, 0, false)).toEqual({ width: 2 * FACES_PADDING, height: 2 * FACES_PADDING + FLOAT_CHROME_BAND + 64 });
    // The band is the float's own top padding, so the chrome never covers a face or the strip.
    expect(cssVar('.face-row[data-density="float"] {', "padding")).toBe(`${FACES_PADDING + FLOAT_CHROME_BAND}px ${FACES_PADDING}px ${FACES_PADDING}px`);
  });
  test("a card holds one spot: the name band is reserved under the faces whether or not the pointer is in", () => {
    ...
    expect(still.height).toBe(2 * FACES_PADDING + FLOAT_CHROME_BAND + 64 + ROW_GAP + NAME_HEIGHT + card.height);
...
describe("the stylesheet agrees", () => {
  ...
  test("the float's tokens and margin", () => {
    ...
    expect(cssVar(rule, "padding")).toBe(`${FACE_ROW_METRICS.float.pad + FLOAT_CHROME_BAND}px ${FACE_ROW_METRICS.float.pad}px ${FACE_ROW_METRICS.float.pad}px`);
  });
