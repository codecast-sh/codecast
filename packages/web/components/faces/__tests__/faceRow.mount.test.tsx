...
describe("one circle, one attribute", () => {
  ...
  test("the marks on a person: mute, unread, ask, followed, the join", async () => {
    ...
    // The name hangs under the chin for the hover.
    expect(h.q(".face-name")!.textContent).toBe("Ann");
  });
  ...
  test("my own face sits at the head while I am engaged, and keys nothing", async () => {
    ...
    await h.click(mine);
    expect(h.q(".people-face-actions")).toBeNull();
  });
...
describe("a face is a button with three actions under it", () => {
  test("a click opens Talk, Ring and Message; Escape closes them; Message opens the DM", async () => {
    const h = await mount(bar(rowOf([entry(ANN, "Ann")])));
    const circle = h.circle(ANN);
    ...
    expect(words).toEqual(["Talk", "Ring", "Message"]);
    await act(async () => {
  ...
  test("calls off: the face is a face, not a key", async () => {
    const h = await mount(bar(rowOf([entry(ANN, "Ann")]), { callsEnabled: false }));
    const circle = h.circle(ANN);
    expect(circle.getAttribute("aria-expanded")).toBeNull();
    await h.click(circle);
    expect(h.q(".people-face-actions")).toBeNull();
  });
});
...
describe("the engagement card renders the model's card", () => {
  ...
  test("the card hangs under the row it is given to", async () => {
    ...
    expect(h.q(".face-row > .engagement-card")).not.toBeNull();
  });
});
