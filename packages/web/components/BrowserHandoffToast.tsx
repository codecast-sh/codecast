...
function toastKey(path: string) {
  return `browser-handoff:${path}`;
}

// Bumped on every show and every close. A resize observer from a dismissed
// (or replaced) card must not mint the toast again under the same key.
let handoffGen = 0;
...
export function showBrowserHandoffToast(path: string, onOpen: (path: string) => void) {
  ...
  toast.custom(
    (toastId) => <BrowserHandoffToast toastId={toastId} path={path} onOpen={onOpen} gen={gen} />,
    { id: toastKey(path), duration: Infinity, unstyled: true, style: TOAST_STYLE },
  );
}

function closeHandoffToast(path: string, toastId: string | number) {
  handoffGen += 1;
  toast.dismiss(toastKey(path));
...
function BrowserHandoffToast({
  ...
  useWatchEffect(() => {
    if (arrived) closeHandoffToast(path, toastId);
  }, [arrived, path, toastId]);

  const open = () => {
    closeHandoffToast(path, toastId);
    onOpen(path);
  };
  const dismiss = () => closeHandoffToast(path, toastId);
  ...
  // re-measures. Skip once this generation is stale: dismiss animates height,
  // and that resize would otherwise mint the toast again under the same id.
  ...
  useWatchEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      if (gen !== handoffGen || !node.isConnected) return;
