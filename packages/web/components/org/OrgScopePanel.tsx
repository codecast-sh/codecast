...
function parentName(tree: OrgTree, ref: OrgParentRef): string {
...
function StateChip({ state }: { state: OrgSession["work_state"] }) {
...
function SessionRow({ s, now, onOpen }: { s: OrgSession; now: number; onOpen: () => void }) {
...
function TaskRow({ t, now }: { t: TaskItem; now: number }) {
...
function DocRow({ d, now }: { d: DocItem; now: number }) {
...
function ScopeEditor({ role, canEdit, onChange }: { role: OrgRole; canEdit: boolean; onChange: (scope: OrgScope) => void }) {
...
function Chip({ children, tone, mono, href, onRemove }: { children: React.ReactNode; tone: "blue" | "magenta"; mono?: boolean; href: string; onRemove?: () => void }) {
...
function InlineEdit({ value, onSave, className, style, placeholder, multiline, canEdit }: { value: string; onSave: (v: string) => void; className?: string; style?: React.CSSProperties; placeholder?: string; multiline?: boolean; canEdit: boolean }) {
...
function RolePanel({ tree, role, sessions, canEdit, onOpenSession, onMove, onUpdateRole, onRetireRole, onSelectNode, now }: {
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10.5px]" style={{ color: "var(--sol-text-dim)", fontFamily: "var(--font-mono)" }}>{role.short_id}</span>
      ...
      <ScopeEditor role={role} canEdit={canEdit} onChange={(scope) => onUpdateRole(role._id, { scope })} />
