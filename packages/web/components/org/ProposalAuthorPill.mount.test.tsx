...
async function verifyAuthorPill() {
  ...
  const briefs: Record<string, any> = { "role-x": { role: { name: "Platform", handle: "platform", short_id: "or-2" } } };
  ...
  mock.module("../../hooks/useQueryNoThrow", () => ({ useQueryNoThrow: (_fn: unknown, args: any) => ({ data: args === "skip" ? undefined : briefs[args.role_id] ?? null }) }));
  mock.module("../../hooks/useSyncOrgProposals", () => ({ useSyncOrgProposal: () => ({ ready: true, missing: false }) }));
  ...
  // A role neither knows: the brief names it (one lookup, only then).
  ...
  assert.match(q('[data-proposal-author="role"]')!.textContent!, /Platform/);
  ...
  // A proposal the store has not fetched yet says so instead of a blank pill.
