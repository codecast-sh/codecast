...
// surface: header, the company's flags, the change list with accept, edit and
// skip, the rationale and evidence of the selected change, accept all, and a
// composer to the chief of staff's standing session with its thread below.
// With no proposal it is the health summary and the composer. With no chief
// of staff it is two buttons. Everything it shows arrives through props from
// the store (OrgPage owns the reads and the actions); it computes nothing
// beyond what staffingModel.ts hands it.
import { useMemo, useState } from "react";
import Link from "next/link";
import { Check, ChevronDown, ChevronRight, ExternalLink, Flag as FlagGlyph, Pencil, Sparkles, UserRoundPlus, X } from "lucide-react";
...
export type StaffingPaneProps = {
   ...
   *  (staffingModel.resolveProposalLink): one line above the body, with a
   *  switch when the proposal lives in a workspace the viewer can open. */
...
export function StaffingPane(props: StaffingPaneProps) {
  ...
  return (
    <div className="flex flex-col min-h-0" data-staffing-mode={mode}>
      {props.link && <LinkLine link={props.link} />}
      ...
      {mode !== "no_chief" && <Composer chief={props.chief} onOpenSession={props.onOpenSession} />}
    </div>
  );
}
...
function ProposalBody(props: StaffingPaneProps & { proposal: OrgProposalRow }) {
  ...
  const selected = proposal.changes.find((c) => c._id === selectedChangeId) ?? null;
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[10.5px] uppercase tracking-[0.08em]" style={{ color: "var(--sol-text-dim)" }}>{proposal.mode === "init" ? "first org" : proposal.mode === "review" ? "review" : "request"}</span>
        {others.length > 0 && (
      ...
      <div className="flex flex-col gap-3">
        {groups.map((g) => (
          <div key={g.kind} data-change-group={g.kind}>
            <div className="text-[10px] font-medium uppercase tracking-[0.08em] mb-1 px-1" style={{ color: "var(--sol-text-dim)" }}>{g.label}</div>
        ...
        <div className="mt-3">
          {!confirmAll ? (
            ...
            <div className="rounded-lg p-3 border" style={{ borderColor: "color-mix(in srgb, var(--sol-cyan) 40%, transparent)", background: "color-mix(in srgb, var(--sol-cyan) 6%, transparent)" }}>
              <p className="text-[12px]" style={{ color: "var(--sol-text-secondary)" }}>Apply the {progress.remaining} remaining changes now, in order: projects, then roles, then the rest. Each applies as proposed.</p>
      ...
      {/* rationale of the selected change */}
      {selected && (
        <div className="mt-4 rounded-xl border p-3 org-pop-in" data-rationale style={{ borderColor: BORDER, background: "var(--sol-card)" }}>
          <div className="flex items-start gap-2">
            <StatusPill status={selected.status} />
            <div className="text-[12.5px] font-medium leading-snug flex-1" style={{ color: "var(--sol-text)" }}>{changeLine(selected.change)}</div>
          </div>
          <p className="mt-2 text-[12.5px] leading-relaxed" style={{ color: "var(--sol-text-secondary)" }}>{selected.rationale}</p>
          {selected.expected_effect && <Fact k="expected" v={selected.expected_effect} />}
          {selected.risk && <Fact k="risk" v={selected.risk} tone="var(--sol-orange)" />}
          {selected.applied_note && <Fact k="applied" v={selected.applied_note} tone="var(--sol-green)" />}
          {selected.evidence.length > 0 && (
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              {selected.evidence.map((e, i) => e.href ? (
...
/** One block per change, coloured by status, in list order: the "N of M" as a strip. */
    ...
    <div className="mt-2 flex gap-[3px]" aria-hidden>
      {changes.map((c) => (
        <button
          ...
          className={cn("h-[6px] flex-1 rounded-sm transition-transform", c._id === selectedId && "scale-y-[1.8]")}
          style={{ background: c.status === "proposed" ? "color-mix(in srgb, var(--sol-violet) 35%, transparent)" : CHANGE_STATUS_META[c.status].color }}
        />
      ))}
    </div>
  );
...
function ChangeRow({ change, selected, editing, onPick, onAccept, onSkip, onEdit, onCancelEdit, onAcceptWithEdits }: {
  ...
  const open = change.status === "proposed";
  return (
    <div className={cn("rounded-lg border transition-colors", selected ? "bg-sol-bg-highlight/70" : "hover:bg-sol-bg-highlight/40")} style={{ borderColor: selected ? "color-mix(in srgb, var(--sol-violet) 45%, transparent)" : "transparent" }} data-change-row={change._id} data-change-status={change.status}>
      <div className="flex items-center gap-2 px-2 py-1.5">
        <button type="button" onClick={onPick} className="flex-1 min-w-0 flex items-center gap-2 text-left" aria-pressed={selected}>
          <StatusPill status={change.status} />
          <span className={cn("truncate text-[12.5px]", change.status === "skipped" && "line-through opacity-60")} style={{ color: "var(--sol-text)" }}>{changeLine(change.change)}</span>
        </button>
        {open && (
  ...
  return (
    <div className="flex flex-col gap-1" data-flags>
      {shown.map((r) => {
        ...
        const body = (
          <>
            <span className="w-1.5 h-1.5 rounded-full shrink-0 mt-[6px]" style={{ background: SEVERITY_COLOR[r.flag.severity] }} />
            <span className="min-w-0 flex-1">
              <span className="block text-[12px] leading-snug" style={{ color: "var(--sol-text)" }}>
                <span className="font-medium" style={{ color: SEVERITY_COLOR[r.flag.severity] }}>{FLAG_LABEL[r.flag.code]}</span>
          ...
          {bottlenecks.map((b) => (
            <button key={b.role_id} type="button" onClick={() => props.onSelectNode(b.nodeId)} className="flex items-center gap-2 px-1.5 py-1.5 rounded-md text-left hover:bg-sol-bg-highlight/70 transition-colors">
              ...
              <span className="flex items-center gap-1 shrink-0">
                {b.flags.map((f, i) => (
                  <span key={i} className="inline-flex items-center gap-1 h-[18px] px-1.5 rounded-md text-[10px]" style={{ background: `color-mix(in srgb, ${SEVERITY_COLOR[f.severity]} 14%, transparent)`, color: SEVERITY_COLOR[f.severity] }} title={f.detail}>
                    <FlagGlyph className="w-2.5 h-2.5" />{FLAG_LABEL[f.code]}
                  </span>
                ))}
              </span>
            </button>
...
function NoChiefBody(props: StaffingPaneProps) {
  return (
    <div data-no-chief>
      ...
      {props.reviewing ? (
        <div className="mt-4 rounded-xl border p-3 flex items-center gap-2.5" style={{ borderColor: BORDER, background: "var(--sol-card)" }} data-reviewing>
 ...
 *  component the anchor page uses. */
function Composer({ chief, onOpenSession }: { chief: OrgRole | null; onOpenSession: (id: string) => void }) {
  ...
  return (
    <div className="mt-6 pt-4 border-t" style={{ borderColor: BORDER }} data-composer>
      ...
      {!chief ? (
        ...
        <div className="rounded-xl border overflow-hidden" style={{ borderColor: BORDER, height: 420 }}>
          <AnchorConversation conversationId={conv} hideHeader />
        </div>
      )}
    </div>
  );
