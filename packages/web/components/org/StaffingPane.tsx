...
export type StaffingPaneProps = {
  ...
  discuss?: { name: string; updated: number; onOpen: () => void } | null;
...
function ProposalBody(props: StaffingPaneProps & { proposal: OrgProposalRow }) {
  ...
  return (
    <>
      ...
      {/* S17: the plain ask, the groups with counts, then three controls
          (evidence, budget, findings) that keep the detail behind the
          summary; the change list follows below. */}
      <ProposalSummary
        proposal={proposal}
        tree={tree}
        groups={groups}
...
type SummaryControl = "evidence" | "budget" | "findings";

/**
 * The summary a person reads before the change list (S17): the ask, folded
 * to its first lines with the whole letter one click away; the groups with
 * counts, each a jump to its rows; and three controls that hold the detail:
 * the evidence page, the budget arithmetic, the findings. The words link
 * opens the glossary. On a phone this block is the first screen, so nothing
 * here grows past what fits at 390 wide.
 */
function ProposalSummary({ proposal, tree, groups, findings, findingsCount, onOpenGlossary }: {
  proposal: OrgProposalRow; tree: OrgTree | null; groups: ChangeGroup[];
  findings: React.ReactNode; findingsCount: number;
  onOpenGlossary?: (page: "how" | "words") => void;
}) {
  const [askOpen, setAskOpen] = useState(false);
  const [control, setControl] = useState<SummaryControl | null>(null);
  const { ask, evidenceHref } = useMemo(() => splitAsk(proposal.summary_md), [proposal.summary_md]);
  const lead = useMemo(() => askLead(ask), [ask]);
  const rows = useMemo(() => groupSummary(groups), [groups]);
  ...
  const jump = (kind: ChangeGroup["kind"]) => {
    const el = document.querySelector<HTMLElement>(`[data-change-group="${kind}"]`);
    el?.scrollIntoView({ block: "start", behavior: "smooth" });
  };
  const foldable = ask.length > lead.length + 1;
  return (
    <div className="mt-3" data-proposal-summary>
      {/* the ask: the first sentence in front, the whole letter behind it */}
      {ask && (
        <div data-ask={askOpen ? "open" : "folded"}>
          {askOpen ? (
            <div className="text-[12.5px] leading-relaxed" style={{ color: "var(--sol-text-secondary)" }}>
              <MarkdownRenderer content={ask} />
            </div>
          ) : (
            <p className="text-[12.5px] leading-relaxed" style={{ color: "var(--sol-text-secondary)" }}>{lead}</p>
          )}
          {foldable && (
            <button type="button" onClick={() => setAskOpen((v) => !v)} className="mt-0.5 inline-flex items-center gap-1 text-[11.5px] px-1 -ml-1 h-6 rounded-md hover:bg-sol-bg-highlight" style={{ color: "var(--sol-violet)" }} data-ask-toggle>
      ...
      {/* the groups with counts: one row each, a jump to its rows */}
      {rows.length > 0 && (
        <div className="mt-2.5 rounded-lg border overflow-hidden" style={{ borderColor: BORDER }} data-group-summary>
          {rows.map((r) => (
            <button key={r.kind} type="button" onClick={() => jump(r.kind)} title={r.describe} className="w-full flex items-center gap-2 px-2.5 h-[30px] text-left border-b last:border-b-0 hover:bg-sol-bg-highlight/60 transition-colors" style={{ borderColor: BORDER }} data-group-summary-row={r.kind}>
              <span className="min-w-0 flex-1 truncate text-[12.5px]" style={{ color: "var(--sol-text)" }}>{r.label}</span>
              <span className="text-[12px] tabular-nums font-medium shrink-0" style={{ color: "var(--sol-text)" }}>{r.count}</span>
              <span className="text-[10.5px] tabular-nums shrink-0 w-[62px] text-right" style={{ color: r.remaining > 0 ? "var(--sol-text-dim)" : "var(--sol-green)" }}>{r.remaining > 0 ? `${r.remaining} to decide` : "decided"}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <div className="mt-2.5 flex items-center gap-1.5 flex-wrap" data-summary-controls>
        {evidenceHref ? (
          <Link href={evidenceHref} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 h-7 px-2.5 rounded-md border text-[11.5px] font-medium hover:bg-sol-bg-highlight/60" style={{ borderColor: BORDER, color: "var(--sol-text-secondary)" }} data-summary-control="evidence">
        ...
        {onOpenGlossary && (
          <button type="button" onClick={() => onOpenGlossary("words")} className="ml-auto inline-flex items-center gap-1 h-7 px-1.5 rounded-md text-[11.5px] hover:bg-sol-bg-highlight" style={{ color: "var(--sol-violet)" }} title="The eight words this page uses, each in one sentence" data-summary-words>
...
function SummaryToggle({ active, onClick, name, count, children }: { active: boolean; onClick: () => void; name: SummaryControl; count?: number; children: React.ReactNode }) {
  return (
    <button type="button" onClick={onClick} aria-expanded={active} className={cn("inline-flex items-center gap-1 h-7 px-2.5 rounded-md border text-[11.5px] font-medium transition-colors", active ? "bg-sol-bg-highlight" : "hover:bg-sol-bg-highlight/60")} style={{ borderColor: active ? "color-mix(in srgb, var(--sol-violet) 45%, transparent)" : BORDER, color: active ? "var(--sol-text)" : "var(--sol-text-secondary)" }} data-summary-control={name}>
      {children}
        ...
        <div className="mt-2 flex flex-col gap-1" data-budget-lines>
          {budget.lines.map((l, i) => (
            <div key={i} className="text-[11.5px] leading-snug" style={{ color: "var(--sol-text-muted)" }}>
              ...
              <span className="block tabular-nums">{l.before && l.after ? `${capsLine(l.before)} → ${capsLine(l.after)}` : l.after ? `+ ${capsLine(l.after)}` : `− ${capsLine(l.before)}`}</span>
            </div>
          ))}
        </div>
      )}
...
function RevisionNote({ change, selected }: { change: OrgProposalChange; selected: boolean }) {
  ...
  return (
    <span className="block mt-0.5 text-[11px] leading-snug" style={{ color: "var(--sol-text-muted)" }} data-revision={r.kind}>
      ...
      {moves.length > 0 && (
        <span className="block mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5" data-revision-moves>
          {moves.map((m) => (
            <span key={m.key} className="inline-flex items-baseline gap-1" title={`${m.label}: was ${m.from ?? "nothing"}, now ${m.to ?? "nothing"}`}>
              <span style={{ color: "var(--sol-text-dim)" }}>{m.label}</span>
              {m.from !== null && <span className="line-through opacity-70">{m.from}</span>}
              {m.to !== null && <span className="font-medium" style={{ color: "var(--sol-text)" }}>{m.to}</span>}
            </span>
          ))}
        </span>
      )}
...
function DiscussBar({ name, updated, onOpen }: { name: string; updated: number; onOpen: () => void }) {
  return (
    <div className="sticky bottom-0 -mx-4 px-4 pt-2 pb-3 mt-4" style={{ background: "linear-gradient(to bottom, transparent, var(--sol-bg) 30%)" }} data-discuss-bar>
      <button type="button" onClick={onOpen} className="w-full h-10 rounded-xl border flex items-center gap-2.5 px-3 text-left shadow-[0_8px_24px_-12px_rgba(0,0,0,0.5)]" style={{ borderColor: "color-mix(in srgb, var(--sol-violet) 45%, transparent)", background: "var(--sol-card)", color: "var(--sol-text)" }}>
        ...
        <span className="min-w-0 flex-1 truncate text-[13px] font-medium">Talk to {name} about this</span>
        {updated > 0 && <span className="shrink-0 inline-flex items-center h-5 px-1.5 rounded-full text-[10.5px] font-semibold tabular-nums" style={{ background: "var(--sol-violet)", color: "var(--sol-bg)" }}>{updated} changed</span>}
