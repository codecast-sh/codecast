...
describe("proposalAsks", () => {
  ...
  test("state and verdict line: open while anything waits, then accepted or skipped", () => {
    ...
    expect(asks[3].verdictLine).toBe("2 of 4 decided");
...
describe("costLine", () => {
  ...
  test("says the share in words, less or more", () => {
    expect(costLine(b(1000, 750))).toBe("After: the daily limits add up to about a quarter less");
    expect(costLine(b(1000, 1100))).toBe("After: the daily limits add up to about a tenth more");
    expect(costLine(b(1000, 500))).toBe("After: the daily limits add up to about half less");
    expect(costLine(b(1000, 1660))).toBe("After: the daily limits add up to about two thirds more");
    expect(costLine(b(1000, 2100))).toBe("After: the daily limits add up to about double");
    expect(costLine(b(1000, 3200))).toBe("After: the daily limits add up to about 3 times as much");
    expect(costLine(b(1000, 50))).toBe("After: the daily limits add up to almost nothing");
    expect(costLine(b(1000, 1010))).toBe("After: the daily limits add up to about the same");
  });
  test("the edges: nothing moves, a first limit, no limit left", () => {
    expect(costLine(b(1000, 1000, 0))).toBe("After: the daily limits stay the same");
    expect(costLine(b(0, 800))).toBe("After: the agents get their first daily limit");
    expect(costLine(b(800, 0))).toBe("After: no agent has a daily limit left to use");
  });
});
