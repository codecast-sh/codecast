...
import { CheckSquare, Layers, MessageCircleQuestion } from "lucide-react";
...
import { sortOrgSessions, type OrgRole, type OrgSession, type OrgTree } from "../orgTypes";
...
export function ScopeSessionsTab({ tree, role }: { tree: OrgTree; role: OrgRole | null }) {
  const now = useCoarseNow(30_000);
  ...
  const onPage = useCallback((cursor: string | undefined, rows: OrgSession[], next?: string) => {
    setPages((prev) => ({ ...prev, [cursor ?? ""]: { rows, next } }));
  }, []);
  ...
  // The tree carries the top 8; the rest page in through sessionsUnder.
  let lastNext: string | undefined;
  { let key = ""; for (let g = 0; g < 100; g++) { const p = pages[key]; if (!p) { lastNext = key === "" ? (role && role.total > role.sessions.length ? "" : undefined) : undefined; break; } lastNext = p.next; if (!p.next) break; key = p.next; } }
  const canLoad = !!role && (lastNext !== undefined) && !requested.includes(lastNext ?? "") && rows.length < total;
  ...
  return (
    <div>
      ...
      {role && requested.map((c) => <RoleSessionsPage key={c} roleId={role._id} teamId={teamId} cursor={c || undefined} onPage={onPage} />)}
      {rows.length === 0 ? (
      ...
      {canLoad && (
        <div className="pt-2 text-center">
          <button type="button" disabled={pending} onClick={() => setRequested((r) => [...r, lastNext ?? ""])} className="text-[11.5px] hover:underline disabled:opacity-50" style={{ color: "var(--sol-text-muted)" }}>
    ...
    return (
      <li>
        <Link href={decisionHref(d)} className="group flex items-start gap-2.5 px-2.5 py-2 rounded-xl transition-colors hover:bg-sol-bg-highlight/70">
          <span className="w-[3px] self-stretch rounded-full shrink-0" style={{ background: tone }} />
          <MessageCircleQuestion className="w-4 h-4 shrink-0 mt-[2px]" style={{ color: tone }} />
          <span className="min-w-0 flex-1">
