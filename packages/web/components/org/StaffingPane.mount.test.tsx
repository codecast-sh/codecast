...
async function verifyStaffingPane() {
  ...
  // No evidence page, no Evidence link; the glossary opens from the intro and the page header, not a third link here.
  assert.equal(q('[data-summary-control="evidence"]'), null);
  assert.equal(q("[data-summary-words]"), null);
  ...
  assert.match(strip!.querySelector("button")!.getAttribute("aria-label")!, /^proposed · Add a standing agent/);
