...
async function verifyStaffingPane() {
  ...
  assert.deepEqual(qa("[data-ask-fold]").map((el) => el.textContent?.trim()), ["2 recordsdecide them one at a time", "1 change", "1 change", "4 changesdecide them one at a time"]);
  ...
  assert.equal(card(3).querySelector("[data-ask-verdict]")!.textContent, "2 of 4 decided");
  ...
  assert.equal(q("[data-cost-line]")!.textContent, "After: the agents get their first daily limitSee the numbers");
