...
async function verifyProposalThread() {
  ...
  const embed: { onSendOverride?: (content: string) => Promise<void>; composerNode?: React.ReactNode; conversationId?: string } = {};
  ...
  assert.equal(said[1], "fixture-chief-conv|op-9|null|why is growth in there");
  ...
  assert.match(amended.querySelector("[data-revision-moves]")!.textContent!, /caps tokens per day600000800000/);
  ...
  await renderPane({ threadNode: null, discuss: { name: "Chief of Staff", updated: 3, onOpen: () => paneCalls.push("discuss") } });
  assert.equal(q("[data-thread-slot]"), null);
  assert.match(q("[data-discuss-bar]")!.textContent!, /Talk to Chief of Staff about this3 changed/);
  ...
  console.log("ProposalThread mount: ok");
}

verifyProposalThread().catch((e) => { console.error(e); process.exit(1); });
