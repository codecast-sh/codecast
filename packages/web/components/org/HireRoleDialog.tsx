...
import { parentNodeId, parentRefOfNodeId, refMatches } from "./orgLayout";
...
export function HireRoleDialog({ open, onClose, tree, meId, onCreate, initialProjects = [], projectPath, title = "Add a role", initial, submitLabel = "Create and start" }: {
  ...
  const [reportsTo, setReportsTo] = useState<string>(initial?.reports_to ? parentNodeId(initial.reports_to) : meId ? parentNodeId({ kind: "user", user_id: meId }) : "");
  ...
  const [projectIds, setProjectIds] = useState<string[]>(() => [
    ...
    ...wsProjects.filter((p) => (initial?.scope?.projects ?? []).some((ref) => refMatches(ref, { id: p._id, title: p.title, short_id: (p as { short_id?: string }).short_id })) && !initialProjects.some((q) => q._id === p._id)).map((p) => p._id),
  ]);
  const [planIds, setPlanIds] = useState<string[]>(() => plans.filter((p) => (initial?.scope?.plans ?? []).some((ref) => refMatches(ref, { id: p._id, title: p.title, short_id: p.short_id }))).map((p) => p._id));
