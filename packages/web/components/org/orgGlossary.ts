...
const DEFINITION: Record<GlossaryWord, string> = {
  role: "A standing agent with a name, an area of work to look after, and a daily limit; it watches its area and leaves the rest alone.",
  scope: "The projects and plans a role looks after: what lands in front of it, and what it never sees.",
  charter: "A short written statement of what a project or a role is for, so an agent can tell its own work from someone else's.",
  hand: "One session doing one piece of work under a role, gone when that work is done.",
  wake: "One turn a role takes when something calls it: a message, a schedule, or a change inside its area.",
  budget: "The most a role may do in one day, counted in hands, wakes and tokens, the units of text a model reads and writes.",
...
export function glossaryEntries(tree: OrgTree | null | undefined, health: OrgHealth | null | undefined, proposal: Pick<OrgProposalRow, "short_id" | "changes" | "counts"> | null | undefined): GlossaryEntry[] {
  ...
  const handEx = healthRole && healthRole.load.live_hands > 0
    ...
    : { text: "A session you start from a task under a role is one of its hands; it ends when the task does.", own: false };
  const wakeEx = healthRole && healthRole.spend.wakes_cap > 0
    ...
    : { text: "A message sent to a role, or its weekly review coming due, is one wake.", own: false };
...
export const HOW_THIS_WORKS: { heading: string; body: string }[] = [
  {
    ...
    body: "The org page is the reporting chart of your workspace: the people, the standing agents they hired, and every session under them. A standing agent is called a role. Each role has a scope, an area of work it looks after, so it knows what to pick up and what to leave alone.",
  },
  {
    heading: "Where a proposal comes from",
    body: "An agent reads how work actually moves through the company: which plans finished, which tasks nobody touched, where decisions pile up. It then writes a proposal: a list of changes to the org, each with the evidence behind it. The chief of staff is the role that does this on a schedule; you can also ask for one review at any time.",
  },
  {
