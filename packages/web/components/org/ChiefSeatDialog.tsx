...
export function ChiefSeatDialog({ open, onClose, teamId, agentName, threadShortId, messageCount, onConfirm }: {
  ...
  // all, so the honest source is the server's own anchor space. Subscribed only
  // while the dialog is open (the owner-candidates rule): the dialog opens at
  // once on what the tree already knows, and the count fills in when it lands.
  const { data: space } = useQueryNoThrow(
    api.anchors.getAnchorSpace,
    open ? { scope_type: teamId ? ("team" as const) : ("user" as const), ...(teamId ? { team_id: teamId as Id<"teams"> } : {}) } : "skip",
  );
