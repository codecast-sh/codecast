    ...
    <div className="h-full flex flex-col overflow-hidden" style={{ background: "var(--sol-bg)", color: "var(--sol-text)" }}>
      ...
      <div className="shrink-0 px-4 sm:px-6 pt-4 pb-3 border-b flex items-end justify-between gap-3 flex-wrap" style={{ borderColor: "color-mix(in srgb, var(--sol-border) 22%, transparent)" }}>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {tree && isAdmin !== false && (
            <button type="button" onClick={() => setAddRoleOpen(true)} className="h-[34px] inline-flex items-center gap-1.5 pl-3 pr-3.5 rounded-lg text-[12.5px] font-semibold transition-colors hover:brightness-110" style={{ background: "var(--sol-violet)", color: "var(--sol-bg)" }}>
              <Plus className="w-3.5 h-3.5" /> Add a role
              {/* No roles yet: the button carries the explanation, so the empty state has one affordance, not two. */}
              {stats?.roles === 0 && <span className="hidden md:inline font-normal opacity-80">· a seat sessions can report to</span>}
            </button>
          )}
        </div>
      </div>
