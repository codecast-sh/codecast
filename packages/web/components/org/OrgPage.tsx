...
import { StaffingPane } from "./StaffingPane";
import { changeNodeId, findChiefOfStaff, openProposals, pickProposal, proposalParam, proposalProgress } from "./staffingModel";
...
export function OrgPageInner() {
  ...
  // The proposals collection and the focus scalar are the ghosts slice's
  // (org-staffing.md S5, registered with the ghost nodes); the pane reads
  // them through the same store, loosely typed until that slice lands.
  const s = useTrackedStore([
    (st) => st.currentUser?._id,
    (st) => st.clientState.ui?.active_team_id,
    (st) => (st as unknown as { orgProposals?: Record<string, OrgProposalRow> }).orgProposals,
    (st) => (st as unknown as { orgFocusChangeId?: string | null }).orgFocusChangeId ?? null,
  ]);
  const storeProposals = (s as unknown as { orgProposals?: Record<string, OrgProposalRow> }).orgProposals;
  const storeFocusChangeId = (s as unknown as { orgFocusChangeId?: string | null }).orgFocusChangeId ?? null;
  ...
  const [focusChangeId, setFocusChangeIdState] = useState<string | null>(null);
  ...
  const proposals = useMemo<OrgProposalRow[]>(() => preview ? previewProposals : Object.values(storeProposals ?? {}), [preview, previewProposals, storeProposals]);
  ...
  const reviewing = reviewingSince !== null && !openProposals(workspaceProposals).some((p) => p.created_at >= reviewingSince);
  ...
  const setFocusChangeId = useCallback((id: string | null) => {
    setFocusChangeIdState(id);
    (useInboxStore.getState() as unknown as { setOrgFocusChangeId?: (id: string | null) => void }).setOrgFocusChangeId?.(id);
  }, []);
  // A ghost click on the chart (the ghosts slice sets the scalar) opens the
  // pane on that change.
  useWatchEffect(() => {
    if (!storeFocusChangeId) return;
    setFocusChangeIdState(storeFocusChangeId);
    setPanelMode("staffing");
  ...
  // -------- staffing actions
  /** The preview decides on its local copy; live, the ghosts slice's action
   *  flips the row and rides dispatch to orgProposals.decide. */
  const decideChange = useCallback((changeId: string, verdict: "accept" | "skip", edits?: Record<string, unknown>) => {
    ...
    const st = useInboxStore.getState() as unknown as { decideOrgProposalChange?: (changeId: string, verdict: "accept" | "skip", edits?: Record<string, unknown>) => void };
    if (!st.decideOrgProposalChange) { toast.error("Deciding a change is not wired on this build yet"); return; }
    st.decideOrgProposalChange(changeId, verdict, edits);
  }, [preview]);
  ...
  }, [proposal, tree, setFocusChangeId]);
  const hireChief = useCallback(() => {
    const host = me?.user_id ?? meId;
    if (!tree || !host) return;
    run("staffChiefOfStaff", { ...(tree.workspace.kind === "team" ? { team_id: tree.workspace.id } : {}), host_user_id: host, client_id: `orgrolestub-chief-${Math.random().toString(36).slice(2)}` });
  ...
  const editRoleInitial = useMemo<HireRoleInitial | undefined>(() => {
    const c = editRoleChange?.change;
    if (!c || c.kind !== "role") return undefined;
    return { name: c.name, handle: c.handle, charter: c.charter, caps: c.caps };
  }, [editRoleChange]);
    ...
    <div className="h-full flex flex-col overflow-hidden" style={{ background: "var(--sol-bg)", color: "var(--sol-text)" }}>
      ...
      {tree && editRoleChange && (
        <HireRoleDialog
          ...
          onCreate={(input) => {
            decideChange(editRoleChange._id, "accept", { name: input.name, handle: input.handle, ...(input.charter ? { charter: input.charter } : {}), ...(input.caps ? { caps: input.caps } : {}), scope: input.scope, reports_to: input.reports_to });
