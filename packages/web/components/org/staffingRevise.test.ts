...
describe("proposalThread", () => {
  test("the server's pointer wins, named after the role", () => {
    ...
    expect(t).toMatchObject({ conversationId: "fixture-chief-conv", shortId: "jx7ch1f", name: "Chief of Staff" });
  ...
  test("a session author's conversation is the thread until the pointer lands", () => {
    ...
    expect(t).toMatchObject({ conversationId: "fixture-conv-review", shortId: "jx7rev1", name: "Org review, September", role: null });
  });
...
describe("what a revise did", () => {
  ...
  test("an amend lists the fields that moved, old and new", () => {
    ...
    expect(amendedMoves(budget)).toEqual([{ key: "caps.tokens_per_day", label: "caps tokens per day", from: "600000", to: "800000" }]);
