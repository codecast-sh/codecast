...
function OrgGraphInner(props: OrgGraphProps) {
  const { tree, view, selectedId, loadingClusters, showMiniMap, onSelect, onToggleCollapse, onExpandCluster, onCollapseCluster, onReparentRequest, onNodeContextMenu, onOpenSession, resetKey, panelWidth = 0, canDrag, changes, health, viewerSession, focusChangeId = null, onFocusChange, onDecideChange, onEditRoleChange } = props;
  ...
  const flagsByNode = useMemo(() => healthFlagsByNode(health), [health]);
  ...
  // pans its ghost to the centre of the free canvas at the current zoom. A
  // programmatic move, so the cue recomputes from the target viewport.
  const focusNodeId = useMemo(() => ghostNodeIdFor(ghosts, focusChangeId), [ghosts, focusChangeId]);
  useWatchEffect(() => {
    const el = wrapRef.current;
    if (!focusNodeId || !el || !viewportReady) return;
    const vp = computeOrgViewport(layout.nodes, el.clientWidth, el.clientHeight, panelWidth, focusId, { id: focusNodeId, zoom: rf.getViewport().zoom });
    if (!vp) return;
    userMoved.current = true;
    rf.setViewport({ x: vp.x, y: vp.y, zoom: vp.zoom }, { duration: document.hidden ? 0 : 280 });
    recomputeCue({ x: vp.x, y: vp.y, zoom: vp.zoom });
  }, [focusNodeId, viewportReady]);
