...
function OrgGraphInner(props: OrgGraphProps) {
  ...
  // Until the user pans or zooms, the canvas keeps the tree fitted as the
  // container settles (boot layout, pane resize, phone rotation). A resize
  // after the user took the wheel is theirs to handle.
  ...
  useWatchEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      if (userMoved.current || !initialized) return;
      rf.fitView({ padding: 0.1, duration: 0, maxZoom: 1 });
    });
    ro.observe(el);
