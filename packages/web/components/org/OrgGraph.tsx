...
export type OrgGraphProps = {
  ...
  /** A node the pane asked the chart to pan to (a flag row): the same target
   *  path as a focused ghost, which wins when both are set. */
...
function OrgGraphInner(props: OrgGraphProps) {
  const { tree, view, selectedId, loadingClusters, showMiniMap, onSelect, onToggleCollapse, onExpandCluster, onCollapseCluster, onReparentRequest, onNodeContextMenu, onOpenSession, resetKey, panelWidth = 0, panelHeightFraction = 0, canDrag, changes, health, viewerSession, focusChangeId = null, focusNodeId: focusNodeProp = null, onFocusChange, onDecideChange, onEditRoleChange } = props;
  ...
  useWatchEffect(() => {
    ...
    recomputeCue({ x: vp.x, y: vp.y, zoom: vp.zoom });
  }, [focusNodeId, viewportReady, panelWidth, panelHeightFraction]);
