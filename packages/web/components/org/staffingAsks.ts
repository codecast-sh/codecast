...
import { resolveOrgAsks } from "@codecast/shared/contracts/orgProposal";
...
export function proposalAsks(p: Pick<OrgProposalRow, "asks" | "changes">): AskView[] {
  const bySeq = new Map(p.changes.map((c) => [c.seq, c]));
  return resolveOrgAsks(p.asks, p.changes).map((a, index) => {
