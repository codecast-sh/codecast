...
export interface FleetCountOpts extends FleetBandOpts {
  ...
  /** The viewer's user id (store.currentUser._id). A cached row that is
   *  definitively a TEAMMATE's counts only while teamInboxIds reports it —
   *  outside the team subscription its liveness is frozen, so counting it
   *  would report last week's "working" forever (isForeignRow). */
  meId?: string | null;
 ...
 * members left out, exactly the default rendered set. The selection doesn't
 * read authorship, so definitively-foreign rows (a teammate's session cached
 * by team mode / a deep-link / search) take the teamInboxIds arm ONLY: they
 * count while their own subscription keeps them live and drop out with it.
...
export function fleetCountedSessions(
  ...
  for (const [id, s] of Object.entries(sessions)) {
    const kept =
      ...
      (membership.members.has(id) &&
        !membership.belowFold.has(id) &&
        !isForeignRow(s, opts.meId)) ||
