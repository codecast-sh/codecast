...
const SIZE = {
  md: "gap-2 rounded-lg px-5 py-2.5 font-medium",
  sm: "gap-2 rounded-md px-2.5 py-1.5 text-xs font-medium",
} as const;
const ICON = { md: "h-5 w-5", sm: "h-3.5 w-3.5" } as const;
const TONE = {
  dark: "bg-[#002b36] text-white hover:bg-[#073642]",
  light: "border border-[#93a1a1] text-[#586e75] hover:bg-[#eee8d5]",
} as const;
 ...
 */
export function AppBadge({ app, location, tone = "dark", size = "md", direct = false }: {
  app: NativeApp;
  location: string;
  tone?: "dark" | "light";
  size?: "md" | "sm";
  ...
  const className = `inline-flex items-center transition-colors ${SIZE[size]} ${TONE[tone]}`;
