...
// forwarded to origin) while #o= stays a fragment — insert before the hash.
function withEditParam(manageUrl: string): string {
    ...
    <div className="group relative flex flex-col rounded-lg border border-sol-border/40 bg-sol-bg-alt/40 overflow-hidden transition-colors hover:border-sol-border hover:bg-sol-bg-alt/70">
      ...
      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="min-w-[190px]">
            ...
            <DropdownMenuSeparator />
            {/* Manage opens the artifact's own owner sheet (#o= key stays in the
                fragment); rollback, gates, viewers and stats all live there. */}
            ...
            <DropdownMenuSeparator />
            {/* Deleting needs an authed convex mutation (deleteForWeb) that the
                backend layer doesn't expose yet — point at the CLI meanwhile. */}
            <DropdownMenuItem disabled title="Web delete isn't wired up yet">
              <Trash2 className="w-3.5 h-3.5 mr-2" />
              <span className="flex flex-col items-start">
                <span>Delete</span>
                <span className="text-[10px] text-sol-text-dim font-mono">use: cast publish rm {a.slug}</span>
              </span>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <div className="flex flex-col gap-1.5 p-3">
        ...
        <div className="flex items-center gap-3 text-[11px] text-sol-text-dim">
          ...
          <GateBadges a={a} />
          {a.session_short_id && (
            <span className="ml-auto min-w-0" onClick={(e) => e.stopPropagation()}>
              <EntityIdPill shortId={a.session_short_id} type="session" />
            </span>
          )}
        </div>
      </div>
