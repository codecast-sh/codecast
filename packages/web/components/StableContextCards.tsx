    ...
    <div className="conv-col mx-auto px-4 sm:px-5 md:px-6 pt-2 pb-1">
      ...
      >
        ...
        <span>
          ...
          {data.items.length === 1 ? "session" : "sessions"} from the{" "}
...
export function StableContextPicker({ conversationId }: { conversationId: string }) {
  ...
  const { data: feed } = useQueryNoThrow(api.conversations.feedForCLI, feedArgs, { breakAfterMs: 20_000 });
  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-4">
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-1.5 text-[11px] text-sol-text-dim">
          <Layers className="w-3 h-3 text-sol-cyan" />
          <span>Context</span>
          {effectiveMode !== "off" && feed !== undefined && (
            <span className="opacity-70">
              — {includedCount} {includedCount === 1 ? "session" : "sessions"} injected at start
            </span>
          )}
        </div>
        <SegmentedToggle
