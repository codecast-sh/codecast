...
import { useSyncProjectGitEvents, useGitEvents, gitEventsNewestFirst } from "../hooks/useSyncGitEvents";
...
function EventBody({ event }: { event: TimelineEvent }) {
  switch (event.type) {
    ...
    case "git":
      // Git rows paint through ExternalEventRow, not here.
      return null;
    case "doc_created":
      return (
        <span className="text-sol-text-muted min-w-0 truncate">
...
export function ProjectTimeline({ projectId }: { projectId: string }) {
  ...
  // Git events do not come from webTimeline: they are their own synced
  // collection, so mount the project feeder and read the store.
  useSyncProjectGitEvents(projectId);
  const gitWhere = useMemo(
    () => (row: GitEventRow) => !!projectId && !!row.project_ids?.includes(projectId),
    [projectId],
  );
  const gitRows = useGitEvents(gitWhere, gitEventsNewestFirst);

  const groups = useMemo(() => {
    const now = Date.now();
    const gitEvents: TimelineEvent[] = gitRows.map((row) => ({
      ts: row.created_at ?? 0,
      type: "git",
      git: row,
    }));
    const list = [...((events ?? []) as TimelineEvent[]), ...gitEvents]
      .filter((e) => {
              ...
              if (e.type === "git" && e.git) {
                return (
                  <div key={e.git._id} className="relative -ml-[13px] py-0.5">
                    <ExternalEventRow
                      event={gitEventToExternalEvent(e.git)}
                      density="compact"
                      omitRefs={["project_id"]}
                    />
                  </div>
