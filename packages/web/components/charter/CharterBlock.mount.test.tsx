...
describe("CharterBlock", () => {
  ...
  it("empty with no chief of staff: the ask still links to /org (the pane opens on the hire) and says none is hired", async () => {
    const t = await setup();
    ...
    assert.match(t.text(), /none hired yet/);
    assert.equal(t.buttons().filter((b) => /Ask the Chief of Staff/.test(b.textContent ?? "")).length, 0, "the ask is never a button that opens blank fields");
  ...
  it("empty with a chief of staff: the ask links to /org with the composer prefilled", async () => {
    ...
    assert.match(link.textContent!, /Ask the Chief of Staff to draft one/);
    assert.equal(link.getAttribute("href"), `/org?compose=${encodeURIComponent("draft a charter for Codecast: Product")}`);
    assert.doesNotMatch(t.text(), /none hired yet/);
    await t.click(t.button("or write it"));
    assert.equal(document.querySelector("[data-charter]")?.getAttribute("data-charter"), "project");
    t.unmount();
  }, SLOW);
  ...
  it("the no owner chip with no role to pick and no hire path is disabled and says why, never a dead button", async () => {
    ...
    await t.render({ charter: { goal: "x" }, tree: { ...ORG_FIXTURE, roles: [] } });
    ...
    await t.render({ charter: { goal: "x" }, tree: null });
    ...
    await t.render({ charter: { goal: "x" }, tree: null, ownerBlockedReason: "Switch to the project's workspace to assign an owner" });
    chip = document.querySelector<HTMLButtonElement>('[data-owner="none"]')!;
  ...
  it("a retired owner renders as 'No owner' so the person reassigns, not as a link to a gone seat", async () => {
    ...
    await t.render({ charter: full, tree });
  ...
  it("a budget written hands first has the server's key order, so the field lock retires on the echo", async () => {
    const t = await setup();
    ...
    assert.deepEqual(t.patches.at(-1), { budget: { hands_per_day: 2 } });
    // The row now carries the hands only budget; tokens land second.
    ...
    assert.deepEqual(Object.keys(budget), ["tokens_per_day", "hands_per_day"], "tokens then hands, the server's order");
    assert.equal(JSON.stringify(budget), JSON.stringify({ tokens_per_day: 400_000, hands_per_day: 2 }));
