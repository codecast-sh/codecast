...
export function PriorityPill({ priority, onChange, size = "sm", className }: {
  ...
  return (
    <span className="relative inline-flex">
      <button type="button" className={cn(cls, "transition-colors hover:brightness-110")} style={style} title={meta?.hint ?? "Set the priority"} data-priority={priority ?? "none"} onClick={() => setOpen((o) => !o)} aria-haspopup="menu" aria-expanded={open}>
        {body}
      </button>
