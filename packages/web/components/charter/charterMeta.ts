...
/** The role lookups take the role LIST, not the org tree: the tree carries
 *  every node's top sessions (their state and updated_at), so a surface that
 *  subscribed to it to look up a role name would re-render on every message
 *  in the workspace. `null` means the roles have not loaded; `[]` means the
 *  workspace has none. */
 ...
 *  longer exists. */
export function ownerRoleOf(roles: OrgRoles, ownerRoleId: string | null | undefined): OrgRole | null {
  if (!roles || !ownerRoleId) return null;
  const role = roles.find((r) => r._id === ownerRoleId);
