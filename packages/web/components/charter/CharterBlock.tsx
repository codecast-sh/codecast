...
function BudgetLine({ budget, canEdit, onChange }: { budget: CharterFields["budget"]; canEdit: boolean; onChange: (b: NonNullable<CharterFields["budget"]> | null) => void }) {
  ...
  // the field lock compares by JSON.stringify, and a budget written hands
  // first would otherwise hold a different key order and never retire.
...
export function CharterBlock({ kind, title, charter, canEdit, onChange, tree, onHire, ownerBlockedReason, className }: CharterBlockProps) {
