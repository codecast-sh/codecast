...
import { Compass, Flag, ListChecks, Ban, AlertTriangle, Coins, Plus, X, Sparkles } from "lucide-react";
...
export type CharterBlockProps = {
  ...
  /** The org tree singleton: the owner chip and the chief of staff come from it. */
...
function ListField({ label, icon: Icon, items, canEdit, onChange, placeholder, marker }: {
  label: string;
  icon: typeof Flag;
  items: string[];
  canEdit: boolean;
      ...
      <ul className="space-y-0.5">
        {items.map((item, i) => (
          <li key={`${i}:${item}`} className="group/item flex items-start gap-2 text-[12.5px] leading-snug" style={{ color: "var(--sol-text)" }}>
            ...
            <InlineEdit value={item} canEdit={canEdit} onSave={(v) => setAt(i, v)} className="text-[12.5px] flex-1" />
            {canEdit && (
              <button type="button" onClick={() => setAt(i, "")} className="mt-[3px] shrink-0 opacity-0 group-hover/item:opacity-60 hover:!opacity-100 transition-opacity" aria-label={`Remove from ${label}`} style={{ color: "var(--sol-text-dim)" }}>
                <X className="w-3 h-3" />
              </button>
            )}
          </li>
        ...
        {canEdit && (
          <li className="flex items-start gap-2 text-[12px]" style={{ color: "var(--sol-text-dim)" }}>
            ...
            <InlineEdit value="" canEdit placeholder={placeholder} onSave={(v) => { if (v) onChange([...items, v]); }} className="text-[12px] flex-1" />
          </li>
        )}
      </ul>
    </div>
...
function BudgetLine({ budget, canEdit, onChange }: { budget: CharterFields["budget"]; canEdit: boolean; onChange: (b: NonNullable<CharterFields["budget"]> | null) => void }) {
  ...
  const set = (patch: Partial<NonNullable<CharterFields["budget"]>>) => {
    const next = { ...(budget ?? {}), ...patch };
    if (next.tokens_per_day === undefined) delete next.tokens_per_day;
    if (next.hands_per_day === undefined) delete next.hands_per_day;
...
export function CharterBlock({ kind, title, charter, canEdit, onChange, tree, onHire, className }: CharterBlockProps) {
  // "No charter yet" until someone opens the fields or a value lands.
  const [opened, setOpened] = useState(false);
  const filled = hasCharter(charter);
  const chief = chiefOfStaffOf(tree);
  ...
  if (!filled && !opened) {
    return (
      <div className={cn("flex items-center gap-2 text-[12px] flex-wrap", className)} data-charter="empty">
        ...
        {chief ? (
          <Link href={composeCharterHref(title)} className="inline-flex items-center gap-1 font-medium hover:underline" style={{ color: "var(--sol-cyan)" }}>
            <Sparkles className="w-3 h-3" /> Ask the Chief of Staff to draft one
          </Link>
        ) : canEdit ? (
          <button type="button" onClick={() => setOpened(true)} className="inline-flex items-center gap-1 font-medium hover:underline" style={{ color: "var(--sol-cyan)" }}>
            <Sparkles className="w-3 h-3" /> Ask the Chief of Staff to draft one
          </button>
        ) : null}
        {chief && canEdit && (
