...
export function CharterBlock({ kind, title, charter, canEdit, onChange, tree, onHire, className }: CharterBlockProps) {
  ...
  // says who is hired when nobody holds the seat yet.
  if (!filled && !opened) {
    return (
      <div className={cn("flex items-center gap-2 text-[12px] flex-wrap", className)} data-charter="empty">
        ...
        <Link
          ...
          title={chief ? `Sends "draft a charter for ${title}" to @${chief.handle}` : "No Chief of Staff yet: the org page opens on hiring one, and the draft request follows the hire"}
        >
          <Sparkles className="w-3 h-3" /> Ask the Chief of Staff to draft one
        </Link>
        {!chief && <span style={{ color: "var(--sol-text-dim)" }}>(none hired yet)</span>}
        {canEdit && (
