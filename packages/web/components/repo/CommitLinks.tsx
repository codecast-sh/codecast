...
export function CommitLinks({
  ...
  // One shape either way, so the pill below can read short_id without the
  // union narrowing it away.
  const tasks: NonNullable<CommitJoins["tasks"]> =
    joins.tasks ?? (joins.task_ids ?? []).map((_id) => ({ _id }));
