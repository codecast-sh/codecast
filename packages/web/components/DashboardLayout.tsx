...
import { useInboxStore, useTrackedStore, sessionsWakeSig, pendingSendWakeSig, getProjectName, resolveShowOld, selectSessionRailOpen, selectCommentRailOpen, selectSessionRailUserClosed, selectNavCollapsed, bucketProjectPath, placeInboxRows, resolveSimpleView } from "../store/inboxStore";
...
function DashboardLayoutInner({ children, hideSidebar }: DashboardLayoutProps) {
    ...
    const onPop = (e: PopStateEvent) => {
      ...
      if (popped.inboxView) {
        ...
        withApplyingViewHistory(() => {
          if (
            v.bucket !== store.activeBucketFilter || v.project !== store.activeProjectFilter || !!v.exclude !== store.chipFilterExclude ||
            !sameFilterExtras(v.extras, store.extraBucketFilters) || !sameFilterExtras(v.projectExtras, store.extraProjectFilters)
          ) {
            if (v.bucket) store.setActiveBucketFilter(v.bucket, v.exclude, v.extras);
            else if (v.project) store.setActiveProjectFilter(v.project, v.projectPath, v.exclude, v.projectExtras);
            else {
  ...
  if (PANE_EMBED) {
    return (
      <div data-cc-shell data-cc-embed className={`bg-sol-bg overflow-hidden${resolveSimpleView(s.clientState.ui) ? " simple-view" : ""}`} style={{ height: zoomHeight }}>
  ...
  return (
    <div data-cc-shell className={`bg-sol-bg flex flex-col overflow-hidden${resolveSimpleView(s.clientState.ui) ? " simple-view" : ""}`} style={{ height: zoomHeight }}>
      <ErrorBoundary name="DashboardSync" level="inline" fallback={null}>
        <DashboardSyncEffects />
      </ErrorBoundary>
