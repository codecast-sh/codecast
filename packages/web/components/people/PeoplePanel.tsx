...
export function PeoplePanel() {
  ...
  return (
    <div className="people-panel flex flex-col bg-sol-bg text-sol-text">
      <PanelHeader />
      ...
      {/* The phone's own ringer and strip live HERE, in the window that hosts
          ...
          it is a call dock, the walkie strip, or nothing at all. */}
...
function WalkieDoorToggle() {
  ...
  return (
    <button
      ...
      title={
        open
          ? "Teammates can talk to you here. Their words still arrive as messages either way."
          : "Nobody plays out loud here. Their words still arrive as messages."
      }
...
function Roster({ callsEnabled }: { callsEnabled: boolean }) {
  ...
  // fields a row draws and reads the array itself out of getState().
  ...
  const members = useMemo(() => useInboxStore.getState().teamMembers ?? [], [sig]);

  // Three list-level subscriptions, each shared by every row: the fleet counts
  // (one pass over the sessions the store holds), the live rooms, and the chat
  // rail. Per row these would be twenty subscriptions to the same data.
  ...
  const groups = useMemo(
    () => groupMembersByBand(members.filter((m: any) => m?._id && String(m._id) !== viewerId)),
    [members, viewerId],
  );
  ...
  if (members.length === 0) {
    ...
    return (
      <div className="px-3 py-6 text-[12px] text-sol-text-dim">
        {strayWorkspace
          ? "This window is pointed at a workspace you are not in. Switch workspace in the main window."
          : "No teammates yet."}
      </div>
    );
  }
...
function RosterRow({
  ...
  const line = useMemo(
    () => presenceActivityLine(member, { now, fleet, room, talking }),
    [member, now, fleet, room, talking],
  );
  ...
  return (
    ...
    >
      ...
      {/* One slot, two faces: the unread count at rest, the gestures under a
          pointer or a focus ring. They share the box so the name column never
          reflows as the pointer crosses the list. */}
      <div className="relative flex h-7 shrink-0 items-center justify-end" style={{ width: 92 }}>
        ...
        <div
          className="people-row-actions absolute inset-y-0 right-0 flex items-center gap-0.5"
