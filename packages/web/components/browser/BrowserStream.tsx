...
export function BrowserStream({
  ...
  return (
    <div ref={boxRef} className="absolute inset-0">
      ...
      {frame && (
        <GhostCursor store={ghost} boxRef={boxRef} imgRef={imgRef} frameSize={frameSize} hidden={driving || !live} />
      )}
      {driving && (
...
function GhostCursor({
  ...
  /** The human has the wheel, or the stream is not live: the arrow hides and
   *  keeps its place, so it returns where it was when the agent is back. */
  hidden: boolean;
}) {
