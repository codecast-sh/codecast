...
// Step 1 and 2 share one visual — the same four cards, first arriving in a
// loose pile, then filed under their sections. Continuity does the teaching.
...
function StepPractice({ cleared, clearedBy, onFinish }: {
  ...
  return (
    <StepShell
      ...
      lede={doneAll
        ...
        : <>These are practice cards. Park each one — the highlighted card is next, and any verb works.</>}
    >
      {doneAll ? (
    ...
    >
      ...
      >
        <div className="min-h-[380px]">
          ...
          {step === 2 && <StepVerbs />}
          {step === 3 && (
            <StepPractice cleared={cleared} clearedBy={clearedBy} onFinish={() => close("finished")} />
          )}
        </div>
        ...
        <div className="flex items-center px-8 py-4 border-t border-sol-border/30">
          <button
            type="button"
            onClick={() => close("skipped")}
            className="text-[11px] text-sol-text-dim hover:text-sol-text-muted transition-colors"
          >
            Skip tour
          </button>
          <div className="flex items-center gap-1.5 mx-auto" aria-hidden>
            {Array.from({ length: STEP_COUNT }, (_, i) => (
              <button
                ...
                className={cn(
                  ...
                  i === step ? "w-5 bg-sol-cyan" : "w-1.5 bg-sol-border/60 hover:bg-sol-border",
                )}
              />
            ))}
          </div>
          <div className="flex items-center gap-2">
            ...
            {step < STEP_COUNT - 1 ? (
              ...
              <span className="text-[10px] text-sol-text-dim">3 cards left</span>
            )}
          </div>
        </div>
      </div>
...
// CLI setup hero, and never when tips are off.
export function TriageNuxGate() {
  ...
  useEffect(() => {
    if (open || !initialized || done || off || !hasSessions) return;
  ...
  }, [open, initialized, done, off, hasSessions]);
