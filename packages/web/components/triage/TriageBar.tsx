...
function VerbButton({ verb, disabled, engaged, onRun }: {
  ...
  const btn = (
    ...
    >
      ...
      <MenuKeyCaps action={verb.action} className="cq-triage-caps flex items-center gap-[2px]" />
    </button>
  );
...
export function TriageBar() {
  ...
  // Minimized: a corner button, no layout height. Sits in the bottom-left of
  // the conversation column so it does not fight the call pill on the right
  // and does not push the composer up.
  if (compact) {
    return (
          ...
          <ShortcutTooltip side="top" label="Triage actions">
            <button
              ...
              className={cn(
                ...
                "border border-sol-border/50 bg-sol-bg-alt/90 text-sol-text-dim",
