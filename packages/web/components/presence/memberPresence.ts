...
/** The name a surface prints for a member, and nothing fancier: their name,
 *  else the local part of their email, else a word rather than a blank. One
 *  helper so the strip, the roster and the card cannot disagree about what a
 *  person is called. */
...
export interface PresenceActivityCtx {
   ...
   *  arrives here, and says so. */
  room?: { label?: string; locked?: boolean } | null;
...
export function presenceActivityLine(member: any, ctx: PresenceActivityCtx): string {
  ...
  if (member?.in_room_key || member?.in_huddle || room) {
    // A locked room the viewer is not in has no name to print (the server
    // redacts it); saying it is locked is the honest and useful thing.
