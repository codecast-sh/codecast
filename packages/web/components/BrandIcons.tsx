// Brand marks for the agent providers, shared by every surface that shows
// one (session lists, notifications, timeline, account usage). Monochrome
// paths that inherit currentColor so each caller picks the tone.
...
// Official Grok logomark from grok.com (Feb 2025). Two interlocking curves;
// viewBox matches the mark as published, so it scales into any square slot.
export function GrokIcon({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0.36 0.5 33.33 32" fill="currentColor">
