  ...
  return (
    <CtxSub>
      ...
      <CtxSubContent className="min-w-[180px]">
        {options.map((o) => (
          <CtxItem key={o.key} onSelect={() => onPick(o.key)}>
            {o.icon ? <o.icon className={cn("size-3.5 shrink-0", o.color)} /> : null}
            <span className={cn("truncate", !o.icon && o.color)}>{o.label}</span>
            {currentKey === o.key && <Check className="size-3.5 ml-auto text-sol-cyan" />}
          </CtxItem>
        ))}
      </CtxSubContent>
    </CtxSub>
...
export function TaskMenuItems({
  ...
  const setStatus = (key: string) => {
    ...
    if (key === "done" || key === "dropped") {
      let deferred = false;
      for (const t of tasks) {
        if (closeTaskWithGuard(t.short_id, key as "done" | "dropped").needsConfirm) deferred = true;
      }
      if (!deferred) toast.success(`${bulkLabel} → ${key === "done" ? "Done" : "Dropped"}`);
    } else {
      applyAll({ status: key });
      toast.success(`${bulkLabel} → ${STATUS_OPTIONS.find((o) => o.key === key)?.label ?? key}`);
    }
  };
      ...
      <CtxSub>
        ...
        <CtxSubContent className="min-w-[190px]">
          {members.map((m: any) => (
            ...
            >
              <span className="truncate">{m.name || m.github_username || "user"}</span>
              {single && (single as any).assignee === m._id && <Check className="size-3.5 ml-auto text-sol-cyan" />}
            </CtxItem>
          ))}
          ...
          {labels.map((b: any) => {
            ...
            return (
              ...
              >
                ...
                {currentBucketId === b._id && <Check className="size-3.5 ml-auto text-sol-cyan" />}
              </CtxItem>
            );
          })}
          {currentBucketId && (
