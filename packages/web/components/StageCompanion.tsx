...
export const StageCompanion = memo(function StageCompanion() {
  ...
  return (
    <div className="h-full flex flex-col border-l border-sol-border/30 bg-sol-bg">
      {/* Standard slot chrome — title, ⤢ promote, ✕ close — from one place, so
          every pane in every slot closes and promotes identically. */}
      <SlotChrome slot="secondary" title={session.title || "Session"} onPromote={handleExpand} />
