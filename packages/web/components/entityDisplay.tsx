 ...
 * is in hand, whichever id the reference arrived with.
 */
export function repoObjectRefOf(type: EntityType, entity: any): string | null {
  ...
  if (type === "commit" && typeof entity.sha === "string") return repoObjectId({ type: "commit", repository: entity.repository, sha: entity.sha.slice(0, 12) });
  return null;
}
...
export function useEntityResolution(rawRef: string, typeProp?: EntityType): EntityResolution {
  ...
  const routeId = isRepoObject && type ? repoObjectRefOf(type, entity) ?? rawId : entity?._id ?? rawId;
