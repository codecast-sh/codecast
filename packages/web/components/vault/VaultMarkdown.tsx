...
/** Headings carry `id="vh-<slug>"` so [[Note#Heading]] links and the outline
 *  pane can scroll to them. The slug matches the index engine's slugifyHeading. */
function anchoredHeading(Tag: "h1" | "h2" | "h3" | "h4" | "h5" | "h6", className: string) {
  return function VaultHeading({ children }: { children?: React.ReactNode }) {
    return (
      <Tag id={`vh-${slugifyHeading(childText(children))}`} className={className}>
        {children}
      </Tag>
    );
  };
...
const VAULT_REHYPE_PLUGINS = [rehypeHighlight, rehypeKatex];
