...
import { useInboxStore, placeInboxRows, classifySession } from "../../store/inboxStore";
...
export function MentionSuggestion({ item }: { item: Omit<MentionItem, "id"> & { id?: string; description?: string } }) {
  ...
  const liveJson = useInboxStore((s) => {
    ...
    if (item.type === "session" && s.sessions[id]) {
      ...
      const verdict = classifySession({ ...row, ...live, agent_status: (live.agent_status ?? undefined) as typeof row.agent_status, is_connected: live.daemon_alive });
      const status = placeInboxRows(s, { now }).placements.get(id)?.work_state
        ?? (!verdict.idle ? "working" : verdict.waiting ? verdict.rest : "idle");
