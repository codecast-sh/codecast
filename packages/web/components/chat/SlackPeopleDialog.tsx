...
export function SlackPeopleDialog({ teamId, onClose }: { teamId: string; onClose: () => void }) {
  const data = useQuery(api.slackSync.listSlackPeople, { team_id: teamId } as any);
  const mapPerson = useMutation(api.slackSync.mapSlackPerson);
  const s = useTrackedStore([(st) => memberListSig(st.teamMembers as any), (st) => st.currentUser?._id]);
