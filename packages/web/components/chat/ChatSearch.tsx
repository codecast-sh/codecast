...
import { authorFor, memberHandles, memberName, type ChatMember } from "../../lib/chatViews";
...
export function ChatSearch({
  ...
  const hits: SearchHit[] = ready ? (data?.results ?? []) : [];
  ...
  const roomLabel = (h: SearchHit): { icon: React.ReactNode; label: string } => {
    if (h.channel_kind === "dm") {
      const others = dmOtherIds(h.dm_key, String(viewer));
      const names = others.map((id) => {
        const full = memberName(byId.get(String(id)));
        return others.length > 1 ? full.split(/\s+/)[0] : full;
      });
      return { icon: <User className="w-3 h-3" />, label: names.join(", ") || "Direct message" };
    }
    const priv = channels.find((c) => c.id === h.channel_id)?.isPrivate;
    return {
      icon: priv ? <Lock className="w-3 h-3" /> : <Hash className="w-3 h-3" />,
      label: h.channel_name || "unknown",
    };
  };
  ...
  let flatIdx = -1;
  return (
    <div className="ch-search-overlay" onClick={onClose} role="presentation">
      <div
        className="ch-search"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="ch-search-bar">
          ...
          />
          <button type="button" className="ch-search-close" aria-label="Close" onClick={onClose}>
                ...
                {g.hits.map((h) => {
                  ...
                  return (
                    <button
                      ...
                      className={`ch-search-hit ${idx === highlight ? "ch-search-hit-hot" : ""}`}
