...
export function useChannelSlackLink(channelId: string | null | undefined): ChatSlackLinkRow | null {
  ...
  for (const id in s.chatSlackLinks) {
    const row = s.chatSlackLinks[id] as ChatSlackLinkRow;
    if (row.chat_channel_id === channelId) return row;
  }
  return null;
}
        ...
        <header className="ch-slack-head">
          ...
          <div className="min-w-0">
            ...
            <div className="ch-slack-sub">
              ...
              {link && (
                <>
                  <span className="ch-slack-arrow" aria-hidden="true">{directionGlyph(link.direction)}</span>
...
function directionGlyph(d: Direction): string {
  return d === "both" ? "⇄" : d === "slack_to_codecast" ? "←" : "→";
}
  ...
  return (
    <div className="ch-slack-body">
      ...
      <p className="ch-slack-fine ch-slack-summary">
        Nothing already posted moves, either way, unless you pick history above. Slack gets one notice
        when this starts. You can unlink any time; copies on both sides stay.
      </p>
      <footer className="ch-slack-foot">
        <span className="ch-slack-fine">
          {picked
            ? picked.is_private && !picked.is_member
              ? "Private: run /invite @Codecast in that Slack channel first."
              : `Mirroring #${chatName ?? "channel"} with Slack #${picked.name}.`
            : "Pick the Slack channel to mirror."}
        </span>
  ...
  return (
    <div className="ch-slack-body">
      ...
      <section className="ch-slack-section">
        <div className="ch-slack-label">What crosses over</div>
        <OptionRows
          options={{ ...DEFAULT_OPTIONS, ...link.options }}
          ...
          <ul className="ch-slack-ledger">
            {activity.map((j: any) => (
              <li key={j._id} className={`ch-slack-ledger-row ch-slack-ledger-${j.status}`}>
                ...
                <span className="ch-slack-ledger-status">{j.status === "done" ? "mirrored" : j.status === "skipped" ? `skipped${j.error ? ` · ${describeSkip(j.error)}` : ""}` : j.status === "failed" ? `failed${j.error ? ` · ${j.error}` : ""}` : "queued"}</span>
...
function OptionRows({
  ...
  const changedAdvanced = ADVANCED_ROWS.filter((r) => options[r.key] !== DEFAULT_OPTIONS[r.key]).length;
  return (
    <>
