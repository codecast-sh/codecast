            ...
            <div className="ch-slack-sub">
              ...
              {link && (
                <>
                  ...
                  {/* The channel wears the Slack name, so the other side is
                      just "Slack"; only a name kept apart (taken on our side)
                      is spelled out. */}
...
function SetupBody({
  ...
  const start = async () => {
    ...
    try {
      ...
      toast(
        res.chat_channel_name && res.chat_channel_name !== chatName
          ? `Mirroring with Slack. The channel is now #${res.chat_channel_name}, the same name as in Slack`
          : `Mirroring #${res.chat_channel_name ?? picked.name} with Slack`,
        { duration: 4000 },
      );
    } catch (e: any) {
