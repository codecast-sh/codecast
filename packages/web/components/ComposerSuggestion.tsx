...
export const ComposerSuggestion = memo(forwardRef<ComposerSuggestionHandle, {
  ...
  // exactly where typed words would. The text is a button (click accepts);
  // the caps on the right are inert.
  return (
    <div className="absolute inset-0 flex items-center gap-2 py-1 min-w-0 text-sm leading-relaxed animate-in fade-in-0 duration-200">
      <button
        type="button"
        // preventDefault so the composer keeps focus through the click.
        onMouseDown={(e) => e.preventDefault()}
        onClick={accept}
        // The row truncates a long prompt; hover shows all of what Tab inserts.
        title={text}
        className="flex items-center gap-1.5 min-w-0 flex-1 text-left cursor-text text-sol-text-dim hover:text-sol-text-muted transition-colors"
      >
        <Sparkles className="w-3 h-3 text-sol-violet/70 shrink-0" />
        <span className="truncate">{text}</span>
      </button>
      <span className="shrink-0 flex items-center gap-1.5 text-[9px] text-sol-text-dim/70 select-none pointer-events-none">
