...
describe("the one card under a face", () => {
  test("opens after a dwell on a face: the person, what they are doing, the profile door and the three actions, and no second photo", async () => {
    const h = await mount();
    ...
    // Inside the row, under the face's own seat band.
    expect(card.closest(".face-row")).not.toBeNull();
    expect(card.textContent).toContain("Ann");
    // The face on the row is the picture; the card never repeats it.
    expect(card.querySelector("img")).toBeNull();
    // Talk, a huddle, Message: the three things you can do, on the card.
  ...
  test("a click pins the same card; there is never a second popup under a face", async () => {
    const h = await mount();
    await h.fire(h.face(ANN), "mouseover");
    await act(async () => {
      await Bun.sleep(180);
    });
    expect(h.all("[data-member-card]").length).toBe(1);
    await h.fire(h.face(ANN), "click");
    expect(h.all("[data-member-card]").length).toBe(1);
    expect(h.q(".people-face-actions")).toBeNull();
    expect(h.face(ANN).getAttribute("aria-expanded")).toBe("true");
    // Pinned: the pointer leaving does not drop it.
    await h.fire(h.seat(ANN), "mouseout");
    await act(async () => {
      await Bun.sleep(260);
    });
    expect(h.q("[data-member-card]")).not.toBeNull();
    // Message on the card opens the DM and the card goes.
    const message = h.all("[data-member-card] .face-action").find((b) => b.textContent?.includes("Message"))!;
    await h.fire(message, "click");
    expect(openedDms).toEqual([[ANN]]);
    expect(h.q("[data-member-card]")).toBeNull();
  });

  test("a hovered card lets go when the pointer leaves, and switches faces at once", async () => {
    const h = await mount();
    await h.fire(h.face(ANN), "mouseover");
    await act(async () => {
      await Bun.sleep(180);
    ...
    await h.fire(h.seat(ANN), "mouseout");
    await h.fire(h.seat(BO), "mouseover");
    expect(h.q("[data-member-card]")!.textContent).toContain("Bo");
    await h.fire(h.seat(BO), "mouseout");
    await act(async () => {
      await Bun.sleep(260);
    });
    expect(h.q("[data-member-card]")).toBeNull();
