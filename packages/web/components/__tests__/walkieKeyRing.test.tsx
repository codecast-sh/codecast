...
// Radix decides ONCE, when its layout-effect shim loads, whether a document
// exists — and in a shared test process a neighbouring file loads it before
// this one has a DOM, after which every Radix portal (the menu's included)
// stays unmounted forever. React's own layout effect is what the shim would
// have chosen with a document, so this is the real thing, not a stub.
...
describe("walkie key: ring under the key", () => {
  test("a right click opens the one-item ring menu, and choosing it rings the room", async () => {
    ...
    console.log("DEBUG shim:", (await import("@radix-ui/react-use-layout-effect")).useLayoutEffect === React.useLayoutEffect, document.body.innerHTML.slice(0, 120));
    const item = k.menuItem();
    expect(item?.textContent).toContain("Ring everyone and start a huddle");

    // Radix selects on the item's click (a pointerup becomes one).
    await k.act(async () => {
