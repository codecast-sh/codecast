...
const noop = () => {};
const actions = { join: noop, snooze: noop, end: noop, mute: noop, camera: noop, answer: noop, decline: noop, cancel: noop };
...
describe("on the line, hands free", () => {
  ...
  test("the badge says so, and End and Mute are both there, beside it", () => {
    ...
    const quiet = rule(".engagement-card .walkie-strip-mute,\n.engagement-card .walkie-strip-snooze,\n.engagement-card .ring-card-decline,\n.engagement-card .walkie-strip-end", rowCss);
    expect(quiet.background).toContain("var(--sol-text) 7%");
    expect(quiet.border).toBe("1px solid transparent");
  });
  ...
  test("muted says so on the button, once", () => {
    ...
    expect(rule(".engagement-card .walkie-strip-mute-on", rowCss).color).toBe("var(--sol-text)");
  });
});
