...
// Measured 2026-09-07 on this fixture: 169 with the sidebar mounted at 100
// rows (51 at 20 rows), and 1 for a card mounted on its own. The fixture size
// is fixed, so the sidebar number IS the per-row cost: it moves only when a
// card or the panel changes how much it subscribes to.
//
// The first measurement here was 1069 and 13 (ct-49548). A card held thirteen
// separate subscriptions — ten inline selectors plus useRosterDevice,
// useTeamRosterIdentity and useAnchorIdentity — and 75 rendered rows made that
// about a thousand selectors run on every publish. ct-49746 folded them into
// one useTrackedStore per card, whose deps are the same narrow projections, so
// a row now costs two: the card's, and the one CardBarStrip holds for the
// row's live-watch bars.
const SIDEBAR_PIN = 169;
const CARD_PIN = 1;
...
// Folding a card's subscriptions must not change WHEN it repaints — a card
// that woke on a heartbeat before would just wake through one subscription
// instead of thirteen, which is no win at all. These pin both directions
// against the real panel (ct-49746).
async function commitsDuring(mutate: () => void, warmUp?: () => void): Promise<number> {
  let commits = 0;
  const host = document.createElement("div");
  document.body.appendChild(host);
  const root = createRoot(host);
  const tree = (
    <React.Profiler id="panel" onRender={() => { commits++; }}>
      <SessionListPanel />
    </React.Profiler>
  );
  await act(async () => root.render(tree));
  // Steady state: the FIRST push of a shape does real work no matter what it
  // carries (syncTable's transform seeds a conversation row per session), and
  // measuring that would say nothing about a heartbeat.
  if (warmUp) await act(async () => { warmUp(); flushSyncPublishes(); });
  commits = 0;
  await act(async () => { mutate(); flushSyncPublishes(); });
  const seen = commits;
  await act(async () => root.unmount());
  host.remove();
  return seen;
...
test("a liveness heartbeat does not repaint the sidebar", async () => {
  seed(SESSION_COUNT);
  const rows = Object.values(useInboxStore.getState().sessions) as any[];
  // What the liveness overlay carries and nothing else: a fresher stamp on
  // every row. `updated_at` is deliberately left alone — that one IS real
  // activity, and the panel is meant to wake on it.
  const heartbeat = () =>
    useInboxStore.getState().syncTable(
      "sessions",
      rows.map((r) => ({ ...r, last_heartbeat: Date.now() })),
    );
  const commits = await commitsDuring(heartbeat, heartbeat);
  expect(commits).toBe(0);
}, 60_000);

test("a status change repaints the sidebar", async () => {
  seed(SESSION_COUNT);
  const rows = Object.values(useInboxStore.getState().sessions) as any[];
  const target = rows.find((r) => r.agent_status === "working")!;
  const commits = await commitsDuring(() => {
    useInboxStore.getState().syncTable("sessions", [
      { ...target, agent_status: "done", is_idle: true, updated_at: target.updated_at + 1 },
    ]);
  });
