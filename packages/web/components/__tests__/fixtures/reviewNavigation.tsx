...
test("passage jumps use the virtualizer and clear the sticky prompt while visible passages stay put", async () => {
  const host = document.createElement("div"); document.body.append(host);
  const root = createRoot(host);
  const scroll = document.createElement("div");
  scroll.innerHTML = '<div data-review-message="m1" tabindex="-1"><div class="cc-content"><p>Quoted passage</p></div></div>';
  document.body.append(scroll);
  scroll.getBoundingClientRect = () => bounds(100, 500);
  scroll.scrollTop = 800;
  const block = scroll.querySelector("p")!;
  block.getBoundingClientRect = () => bounds(600, 680);
  const sticky = document.createElement("div");
  Object.defineProperty(sticky, "offsetHeight", { value: 60 });
  const scrollRef = createRef<HTMLDivElement>(); scrollRef.current = scroll;
  const stickyRef = createRef<HTMLDivElement>(); stickyRef.current = sticky;
  const offsets: number[] = [];
  const messages: string[] = [];
  let navigations = 0;
  const virtualizer = { scrollToOffset: (offset: number) => offsets.push(offset) } as any;
  let bridge: ReturnType<typeof useReviewNavigation>;
  function Harness() {
    bridge = useReviewNavigation(scrollRef, virtualizer, id => messages.push(id), () => navigations++, stickyRef);
    return null;
  }
  try {
    await act(() => root.render(<Harness />));
    await act(() => bridge.jumpToComment(comment("one", "m1")));
    expect(offsets).toEqual([1110]);
    expect(messages).toEqual([]);
    expect(navigations).toBe(1);
    block.getBoundingClientRect = () => bounds(300, 380);
    bridge!.scrollToBlock(block, "nearest");
    expect(offsets).toEqual([1110]);
    await act(() => bridge.jumpToComment(comment("other", "unmounted")));
    expect(messages).toEqual(["unmounted"]);
  } finally { await act(() => root.unmount()); host.remove(); scroll.remove(); }
});
