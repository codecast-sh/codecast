...
describe("a machine that told us nothing is never drawn as a machine with nothing", () => {
  ...
  test("a machine that never reported outranks whatever the map says about it", () => {
    ...
    // it as a reporter.
    const rows = buildFleetRows([machine("a", "MacBook"), machine("b", "Studio", null)], {
      a: [skill("deep-research")],
...
// ------------------------------------------------ the badge and the gate agree

// `ExecutionSurface` used to name two different things: the contract's eight
// observable surfaces, and this UI's three-way display bucket. One exported name
// for two concepts is how a reassuring badge ends up on a capability the consent
// gate would have stopped. The bucket is now `ExecutionConfidence`, and it is
// DERIVED from the gate rather than decided again beside it.
describe("the execution badge cannot contradict the consent gate", () => {
  const CASES: Array<readonly ExecutionSurface[]> = [
    [],
    ["reads_files"],
    ["runs_commands"],
    ["network"],
    ["reads_files", "runs_commands"],
  ];

  test("every surface set renders code exactly when consent is required", () => {
    for (const surfaces of CASES) {
      expect(confidenceFromSurfaces(surfaces) === "code").toBe(requiresExplicitConsent(surfaces));
    }
  });
  ...
  test("surfaces never looked at are unknown, which is not prose", () => {
    // Undefined means no scan has run. Rendering that as prose would be the
    // same lie as an empty classification, one step earlier.
    expect(confidenceFromSurfaces(undefined)).toBe("unknown");
  });

  test("the kind floor never calls an uninspected kind safe", () => {
    expect(defaultExecutionSurface("snippet")).toBe("prose");
    for (const kind of ["mcp", "hook", "plugin"]) {
      expect(defaultExecutionSurface(kind)).toBe("code");
    }
    ...
    for (const kind of ["skill", "command", "subagent", "something-new"]) {
      expect(defaultExecutionSurface(kind)).toBe("unknown");
    }
  });
});
