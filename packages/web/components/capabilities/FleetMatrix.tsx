...
/**
 * How good an observation is, when one machine reports the same capability more
 * than once — Claude Code's scopes stack rather than override, and several
 * clients can see the same thing.
 *
 * The question a fleet page answers is "does this machine have it working
 * anywhere", so the best observation is the machine's answer. Same ranking as
 * the store's `PRESENCE_RANK` (store/capabilities.ts), which is what keeps the
 * two sides agreeing about one machine.
 */
function observedRank(item: FleetInventoryItem): number {
  if (item.enabled) return item.installed === false ? 2 : 3; // broken, else active
  return item.installed === false ? 0 : 1; // absent, else downloaded and off
}
 ...
 * per scope that switched it on — so several reports can land in one cell, and
 * the best of them is the machine's answer (`observedRank`).
 */
export function buildFleetRows(
  ...
  const rows = new Map<
    string,
    {
      row: Omit<
        FleetRow,
        "drift" | "missingOn" | "mismatchedOn" | "disabledOn" | "brokenOn" | "majorityPin"
      >;
  ...
  for (const device of reporting) {
    for (const item of reports[device.deviceId] ?? []) {
      ...
      if (!prev || observedRank(item) > observedRank(prev)) {
        bucket.byDevice.set(device.deviceId, item);
      }
    }
  }
