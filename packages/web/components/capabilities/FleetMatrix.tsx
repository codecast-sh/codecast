...
import type { TokenCost } from "./TokenCostBadge";
...
/**
 * Fold every machine's inventory into one row per capability.
 *
 * Pure and exported so the same answer can be asserted in a test and, later,
 * computed by `cast cap status` without a second implementation of "what counts
 * as drift".
 *
...
export function buildFleetRows(
  ...
  for (const { row, byDevice } of rows.values()) {
    ...
    // disagree with itself, and a fleet of one has nothing to compare against.
    // "Switched off on every machine that has it" is agreement, not drift — a
    // deliberate, consistent decision should not be reported as a problem.
    const offEverywhere =
      disabledOn.length === reporting.length && missingOn.length === 0 && mismatchedOn.length === 0;
    const drift =
      reporting.length > 1 &&
      (missingOn.length > 0 || mismatchedOn.length > 0 || disabledOn.length > 0) &&
      !offEverywhere;
...
function Cell({ cell, device, row }: { cell: FleetCell; device: CapabilityDevice; row: FleetRow }) {
  ...
  return (
    <ShortcutTooltip label={label} side="top">
      <div
  ...
  return (
    <div className="px-1.5 py-2 border-l border-l-[color-mix(in_srgb,var(--sol-border)_60%,transparent)] min-w-0">
      ...
      {d.reportedAt === null ? (
        ...
        <div className="mt-0.5 text-[9px] text-sol-text-dim truncate" title={d.clientVersion}>
          {seen}
        </div>
      )}
      {d.error && (
...
function RowHead({
  ...
  return (
    ...
    >
      {Icon && (
        <ShortcutTooltip label={meta!.label} side="right">
      ...
      {row.description && (
        <span className="hidden lg:inline text-[11px] text-sol-text-dim truncate min-w-0">
          {row.description}
        </span>
      )}
    </div>
...
export function FleetMatrix({
  ...
  // nothing; the device columns are fixed so N machines scroll rather than
  // squeeze into unreadable slivers.
  const template = `minmax(240px, 1fr) repeat(${devices.length}, minmax(104px, 132px))`;
...
function MatrixRow({
  ...
  return (
    <div
      className={`contents group/row ${selected ? "" : ""}`}
