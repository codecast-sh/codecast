 ...
 *
 * The kind only sets a floor: a skill directory can ship an executable, and a
 * scanner that has looked inside says so by setting `executionSurface`
 * explicitly. A derived value is never allowed to overrule an observed one.
 */
...
export function toCapabilityDevice(
  d: Device,
  report?: { reportedAt?: number; error?: string; clientVersion?: string },
): CapabilityDevice {
  return {
    ...
    reportedAt: typeof report?.reportedAt === "number" ? report.reportedAt : null,
    error: report?.error,
