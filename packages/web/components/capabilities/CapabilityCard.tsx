 ...
 *
 * This is the RENDERED verdict, two-valued on purpose. The classification itself
 * belongs to the shared contract: `EXECUTION_SURFACES` enumerates the eight
 * surfaces a scanner observes and `requiresExplicitConsent(surfaces)` decides
 * which of them mean "code", including the deliberate rule that an unclassified
 * capability counts as dangerous. Whatever adapts a catalog row for this surface
 * sets `executionSurface` from that call — never by re-deciding it here.
 *
 * The kind is only the floor used when nothing has looked inside: a skill
 * directory can ship an executable, and an observed value always wins over this.
 */
export type ExecutionSurface = "prose" | "code" | "unknown";

export function defaultExecutionSurface(kind: CapabilityKind): ExecutionSurface {
  return kind === "mcp" || kind === "hook" || kind === "plugin" ? "code" : "prose";
}
...
/** Only shown when a capability can execute. Absence of the marker is not a
 *  safety claim, so nothing is rendered for prose. */
...
export function InstallFootprint({
  ...
  const byDevice = useMemo(() => {
    ...
    for (const site of entry.installs) {
      ...
      // Scopes stack; an enabled site is the more useful truth to show.
      if (!prev || (!prev.enabled && site.enabled)) m.set(site.deviceId, site);
    }
    return m;
  }, [entry.installs]);

  const reporting = devices.filter((d) => d.reportedAt !== null);
  const present = reporting.filter((d) => byDevice.has(d.deviceId));
  ...
  return (
    ...
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      {devices.map((d) => {
        ...
        const unknown = d.reportedAt === null;
        const state = unknown ? "unknown" : site ? (site.enabled ? "on" : "off") : "absent";
        return (
