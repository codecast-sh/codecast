...
function installedCount(entry: CatalogEntry, reportingIds: Set<string>): number {
  ...
  for (const site of entry.installs) {
    if (reportingIds.has(site.deviceId)) seen.add(site.deviceId);
  }
  return seen.size;
}
...
export function LibraryBrowse({
  ...
  // vocabulary: `/` focuses search, j/k walk the grid, Enter opens. The context
  // is only claimed while this tab is mounted, so the keys go back to whatever
  // else owns them the moment it unmounts.
  useShortcutContext("list", true);
  ...
  // tab that always shows zero is noise, and a kind we have never modelled would
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <SegmentedToggle
          value={kind}
          onChange={(k) => {
            setKind(k as CapabilityKind | "all");
            setFocusIndex(0);
          }}
          items={kindItems}
          collapse
