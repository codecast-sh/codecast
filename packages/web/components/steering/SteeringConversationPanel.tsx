...
export function SteeringConversationPanel({
  ...
  const sidePanelOpen = useInboxStore((state) => state.sidePanelOpen);
  ...
  const effectiveConversationId = selectedConversationId === "__new__"
    ? null
    : selectedConversationId ?? (sidePanelOpen && related.some(({ conversation }: any) => conversation._id === visibleSidePanelId) ? visibleSidePanelId : null);
