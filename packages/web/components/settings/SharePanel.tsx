 ...
 */
export function SharePanel({ name, paths, isRepository = false, projects, teams, current, busy, onApply, onClose }: {
  name: string;
  ...
  const impact = { ...useShareImpact(paths, projects, team ? since : null), repos: 1 };
  const list = useQueryNoThrow(api.users.listConversationsForPath, team ? { path_prefixes: paths.slice(0, 12) } : "skip").data as
    ...
    <div className="space-y-3 border-t border-sol-border/40 bg-sol-bg-alt/40 px-4 py-3 sm:px-5" data-share-panel>
      ...
      {team && level ? (
        <>
          ...
          {(!list || list.total > 0) && <div className="rounded-md border border-sol-border/50">
            <div className="flex items-baseline justify-between gap-2 border-b border-sol-border/40 px-3 py-1.5 text-[11px] text-sol-text-dim">
              <span>Untick a session to keep it private</span>
              <span className="tabular-nums">
                {list ? (list.rows.length < list.total ? `newest ${list.rows.length} of ${formatSessionCount(list.total, list.truncated)}` : formatSessionCount(list.total, list.truncated)) : "loading"}
