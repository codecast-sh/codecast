...
function ProjectSwitcher({ conversation, handleRef, machineSlot }: {
  ...
  const pickList = useMemo<{ path: string; custom?: boolean; suggested?: boolean }[]>(() => {
    ...
    return base.concat(
      otherProjects.filter((p) => !p.suggested).map((p) => ({ path: p.path })),
      otherProjects.filter((p) => p.suggested).map((p) => ({ path: p.path, suggested: true })),
    );
        ...
        {picking && (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-1 pb-2 mb-2 border-b border-sol-cyan/20">
            ...
            <input
              ...
              placeholder="search folders, or paste any path (~/ or /)"
              spellCheck={false}
              autoComplete="off"
