...
export function moveAgentOptions(agentType: string | undefined, messageCount: number | undefined): Record<MoveVerb, AgentOption[]> {
  ...
  const cannotRebuild = `can't rebuild ${count} message${count === 1 ? "" : "s"} of history`;
...
function StepHeader({ onBack, children }: { onBack: () => void; children: ReactNode }) {
  return (
    <div className="flex items-center gap-2 border-b border-sol-border/30 px-3 py-2">
      ...
      <span className="flex min-w-0 items-center gap-1.5 text-xs text-sol-text">{children}</span>
    </div>
  );
}
  ...
  return (
    <div data-pick-step={verb} className="animate-in fade-in-0 slide-in-from-right-4 duration-200 motion-reduce:animate-none">
      <StepHeader onBack={onBack}>
        ...
        <span className="truncate text-sol-text-dim">· {meta.hint}</span>
      </StepHeader>
      <div className="py-1">
        {options.map((o) => (
          <DropdownMenuItem
          ...
          >
            ...
            <span className={`text-xs ${o.disabledReason ? "text-sol-text-secondary" : "text-sol-text"}`}>{o.label}</span>
            {o.disabledReason && <span className="ml-auto truncate text-[10px] text-sol-text-dim">{o.disabledReason}</span>}
