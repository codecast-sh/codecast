...
let shellPromise: Promise<{ html: string; injected: string } | null> | null = null;

async function loadShell(): Promise<{ html: string; injected: string } | null> {
  try {
    ...
    return { html, injected: links };
  } catch {
    return null;
  }
}

export function getShell(): Promise<{ html: string; injected: string } | null> {
