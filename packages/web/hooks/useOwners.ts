...
export function useOwners(conversationId: string, env: OwnersEnv) {
  ...
  const handoffTo = async (id: string, note: string, opts: { keepSelf?: boolean } = {}) => {
    ...
    try {
      if (env.reparent) {
        // One path (S11): set the owner line to `desired`, first listed becomes
        // the reporting parent. The binding toasts the "now reports to" line.
        await env.reparent({ kind: "user", owners: Array.from(desired), mode: "set" }, { note: trimmed, parentName: disp.name, toastFallback: `Handed off to ${disp.name}` });
      } else {
  ...
  return { ownerIds, ownerList, displayFor, toggle, moveToRole, handoffTo, handoffs, clearAll, selectable, currentUser, handoff, myAssignment, ack, canManage };
}
