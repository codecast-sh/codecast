...
// crawl: an armed boot gate must page despite a fresh persisted watermark, then
// throttle — and a waiting gate must not page at all.
describe("runReconcileCrawl — boot gate wiring", () => {
  beforeEach(() => {
  ...
  it("a WAITING gate doesn't page at all — no watermark stands in for the gate", async () => {
    // A fresh profile (or any gap past the throttle) has no persisted watermark,
    // so falling back to the durable throttle while un-armed threw the gate away:
    // the crawl ran anyway and its CLEAR pass un-hid a kill still parked in the
    // outbox. Waiting must mean not one page.
    let calls = 0;
    const crawl = (bootGate: "waiting" | "armed") => runReconcileCrawl({
      namespace: "bGate",
      wsKey: "wsGate",
      throttleMs: THROTTLE,
      pageDelayMs: 0,
    ...
    // Arming is what re-fires the effect; the held-back crawl then runs.
    crawl("armed");
    await delay(40);
    expect(calls).toBe(1);
  });
