...
import { MESSAGE_PAGE_SIZE } from "../store/cacheRetention";
...
export function useConversationMessages(
  ...
  const jumpToStart = useCallback(() => {
    ...
    targetInitializedRef.current = false;
    // Keep the tail on screen until the start page arrives. Clearing
    // targetAroundData first made rawMessages fall through to an empty
    // window when the store key had not hydrated, which is a blank feed.
    setJumpTimestamp(0);
    setJumpMode("start");
    setTargetMode(true);
    ...
    void fetchMessagesAround(convex, conversationId, 0, 0, 50)
      .then((res: any) => {
        ...
        // Flip the loading ref BEFORE the data setState so the first render
        // with the start window is not treated as still in-flight (that left
        // the up-arrow spinner on and skipped the scroll-to-top layout effect).
        targetInitializedRef.current = true;
        setTargetIsLoadingOlder(false);
