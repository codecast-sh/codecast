 ...
 *
 * When somebody ELSE unmutes mid-hold the room stops being a burst and the
 * ordinary dock is the right answer — but that is the deliberate upgrade
 * moment, and ct-46032 owns building it. Nothing here tests for it, so today a
 * hold stays a strip until it ends, which is the behaviour this pins.
 */
export function callDockSurface(
  status: WalkieStatus,
  call: { roomKey: string | null; phase: string; muted: boolean },
  opts: { expanded: boolean; lingerRoom?: string | null; guest?: boolean },
): DockSurface {
