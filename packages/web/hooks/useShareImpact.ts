 ...
 * of every selected directory, folded into one total. A one shot preview read
 * (not a registered feed); it fails soft to the rows' own counts, so the band
 * always has something honest to say.
 */
export function useShareImpact(selectedPaths: string[], rows: ProjectCounts[] | undefined): ShareImpact {
  ...
  const { data } = useQueryNoThrow(
    ...
    stable.length > 0 ? { path_prefixes: stable } : "skip",
  );
  return useMemo(
