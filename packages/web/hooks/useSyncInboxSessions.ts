...
export function useSyncInboxSessions() {
  ...
  useEffect(() => {
    ...
    runReconcileCrawl({
      ...
      wsKey: sessWsKey,
      throttleMs: SESSIONS_RECONCILE_THROTTLE_MS,
      // Once armed, the first run after a page load ignores the durable watermark:
      ...
      // the full 30 minutes. Until it arms the crawl doesn't run AT ALL — a
      // pre-replay CLEAR pass would un-hide a dismiss still parked in the outbox,
      // and a client with no persisted watermark has no throttle to fall back on.
      bootGate: bootEagerReady ? "armed" : "waiting",
      pageDelayMs: SESSIONS_RECONCILE_PAGE_DELAY_MS,
      maxPages: 50,
      fetchPage: async (cursor) => {
    ...
    // bootEagerReady flipping is what re-fires this effect to run the FIRST pass
    // (the pre-arm firings return without crawling); the crawls' own in-session
    // doneAt keeps the later re-fires from double-crawling.
