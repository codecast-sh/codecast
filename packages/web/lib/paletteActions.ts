...
import { getShortcutsForAction, inputGuardBypass, matchShortcut, type ShortcutAction } from "../shortcuts/registry";
  ...
  return actions.find(action => {
    if (action.shortcutAction) {
      return getShortcutsForAction(action.shortcutAction).some(def =>
        matchShortcut(event, def) && inputGuardBypass(def, event.target as HTMLElement | null),
      );
    }
