...
// a codecast timeline: a git push, a check result, a review, and later a
// GitHub or Linear issue change.
//
...
export function filePath(ref: ExternalEventRef["file"]): string | null {
  ...
  return `/repo/${parts.owner}/${parts.repo}/blob/${ref.ref ?? "HEAD"}/${ref.path}${line}`;
}

/** A git_events row as it arrives from convex. Fields are all optional here
 *  because the row is server data a client of any age may hold. */
...
export function gitEventToExternalEvent(row: GitEventRow): ExternalEvent {
  const meta = row.meta ?? {};
  ...
  return {
    ...
    at: row.created_at ?? 0,
    refs,
    meta: { ...meta, repository, branch: row.branch },
  };
}
