...
// Globals bridging the inlined pre-boot script and the app entry (src/main.tsx).
const HANDOFF_PENDING_FLAG = "__ccHandoffPending";
const APP_BOOT_FN = "__ccBootApp";
...
/**
 * The app entry hands its boot function here instead of calling it directly.
 * When the pre-boot handoff fired, the app must NOT boot — that is the whole
 * point — but the escape-hatch buttons on the static screen still need a way to
 * start it. Either order works: if the entry runs first it registers and waits;
 * if the button is clicked first, the flag is already cleared by then.
 */
export function registerAppBoot(boot: () => void): void {
  const w = window as any;
  w[APP_BOOT_FN] = boot;
  if (!w[HANDOFF_PENDING_FLAG]) boot();
}
...
export function runPreBootHandoff(appPreloadUrls: string[]): void {
  ...
  // Yield to the parser before launching the external protocol: a top-level
  // navigation issued mid-parse can abort the document, and the escape-hatch
  // screen has to survive. The document is a few kilobytes, so this costs
  // roughly a millisecond.
  setTimeout(() => openDesktop({ auto: true }), 0);

  const ready = () => installScreenHandlers(appPreloadUrls);
...
function hideScreen(): void {
  document.documentElement.removeAttribute(HANDOFF_SCREEN_ATTR);
}

function bootApp(urls: string[]): void {
  const w = window as any;
  w[HANDOFF_PENDING_FLAG] = false;
  preloadApp(urls);
  const boot = w[APP_BOOT_FN];
  if (typeof boot === "function") boot();
}

function installScreenHandlers(urls: string[]): void {
  document.addEventListener("click", (e) => {
    ...
    if (action === "retry") {
      // Explicitly user-driven, so no auto marker: the desktop applies a
      // clicked link unconditionally.
      openDesktop();
      return;
    }
    ...
    // Both "browser" and "always" mean: use the browser for this page. Load the
    // app that the handoff skipped.
