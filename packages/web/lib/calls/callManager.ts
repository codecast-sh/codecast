...
import { RING_STUB, RING_STUB_MS, ringStubId, type RingStub } from "./ringStubs";
 ...
 *  invite is in flight (lib/calls/ringStubs). A person the list already
 *  shows ringing gets no stub; a withdrawal expires the stub's clock, and
 *  the list's merge drops it. */
function ringStubs(roomKey: string, toUserIds: string[], on: boolean): void {
  ...
  if (on) {
    ...
    const ringing = (id: string) => outgoing.some((r) => String(r.to_user) === id && r.room_key === roomKey && (r.status ?? "ringing") === "ringing");
    const stubs: RingStub[] = toUserIds.filter((id) => !ringing(id)).map((id) => ({
