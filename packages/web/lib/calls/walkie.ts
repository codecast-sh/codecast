...
async function finishBurst(b: Burst, releasedAt: number): Promise<void> {
  ...
  if (burst === b) {
    ...
    // message lands. Everything below this is network — the audio upload, then
    // the finalize — and the linger used to begin after it. Across that window
    // `sending`, `incoming` and `lingerUntil` were all null while this client
    // was still seated and connected, so `walkieOwnsCall` answered false for a
    // room the walkie had not let go of, and the ordinary call dock took it:
    // measured at 861ms on a warm localhost with a ten second recording, and it
    // grows with the recording and the network because it IS the upload. What
    // the person saw was a floating call window — or the full stage — appearing
    // by itself a beat after they stopped talking (ct-46031).
    //
