...
export function railRowTip(
  ...
  const slackLine = c.slack ? `${MIRROR_STATE_LABEL[c.slack.state]} · #${c.slack.name}` : undefined;
  const detail = c.kind === "dm" ? undefined : [slackLine, c.topic || undefined].filter(Boolean).join(" · ") || undefined;
