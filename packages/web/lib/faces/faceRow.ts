...
export function readFaceRow(): FaceRow {
  ...
  const walkie = getWalkieStatus();
  const announcement = getJoinAnnouncement();
  ...
  const sig = faceRowInputSig(st, walkie, announcement, tiles, now);
  if (sig === lastSig) return lastRow;
  lastSig = sig;
  lastRow = deriveFaceRow(faceRowInputFrom(st, walkie, announcement, tiles, now), lastRow);
  return lastRow;
}
