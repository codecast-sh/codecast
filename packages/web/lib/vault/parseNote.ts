...
function toPlainText(line: string): string {
  if (TABLE_DIVIDER_RE.test(line) && line.includes("|")) return "";
  ...
  s = s.replace(/\[\[([^[\]\n]+)\]\]/g, (_m, inner: string) => {
    const { target, subpath, alias } = parseWikiTarget(inner);
    return alias || [target, subpath].filter(Boolean).join(" ");
  });
  s = s.replace(/!\[[^\]]*\]\([^)]*\)/g, " ");
  s = s.replace(/\[([^\]]*)\]\([^)]*\)/g, "$1");
