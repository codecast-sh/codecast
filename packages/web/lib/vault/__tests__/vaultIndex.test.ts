...
describe("the ambiguity flip", () => {
  ...
  test("deleting a file hands its name back to an alias that wanted it", () => {
    const index = build({
      "Note.md": note(""),
      "People/Ada.md": note("---\naliases: [Note]\n---\n"),
      "Source.md": note("[[Note]]"),
    });
    // The real filename wins while it exists...
    expect(index.outgoing("Source.md")[0].resolved).toBe("Note.md");

    index.remove("Note.md");

    // ...and the alias inherits the name when it stops existing.
    expect(index.outgoing("Source.md")[0].resolved).toBe("People/Ada.md");
  });
...
describe("rename", () => {
  ...
  test("renaming ONTO an existing note replaces it, leaving no trace of it", () => {
    const index = build({
      "A.md": note("---\naliases: [Foo]\ntags: [atag]\n---\n\n[[Hub]]"),
      "B.md": note("---\naliases: [Bar]\ntags: [btag]\n---\n\n[[Hub]]"),
      "Hub.md": note(""),
    ...
    // B's tag, alias and backlink all belonged to content that no longer exists.
    expect(index.tags()).toEqual(["atag"]);
    expect(index.resolveLink("Bar", "Anywhere.md")).toBeNull();
    expect(index.resolveLink("Foo", "Anywhere.md")).toBe("B.md");
    expect(index.backlinks("Hub.md").map((b) => b.source)).toEqual(["B.md"]);
    expect(dump(index)).toEqual(
      dump(build({ "B.md": note("---\naliases: [Foo]\ntags: [atag]\n---\n\n[[Hub]]"), "Hub.md": note("") })),
    );
  });
  ...
  test("random operation streams stay identical to a rebuild", () => {
    ...
    for (let step = 0; step < 300; step++) {
      ...
      if (roll < 0.55 || existing.length === 0) {
        ...
        if (to === from) continue;
        // `to` may already exist — a rename that clobbers its destination is a
        // real case (rename(2) replaces), and the map bookkeeping for it is
        // easy to get wrong, so let the stream generate it.
