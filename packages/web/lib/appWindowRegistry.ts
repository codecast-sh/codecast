...
export function installAppWindowRegistry(onOpen: (path: string | null) => void): void {
  ...
  // A goodbye can die with the window that meant to send it (a message
  // posted on the way out is not guaranteed delivery), so the lock manager
  // is asked again on a slow clock; the lock itself is already gone.
  setInterval(() => void refreshAppWindowPresence(), 10_000);
...
export function announceAppWindow(app: DesktopApp): () => void {
  ...
  let released = false;
  // Only the window that holds the lock answers to the app's name: a window
  // that lost the race must not apply an "open" meant for the winner.
  const hello = () => {
    announced = app;
