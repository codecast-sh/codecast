...
/** `personifyAll` is the workspace's "give every session a character" switch;
 *  components get it from usePersonifyAll() rather than passing it by hand. */
...
/** A scalar subscription: the switch changes rarely, and a component that
 *  reads it must not re-render on unrelated client-state churn. */
export function usePersonifyAll(): boolean {
  return useInboxStore((s) => !!(s.clientState as { ui?: { personify_sessions?: boolean } } | undefined)?.ui?.personify_sessions);
}
