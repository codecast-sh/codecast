...
export function buildNavigatorRows(
  ...
  for (const m of userMessages) {
    ...
    if (machine) {
      ...
      rows.push({ _id: m._id, ...(images?.length ? { images } : {}), display, isCmd: false, timestamp: m.timestamp, commentCount, kind: machine.kind, source, originalIndex: -1 });
      continue;
    }
    ...
    rows.push({ _id: m._id, ...(images?.length ? { images } : {}), ...user, timestamp: m.timestamp, commentCount, kind: "user", originalIndex: humanOrdinal++ });
  }
  return rows;
}
