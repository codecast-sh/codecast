    ...
    case "overloaded": {
      ...
      const detail =
        health.hourMs !== undefined
          // The hour is a record, not a live symptom, so it does not promise
          // that anything is late right now. blocksDelivery draws the same
          // line: this tier colours the header chip, it does not tell a stuck
          // message to blame the daemon.
          ? `The CLI daemon's event loop was frozen ${secs(health.hourMs)} in the last hour${worst}.${cause} Deliveries and echoes on this machine run late whenever it blocks.`
