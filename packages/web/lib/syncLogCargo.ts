...
export const ENRICH_TRIGGER_FIELDS: Record<CargoCollection, ReadonlySet<string>> = {
  tasks: new Set(["plan_id"]),
  docs: new Set(["plan_id", "doc_type"]),
  plans: new Set([]),
  projects: new Set([]),
  sessions: new Set(["active_plan_id", "active_task_id", "workflow_run_id", "status"]),
};
...
export function planCargoApply(
  ...
  if (coll === "tasks") {
    ...
    // A patch that bumps only the clock means something JOINED changed (a
    // comment landed, a link moved): the row itself has nothing new to show,
    // so refetch to pick the join up.
    const keys = Object.keys(raw);
    if (keys.length > 0 && keys.every((k) => k === "updated_at") && unsetIn.length === 0) refetch = true;
  }
