...
export type CommentThread = {
  ...
  lastActivity: number;
  // Every comment carries a resolution stamp (see Comment.resolved_at).
  resolved: boolean;
};

export function isThreadResolved(comments: Comment[]): boolean {
  return comments.length > 0 && comments.every((c) => !!c.resolved_at);
}
...
export function groupComments(comments: Comment[]): { global: CommentThread; anchored: CommentThread[]; files: CommentThread[] } {
  ...
  const make = (key: string, messageId: string | undefined, list: Comment[]): CommentThread => {
    ...
    return {
      ...
      messageId,
      comments: sorted,
      lastActivity: sorted.length ? sorted[sorted.length - 1].created_at : 0,
      resolved: isThreadResolved(sorted),
    };
  };
