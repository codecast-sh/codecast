...
// Keeping it whole means the tag id and the conversion label cannot drift.
const SEND_TO: string | undefined = META_ENV.VITE_GOOGLE_ADS_SEND_TO;
const ADS_ID: string | undefined = SEND_TO?.split("/")[0];
...
function measurable(): boolean {
  if (!SEND_TO || !ADS_ID) return false;
