...
export type SwitchBlock = "active" | "offline" | "remote" | "login_expired" | "busy";
...
export function machineSwitchBlock(opts: {
  ...
  if (opts.busyProfile && opts.busyProfile !== opts.thisProfile) {
    return { block: "busy", label: `Already switching to "${opts.busyProfile}"` };
  }
  return null;
}
...
export function resolveMachineSwitch(opts: {
  ...
  if (opts.pending.email && opts.activeEmail && opts.pending.email === opts.activeEmail) {
