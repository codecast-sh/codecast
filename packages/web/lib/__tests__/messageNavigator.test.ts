...
describe("buildNavigatorRows", () => {
  ...
  test("a machine body that is a JSON wire payload shows the type in plain words", () => {
    const wire = formatSessionMessage(
      "review-ct39371",
      '{"type":"idle_notification","from":"review-ct39371","timestamp":1724800000}',
    );
    const [row] = buildNavigatorRows([msg("j", wire)]);
    expect(row.display).toBe("idle notification");
  });

  test("a machine body that only looks like JSON keeps the raw text", () => {
    const brace = "{not json at all";
    const [row] = buildNavigatorRows([msg("j2", formatSessionMessage("jx7c6zk", brace))]);
    expect(row.display).toBe(brace);
    const noType = '{"kind":"ping"}';
    const [row2] = buildNavigatorRows([msg("j3", formatSessionMessage("jx7c6zk", noType))]);
    expect(row2.display).toBe(noType);
  });

  test("a message of only zero width characters produces no row", () => {
    const rows = buildNavigatorRows([msg("z", "\u200B\u200B"), msg("ok", "real prompt")]);
    expect(rows.map((r) => r._id)).toEqual(["ok"]);
    expect(rows[0].originalIndex).toBe(0);
  });

  test("a trigger whose body starts with its title drops the source line", () => {
    const [row] = buildNavigatorRows([
      msg("t", '<scheduled-task title="Daily monitor: run X">Daily monitor: run X for the full protocol</scheduled-task>'),
    ]);
    expect(row.kind).toBe("schedule");
    expect(row.source).toBeUndefined();
    expect(row.display).toBe("Daily monitor: run X for the full protocol");
    const [kept] = buildNavigatorRows([msg("e", TRIGGER_WIRE)]);
    expect(kept.source).toBe("Nightly audit");
  });

  test("a command tag with an empty name falls back to the cleaned content", () => {
    const [row] = buildNavigatorRows([
      msg("cmd", "<command-name>/</command-name><command-args>build the thing</command-args>"),
    ]);
    expect(row.isCmd).toBe(true);
    expect(row.display).toBe("build the thing");
  });

  test("a row whose content reaches the server snippet cap is marked clipped", () => {
    const snippet = "y".repeat(NAV_ROW_SNIPPET_CHARS);
    const rows = buildNavigatorRows([msg("short", "hi"), msg("long", snippet), msg("wire", formatSessionMessage("jx7c6zk", snippet))]);
    expect(rows.map((r) => [r._id, r.clipped])).toEqual([["short", false], ["long", true], ["wire", true]]);
  });
});

describe("navigatorRowBody", () => {
  const snippet = "y".repeat(NAV_ROW_SNIPPET_CHARS);
  const full = `${snippet} and the rest of the prompt`;

  test("without a full body the snippet stands", () => {
    const [row] = buildNavigatorRows([msg("long", snippet)]);
    expect(navigatorRowBody(row, undefined)).toBe(row.display);
  });

  test("a full body runs through the same pipeline: context tags stripped, wire payloads humanized", () => {
    const [row] = buildNavigatorRows([msg("long", snippet)]);
    expect(navigatorRowBody(row, `${full}<system-reminder>noise</system-reminder>`)).toBe(full);
    const [wire] = buildNavigatorRows([msg("wire", formatSessionMessage("jx7c6zk", snippet))]);
    expect(navigatorRowBody(wire, formatSessionMessage("jx7c6zk", full))).toBe(full);
  });

  test("a full body that yields no row keeps the snippet", () => {
    const [row] = buildNavigatorRows([msg("long", snippet)]);
    expect(navigatorRowBody(row, "   ")).toBe(row.display);
  });
});
...
describe("matchSnippet", () => {
  const text = "Hard data on all three threads: PILOT MARKER: email_messages.metadata carries the experiment id for every send";

  test("a hit before the radius returns the text unchanged", () => {
    expect(matchSnippet(text, "three")).toBe(text);
    expect(matchSnippet(text, "HARD")).toBe(text);
  });

  test("a hit past the radius starts at a word boundary with an ellipsis", () => {
    const out = matchSnippet(text, "experiment");
    expect(out.startsWith("\u2026")).toBe(true);
    expect(out).toBe("\u2026MARKER: email_messages.metadata carries the experiment id for every send");
    // The slice starts on a word, never inside one.
    expect(text.includes(" " + out.slice(1))).toBe(true);
    expect(matchIndex(out, "experiment")).toBeGreaterThan(0);
  });

  test("no hit returns the text unchanged", () => {
    expect(matchSnippet(text, "nothing here")).toBe(text);
    expect(matchSnippet(text, "")).toBe(text);
    expect(matchIndex(text, "")).toBe(-1);
  });
});
