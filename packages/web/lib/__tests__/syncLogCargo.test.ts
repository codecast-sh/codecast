...
describe("planCargoApply — tasks/docs/plans/projects", () => {
  ...
  test("tasks: session_count derives from conversation_ids; a clock-only bump refetches (a join changed)", () => {
    expect(planCargoApply("tasks", { patch: { conversation_ids: ["a", "b"] } }, {}).fields.session_count).toBe(2);
    expect(planCargoApply("tasks", { patch: { updated_at: 5 } }, {}).refetch).toBe(true);
...
describe("planCargoApply — sessions adapter", () => {
  ...
  test("a status that byIds would omit forces the refetch that prunes it", () => {
    ...
    expect(planCargoApply("sessions", { patch: { status: "active" } }, {}).refetch).toBe(true); // in triggers: status
