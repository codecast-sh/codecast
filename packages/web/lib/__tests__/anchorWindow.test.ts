...
describe("the seat's provisioning prompt folds away (F4.1)", () => {
  ...
  test("the cut is the first message after the prompt, only when the window starts on it", () => {
    expect(bootstrapCut({ messages: [boot, reply, later] })).toBe(200);
    expect(bootstrapCut({ messages: [reply, later] })).toBeUndefined();
    expect(bootstrapCut({ messages: [boot, reply], loaded_start_index: 40 })).toBeUndefined();
    expect(bootstrapCut({ messages: [boot] })).toBeUndefined();
