...
/**
 * A command that moves out of the session's checkout (`cd`, `git -C`, a temp
 * directory) may have committed in some other repository, so its commit must
 * not be linked as if it belonged to the session's. It still renders, unlinked.
 */
export function commandLeavesCheckout(command: string): boolean {
  return /(^|[;&|(]\s*)cd\s|\bgit\s+-C\s|\bmktemp\b|\bpushd\s/.test(command);
}
