// Dual write storage: localStorage (fast sync read) + IndexedDB (durable
// backup). Parameterized by the IndexedDB database name so two apps on one
// origin do not share a store.

const STORE_NAME = "tokens";
const DB_VERSION = 1;

export type DurableAuthStorage = {
  /** Pass this as `storage` to ConvexAuthProvider. */
  storage: Storage;
  /** Read a namespaced auth value from either durable tier without restoring it. */
  readDurableAuthValue: (key: string) => Promise<string | null>;
  /**
...
export function createDurableAuthStorage(params: { dbName: string }): DurableAuthStorage {
  ...
  async function readDurableAuthValue(key: string): Promise<string | null> {
  ...
  async function purgeDurableAuthValues(keys: readonly string[]): Promise<void> {
