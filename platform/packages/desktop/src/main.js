...
function createDesktopApp(userConfig, electron = require("electron")) {
  ...
  function openWindow(opts) {
    ...
    if (opts.file) win.loadFile(opts.file);
    else if (opts.url) win.loadURL(opts.url);
    return win;
  }
  ...
  // called with the renderer's arguments and then the shell API.
  for (const [name, fn] of Object.entries(cfg.ipc.handlers)) {
    ipcMain.handle(`app:${name}`, (_e, ...args) => fn(...args, api));
  }
  function emit(name, payload) {
    if (!cfg.ipc.events.includes(name)) throw new Error(`ipc.events does not list "${name}"`);
    for (const w of BrowserWindow.getAllWindows ? BrowserWindow.getAllWindows() : []) {
      if (!w.isDestroyed()) w.webContents.send(`app:${name}`, payload);
    }
  }
