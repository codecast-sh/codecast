...
if [ ! -f "$MARKER" ]; then
  ...
  # A failed copy must not turn into a tight restart loop: every attempt forces
  # a checkpoint on the primary and creates then drops the slot (2026-09-13: a
  # root-owned stray file in the primary's PGDATA failed the copy 40 times in
  # 40 minutes). Wait before letting the supervisor restart us.
  if ! gosu postgres pg_basebackup -d "$conninfo" -D "$PGDATA" -R -X stream \
    --create-slot --slot "$REPLICATION_SLOT" --checkpoint=fast --progress --verbose; then
    log "base backup FAILED; sleeping ${BOOTSTRAP_RETRY_SECONDS:-600}s before exit so the restart is not a tight loop"
    sleep "${BOOTSTRAP_RETRY_SECONDS:-600}"
    exit 1
  fi
