...
4. Inspect internal `cloudWake:status`. A cron dispatches due bound triggers every minute; another reconciles expired leases and lost wake schedules. Each wake has at most five attempts. Success is followed by a heartbeat check; a failed or missing heartbeat remains visible as failure, not silently done.
