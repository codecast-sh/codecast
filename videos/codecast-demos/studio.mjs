...
export function renderComposition(scene, voDir) {
  ...
  runs.forEach((run, ri) => {
    ...
    // per-beat spotlight + caption
    run.beats.forEach((b) => {
      if (b.spot) {
        const sId = nid("spot");
        const [sx, sy, sw, sh] = b.spot;
        dom.push(spotHTML(sId, sx, sy, sw, sh, b.spotLabel));
        domSetClip(dom, sId, b.start, b.dur + 0.4, 20);
...
function spotHTML(id, x, y, w, h, label) {
  const L = x * W, T = y * H, WW = w * W, HH = h * H;
  return (
    `<div id="${id}" class="clip spot" style="position:absolute;left:${fx(L)}px;top:${fx(T)}px;width:${fx(WW)}px;height:${fx(HH)}px;` +
    `border:3px solid ${TOK.cyan};border-radius:14px;box-shadow:0 0 0 2px rgba(42,161,152,0.25),0 0 40px 6px rgba(42,161,152,0.45);` +
    `pointer-events:none;">` +
      (label ? `<div style="position:absolute;top:-42px;left:-3px;font-family:var(--mono);font-size:19px;font-weight:700;letter-spacing:2px;` +
